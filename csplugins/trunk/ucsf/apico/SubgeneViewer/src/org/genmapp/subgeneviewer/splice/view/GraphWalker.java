...
public class GraphWalker {
	...
	private static final int FLAG_WIDTH = 10;  // width of bounding box for triangle of flag on StartSite
	private static final int FLAG_HEIGHT = 10;  // height of bounding box for triangle of flag on StartSite
	...
	public static void renderView(SpliceNetworkView view) {
		...
		while (blocks.hasNext()) {
			...
			while (regions.hasNext()) {
				...
				Iterator<SpliceEvent> splices = region.listOfSpliceEvents.iterator();
				...
				while (splices.hasNext()) {
					...
					System.out.println("Getting bounds for region: " + toRegion + " with name: " + regionString);
					...
					g.drawLine(startX, startY, ((startX + endX) / 2), startY + NODE_HEIGHT);
					g.drawLine(((startX + endX) / 2), startY + NODE_HEIGHT, endX, endY);
				}
				...
				if (region.containsStartSite())
				{
					...
					int startY = regionBounds.y - START_SITE_HEIGHT; // draw flag above region
