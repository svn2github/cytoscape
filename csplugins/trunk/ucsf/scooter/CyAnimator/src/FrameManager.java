...
public class FrameManager {
	...
	public void recordAnimation(String directory) throws IOException {
		WriteTask task = new WriteTask("Writing output files", directory);
	...
	public class WriteTask implements Task {
		...
		public WriteTask(String title, String directory) {
			super();
			this.title = title;
		...
		public void run() {
			...
			monitor.setStatus("Writing frames");
			...
			for(int i=0; i<frames.length; i++) {
				...
				try {
					...
					monitor.setException(e, "Failed to write file "+name);
					return;
				}
				monitor.setPercentCompleted((i*100)/frames.length);
			}
