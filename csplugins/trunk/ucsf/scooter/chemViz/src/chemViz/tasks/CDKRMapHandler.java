...
public class CDKRMapHandler {
    ...
    public void calculateOverlapsAndReduce(IAtomContainer Molecule1, IAtomContainer Molecule2, boolean shouldMatchBonds) throws CDKException {
        ...
        if ((Molecule1.getAtomCount() == 1) || (Molecule2.getAtomCount() == 1)) {
            List<CDKRMap> overlaps = CDKMCS.checkSingleAtomCases(Molecule1, Molecule2);
            ...
            if (nAtomsMatched > 0) {
                ...
                //int counter = 0;
                identifySingleAtomsMatchedParts(overlaps, Molecule1, Molecule2);

            }

        } else {
            List<List<CDKRMap>> overlaps = CDKMCS.search(Molecule1, Molecule2, new BitSet(), new BitSet(), true, true, shouldMatchBonds);
...
//                System.out.println("source: " + source.getAtomCount() + ", target: " + target.getAtomCount() + ", overl: " + allMaxOverlaps.peek().size());
                List<List<CDKRMap>> maxOverlapsAtoms = makeAtomsMapOfBondsMap(allMaxOverlaps.peek(), Molecule1, Molecule2);
...
//                identifyMatchedParts(allMaxOverlaps.peek(), source, target);
    ...
    public void calculateOverlapsAndReduceExactMatch(IAtomContainer Molecule1, IAtomContainer Molecule2, boolean shouldMatchBonds) throws CDKException {
        ...
        if ((Molecule1.getAtomCount() == 1) || (Molecule2.getAtomCount() == 1)) {
            ...
            List<List<CDKRMap>> overlaps =
                    CDKMCS.search(Molecule1, Molecule2, new BitSet(), new BitSet(), true, true, shouldMatchBonds);
    ...
    public void calculateSubGraphs(IAtomContainer Molecule1, IAtomContainer Molecule2, boolean shouldMatchBonds) throws CDKException {
        ...
        if ((Molecule1.getAtomCount() == 1) || (Molecule2.getAtomCount() == 1)) {
            ...
            List<List<CDKRMap>> overlaps =
                    CDKMCS.getSubgraphMaps(Molecule1, Molecule2, shouldMatchBonds);
    ...
    public void calculateIsomorphs(IAtomContainer Molecule1, IAtomContainer Molecule2, boolean shouldMatchBonds) throws CDKException {
        ...
        if ((Molecule1.getAtomCount() == 1) || (Molecule2.getAtomCount() == 1)) {
            ...
            List<List<CDKRMap>> overlaps =
                    CDKMCS.getIsomorphMaps(Molecule1, Molecule2, shouldMatchBonds);
     ...
     */
    private List<List<CDKRMap>> makeAtomsMapOfBondsMap(List<CDKRMap> rMapList, IAtomContainer graph1, IAtomContainer graph2) {
        if (rMapList == null) {
            return (null);
        }
     ...
     */
    private List<List<CDKRMap>> makeAtomsMapOfBondsMapSingleBond(List<CDKRMap> list, IAtomContainer sourceGraph, IAtomContainer targetGraph) {
        if (list == null) {
            return null;
        }
