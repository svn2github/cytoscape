...
public class CustomGraphicsVisitor implements IDrawVisitor {
	...
	public CustomGraphicsVisitor(NodeView view, double scale) {
		...
		networkView = (CyNetworkView)(view.getGraphView());
	}
	...
	private void visit(LineElement line) {
		...
		Shape s = stroke.createStrokedShape(lineShape);
	...
	private void visit(PathElement path) {
		...
		Shape s = currentStroke.createStrokedShape(pathShape);
		PaintFactory p = new DefaultPaintFactory(path.color);
	...
	private void visit(GeneralPath path) {
		...
		PaintFactory p = new DefaultPaintFactory(path.color);
		Shape s = currentStroke.createStrokedShape(generalPath);
		CustomGraphic c = new CustomGraphic(scaleTransform.createTransformedShape(s), p);
		cgList.add(c);
	}
	...
	private void visit(ArrowElement line) {
		...
		PaintFactory p = new DefaultPaintFactory(line.color);
	...
	private void createTextCustomGraphics(Shape textShape, Color textColor, 
		...
		if (paintBackground && !textColor.equals(backgroundColor)) {
			...
			Rectangle2D bg = new Rectangle2D.Double(bbox.getX()-1, bbox.getY()-1, bbox.getWidth()+2, bbox.getHeight()+2);
			PaintFactory pb = new DefaultPaintFactory((Color)backgroundColor);
			CustomGraphic cb = new CustomGraphic(scaleTransform.createTransformedShape(bg), pb);
			cgList.add(cb);
		}

		PaintFactory p = new DefaultPaintFactory(textColor);
		CustomGraphic c = new CustomGraphic(scaledShape, p);
		cgList.add(c);
	}
	...
	private void drawFilledWedge(Point2d vertexA, Point2d vertexB, Point2d vertexC, Color clr) {
		...
		CustomGraphic c = new CustomGraphic(scaleTransform.createTransformedShape(wedge), new DefaultPaintFactory(clr));
		cgList.add(c);
	}
	...
	private void drawDashedWedge(Point2d vertexA, Point2d vertexB, Point2d vertexC, Color clr) {
		...
		Shape s = currentStroke.createStrokedShape(path);
		PaintFactory p = new DefaultPaintFactory(clr);
