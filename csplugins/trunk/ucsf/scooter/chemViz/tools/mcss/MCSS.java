...
final public class MCSS {
    ...
    private synchronized List<IAtomContainer> calculateMCSS(List<IAtomContainer> mcssList, JobType jobType, int nThreads) {
        ...
        if (nThreads == 1) {
            ...
            IAtomContainer result = task.call();
            if (result != null) {
                newMCSSList.add(result);
            }
            return newMCSSList;
        } else {
            /*
    ...
    private synchronized List<IAtomContainer> submitMultiThreadedJob(List<IAtomContainer> mcssList, 
                                                                     JobType jobType, TaskUpdater updater, int nThreads) {
        int taskNumber = 1;
        List<IAtomContainer> newMCSSList = Collections.synchronizedList(new ArrayList<IAtomContainer>(nThreads));
        ...
        for (int i = 0; i < mcssList.size(); i += step) {
            ...
            if (subList.size() > 1) {
                ...
                Future<IAtomContainer> callMCSSThread = threadPool.submit(mcssJobThread);
        ...
        for (Future<IAtomContainer> results : futureList) {

            if (results == null) {
                continue;
            }
            try {
                IAtomContainer f = results.get();
                if (f != null) {
                    newMCSSList.add(f);
                }
            } catch (Exception e) {
