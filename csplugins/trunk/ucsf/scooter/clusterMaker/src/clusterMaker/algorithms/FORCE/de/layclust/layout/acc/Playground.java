...
import de.layclust.layout.postprocessing.PostProcessingTobi;
...
public class Playground {
	...
	private LinkedList uncarriedItems;
	...
	public Playground(ConnectedComponent cc, IParameters parameters) {
		...
		this.dimension = ((ACCParameters)parameters).getDimension();
		this.noNodes = cc.getNodeNumber();
		int noAnts = ((ACCParameters)parameters).getNoAnts();
		this.noOfIterations = noNodes * ((ACCParameters)parameters).getMultiplicatorForIterations();
		...
		this.size = (int) Math.round(Math.pow(((ACCParameters)parameters).getMultiplicatorForGridSize()*noNodes, 1.0/dimension));
	...
	public Playground(ConnectedComponent cc) {
		...
		this.ants = new IAnt[noAnts];
		this.size = (int) Math.round(Math.pow(ACCConfig.MULTIPLICATOR_FOR_SIZE*noNodes, 1.0/dimension));
		this.itemPositions = new int[noNodes][dimension];
	...
	private void initializeAnts(){
		for (int i = 0; i < ants.length; i++) {
			if(antType == "JumpingAnt") {
				ants[i] = new JumpingAnt(this);
			} else if (antType == "JumpingAntWithIncreasingViewField") {
				ants[i] = new JumpingAntWithIncreasingViewField(this);
			} else if (antType == "MemoryAnt") {
...
//		System.out.println();
		...
		if (dimension == 2) {
			int xBegin = (pos[0]-view >= 0) ? (pos[0]-view) : 0;
			int xEnd = (pos[0]+view <= size-1) ? (pos[0]+view) : (size-1);
			int yBegin = (pos[1]-view >= 0) ? (pos[1]-view) : 0;
			int yEnd = (pos[1]+view <= size-1) ? (pos[1]+view) : (size-1);
			for(int i = xBegin; i <= xEnd; i++) {
				//if(i < 0 || i > size-1) continue;
				for(int j = yBegin; j <= yEnd; j++) {
					//if(j < 0 || j > size-1) continue;
					int item = getLocation(new int[] {i,j});
			...
			int yBegin = (pos[1]-view >= 0) ? (pos[1]-view) : 0;
			int yEnd = (pos[1]+view <= size-1) ? (pos[1]+view) : (size-1);
			int zBegin = (pos[2]-view >= 0) ? (pos[2]-view) : 0;
			int zEnd = (pos[2]+view <= size-1) ? (pos[2]+view) : (size-1);
			for(int i = xBegin; i <= xEnd; i++) {
				for(int j = yBegin; j <= yEnd; j++) {
					for(int k = zBegin; k <= zEnd; k++) {
						int item = getLocation(new int[] {i,j,k});
						if (item != 0) items.add(item);
					}
				}
			}
	 ...
	 public void runInPanel(JPanel panel, int time, GUI gui) {
		...
		Thread t = new Thread("Gui-Update") {
			public void run() {
				...
				cc.getCCEdges().denormaliseWithThreshold();
				SingleLinkageClusterer slc = new SingleLinkageClusterer(cc);
				slc.run();
				PostProcessingTobi pp = new PostProcessingTobi();
...
//		s.release();
	...
	public int moveWorstPlacedItem() {
		...
		// Not in Java 1.5
		// Entry entry = itemsByPickUpProbability.lastEntry();
