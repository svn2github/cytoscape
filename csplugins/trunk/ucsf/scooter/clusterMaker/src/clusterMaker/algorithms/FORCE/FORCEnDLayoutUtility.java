...
//       --------------------- FIXED VARIABLES ---------------------- //
	 ...
	 */
	public static double calculateTemperature(int iteration, int node_no, Parameters param) {
		int k = 2;
	 ...
	 */
	public static void calculateDisplacementVectors(double[][] allDisplacements, ConnectedComponent cc, int dim, Parameters param) {

	        double[][] node_pos = cc.getCCPositions();
		DoubleMatrix2D edges = cc.getCCEdges();
			...
			for (int j = 0; j < i; j++) {
				...
				for (int d = 0; d < dim; d++) {
					...
					if (differenceInPosForDim == 0) {
						differenceInPosForDim = MIN_DISTANCE; // TODO
																					// why?
