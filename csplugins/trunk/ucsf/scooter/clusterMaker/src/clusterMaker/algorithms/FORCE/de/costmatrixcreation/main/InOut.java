...
import de.costmatrixcreation.dataTypes.*;
...
public class InOut {
	...
	public static BlastFile readBlastFileWithArray(String blastFile,HashMap<String ,Integer> integers2proteins) throws IOException{
		...
		if(Config.gui){
			...
			System.out.println("\t start reading blast file ...");
		}
		...
		try {
			while(true){
				...
				bf.setAll(i, startQuery, endQuery, startSubject, endSubject, sourceInt, targetInt, evalue);
	...
	public static Edges readSimilarityFile(String file, HashMap<Integer, String> proteins2integers, HashMap<String, Integer> integers2proteins) throws IOException{
		...
		if(!Config.createSimilarityFile){
			...
			if(Config.gui){
				...
				System.out.println("\t start counting lines of similarity file ...");
			}
		...
		for (int i = 0; i < clusters.size(); i++) {
			...
			if(Config.reducedMatrix){
				...
				if(countRealEdges==numberEdges){
					...
					String costMatrixFile = "costMatrix_size_"+cluster.size()+"_nr_"+countCostMatrices + ".rcm";
...
//						if(value<0) Console.println(value);
		...
		for (Iterator iter = integers2proteins.keySet().iterator(); iter.hasNext();) {
	...
	private static float getEdgeValue(int source, int target, Edges es) {
