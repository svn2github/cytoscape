...
public class PostProcessingUtility {
	 ...
	 * 
	 * @param clusterObject The vector of vectors containing the actual clusters.
	 * @param comparator The comparator for comparing the size of two vectors
	 ...
	 */
	protected static void sortedMerge(Vector<Vector<Integer>> clusterObject, 
...
//				String combination = v1.hashCode()+"#"+v2.hashCode();
	 ...
	 */
	protected static double calculateCostChange(Vector<Integer> cluster1, Vector<Integer> cluster2, ConnectedComponent cc){
		double costChange = 0;
	...
	protected static void rearrangeNodeToLevelTwo(Vector<Vector<Integer>> clusterObject,
			ConnectedComponent cc){
		
		int[] clusters = cc.getClusters();
		
		for(int c1=0;c1<clusterObject.size();c1++){
			Vector<Integer> oldVector = clusterObject.get(c1);
			for(int i=0;i<oldVector.size();i++){
				Integer node_i = oldVector.get(i);
				for(int j=i+1;j<oldVector.size();j++){
					Integer node_j = oldVector.get(j);
					
					//TODO
				}
			}
			
			for (int c2 = 0; c2 < clusterObject.size(); c2++) {
				Vector<Integer> newVector = clusterObject.get(c2);
			}
		}
	}
	
	protected static void rearrangeNodeToLevelN(Vector<Vector<Integer>> clusterObject,
			ConnectedComponent cc, int level){
		int[] clusters = cc.getClusters();
		
		for(int c1=0;c1<clusterObject.size();c1++){
			Vector<Integer> oldCluster = clusterObject.get(c1);
			for(int i=0;i<level;i++){
				
				createCombinationsAndRearrange(clusterObject, oldCluster, level, new HashSet(), 0);
...
//			}
	...
	private static void createCombinationsAndRearrange(Vector<Vector<Integer>> clusterObject, Vector<Integer> oldCluster, int level, HashSet<Integer> nodeSet, int current_i) {
		
		/* recursion end - so calculate costs and rearrange if this improves the overall cost */
		if(level==0){
			//TODO calculate cost
		}
		...
		nodeSet.add(oldCluster.get(current_i));
		createCombinationsAndRearrange(clusterObject, oldCluster, level, nodeSet, current_i+1);
		
		for (int i = current_i+1; i < oldCluster.size(); i++) {
			current_i = i;
	...
	protected static double calculateCostChange(HashSet<Integer> moveNodes, Vector<Integer> oldCluster,
			Vector<Integer> newCluster, ConnectedComponent cc){
		double costChange = 0;
	...
	protected static double calculateCostChange(int node_i, int node_j, Vector<Integer> oldCluster, 
			Vector<Integer> newCluster, ConnectedComponent cc){
		double costChange = 0;
	 ...
	 * @param cc
	 */
	protected static void rearrangeEachNode(Vector<Vector<Integer>> clusterObject,
			ConnectedComponent cc){
		...
		for (int i = 0; i < clusters.length; i++) {
			...
			Vector<Integer> oldCluster = clusterObject.get(oldClusterNo);
			...
			for (int c = 0; c < clusterObject.size(); c++) {
				if(oldClusterNo!=c){
					Vector<Integer> newCluster = clusterObject.get(c);
			...
			costChange = calculateCostChange(i, oldCluster, newEmptyVector, cc);
			if(costChange<0){
				oldCluster.remove(node_i);
				newEmptyVector.add(node_i);
	 ...
	 */
	protected static double calculateCostChange(int node_i, Vector<Integer> oldCluster, 
			Vector<Integer> newCluster, ConnectedComponent cc){
		double costChange = 0;
	...
	/**
	 * 1. Updates the clusters array from the information in the cluster vector object.
	 ...
	 */
	protected static double updateClusterInfoInCC(Vector<Vector<Integer>> clusterObject,
			ConnectedComponent cc){
	 ...
	 * a Vector exists with the nodes that are in this cluster. These Vectors are added to the
	 * overall Vector, which represents the collection of all clusters. If the given connected component
	 ...
	 * @param isSubConnected If the cc is a sub component of the original cc or not.
	 * @return The clusters object.
	 */
	protected static Vector<Vector<Integer>> createClusterObject(ConnectedComponent cc, boolean isSubConnectedComponent){
		
		int noOfClusters = cc.getNumberOfClusters();
		int[] clusters = cc.getClusters();
		
		/* initialise clusters object */
		Vector<Vector<Integer>> clusterObject = new Vector<Vector<Integer>>(noOfClusters);
		int[] clusterDistribution = cc.getClusterInfo();
		for (int i = 0; i < clusterDistribution.length; i++) {
			clusterObject.add(new Vector<Integer>(clusterDistribution[i]));
		}
		
		/* fill clusters object */
		for (int i = 0; i < clusters.length; i++) {
			Vector<Integer> cluster = clusterObject.get(clusters[i]);
	...
	protected static void mergeCluster(Vector<Vector<Integer>> h,  
...
//		System.out.println("test" + bestScore);
	...
	public static void main(String[] args){
		...
		Vector<Vector<Integer>> object = new Vector<Vector<Integer>>(3);
		object.add(v1);
		object.add(v2);
		object.add(v3);
...
//		PostProcessingUtility.sortedMerge(object, comparator);
	}
