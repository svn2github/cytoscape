...
public class GeneralParameterTraining {
	...
	private static Logger log = Logger.getLogger(GeneralParameterTraining.class
			.getName());
	...
	public IParameters[] runGeneralTraining() throws InvalidInputFileException, 
		...
		ArrayList<ConnectedComponent> connectedComponents = clusteringManager.
		...
		log.fine("terminate training? "+terminateTraining);
	...
	private boolean terminateTraining(IParameters[][] parameters) {
		...
		log.fine("Terminate training: "+terminate);
		return terminate;
	}

	private void runOneGeneration(IParameters[][] parameters, 
			ArrayList<ConnectedComponent> connectedComponents, int gen) throws InvalidInputFileException, InvalidTypeException {
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = 0; j < connectedComponents.size(); j++) {
				ConnectedComponent cc = connectedComponents.get(j);
	...
	private IParameters[][] createInitialParameterGeneration() {
		...
		IParameters[][] paramsGen = new IParameters[initialSize][TaskConfig.
		                                                         layouterEnumTypes.length];
