...
public class ChildDataPropertyMap 
  implements DataPropertyMap {

  /**
   * ChildDataPropertyMaps inherit from a ParentDataPropertyMap
   */
   ...
   * ints from the unique int, to the current index
   *
   * This hash map takes a uid, and returns the current row.
   *   
   * Usage:
   * <code>
   * public Object getValue ( String object_name, String att_name ) {
   *    if ( !child ) {
   *       return dataMatrix.getQuick( sharedIndentifierUniqueIntMap.get( object_name ),
   *                                   attributeNameIntMap.get( att_name );
   *    }
   *    return dataMatrix.getQuick( 
   *parentChildIntHashMap.get( sharedIndentifierUniqueIntMap.get( object_name ) ),
   *parentChildAttributeHashMap.get( attributeNameIntMap.get( att_name ) ) ); }
   *
   * parent --> child
   */
  protected OpenIntIntHashMap parentChildIntHashMap;

  /**
   * Child DataPropertyMaps need to be able to re-direct their 
   * ints from the unique int, to the current index
   * This hash map takes an attribute id and returns the appropraite child id.
   * 
   * parent --> child
   */
  protected OpenIntIntHashMap parentChildAttributeHashMap; 

  /**
   * In order to maintain compatibility with Paul/Dan DataCube plugin
   * it is necessary to keep track of which columns were the "seeds" of
   * a child DataPropertyMap.
   */
  protected int[] originalColumns;
   ...
   */
  protected DataPropertyMap ( ParentDataPropertyMap parentDataPropertyMap,  
                              int[] shared_identifiables,
                              int[] attributes 
                              ) {

    // assign the parent matrix
    this.parentDataPropertyMap = parentDataPropertyMap;

    // Assign our DataMatrix to the Selection specified, using the colt view
    this.dataMatrix = parentDataPropertyMap.
      getDataMatrix().
      viewSelection( shared_identifiables, attributes );
    
    // Assign the AttributeIntNameMatrix, using the colt view.
    this.attributeIntNameMatrix = parentDataPropertyMap.
      getAttributeNameMatrix().
      viewSelection( attributes );

    // provide a direct reference to the look up hashes
    this.attributeNameIntMap = parentDataPropertyMap.getAttributeNameIntMap();
    this.sharedIndentifierUniqueIntMap = parentDataPropertyMap.getSharedIndentifierUniqueIntMap();

    // now we need to update the conversion hashes
    this.parentChildAttributeHashMap = createParentChildAttributeHashMap();
    this.parentChildIntHashMap = createParentChildIntHashMap();
  ...
  protected OpenIntIntHashMap getParentChildIntHashMap () {
    return parentChildIntHashMap;
  }

  protected OpenIntIntHashMap getParentChildAttributeHashMap () {
  ...
  /**
   * This method populates the 1D attributeIntNameMatrix with new values,
   * and update the attributeNameMap with the appropriate values.
   */
  public void setColumnTitles ( String [] new_titles ) {
    this.attributeIntNameMatrix = create1DMatrix( new_titles );
    this.attributeNameMap = createStringIntHashMap( this.attributeIntNameMatrix );
  }
  ...
  public String[] getUnmaskedColumnTitles () {
    String[] column_names = new String[ attributeIntNameMatrix.size() ];
    for ( int i = 0; i < column_names.length; ++i ) {
      column_names[i] = attributeIntNameMatrix.getQuick( i );
    }
    return column_names;
  }
