	...
	private void loadnetwork_phyloXML(File pFile) {
		...
		try {
			trees = factory.create(pFile, parser);
					...
					JOptionPane.showMessageDialog(Cytoscape.getDesktop(), "PhloXML Parser error occured. \n" +
						"Verify that file is a valid PhyloXML file.", "phyloXML parse error", JOptionPane.ERROR_MESSAGE);
					
			return;
		}
		catch(IOException ioe){
			JOptionPane.showMessageDialog(Cytoscape.getDesktop(), "I/O error occured while attempting to parse the file. \n" +
					"Verify that file is a valid PhyloXML file.", "phyloXML parse error", JOptionPane.ERROR_MESSAGE);
			return;
		}	
		...
		{
			JOptionPane.showMessageDialog(Cytoscape.getDesktop(), "PhloXML Parser error occured. \n" +
					"The file does not contain any phyloXML trees.", "phyloXML parse error", JOptionPane.ERROR_MESSAGE);
		...
		for (int i=0; i< trees.length; i++){
			...
			{
				JOptionPane.showMessageDialog(Cytoscape.getDesktop(), "PhloXML Parser error occured. \n" +
						"The phyloXML tree has no nodes", "phyloXML parse error", JOptionPane.ERROR_MESSAGE);
					continue;
			}
			CyNetwork cyNetwork;
			// Create a network
	...
	private boolean validatePhylipFile(File file)
	{
		...
		if(file!=null)
		{
			...
			if(fileStr!=null && !fileStr.equals(""))
			{
				...
				{
					JOptionPane.showMessageDialog(Cytoscape.getDesktop(), "File is not a valid Phylip/Newick format file. \n" +
							"Number of '(' not equal to ')'.", "Phylip/Newick file parse error", JOptionPane.ERROR_MESSAGE);
					return false;
				}
				...
				{
					JOptionPane.showMessageDialog(Cytoscape.getDesktop(), "File is not a valid Phylip/Newick format file. \n" +
							"Tree in the file does not consist of any nodes.", "Phylip/Newick file parse error", JOptionPane.ERROR_MESSAGE);
				...
				{
				JOptionPane.showMessageDialog(Cytoscape.getDesktop(), "File is not a valid Phylip/Newick format file. \n" +
				"Please check the file.", "Phylip/Newick file parse error", JOptionPane.ERROR_MESSAGE);
				return false;
				}
		}
