...
//		Color overColor = new Color(140,201,77);
	...
	/**
	 * Creates the calculator that assigns node size on a continuous gradient based on node depth
	 * @param network - the network containing the nodes to be resized
	 * @return - the node size calculator
	 */
	private Calculator createNodeSizeCalculator(CyNetwork network) {
		// Determine the min and max for degree
		CyAttributes cyNodeAttrs = Cytoscape.getNodeAttributes();
		...
		// Create a calculator
		return new BasicCalculator("Node Size calcualtor", cm, VisualPropertyType.NODE_SIZE);			
	}
}
