 ...
 * <p/>
 * JOGL requires Java 1.4 or higher, so there are no restrictions on this code
 * to be compatible with Java 1.1 or Java 1.3.
 * <p/>
 * This code relies on PGraphics3D for all lighting and transformations. Meaning
 * that translate(), rotate(), and any lighting will be done in PGraphics3D, and
 * OpenGL is only used to blit lines and triangles as fast as it possibly can.
 * <p/>
 * For this reason, OpenGL may not be accelerated as far as it could be, but I
 * don't have the time to maintain two separate versions of the renderer. My
 * development time must always be focused on implementation and covering
 * features first, and optimizing later.
 * <p/>
 * Further, the difference may be negligible, as the primary slowdown in Java is
 * moving pixels (i.e. a large frame buffer is nearly impossible because Java
 * just can't do a MemoryImageSource at screen resolution) and the overhead from
 * JNI tends to be significant. In the latter case, we may even save time in
 * some cases where a large number of calls to OpenGL would otherwise be used,
 * but that's probably a stretch.
 * <p/>
 * The code is also very messy, while features are being added and removed
 * rapidly as we head towards 1.0. Things got particularly ugly as we approached
 * beta while both Simon and I were working on it. Relax, we'll get it fixed up
 * later.
 * <p/>
 * When exporting applets, the JOGL Applet Launcher is used. More information
 * about the launcher can be found at its <A HREF="http://download.java.net/media/jogl/builds/nightly/javadoc_public/com/sun/opengl/util/JOGLAppletLauncher.html"
 * >documentation page</A>.
 */
public class PGraphicsOpenGL extends PGraphics3D {
	protected GLDrawable drawable; // the rendering 'surface'
	protected GLContext context; // the rendering context (holds rendering state
	// info)
	...
	// / Used to store empty values to be passed when a light has no ambient
	// value
	...
	public void beginDraw() {
		...
		if (drawable != null) {
			...
			// System.out.println("OpenGL beginDraw() setting realized " +
		...
		if (projectionFloats == null) {
			projectionFloats = new float[] { projection.m00, projection.m10,
					projection.m20, projection.m30, projection.m01,
					projection.m11, projection.m21, projection.m31,
					projection.m02, projection.m12, projection.m22,
					projection.m32, projection.m03, projection.m13,
					projection.m23, projection.m33 };
		} else {
		...
		// System.out.println("beginDraw() stop error " +
		// PApplet.hex(gl.glGetError()));
	}

	public void endDraw() {
		// System.out.println("endDraw() error " +
		// PApplet.hex(gl.glGetError()));
	...
	protected void endShapeLighting(boolean lights) {
		...
		// For now do our own lighting--sum the specular and diffuse light
		// colors
		if (lights) {
	 ...
	 * This will obviously have to be revisited if/when proper depth sorting is
	 * implemented.
	 */
	...
	/*
	 * boolean ellipseInited; int ellipseFillList; int ellipseStrokeList;
	 * 
	 * protected void ellipseImpl(float x1, float y1, float w, float h) { float
	 * hradius = w / 2f; float vradius = h / 2f;
	 * 
	 * float centerX = x1 + hradius; float centerY = y1 + vradius;
	 * 
	 * // adapt accuracy to radii used w/ a minimum of 4 segments [toxi] // now
	 * uses current scale factors to determine "real" transformed radius
	 * 
	 * //int cAccuracy = (int)(4+Math.sqrt(hradiusabs(m00)+vradiusabs(m11))2);
	 * //int cAccuracy = (int)(4+Math.sqrt(hradius+vradius)2);
	 * 
	 * // notched this up to3 instead of2 because things were // looking a
	 * little rough, i.e. the calculate->arctangent example [fry]
	 * 
	 ...
	 * //System.out.println("accuracy is " + accuracy); //accuracy = 5;
	 * 
	 * // [toxi031031] adapted to use new lookup tables float inc =
	 * (float)SINCOS_LENGTH / accuracy;
	 * 
	 * float val = 0;
	 * 
	 * if (fill) { boolean savedStroke = stroke; stroke = false;
	 * 
	 * beginShape(TRIANGLE_FAN); normal(0, 0, 1); vertex(centerX, centerY); for
	 * (int i = 0; i < accuracy; i++) { vertex(centerX + cosLUT[(int) val]
	 * hradius, centerY + sinLUT[(int) val] vradius); val += inc; } // back to
	 * the beginning vertex(centerX + cosLUT[0] hradius, centerY + sinLUT[0]
	 * vradius); endShape();
	 * 
	 * stroke = savedStroke; }
	 * 
	 * if (stroke) { boolean savedFill = fill; fill = false;
	 * 
	 * val = 0; beginShape(); //LINE_LOOP); for (int i = 0; i < accuracy; i++) {
	 * vertex(centerX + cosLUT[(int) val] hradius, centerY + sinLUT[(int) val]
	 * vradius); val += inc; } endShape(CLOSE);
	 * 
	 * fill = savedFill; } }
	 */

	/*
	 * pgl.beginGL(); //PGraphics gr = PApplet.this.g; //GL gl =
	 * ((PGraphicsOpenGL).gr).beginGL(); if (!ellipseInited) { ellipseList =
	 * gl.glGenLists(1); gl.glNewList(ellipseList, GL.GL_COMPILE);
	 * gl.glBegin(GL.GL_LINE_LOOP); int seg = 15; float segf = 15; for (int i =
	 * 0; i < seg; i++) { float theta = TWO_PI (float)i / segf;
	 * gl.glVertex2f(cos(theta), sin(theta)); } gl.glEnd(); gl.glEndList();
	 * ellipseInited = true; }
	 * 
	 * for (int i=1; i<numSegments-1; i++) { gl.glPushMatrix();
	 * gl.glTranslatef(x[i], y[i], z); float r = w[i]/2f; gl.glScalef(r, r, r);
	 * gl.glColor4f(1, 1, 1, 150.0/255.0); gl.glCallList(ellipseList);
	 * gl.glScalef(0.5, 0.5, 0.5); gl.glColor4f(1, 1, 1, 50.0/255.0);
	 * gl.glCallList(ellipseList); gl.glPopMatrix(); } pgl.endGL();
	 */
	...
	public void textFont(PFont which) {
		...
		if (textMode == SHAPE) {
			if (textFont.findFont() == null) {
				showWarning("Cannot use " + which.name
						+ " as with textMode(SHAPE) "
	...
	// super.textSize(size);
	/*
	 * // derive the font just in case the user is gonna call // textMode(SHAPE)
	 * afterwards if (textFontNative != null) { textFontNative =
	 * textFontNative.deriveFont(size); Graphics2D graphics = (Graphics2D)
	 * parent.getGraphics(); graphics.setFont(textFontNative);
	 * 
	 * // get the metrics info textFontNativeMetrics =
	 * graphics.getFontMetrics(textFontNative); }
	 */
	// }
	...
	protected float textWidthImpl(char buffer[], int start, int stop) {
		...
		/*
		 * // maybe should use one of the newer/fancier functions for this? int
		 * length = stop - start; return
		 * textFontNativeMetrics.charsWidth(buffer, start, length);
		 */
		Graphics2D graphics = (Graphics2D) parent.getGraphics();
		// otherwise smaller sizes will be totally crapped up
		// seems to need to be before the getFRC, but after the
		// canvas.getGraphics
		// (placing this inside textSize(), even though it was called, wasn't
		// working)
		...
		/*
		 * if (start == 0 && stop == buffer.length) { gv =
		 * textFontNative.createGlyphVector(frc, buffer); } else { char[] fellas
		 * = PApplet.subset(buffer, start, length); gv =
		 * textFontNative.createGlyphVector(frc, fellas); }
		 */
	...
	/**
	 * There must be a better way to do this, but I'm having a brain fart with
	 * all the inner class crap. Fix it later once this stuff is debugged.
	 * <p/>
	 * The method "void vertex(float $1, float $2, float $3);" contained in the
	 * enclosing type "processing.core.PGraphics3" is a perfect match for this
	 * method call. However, it is not visible in this nested class because a
	 * method with the same name in an intervening class is hiding it.
	 */
	/*
	 * public void vertexRedirect(float x, float y, float z) { vertex(x, y, z);
	 * }
	 */
	...
	public class TessCallback extends GLUtessellatorCallbackAdapter {
		...
		public void vertex(Object data) {
			if (data instanceof double[]) {
				...
				PGraphicsOpenGL.this.vertex((float) d[0], (float) d[1],
						(float) d[2]);
				/*
				 * if (d.length == 6) { double[] d2 = {d[0], d[1], d[2]};
				 * gl.glVertex3dv(d2); d2 = new double[]{d[3], d[4], d[5]};
				 * gl.glColor3dv(d2); } else if (d.length == 3) {
				 * gl.glVertex3dv(d); }
				 */
			} else {
	...
	private void glLightAmbient(int num) {
		...
		gl.glLightfv(GL.GL_LIGHT0 + num, GL.GL_AMBIENT, lightDiffuse[num], 0);
	}
	...
	private void glLightNoAmbient(int num) {
		...
		gl.glLightfv(GL.GL_LIGHT0 + num, GL.GL_AMBIENT, zeroBuffer);
	}
	...
	private void glLightDiffuse(int num) {
		...
		gl.glLightfv(GL.GL_LIGHT0 + num, GL.GL_DIFFUSE, lightDiffuse[num], 0);
	}
	...
	private void glLightDirection(int num) {
		...
		if (lightType[num] == DIRECTIONAL) {
			...
			// it seems a bit "hack"
			gl.glLightfv(GL.GL_LIGHT0 + num, GL.GL_POSITION, lightNormal[num]
					.array(), 0);
		} else { // spotlight
	...
	private void glLightPosition(int num) {
		gl.glLightfv(GL.GL_LIGHT0 + num, GL.GL_POSITION, lightPosition[num]
				.array(), 0);
	}
	...
	/**
	 * Convert ARGB (Java/Processing) data to native OpenGL format. This
	 * function leaves alone (ignores) the alpha component. Also flips the image
	 * vertically, since images are upside-down in GL.
	 */
		...
		for (int y = 0; y < height / 2; y++) {
			if (BIG_ENDIAN) {
				...
				for (int x = 0; x < image.width; x++) {
					 ...
					 * ((pixels[yindex] << 8) & 0xffffff00); pixels[yindex] =
					 * ((temp >> 24) & 0xff) | ((temp << 8) & 0xffffff00);
					 */
	...
	/**
	 * Convert Java ARGB to native OpenGL format. Also flips the image
	 * vertically, since images are upside-down in GL.
	 */
	...
	/**
	 * TODO - extremely slow and not optimized. Currently calls a beginPixels()
	 * on the whole canvas, then does the copy, then it calls endPixels().
	 */
