...
public class CircularLayoutAlgorithm extends AbstractGraphPartition
{
	...
	{		
		...
		System.out.println ("Na prvom krugu ima " + bc[compIndex].length + " cvorova, a na drugom " + rnc + 
				" cvorova kojima je potrebno " + outerNodesCount + " mesta.");
		...
		System.out.println(bc[compIndex].length);
		...
		for (int i = 0; i < bc[compIndex].length; i++)
		{
			...
			//System.out.println("cvor: " + bc[compIndex][i] + ", i: " + i);
			
			if (i == bc[compIndex].length - 1)
			{
				System.out.println("idealPos: " + ((int) ((theAngle / outerDeltaAngle) - noOfNeighbours / 2.0)) + 
						 ", theAngle: " + theAngle + ", noOfNeighbours: " + noOfNeighbours + ", startPos: " + startPos
			...
			while (iter.hasNext())
			{
				...
				if (!posSet[currentNeighbour])
				{
					...
					System.out.print(" rupa: " +  holeDepth);
					for (int j = 0; j < (holeDepth / 2); j++)
					{
						System.out.print ("rupa ");
						outerPositionsOwners[(startPos) % outerPositionsOwners.length] = -3; // free but it must not be used (add. space for tree-like struct.)
	...
	private void DFSSetPos (int nodeID, double theAngle, double theRadius)
	{
		...
		{			
			...
			if (neighboursCount > 2)
			{
			//	System.out.println("Evo ga, ima " + neighboursCount + " suseda, min2 = " + min2 + ", r = " + r + ", theAngle = " + theAngle);
				...
				if (!posSet[currentNeighbour]  && !tmp.containsKey(new Integer(currentNeighbour)))
				{
					...
					{			
						...
						if ((Math.abs(startAngle - (remStartAngle - (neighboursCount / 2) * deltaAngle)) < 0.0001 || 
								...
								&& neighboursCount > 2)
						{
							System.out.print("preskacem srednju poziciju! ");
							startAngle -= deltaAngle;
							if (startAngle < 0)
								startAngle += 2 * Math.PI;
						}
