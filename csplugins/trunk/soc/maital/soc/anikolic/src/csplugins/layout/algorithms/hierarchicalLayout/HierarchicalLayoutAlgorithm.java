...
public class HierarchicalLayoutAlgorithm extends AbstractLayout {
		...
		*/
		...
		for (nodeIndex = 0; nodeIndex < numLayoutNodes; nodeIndex++) {
			...
			if (lastComponent != currentComponent) 
			{
				...
				int maxX = HorizontalNodePositioning(nodeIndex - flowLayoutOrder[nodeIndex - 1].componentSize, nodeIndex - 1, flowLayoutOrder, 
						graph);
	...
	private int HorizontalNodePositioning(int startInd, int endInd, HierarchyFlowLayoutOrderNode[] nodes, Graph theGraph)
	{
		...
		{
			int idealPosX = 0, neighsCount = 0;
			Iterator iter = edgesFrom[nodes[x].graphIndex].iterator();
			while (iter.hasNext())
			{
				Integer neigh = (Integer) iter.next();
				if (nodes2HFLON.get(neigh).layer == nodes[x].layer + 1 
					|| nodes2HFLON.get(neigh).layer == nodes[x].layer - 1)
				{
					idealPosX += nodes2HFLON.get(neigh).xPos;
					neighsCount++;
				}
			}			
			if (neighsCount > 0)
			{
				idealPosX /= neighsCount;
				if (idealPosX > nodes[x].xPos)
				...
				if ((idealPosX > nodes[x].xPos) && (x < endInd) && (nodes[x + 1].layer == nodes[x].layer)
						&& (nodes[x + 1].xPos < idealPosX + nodeHorizontalSpacing))
				{
					for (int i = x + 1; i <= endInd; i++)
						if (nodes[i].layer == nodes[x].layer)
							nodes[i].xPos += idealPosX - nodes[x + 1].xPos + nodeHorizontalSpacing;
						else 
						{
							if (nodes[i-1].xPos > maxX)
								maxX = nodes[i-1].xPos;
							break;
						}
					
				}
				else if ((idealPosX < nodes[x].xPos) && (x > 0) && (nodes[x - 1].layer == nodes[x].layer) 
						&& (nodes[x - 1].xPos > idealPosX + nodeHorizontalSpacing))
				{
					for (int i = x - 1; i >= 0; i--)
						if (nodes[i].layer == nodes[x].layer)
							nodes[i].xPos -= nodes[x - 1].xPos - idealPosX + nodeHorizontalSpacing;
						else break;
					
				}
				nodes[x].xPos = idealPosX;
				if (nodes[x].xPos > maxX)
					maxX = nodes[x].xPos;
			}
		}
		return maxX;
