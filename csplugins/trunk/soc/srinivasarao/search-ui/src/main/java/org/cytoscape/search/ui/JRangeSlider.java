 ...
 * @author jeffrey heer
 */
public class JRangeSlider extends JComponent implements MouseListener,
	 ...
	 */
	public JRangeSlider(int minimum, int maximum, int lowValue, int highValue,
			int orientation) {
		this(
				new DefaultBoundedRangeModel(lowValue, highValue, minimum,
	...
	public void setLowValue(int lowValue) {
		...
		model.setRangeProperties(lowValue, e, model.getMinimum(), model
				.getMaximum(), false);
	...
	public void setRange(int lowValue, int highValue) {
		model.setRangeProperties(lowValue, highValue - lowValue, model
	...
	public void paintComponent(Graphics g) {
		...
		if (empty) {
			if (direction == LEFTRIGHT_TOPBOTTOM) {
				min = ARROW_SZ;
				max = (orientation == VERTICAL) ? (height - ARROW_SZ)
						: (width - ARROW_SZ);
			} else {
				min = (orientation == VERTICAL) ? (height - ARROW_SZ)
						: (width - ARROW_SZ);
				max = ARROW_SZ;
			}
		}
		...
		if (orientation == VERTICAL) {
			if (direction == LEFTRIGHT_TOPBOTTOM) {
				...
				paintArrow(g2, (width - ARROW_WIDTH) / 2.0, min - ARROW_SZ
						+ ((ARROW_SZ - ARROW_HEIGHT) / 2.0), ARROW_WIDTH,
						ARROW_HEIGHT, true);
				paintArrow(g2, (width - ARROW_WIDTH) / 2.0, max
						+ ((ARROW_SZ - ARROW_HEIGHT) / 2.0), ARROW_WIDTH,
						ARROW_HEIGHT, false);
			} else {
				g2.setColor(getForeground());
				...
				// Draw arrows
				g2.setColor(Color.black);
				paintArrow(g2, (width - ARROW_WIDTH) / 2.0, min
						+ ((ARROW_SZ - ARROW_HEIGHT) / 2.0), ARROW_WIDTH,
						ARROW_HEIGHT, false);
			...
			if (direction == LEFTRIGHT_TOPBOTTOM) {
				...
				// Draw arrows
				g2.setColor(Color.black);
				paintArrow(g2, min - ARROW_SZ
						+ ((ARROW_SZ - ARROW_HEIGHT) / 2.0),
						(height - ARROW_WIDTH) / 2.0, ARROW_HEIGHT,
						ARROW_WIDTH, true);
		...
		if (orientation == VERTICAL) {
			...
			if (topDown) {
				for (int i = 0; i < ((w / 2) + 1); i++) {
					g2.drawLine(intX + i, intY + i, (intX + w) - i - 1, intY
							+ i);
				}
			} else {
				for (int i = 0; i < ((w / 2) + 1); i++) {
					g2.drawLine((intX + (w / 2)) - i, intY + i, ((intX + w)
							- (w / 2) + i) - 1, intY + i);
				}
			}
		} else {
			...
			if (topDown) {
				for (int i = 0; i < ((h / 2) + 1); i++) {
					g2.drawLine(intX + i, intY + i, intX + i, (intY + h) - i
							- 1);
				}
			} else {
				for (int i = 0; i < ((h / 2) + 1); i++) {
	 ...
	 */
	protected void paint3DRectLighting(Graphics2D g2, int x, int y, int width,
			int height) {
	...
	protected int toLocal(int xOrY) {
		...
		if (orientation == VERTICAL) {
			scale = (sz.height - (2 * ARROW_SZ))
					/ (double) (getMaximum() - min);
		} else {
	...
	protected double toScreenDouble(int xOrY) {
		...
		if (orientation == VERTICAL) {
			scale = (sz.height - (2 * ARROW_SZ))
					/ (double) ((getMaximum() + 1) - min);
		} else {
			scale = (sz.width - (2 * ARROW_SZ))
					/ (double) ((getMaximum() + 1) - min);
		}
	...
	private void grow(int increment) {
		model.setRangeProperties(model.getValue() - increment, model
				.getExtent()
				+ (2 * increment), model.getMinimum(), model.getMaximum(),
				false);
	}
