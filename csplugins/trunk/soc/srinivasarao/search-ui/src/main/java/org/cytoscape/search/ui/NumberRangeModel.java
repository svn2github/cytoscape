 ...
 */
public class NumberRangeModel extends DefaultBoundedRangeModel implements
		ValuedRangeModel {
	...
	public NumberRangeModel(int lo, int hi, int min, int max) {
		this(new Integer(lo), new Integer(hi), new Integer(min), new Integer(
				max));
	}
	...
	protected void updateRange() {
		if (m_type == int.class) {
			setRange(m_lo.intValue(), m_hi.intValue() - m_lo.intValue(), m_min
					.intValue(), m_max.intValue());
		} else if (m_type == long.class) {
			int v = 10000 * (int) ((m_lo.longValue() - m_min.longValue()) / (m_max
					.longValue() - m_min.longValue()));
			int e = (10000 * (int) ((m_hi.longValue() - m_min.longValue()) / (m_max
					.longValue() - m_min.longValue())))
					- v;
			setRange(v, e, 0, 10000);
		} else {
			int v = (int) Math.round(10000 * ((m_lo.doubleValue() - m_min
					.doubleValue()) / (m_max.doubleValue() - m_min
					.doubleValue())));
			int e = (int) Math.round(10000 * ((m_hi.doubleValue() - m_min
					.doubleValue()) / (m_max.doubleValue() - m_min
					.doubleValue())))
					- v;
	...
	protected Object value(int val) {
		...
		if ((m_type == double.class) || (m_type == float.class)) {
			...
			long v = m
					+ (((val - min) * (m_max.longValue() - m)) / (max - min));
