...
public class AttributeManager {
	 ...
	 */
	public void setValuesAttribute(String setName, String[] criteriaLabelColor){
		...
		for(int i=0; i<nodesList.size(); i++){
			...
			for(int j=0; j<labels.length; j++){
				if(!(nodeAttributes.hasAttribute(nodeID, labels[j]))){
					throw new Exception("Node Attribute for node "+nodeID +" at " + labels[j] + " has not been calculated");
				}
