...
public class BooleanSettingsDialog extends JDialog implements ActionListener, FocusListener, ListSelectionListener,
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (command.equals("add")){
			...
			String[] checkDuplicate = Cytoscape.getNodeAttributes().getAttributeNames();
			if(allCurrentLabels.contains(label)){ 
				JOptionPane.showMessageDialog(new JPanel(), "Label: "+label+" already exists.");
			    return;
			}else{
				allCurrentLabels.add(label);
			}
			
			for(int i=0; i<checkDuplicate.length; i++){
				if(checkDuplicate[i].equals(label)){
				    JOptionPane.showMessageDialog(new JPanel(), "Label: "+label+" already exists.");
					return;
				}
			}
		...
		if(command.equals("listChanged")){
			...
			if(value.equals("New...")){
				...
				if(saveFlag){ saveSettings(value); saveFlag = false; }
				else { loadSettings(value); }
			}
		}
		
		if(command.equals("mapToBoxChanged")){
	...
	public void applyCriteria(){
		...
		if(labels.size() == 1){
			colorMapper.createDiscreteMapping(labelsA[0]+"_discrete", labelsA[0], colorsA[0], mapTo);
		}else{
			colorMapper.createCompositeMapping(compositeLabel+"_discrete", compositeLabel, colorsA, mapTo);
		}
