...
float3 * radixMedianD(float3 * a, float3 * b, int axis,int k, unsigned int bit,
		unsigned int * data_out, unsigned int * d_temp_addr_uint, float3 * d_out,CUDPPScanConfig * config,unsigned int * nD){
	...
	if(a==b)
		...
		return s;	
		if( ns > k )
			return radixMedianD(a,s,axis,k,bit>>1,data_out,d_temp_addr_uint,d_out,config,nD);
		else
			return radixMedianD(s+1,b,axis,k-ns,bit>>1,data_out,d_temp_addr_uint,d_out,config,nD);
	}
}
...
void constructD(float3 * NodePos1, float3 * NodePos2, kdNodeInt * n,kdNodeFloat * nf,
			   ...
			   unsigned int * data_out, unsigned int * d_temp_addr_uint, float3 * d_out,CUDPPScanConfig * config,unsigned int * nD,complexDevice * OuterD){
	...
	float3 * mid = radixMedianD(NodePos1,NodePos2,a,(int)((NodePos2-NodePos1)/2)+1,bit,data_out,d_temp_addr_uint,d_out,config,nD);
	...
	constructD(NodePos1,mid-1,n,nf,c,1-a,d1,u1,l1,r1,limit,data_out, d_temp_addr_uint, d_out,config,nD,OuterD);
	constructD(mid,NodePos2 ,n,nf,c2,1-a,d2,u2,l2,r2,limit,data_out, d_temp_addr_uint, d_out,config,nD,OuterD);	 
}