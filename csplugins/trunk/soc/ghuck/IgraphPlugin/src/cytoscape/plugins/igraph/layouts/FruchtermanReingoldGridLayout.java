...
public class FruchtermanReingoldGridLayout extends AbstractIgraphLayout {
    ...
    private double  cellSizeCoefficient   = 1.0;   // The size of the grid cells, in which repulsions are taken into account
	...
	layoutProperties.add(new Tunable("cellSize",
					 "The size of the grid cells in which node repulsions are computed.",
					 Tunable.DOUBLE, 
	...
	if (supportWeights)
	    return "Igraph Edge-Weighted Fruchterman-Reingold Layout (for large graphs)";
	else
	...
	if (supportWeights)
	    return "Edge-Weighted Fruchterman-Reingold Layout (for large graphs)";
	else
