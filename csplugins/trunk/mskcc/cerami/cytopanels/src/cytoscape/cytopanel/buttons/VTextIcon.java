 ...
 *
 * @author Lee Ann Rucker
 */
public class VTextIcon implements Icon, PropertyChangeListener {
    String fLabel;
    // for efficiency, break the fLabel into one-char strings to be
    // passed to drawString
    String[] fCharStrings;
    // Roman characters should be centered when not rotated
     ...
     * Creates a <code>VTextIcon</code> for the specified <code>component</code>
     * with the specified <code>label</code>.
     * It sets the orientation to the provided value if it's legal for
     * the string
     *
     * @see #verifyRotation
     */
...
// Make sure the current rotation is still legal
    ...
    void calcDimensions() {
        ...
        if (fRotation == ROTATE_NONE) {
            ...
            // and height is the font height * the char count, + one
            // extra leading at the bottom
            ...
            for (int i = 0; i < fCharStrings.length; i++) {
                ...
                switch (fPosition[i]) {
                    case POSITION_NORMAL:
                        // Roman fonts should be centered. Japanese fonts
                        // are always monospaced.
                    ...
                    case POSITION_TOP_RIGHT:
                        ...
                        tweak = fCharHeight / 3;
                        g.drawString(fCharStrings[i], x + (tweak / 2),
                                yPos - tweak);
                        break;
     ...
     * ROTATE_RIGHT)
     * <p/>
     * This is public static so you can use it to test a string without
     * creating a VTextIcon
     * <p/>
     ...
     * <p/>
     * Rotation rules
     * - Roman can rotate left, right, or none - default right
     *  (counterclockwise)
     * - CJK can't rotate
     * - Arabic must rotate - default left (clockwise)
     * <p/>
     * from the online edition of _The Unicode Standard, Version 3.0_,
     * file ch10.pdf page 4
     ...
     * for vertical text nor provide directionality controls designed to
    ...
    // The small kana characters and Japanese punctuation that draw in
