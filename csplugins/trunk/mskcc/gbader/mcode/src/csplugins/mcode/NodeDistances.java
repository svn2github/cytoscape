...
public class NodeDistances implements MonitorableTask {
	...
	protected HashMap nodeIndexToMatrixIndexMap; // a root node index to matrix
	 ...
	 * 
	 * @deprecated Can't use this now that GraphPerspective uses root graph
	 *             indices.
	 * 
	 * @param nodes_list
	 *            a <code>List</code> of <code>giny.model.Node</code> objects
	 *            for which distances will be calculated
	 * @param distances
	 *            the 2 dimensional array that will hold the calculated
	 *            distances (possibly null)
	 * @param perspective
	 *            the <code>giny.model.GraphPerspective</code> in which the
	 *            nodes reside
	 */
	public NodeDistances(List nodes_list, int[][] distances,
			GraphPerspective perspective) {
		this(nodes_list, distances, perspective, false);
	}// NodeDistances

	/**
	 * Constructor, specifies whether a the graph should be treated as a
	 ...
	 *            nodes reside
	 * @param directed
	 *            if <code>true</code>, then APSP is calculated assuming a
	 *            directed graph, if <code>false</code> an undirected graph is
	 *            assumed
	 */
	public NodeDistances(List nodes_list, int[][] distances,
			GraphPerspective perspective, boolean directed) {
	 ...
	 *            matrix indices
	 */
	public NodeDistances(List nodesList, GraphPerspective perspective,
			HashMap nodeIndexToMatrixIndexMap) {
	 ...
	 *         <code>MonitorableSwingWorker.stop()</code>, <code>false</code>
	 *         otherwise
	 */
	...
	public int[][] calculate() {
		...
		for (int i = 0; i < nodes.length; i++) {
			...
			index = ((Integer) nodeIndexToMatrixIndexMap.get(new Integer(
					from_node.getRootGraphIndex()))).intValue();
...
//		Node neighbor;
		...
		for (int from_node_index = 0; from_node_index < nodes.length; from_node_index++) {
			...
			while (!(queue.isEmpty())) {
				...
				for(Node neighbor: neighbors) {
					...
					neighbor_index = ((Integer) nodeIndexToMatrixIndexMap
							.get(new Integer(neighbor.getRootGraphIndex())))
							.intValue();
	...
	private Collection<Node> getNeighbors(Node node) {
		...
		final int[] adjArray = perspective.getAdjacentEdgeIndicesArray(node
				.getRootGraphIndex(), true, true, true);
		
		if(adjArray == null || adjArray.length == 0) return result;
		
		int targetIndex = node.getRootGraphIndex();
		Edge curEdge;
		for(int i=0; i<adjArray.length; i++) {
			curEdge = Cytoscape.getRootGraph().getEdge(adjArray[i]);
			if(curEdge.getSource().getRootGraphIndex() != targetIndex) {
				result.add(curEdge.getSource());
				continue;
			}
			
			if(curEdge.getTarget().getRootGraphIndex() != targetIndex)
