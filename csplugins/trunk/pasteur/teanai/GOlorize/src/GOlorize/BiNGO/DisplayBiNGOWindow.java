...
public class DisplayBiNGOWindow{
	...
	private String clusterName ;
  	/** the SettingsSavePanel for choosing the directory for the data file.*/
	...
	public void buildNodeAttributes(CyNetwork network){
   	...
   	Iterator i = network.nodesIterator();
	 	while (i.hasNext()){
			...
			network.setNodeAttributeValue(node, "pValue_" + clusterName, pValue);
			network.setNodeAttributeValue(node, "adjustedPValue_" + clusterName, adj_pValue);
			network.setNodeAttributeValue(node, "x_"+ clusterName, smallX);
			network.setNodeAttributeValue(node, "X_"+ clusterName, bigX);
			network.setNodeAttributeValue(node, "n_"+ clusterName, smallN);
			network.setNodeAttributeValue(node, "N_"+ clusterName, bigN);
			network.setNodeAttributeValue(node, "description_"+ clusterName, description);	
			network.setNodeAttributeValue(node, "nodeFillColor_"+ clusterName, color);
			network.setNodeAttributeValue(node, "nodeSize_"+ clusterName, size );
			network.setNodeAttributeValue(node, "nodeType_"+ clusterName, shape );		
		}
	}
	...
	public void buildEdgeAttributes(CyNetwork network){
		Iterator i = network.edgesIterator();
	 	while (i.hasNext()){
			...
			network.setEdgeAttributeValue(edge, "edgeType_"+ clusterName, "black");
		}
	}
