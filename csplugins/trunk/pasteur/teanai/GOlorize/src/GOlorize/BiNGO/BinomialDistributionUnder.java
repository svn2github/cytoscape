...
public class BinomialDistributionUnder  {
    ...
    public String calculateBinomialDistribution(){
		...
		BigDecimal nByN = (new BigDecimal(n)).divide(new BigDecimal(bigN),
						 							SCALE_RESULT,
		...
		BigDecimal n_pow_i ;
		BigDecimal N_pow_i ;
		BigDecimal N_min_n_pow_X_min_i ;
		BigDecimal N_pow_X_min_i ;
		
		
		for (int i = 0; i <= x ; i++){
						
	    	c_X_i = bi.calculateBinomialCoefficients(bigX, i);
			n_pow_i = new BigDecimal((new BigInteger(new String(n + ""))).pow(i)) ;
			N_pow_i = new BigDecimal((new BigInteger(new String(bigN + ""))).pow(i)) ;
			N_min_n_pow_X_min_i = new BigDecimal((new BigInteger(new String(bigN-n + ""))).pow(bigX-i)) ;
			N_pow_X_min_i = new BigDecimal((new BigInteger(new String(bigN + ""))).pow(bigX-i)) ;
	    	
			BigDecimal n_div_N_i = n_pow_i.divide(N_pow_i,
						 							SCALE_RESULT,
						 							BigDecimal.ROUND_HALF_UP);	    	
	    	
	    	BigDecimal one_min_n_div_N_pow_X_min_i = N_min_n_pow_X_min_i.divide(N_pow_X_min_i,
						 							SCALE_RESULT,
						 							BigDecimal.ROUND_HALF_UP);	
	    	
			BigDecimal product = new BigDecimal(c_X_i).setScale(SCALE_RESULT,BigDecimal.ROUND_HALF_UP);
	    	product = product.multiply(n_div_N_i).setScale(SCALE_RESULT,BigDecimal.ROUND_HALF_UP);
	    	product = product.multiply(one_min_n_div_N_pow_X_min_i).setScale(SCALE_RESULT,BigDecimal.ROUND_HALF_UP);
	    	sum = sum.add(product).setScale(SCALE_RESULT,BigDecimal.ROUND_HALF_UP);
		}
