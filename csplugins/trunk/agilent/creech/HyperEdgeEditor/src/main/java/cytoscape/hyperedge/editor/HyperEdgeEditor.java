...
public class HyperEdgeEditor extends DefaultCytoscapeEditor {
    ...
    public void buildVisualStyle() {
        ...
        BioChemicalReactionVisualStyle.getVisualStyle().setupVisualStyle(true, false);
    ...
    protected void generateEdgePaletteEntries(String controllingAttribute) {
        ...
        //                                                                            this,
        //                                                                            null);
        Iterator<ShapePaletteInfo> spEntries = palGen
                .buildShapePaletteInfo(eac, new VisualPropertyType[] {
        ...
        if (!spEntries.hasNext()) {
            getShapePalette().addShape(controllingAttribute, "DirectedEdge",
            // MLC 05/10/07:
            ...
            while (spEntries.hasNext()) {
                ...
                getShapePalette()
                        .addShape(spi.getControllingAttributeName(),
                                  spi.getKey(),
                                  // MLC 05/10/07:
                                  ...
                                  // MLC 10/20/07:
                                  new CytoShapeIcon(arrowShapeToArrow((ArrowShape) spi
                                          .getValue(VisualPropertyType.EDGE_TGTARROW_SHAPE))),
    ...
    protected void generateNodePaletteEntries(String controllingAttribute) {
        ...
        VisualPropertyType[] calcsToUse = new VisualPropertyType[] {
                VisualPropertyType.NODE_FILL_COLOR, VisualPropertyType.NODE_SHAPE,
                VisualPropertyType.NODE_SIZE };
        // MLC 05/10/07 END.
        Iterator<ShapePaletteInfo> spEntries = palGen
                .buildShapePaletteInfo(nac,
                                       calcsToUse,
                                       controllingAttribute,
                                       this,
        ...
        if (!spEntries.hasNext()) {
            getShapePalette().addShape(controllingAttribute,
                                       ...
                                       new CytoShapeIcon((NodeShape) (nac.getDefaultAppearance()
                                               .get(VisualPropertyType.NODE_SHAPE)), (Color) (nac
            ...
            while (spEntries.hasNext()) {
                ...
                int nodeSize = (int) ((Double) spi.getValue(VisualPropertyType.NODE_SIZE))
                        .longValue();
                // MLC 05/10/07 END.
                getShapePalette().addShape(spi.getControllingAttributeName(),
                                           spi.getKey(),
                                           new CytoShapeIcon(nodeShape, nodeColor, new Dimension(
                                                   nodeSize, nodeSize)),
                                           spi.getKey(),
                                           null);
            }
    ...
    private void generateHyperEdgePaletteEntries() {
        ...
        // MLC 1/06/07:
        ImageIcon icon = MiscGUI
                .loadImageIcon("addReaction.png",
                               null,
                               IMAGE_REL_LOC,
    ...
    public void determineAction(DGraphView view, String attributeValue, Point2D location) {
    ...
    public CyEdge addEdge(Node node_1,
                          Node node_2,
                          String attribute,
        ...
        if (!create || !(attribute_value instanceof String)) {
            ...
            return super.addEdge(node_1, node_2, attribute, attribute_value, create, edgeType);
        }
        ...
        // MLC 05/23/11:
        try {
            // We need to turn off Cytoscape logging for HyperEdge so that execeptions thrown are
            // not logged by Cytoscape: 
            if ((sourceHE != null) && (targetHE != null)) {
    ...
    private void addReaction(DGraphView netView, Point2D location) {
        ...
        HyperEdge he = _factory.createHyperEdge(s1,
                                                EdgeTypeMap.SUBSTRATE,
                                                m1,
                                                EdgeTypeMap.ACTIVATING_MEDIATOR,
                                                p1,
    ...
    private void addMediator(DGraphView view, Point2D location, String mediatorType) {
        ...
        addAndPositionMediator(he, cNodeView, cNodeView.getGraphView(), mediatorType);
    ...
    private void addProduct(DGraphView view, Point2D location) {
        ...
        //        Cytoscape.firePropertyChange(Cytoscape.NETWORK_MODIFIED,
        //            CytoscapeEditorManager.CYTOSCAPE_EDITOR, net);
        addAndPositionProduct(he, cNodeView, cNodeView.getGraphView());
        ((CyNetworkView) view).redrawGraph(true, true);
    }
    ...
    private void addSubstrate(DGraphView view, Point2D location) {
        ...
        addAndPositionSubstrate(he, cNodeView, cNodeView.getGraphView());
        ((CyNetworkView) view).redrawGraph(true, true);
    }
    ...
    public void convertEdgeIntoHyperEdge(Point2D location,
                                         CyEdge edge,
                                         CyNode addedNode,
        ...
        if ((sourceHe != null) && (!sourceHe.hasEdge(edge) || !sourceHe.inNetwork(net))) {
            sourceHe = null;
        }
        if ((targetHe != null) && (!targetHe.hasEdge(edge) || !targetHe.inNetwork(net))) {
            targetHe = null;
        }
        ...
        if ((sourceHe == null) && (targetHe == null)) {
            ...
            net.removeEdge(Cytoscape.getRootGraph().getIndex(edge), false);
            ...
            // connectHyperEdges:
            newHe = handleEdgeDropOnHyperEdgeEdge(sourceHe,
                                                  edge,
                                                  addedNodeHe,
                                                  addedNode,
                                                  attributeValue,
            ...
            // make a new hyperedge with 2 edges and connect the third, shared edge via
            // connectHyperEdges:
            newHe = handleEdgeDropOnHyperEdgeEdge(targetHe,
                                                  edge,
                                                  addedNodeHe,
                                                  addedNode,
                                                  attributeValue,
    ...
    // MLC 01/31/07 END.
    public void positionAndSelectHyperEdge(HyperEdge he, CyNetworkView netView, Point2D location) {
        // place the connector node where we clicked on the canvas:
        positionNode(netView, he.getConnectorNode(), location);
        SelectHyperEdgeAction selAct = new SelectHyperEdgeAction(he, netView.getNetwork());
        selAct.actionPerformed(new ActionEvent(this, 1, ""));
    }

    private void addAndPositionSubstrate(HyperEdge he, NodeView cNodeView, GraphView gv) {
        ...
        if (!it.hasNext()) {
            ...
            //         height.
            double newNodeY = adjNode.getYPosition() + (.5 * adjNode.getHeight())
    ...
    private void addAndPositionProduct(HyperEdge he, NodeView cNodeView, GraphView gv) {
        // GraphPerspective gp = gv.getGraphPerspective();
        CyNode p1 = ensureUniqueNode("P");
        // MLC 08/07/06:
        // addAttribute(p1, CytoscapeEditorManager.NODE_TYPE, EdgeTypeMap.PRODUCT);
    ...
    private void addAndPositionMediator(HyperEdge he,
                                        NodeView cNodeView,
                                        GraphView gv,
                                        String mediatorType) {
        CyNode m1 = ensureUniqueNode("M");
        ...
        if (!it.hasNext()) {
            ...
            //         width.
            double newNodeX = adjNode.getXPosition() + (.5 * adjNode.getWidth())
    ...
    // MLC 01/09/07 END.
    private void addEdgeAndToolTips(GraphView netView, HyperEdge he, CyNode node, String edgeIType) {
    ...
    private void addNewHyperEdgeTooltips(HyperEdge he, GraphView netView) {
        ...
        while (nodes.hasNext()) {
            ...
            while (edges.hasNext()) {
                addEdgeToolTip(edges.next(), netView);
            }
        }
    ...
    private void addNodeToolTip(CyNode cn, GraphView netView) {
        ...
        nv.setToolTip(Cytoscape.getNodeAttributes()
                .getStringAttribute(cn.getIdentifier(), HyperEdgeImpl.LABEL_ATTRIBUTE_NAME));
    }
    ...
    private void addEdgeToolTip(CyEdge ce, GraphView netView) {
        ...
        ev.setToolTip(Cytoscape.getEdgeAttributes()
                .getStringAttribute(ce.getIdentifier(), HyperEdgeImpl.LABEL_ATTRIBUTE_NAME));
    }
    ...
    private void selectNodes(NodeView cNodeView, CyNode newNode) {
        CyNetwork net = (CyNetwork) (cNodeView.getGraphView().getGraphPerspective());
        net.unselectAllNodes();
    ...
    private NodeView getConnectorNodeNodeView(Point2D location, DGraphView view) {
        ...
        if (!_manager.isConnectorNode((CyNode) nv.getNode(),
                                      (CyNetwork) (nv.getGraphView().getGraphPerspective()))) {
            return null;
        }
        return nv;
    }

    public void positionHyperEdge(HyperEdge he, Point2D heLocation, GraphView netView) {
        if (heLocation != null) {
            CyNode connectorNode = he.getConnectorNode();
            NodeView connectorNodeView = positionNode(netView, connectorNode, heLocation);
    ...
    private void positionSubstrates(HyperEdge he, NodeView connectorNodeView, GraphView netView) {
        positionLeftOrRight(he, connectorNodeView, netView, EdgeTypeMap.SUBSTRATE, "left");
    }
    ...
    // connector node, separated by EDGE_LENGTH distance.
    private void positionProducts(HyperEdge he, NodeView connectorNodeView, GraphView netView) {
        positionLeftOrRight(he, connectorNodeView, netView, EdgeTypeMap.PRODUCT, "right");
    }
    ...
    // connector node, separated by EDGE_LENGTH distance.
    private void positionMediators(HyperEdge he, NodeView connectorNodeView, GraphView netView) {
    ...
    private void positionLeftOrRight(HyperEdge he,
                                     NodeView connectorNodeView,
                                     GraphView netView,
                                     String edge_i_type,
                                     String direction) {
        double startX;
        ...
        double totalHeightAllNodes = computeTotalNodeHeight(he.getNodes(edge_i_type), netView);
    ...
    private void positionAboveOrBelow(HyperEdge he,
                                      NodeView connectorNodeView,
                                      GraphView netView,
        ...
        findFurthestNodeViewAlongDimension(he.getNodesByEdgeTypes(edgeITypes), netView, false);
    }

    private double computeTotalNodeHeight(Iterator<CyNode> nodeIt, GraphView netView) {
        if (!nodeIt.hasNext()) {
            return 0.0;
        }
        double nodeHeight;
    ...
    private double computeTotalNodeWidth(Iterator<CyNode> nodeIt, GraphView netView) {
        if (!nodeIt.hasNext()) {
            return 0.0;
        }
        double nodeWidth;
    ...
    private class HyperEdgePaletteItemDragCursorSetter implements DragSourceContextCursorSetter {
        public Cursor computeCursor(CyNetworkView netView,
                                    Point netViewLoc,
                                    DragSourceDragEvent dsde) {
            Point2D netViewLoc2D = new Point2D.Float(netViewLoc.x, netViewLoc.y);
            // Now check if we are on a NodeView of a ConnectorNode:
            NodeView nv = getConnectorNodeNodeView(netViewLoc2D, (DGraphView) netView);
            if (nv != null) {
