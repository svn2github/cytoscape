...
public class QuickFindConfigDialog extends JDialog {
     ...
     * Reindex Text.
     */
    private static final String BUTTON_REINDEX_TEXT = "Select";
...
class ReindexQuickFind implements Task {
    ...
    public void run() {
        ...
        BubbleRouterPlugin.setAttributeName(newAttributeKey);
    }
...
class DetermineDistinctValuesTask implements Task {
    ...
    public void run() {
        ...
        String values[] = CyAttributesUtil.getDistinctAttributeValues
                (nodeIterator, nodeAttributes, attributeKey, 50); //allows for 50 values to be sent to bubble router
        if (values != null && values.length > 0) {
            for (int i = 0; i < ((values.length >= 5)? 5 : values.length); i++) { //modified to only display 5
                tableModel.setValueAt(values[i], i, 0);
            }
            parentDialog.enableApplyButton(true);
            
            //APico 9.17.06
            //Append list with "unassigned" value for bubble router            
            Object[] objects = new Object[values.length+1];
             for (int i = 0; i < values.length; i++) {
            	Object o = values[i];
            	objects[i] = o;
            }
            objects[values.length] = "unassigned";
             //Send sample attribute values to bubble router
            BubbleRouterPlugin.setAttributeValues(objects);
