...
public class CytoscapeEditorManagerSupport implements PropertyChangeListener,
		ChangeListener, GraphPerspectiveChangeListener, CytoPanelListener {
	...
	/**
	 * respond to a ChangeEvent, typically this is caused by switching visual
	 * styles
	 */
	...
	public void updateEditorPalette(VisualStyle style) {
		...
		if (editorForStyle != null) {
			...
			// System.out.println("Initializing controls for " +
			// editorForStyle);
	...
	/**
	 * respond to a PropertyChangeEvent. This is typically the Creation or
	 * Modification of a Network or NetworkView. If networkView focus changes,
	 * then bring up the appropriate editor for the enw network view.
	 */
	public void propertyChange(PropertyChangeEvent e) {
		...
		// redraw graph if the network is modified, e.g. by an undoable edit
			...
			if (e.getOldValue() != null) {
				// AJK: 06/19/06 hack that uses OldValue field to indicate that
				// this
				...
				if ((e.getOldValue()
						...
						|| (e.getOldValue()
								.equals("cytoscape.util.UndoManager"))) {
		...
		// AJK: 06/15/06: enable editing once Cytoscape has been initialized
			...
			// AJK: 08/30/06 BEGIN
			// for incremental graph layout
//			CyNetworkView newView = Cytoscape.getCurrentNetworkView(); // added
//																		// 8/17
...
//				cyAnnPanel.setBounds(component.getX(), component.getY(),
//						component.getWidth(), component.getHeight());
//				j.getGlassPane().setBackground(Color.RED);
//				j.getGlassPane().setVisible(true);
//				System.out
//						.println("AP: Glass pane set to: " + j.getGlassPane());
				// AJK: 08/30/06 END
				...
				if (cyEditor == null) {
					...
					if (cyEditor == null) {
						// this would be because no editor has been set yet.
						// Just
						// return
						return;
					} else {
						// at this point there is an editor but it is not
						// assigned
						// to this
						// view
						// this is probably the case if we are loading a
						// network,
						// rather
						// than creating a new one
						// in this case, we need to setup the network view,
						// which
						// sets all
	...
	private void setupUndoableNodeAdditionEdit(CyNetwork net, CyNode node) {
		...
		// AJK: 06/21/06 gevalt, what a hack! store coordinate position on node
		// attributes so
		// that a subsequent redo will restore node to its coordinate position
		CyAttributes nodeAttribs = Cytoscape.getNodeAttributes();
		nodeAttribs.setAttribute(node.getIdentifier(), "X_pos", new Double(
				offset.getX()));
		nodeAttribs.setAttribute(node.getIdentifier(), "Y_pos", new Double(
				offset.getY()));
		...
		CytoscapeEditorManager.addEdit(new AbstractUndoableEdit() {
			...
			public void redo() {
				...
				if (network != null) {
					...
					CyAttributes nodeAttribs = Cytoscape.getNodeAttributes();
					Double xPos = nodeAttribs.getDoubleAttribute(n
							.getIdentifier(), "X_pos");
					Double yPos = nodeAttribs.getDoubleAttribute(n
							.getIdentifier(), "Y_pos");
	...
	public void graphPerspectiveChanged(GraphPerspectiveChangeEvent event) {
		...
		if (nodeChanges || edgeChanges) {
			Cytoscape.firePropertyChange(Cytoscape.NETWORK_MODIFIED,
					CytoscapeEditorManager.CYTOSCAPE_EDITOR, // for
																// distinguishing
																// from batch
																// firing of
																// event
