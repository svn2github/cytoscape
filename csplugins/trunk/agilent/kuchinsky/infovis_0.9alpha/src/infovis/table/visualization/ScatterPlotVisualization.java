...
//    private static final Logger             logger                = Logger.getLogger(ScatterPlotVisualization.class);
    ...
    public void computeShapes(Rectangle2D bounds) {
        ...
        if (xCol.getMinIndex() == -1 || yCol.getMinIndex() == -1) {
            return;
        }
        ...
        double ymax;
        if (xAxisModel == null) {
            xmin = xCol.getDoubleAt(xCol.getMinIndex());
            xmax = xCol.getDoubleAt(xCol.getMaxIndex());
        }
        else {
        ...
        if (yAxisModel == null) {
            ymin = yCol.getDoubleAt(yCol.getMinIndex());
            ymax = yCol.getDoubleAt(yCol.getMaxIndex());
        }
        else {
