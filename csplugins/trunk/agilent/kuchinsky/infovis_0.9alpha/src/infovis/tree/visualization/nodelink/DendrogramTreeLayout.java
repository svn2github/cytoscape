...
public class DendrogramTreeLayout extends NodeLinkTreeLayout {
    protected float                 siblingSeparation = 5;
    ...
    public DoubleColumn createPosColumn(final NodeLinkTreeVisualization tree) {
        posColumn.clear();
            ...
            public void postorder(int node) {
                if (tree.isLeaf(node)) {
                    ...
                    for (RowIterator iter = tree.childrenIterator(node); iter
                            .hasNext();) {
                        int c = iter.nextRow();
                        max = Math.max(max, posColumn.getIntAt(c));
                    }
                    posColumn.set(node, max + 1);
                }
            }
        }, tree.getVisibleRoot());
        return posColumn;
    ...
    public DoubleColumn createPosColumn(
            ...
            public void postorder(int node) {
                if (tree.isLeaf(node)) {
                    ...
                    for (RowIterator iter = tree.childrenIterator(node); iter
                            .hasNext();) {
                        int c = iter.nextRow();
                        max = Math.max(max, posColumn.get(c));
                    }
                    posColumn.set(node, max + 1);
                }
                if (!lengthColumn.isValueUndefined(node)) {
    ...
    public void computeShapes(Rectangle2D bounds, NodeLinkTreeVisualization vis) {
        ...
        if (bounds == null || bounds.getWidth() == 0 || bounds.getHeight() == 0
            ...
            if (o == ORIENTATION_SOUTH || o == ORIENTATION_EAST) {
                origin = posColumn.getDoubleAt(posColumn.getMaxIndex());
                scale = len
                        / (posColumn.getDoubleAt(posColumn.getMinIndex()) - origin);
            }
            else {
                origin = posColumn.getDoubleAt(posColumn.getMinIndex());
                scale = len
                        / (posColumn.getDoubleAt(posColumn.getMaxIndex()) - origin);
            }
        }
        computeSizes();
    ...
    float computeShapes(NodeLinkTreeVisualization vis, int node, float start) {
        float pos = start;
        if (!vis.isLeaf(node)) {
    ...
    public float getLength(int node) {
        Rectangle2D.Float rect = getRectAt(node);
        if (rect == null) {
            return 0;
        }
    ...
    public float computeLength(NodeLinkTreeVisualization vis, int node) {
        if (isLeaf(node)) {
    ...
    public Dimension getPreferredSize(Visualization vis) {
        ...
        try {
            ...
            int depth = Algorithms.treeDepth(
                    visualization, 
                    visualization.getVisibleRoot());
            Dimension d;
            if (Orientation.isHorizontal(getOrientation())) {
                d = new Dimension(
                        (int)(depth*vs.getMaxSize()), 
                        (int) len);
            }
            else {
                d = new Dimension(
