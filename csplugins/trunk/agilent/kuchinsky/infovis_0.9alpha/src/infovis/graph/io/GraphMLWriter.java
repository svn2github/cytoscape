...
public class GraphMLWriter extends AbstractXMLWriter {
    ...
    public String getVertexId(int node) {
        if (vertexIdColumn == null
                || vertexIdColumn.isValueUndefined(node)) {
            return "n" + node;
        } else {
    ...
    protected void writeEdge(int edge) throws SAXException {
        ...
        attrs.addAttribute("", "source", "source", "IDREF",
                getVertexId(graph.getInVertex(edge)));
        attrs.addAttribute("", "target", "tager", "IDREF",
                getVertexId(graph.getOutVertex(edge)));
        ...
        for (int col = 0; edgeLabels != null && col < edgeLabels.size(); col++) {
            ...
            if (!c.isValueUndefined(edge)) {
                indent(writer);
                attrs.clear();
                attrs.addAttribute("", "key", "key", "IDREF", c
                        .getName());
    ...
    protected void writeVertex(int vertex) throws SAXException {
        ...
        depth++;
        for (int col = 0; columnLabels != null
                && col < columnLabels.size(); col++) {
            String label = getColumnLabelAt(col);
            Column c = graph.getVertexTable().getColumn(label);
            if (c == vertexIdColumn) continue;

            if (!c.isValueUndefined(vertex)) {
                indent(writer);
                attrs.clear();
                attrs.addAttribute("", "key", "key", "IDREF",
                        c.getName());
                writer.startElement("", "data", "", attrs);
    ...
    public boolean write() {
        ...
        try {
            ...
            indent(writer);
            if (graph.isDirected()) {
                attrs.addAttribute("", "edgedefault", "edgedefault",
                        "CDATA", "directed");
            }
            else {
                attrs.addAttribute("", "edgedefault", "edgedefault",
