...
public class VCGParser extends antlr.LLkParser       implements VCGTokenTypes
 {
	...
	public int findEdge(int in, int out) {
		for (RowIterator iter = graph.outEdgeIterator(in); iter.hasNext(); ) {
		...
		try {      // for error handling
			...
			do {
				if ((_tokenSet_0.member(LA(1)))) {
					...
					if ( _cnt1033>=1 ) { break _loop1033; } else {throw new NoViableAltException(LT(1), getFilename());}
				}
		...
		try {      // for error handling
			...
			do {
				if ((LA(1)==ID)) {
					...
					if ( _cnt1055>=1 ) { break _loop1055; } else {throw new NoViableAltException(LT(1), getFilename());}
				}
