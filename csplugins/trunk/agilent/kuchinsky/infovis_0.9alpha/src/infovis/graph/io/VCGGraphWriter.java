...
public class VCGGraphWriter extends DOTGraphWriter {
    public static final String []VCGOrientation = {
        "bottom_to_top", // north
        "top_to_bottom", // south
        "left_to_right", // east
        "right_to_left"  // west
    ...
    public VCGGraphWriter(Writer out, Graph graph) {
        super(out, graph);
    }
    ...
    public void writeEdge(int edge) throws IOException {
        write("edge: { ");
        write("sourcename: \""+getVertexId(graph.getInVertex(edge))+"\" ");
        write("targetname: \""+getVertexId(graph.getOutVertex(edge))+"\" ");
        write("label: \""+getEdgeId(edge)+"\" ");
        write("}");
    }
    
    public String getVCGOrientation() {
        return VCGOrientation[orientation]; 
    }
    ...
    public boolean write() {
        try {
            ...
            write("\torientation: "+getVCGOrientation()+"\n");
            ...
            for (RowIterator iter = graph.getVertexTable().iterator();
                ...
                int vertex = iter.nextRow();
                for (RowIterator eiter = graph.outEdgeIterator(vertex);
                    eiter.hasNext();
                    ) {
