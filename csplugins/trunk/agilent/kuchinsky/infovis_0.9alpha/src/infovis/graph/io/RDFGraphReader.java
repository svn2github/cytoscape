 ...
 * 
 * at infovis.factory GraphReaderFactory rdf
 * at infovis.factory GraphReaderFactory owl
 */
...
//        }
    ...
    /*
    public int findVertex(AResource res) {
        String uri;
        if (res.isAnonymous()) {
            uri = res.getAnonymousID();
        }
        else {
            uri = res.getURI();
        }
        
        int v1 = findVertex(uri);
        if (res.isAnonymous()) {
            isAnonymousColumn.setExtend(v1, true);
        }
        return v1;
    }
    
    public void statement(AResource subj, AResource pred, ALiteral lit) {
        int v1 = findVertex(subj);
        String value = lit.toString();
        int v2 = findVertex(value);
        isLiteralColumn.setExtend(v2, true);
        int e = graph.findEdge(v1, v2);
        predColumn.setValueOrNullAt(e, pred.getURI());
    }
    
    public void statement(AResource subj, AResource pred, AResource obj) {
        int v1 = findVertex(subj);
        int v2 = findVertex(obj);
        int e = graph.findEdge(v1, v2);
        predColumn.setValueOrNullAt(e, pred.getURI());
    }
    */
