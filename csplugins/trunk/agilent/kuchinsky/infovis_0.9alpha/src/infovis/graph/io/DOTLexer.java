	...
	public final void mATOM(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
		...
		case 'w':  case 'x':  case 'y':  case 'z':
		{
			...
			do {
				...
				default:
				{
					if ( _cnt919>=1 ) { break _loop919; } else {throw new NoViableAltForCharException((char)LA(1), getFilename(), getLine(), getColumn());}
				}
				}
				_cnt919++;
			} while (true);
		...
		if ((LA(1)=='<')) {
			...
			do {
				if ((_tokenSet_4.member(LA(1))) && ((LA(2) >= '\u0003' && LA(2) <= '\u00ff')) && (true) && (true)) {
					...
					if ( _cnt932>=1 ) { break _loop932; } else {throw new NoViableAltForCharException((char)LA(1), getFilename(), getLine(), getColumn());}
				}
