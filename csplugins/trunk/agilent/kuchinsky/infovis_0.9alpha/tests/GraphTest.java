...
public class GraphTest extends TestCase {
    ...
    protected void checkRemoved(BitSet removed, Graph graph) {
        for (RowIterator iter = graph.edgeIterator(); iter.hasNext(); ) {
            ...
            if (graph.getInVertex(e) < 0) {
                int v1 = graph.getInVertex(e);
                int v2 = graph.getOutVertex(e);
                assertTrue("Edge removed by vertex "+(-v1)+" or "+(-v2),false);
                assertTrue(
                        "Invalid 'in' vertex: "+graph.getInVertex(e),
                        graph.getVertexTable().isRowValid(graph.getInVertex(e)));
            }
            assertTrue(
                    "Invalid 'in' vertex: "+graph.getInVertex(e),
                    graph.getVertexTable().isRowValid(graph.getInVertex(e)));
            assertTrue(
                    "Invalid 'out' vertex: "+graph.getOutVertex(e),
                    graph.getVertexTable().isRowValid(graph.getOutVertex(e)));
            assertTrue(
                    "Edge "+e+" references vertex "+graph.getInVertex(e),
                    removed.get(graph.getInVertex(e)));
            
            assertTrue(
                    "Edge "+e+" references vertex "+graph.getOutVertex(e),
                    removed.get(graph.getOutVertex(e)));
        }
    }
