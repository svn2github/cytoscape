...
public class CytoscapeDesktop extends JFrame implements PropertyChangeListener {
	...
	public CytoscapeDesktop() {
		...
		// MLC 12/08/09 BEGIN:
		// In our simplified version of Cytoscape, we don't
		// want popup menu items on Network nodes and edges
		// for changing the visual style properties of these
		// Nodes and Edges. This is done by not using the
		...
		if (CyMain.isAGMode()) {
		    ...
		    // We need to still set up the cyto panels because
		    // they are used to get and place other components
		    // (e.g., the attribute browser is placed in the
		    // SOUTH cyto panel):
    ...
    // MLC 12/08/09 BEGIN:
    // We need this method because the BirdsEyeViewHandler is not a public class, so we can't use it
    // in another package to get at the view.
    /**
     * Get the BirdsEyeViewer.
     */
