...
public class SVD
{
	...
	public static double[][] eigenVectorsApprox(double[][] X, double percentVariance, int maxPC, int c)
	{
		//DoubleMatrix.centerCols(X);
		
		return eigenVec(RandomSVD.randomMethod(X,c),percentVariance,maxPC);
	}
	...
	private static double[][] eigenVec(SVD svd, double percentVariance, int maxPC)
	{
		...
		double target = utilities.math.VectorOps.sum(svals)*percentVariance;
		...
		//System.out.println(new DoubleVector(svals));
