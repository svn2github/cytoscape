...
public class HCSearch_Runner {
	...
	public static void main(String args[]) {
		...
		if (args.length != 6 && args.length != 7) {
			...
			System.out
					.println("Searches for enriched modules and their connections.");
		...
		// FloatMatrixNetwork pnet = new
		// FloatMatrixNetwork(fpnet,false,false,0,1,2);
		// FloatMatrixNetwork gnet = new
		// FloatMatrixNetwork(fgnet,false,false,0,1,2);
		...
		System.out
				.println("Number of edges remaining in the physical network: "
						+ pnet.numEdges());
		// System.out.println("Number of nodes remaining in the genetic network: "+gnet.numNodes());
		System.out.println("Number of edges remaining in the genetic network: "
				+ gnet.numEdges());
		...
		if (randomize) {
			...
			int taskID = 1;
			try {
				taskID = Integer.valueOf(System.getenv("SGE_TASK_ID"))
						.intValue();
			} catch (Exception e) {
				System.out
						.println("System variable SGE_TASK_ID has not been set. Defaulting to "
								+ taskID + ".");
			}
		...
		TypedLinkNetwork<TypedLinkNodeModule<String, BFEdge>, BFEdge> results = HCSearch2
				.search(pnet, gnet, sfunc);
		...
		Map<String, SNodeModule> genecomplexes = new HashMap<String, SNodeModule>(
				testedgenes.size());
		...
		int i = 0;
		for (TypedLinkNode<TypedLinkNodeModule<String, BFEdge>, BFEdge> mod : results
				.nodeIterator()) {
			...
			// if (!randomize && hm.size()>2)
			// ONetwork.saveModuleToSif(fout+"complex_"+ni+".sif", hm, pnet,
			// gnet);
		...
		System.out.println("Saving properties for " + testedgenes.size()
				+ " genes.");
		i = 0;
		...
		if (results.numEdges() > 0) {
			...
			i = 0;
			for (TypedLinkEdge<TypedLinkNodeModule<String, BFEdge>, BFEdge> edge : results
					.edgeIterator()) {
				...
				int gconnectedness = gn.getConnectedness(hm1.asStringSet(), hm2
						.asStringSet());
				int pconnectedness = pn.getConnectedness(hm1.asStringSet(), hm2
						.asStringSet());
				...
				eprops.set(i, 7, String.valueOf(edge.value().link()
						/ hm1.size() / hm2.size()));

				// if (!randomize && edge.value().link()>0 && hm1.size()>2 &&
				// hm2.size()>2 && gconnectedness>2)
				// ONetwork.saveModuleToSif(fout+"edge_"+n1+","+n2+"_"+gconnectedness+".sif",
				// HyperModule.union(hm1, hm2), pnet, gnet);
				i++;
			}
