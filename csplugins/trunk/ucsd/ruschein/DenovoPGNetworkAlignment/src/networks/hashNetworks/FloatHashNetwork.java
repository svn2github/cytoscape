...
public class FloatHashNetwork extends SFNetwork implements Iterable<SFEdge>
{
	...
	public FloatHashNetwork(String file, boolean selfOk, boolean directed, int n1col, int n2col, int valcol)
	{
		super(selfOk,directed);
	...
	protected void Load(String file, int n1col, int n2col, int valcol)
	{
		...
		for (String line : new FileIterator(file))
		{
			...
			this.add(new UndirectedSFEdge(cols[n1col],cols[n2col], Float.valueOf(cols[valcol])));
		}
	}
	...
	{
		if (this.directed) this.add(new DirectedSFEdge(n1,n2,value));
		else this.add(new UndirectedSFEdge(n1,n2,value));
	}
	...
	public FloatHashNetwork subNetwork(Set<String> nodes)
	{
		...
		FloatHashNetwork subnet = new FloatHashNetwork(this.selfOk, this.directed, this.numEdges());
	...
	{
		TypedLinkNetwork<String,Float> out = new TypedLinkNetwork<String,Float>(this.selfOk,this.directed);
	...
	public FloatHashNetwork shuffleNodes()
	{
		...
		FloatHashNetwork out = new FloatHashNetwork(this.selfOk,this.directed,this.numEdges());
		for (SFEdge i : this)
			out.add(i);
