...
import utilities.math.svd.LinpackSVD;
import utilities.math.svd.SVD;
import data.matrixMath.*;
...
public class DoubleMatrix extends DataMatrix{
	...
	{
		if (j1>j2) throw new java.lang.IllegalArgumentException("j1>j2");
	...
	public double min()
	{
		...
		for (int i=0;i<dim(0);i++)
			for (int j=0;j<dim(1);j++)
				{
					if (!Double.isNaN(get(i,j)) && get(i,j)<mn) mn = get(i,j);
					found = true;
				}
	...
	{
		if (x.length!=y.length || x[0].length!=y[0].length) throw new java.lang.IllegalArgumentException("Data tables must be the same dimensions.");
	...
	public void setCol(int index, DoubleVector vec)
	{
		if (vec.size()!=this.numRows())
			throw new java.lang.IllegalArgumentException("Error setCol(int, DoubleVector): Vector size must equal number of rows. vec="+vec.size()+",rows="+this.numRows());
	...
	public static double[] times(double[][] x, double[] vec)
	{
		if (vec.length!=x[0].length) throw new java.lang.IllegalArgumentException("Dimension mismatch: xcol!=vlength, "+x[0].length+" != "+vec.length);
	...
	{
		DoubleMatrix out = new DoubleMatrix(dm1.numRows()+dm2.numRows(),dm1.numCols());
	...
	public static double[][] corrSymmetric_IgnoreNaN(double[][] m)
	{
		...
		for (int i=0;i<mmm1;i++)
			for (int j=i+1;j<m.length;j++)
			{
				if (count%1000000==0) System.out.println(utilities.text.DataFormatter.printRatio(count, total));
