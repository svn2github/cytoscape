...
public class IntVector extends DataVector {
	...
	public IntVector(String file, boolean arerownames, boolean arecolname, int column)
	{
	...
	public int[] getData()
	{
		...
		else return IntVector.resize(data, this.size);
	}
	...
	{
		if (data.length==0) data = new int[10];
		else if (this.size==data.length)	data = IntVector.resize(data,data.length*2);
	...
	{
		if (data.length<this.size+vals.size()) data = IntVector.resize(data,data.length+vals.size());
	...
	public static double getEmpiricalPvalue(int[] data, double score, boolean upperTail)
	{
	 ...
	 * Replace = false
	 */
	public IntVector sample(int sampleSize, boolean replace, DoubleVector weights)
	{
		if (replace)
		{
	...
	{
		com.google.common.collect.Multiset<Integer> ms = new com.google.common.collect.HashMultiset<Integer>(this.asIntegerList());
	...
	{
		if (this.size()==0) return new IntVector(0);
	...
	public void plot()
	{
		Plotter pl = new Plotter(PlotType.XYLinePlot,this);
		if (this.hasListName()) pl.setTitle(this.listname);
		pl.show();
	}
