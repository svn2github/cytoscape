...
std::string FormatTime(const double time_in_millisecs, const std::string &separator)
{
        if (time_in_millisecs < 0.0)
                throw Exception("in TimeUtil::FormatTime: 'time_in_millisecs' must be non-negative!");
	...
	if (days != 0) {
	       ...
	       output += StringUtil::ToString(days) + 'd';
	...
	if (hours != 0) {
	       show_millis_when_zero = false;
	       output += StringUtil::ToString(hours) + 'h';
	       if (no_of_non_zero_values > 1) {
	               output += separator;
	               --no_of_non_zero_values;
	       }
	...
	if (mins != 0) {
	       show_millis_when_zero = false;
	       output += StringUtil::ToString(mins) + 'm';
	       if (no_of_non_zero_values > 1) {
	               output += separator;
	               --no_of_non_zero_values;
	       }
	...
	if (secs != 0) {
	       show_millis_when_zero = false;
	       output += StringUtil::ToString(secs) + 's';
	       if (no_of_non_zero_values > 1) {
	               output += separator;
	               --no_of_non_zero_values;
	       }
	...
	if (show_millis_when_zero or millis != 0.0) {
	       if (millis == static_cast<unsigned>(millis))
                       output += StringUtil::ToString(static_cast<unsigned>(millis)) + "ms";
               else
                       output += StringUtil::ToString(millis) + "ms";
	}
...
time_t Iso8601StringToTimeT(const std::string &iso_time, const TimeZone time_zone)
{
	...
	if (match_count == 6 and is_definitely_zulu_time) {
		if (time_zone == LOCAL)
			throw Exception("in TimeUtil::Iso8601StringToTimeT: local time requested in Zulu time format!");
		...
		if (the_time == static_cast<time_t>(-1))
			throw Exception("in TimeUtil::Iso8601StringToTimeT: cannot convert '" + iso_time +"' to a time_t!");
	}
...
time_t ConvertHumanDateTimeToTimeT(const std::string &human_date)
{
	...
	// Using function static (initialized once per program) syntax, load up an std::map with time extraction patterns
