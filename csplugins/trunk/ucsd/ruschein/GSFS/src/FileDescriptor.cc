...
FileDescriptor::FileDescriptor(const FileDescriptor &rhs)
{
	if (rhs.fd_ == -1)
		...
		fd_ = ::dup(rhs.fd_);
		if (unlikely(fd_ == -1))
			throw Exception("in FileDescriptor::FileDescriptor: dup(2) failed (" + MsgUtil::ErrnoToString() + ")!");
	}
}
...
const FileDescriptor &FileDescriptor::operator=(const FileDescriptor &rhs)
{
	...
	if (likely(&rhs != this)) {
		...
		fd_ = ::dup(rhs.fd_);
		if (unlikely(fd_ == -1))
			throw Exception("in FileDescriptor::operator=: dup(2) failed (" + MsgUtil::ErrnoToString() + ")!");
	}
