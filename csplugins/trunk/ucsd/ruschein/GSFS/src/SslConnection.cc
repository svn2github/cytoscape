...
ThreadUtil::Mutex SslConnection::mutex_;
...
{
	std::auto_ptr<ThreadUtil::MutexLocker> mutex_locker;
	if (threading_support_mode == SUPPORT_MULTITHREADING)
		mutex_locker.reset(new ThreadUtil::MutexLocker(&SslConnection::mutex_));
	...
	if (unlikely(ssl_connection_ == NULL))
		throw Exception("in SslConnection::SslConnection: ::SSL_new() failed!");

	if (unlikely(not ::SSL_set_fd(ssl_connection_, fd)))
		throw Exception("in SslConnection::SslConnection: ::SSL_set_fd() failed!");
	...
	for (unsigned try_no(0); try_no < NO_OF_TRIES; ++try_no) {
		...
		if (unlikely(ret_val == 0))
			throw Exception("in SslConnection::SslConnection: ::SSL_connect() failed with return value (0)!");
	...
	throw Exception("in SslConnection::SslConnection: ::SSL_connect() failed after " + StringUtil::ToString(NO_OF_TRIES) + " tries!");
}


SslConnection::~SslConnection()
{
	// Several of the following functions called here throw exceptions which could cause an unexpected exception if called while an exception is
	// already in progress, destroying this object. So, don't cleanup if this is the case.
...
void SslConnection::ReleaseContext(const SSL_CTX * const ssl_context)
{
	...
	if (unlikely(matching_context_info == context_infos_.end()))
		throw Exception("in SslConnection::ReleaseContext: can't find context to release!");
...
SSL_CTX *SslConnection::InitClient(const Method method)
{
	...
	case ALL_METHODS:
		...
		break;
	default:
		throw Exception("in SslConnection::InitClient: unknown method!");
	}

	if (unlikely(ssl_method == NULL))
		throw Exception("in SslConnection::InitClient: unexpected NULL SSL method!");
...
SSL_CTX *SslConnection::InitServer(const Method method)
{
	...
	case ALL_METHODS:
		...
		break;
	default:
		throw Exception("in SslConnection::InitServer: unknown method!");
	}

	if (unlikely(ssl_method == NULL))
		throw Exception("in SslConnection::InitServer: unexpected NULL SSL method!");
...
SSL_CTX *SslConnection::InitClientAndServer(const Method method)
{
	...
	case ALL_METHODS:
		...
		break;
	default:
		throw Exception("in SslConnection::InitClientAndServer: unknown method!");
	}

	if (unlikely(ssl_method == NULL))
		throw Exception("in SslConnection::InitClientAndServer: unexpected NULL SSL method!");
