...
{
	static const unsigned char *character_table = NULL;
...
PerlCompatRegExp::PerlCompatRegExp(const std::string &pattern, const ProcessingMode processing_mode, const int options)
	: compiled_pattern_(NULL), extra_pattern_info_(NULL), substring_match_count_(0)
{
...
PerlCompatRegExp::PerlCompatRegExp(const PerlCompatRegExp &rhs)
	: compiled_pattern_(NULL), extra_pattern_info_(NULL), substring_match_count_(rhs.substring_match_count_)
{
...
{
	if (compiled_pattern_ != NULL)
		::pcre_free(compiled_pattern_);
	if (extra_pattern_info_ != NULL)
...
void PerlCompatRegExp::resetPattern(const std::string &new_pattern, const ProcessingMode new_processing_mode, const int new_options)
{
	...
	if (not internalResetPattern(new_pattern, new_processing_mode, new_options, &error_message))
		throw Exception("in PerlCompatRegExp::resetPattern: " + error_message);
}
...
{
	MSG_UTIL_ASSERT(compiled_pattern_ != NULL);

	subject_text_ = subject_text;
	const int match_count = ::pcre_exec(compiled_pattern_, extra_pattern_info_, subject_text_.c_str(), static_cast<int>(subject_text_.length()),
					    static_cast<int>(start_offset), options, offset_vector_, OFFSET_VECTOR_SIZE);
	if (match_count > 1)
...
bool PerlCompatRegExp::Match(const std::string &pattern, const std::string &subject_text, const size_t start_offset, size_t * const start_pos,
			     size_t * const length, const int options)
{
...
bool PerlCompatRegExp::multiMatch(const std::string &subject_text, std::vector<std::string> * const matched_substrings, const int options) const
{
...
bool PerlCompatRegExp::MultiMatch(const std::string &pattern, const std::string &subject_text, std::vector<std::string> * const matched_substrings, const int options)
{
	PerlCompatRegExp perl_compat_reg_exp(pattern, DONT_OPTIMIZE_FOR_MULTIPLE_USE, options);
...
std::string PerlCompatRegExp::GenerateReplacementText(const PerlCompatRegExp &reg_exp, const std::string &replacement)
{
	...
	if (escaped)
		throw Exception("in PerlCompatRegExp::GenerateReplacementText: trailing backslash escape in replacement!");
	if (scanning_reference) {
		std::string matched_substring;
		reg_exp.getMatchedSubstring(substring_reference, &matched_substring);
		replacement_text += matched_substring;
...
bool PerlCompatRegExp::getMatchedSubstring(unsigned index, std::string * const matched_substring) const
{
	if (substring_match_count_ == 0)
		throw Exception("in PerlCompatRegExp::getMatchedSubstring: no matches available!");

	if (index == 0)
		throw Exception("in PerlCompatRegExp::getMatchedSubstring: index out of range (indexes start at 1)!");
...
std::string PerlCompatRegExp::getMatchedSubstring(unsigned index) const
{
	...
	if (index > substring_match_count_)
		throw Exception("in PerlCompatRegExp::getMatchedSubstring: asked for substring beyond available count!");
...
std::string PerlCompatRegExp::Subst(const std::string &subst_expression, const std::string &subject_text, const int options)
{
	if (subst_expression.length() < 3)
		throw Exception("in PerlCompatRegExp::Subst: subst_expression.length() < 3!");
	...
	for (++ch; /* forever */; ++ch) {
		if (ch == subst_expression.end())
			throw Exception("in PerlCompatRegExp::Subst: missing final delimiter!");
		else if (escaped) {
			escaped = false;
			if (scanning_pattern)
				pattern += *ch;
			...
			if (scanning_pattern)
				...
				if (ch != subst_expression.end())
					throw Exception("in PerlCompatRegExp::Subst: unexpected delimiter before end of replacement text!");
...
bool PerlCompatRegExps::matchAny(const std::string &subject_text, const size_t start_offset, size_t * const start_pos, size_t * const length,
				 const int options) const
{
	for (std::list<PerlCompatRegExp>::const_iterator reg_exp(reg_exps_.begin()); reg_exp != reg_exps_.end(); ++reg_exp) {
...
bool PerlCompatRegExps::multiMatch(const std::string &subject_text, std::vector<std::string> * const matched_substrings, const int options) const
{
	matched_substrings->clear();
...
PerlCompatSubst::PerlCompatSubst(const std::string &subst_expr)
	: subst_expr_(subst_expr), global_(false), perl_compat_regexp_(NULL)
{
	if (subst_expr.length() < 3)
		throw Exception("in PerlCompatSubst::PerlCompatSubst: subst_expression.length() < 3!");
	...
	for (++ch; /* forever */; ++ch) {
		if (ch == subst_expr.end())
			throw Exception("in PerlCompatRegExp::PerlCompatRegExp: missing final delimiter!");
		else if (escaped) {
			escaped = false;
			if (scanning_pattern)
				pattern += *ch;
			...
			if (scanning_pattern)
				...
				if (ch != subst_expr.end())
					throw Exception("in PerlCompatSubst::PerlCompatSubst: unexpected delimiter before end of "
							"replacement text!");
...
void PerlCompatSubsts::addSubstExpression(const std::string &new_subst_expression)
{
	try {
		...
		throw Exception("in PerlCompatSubsts::addSubstExpression: \"" + new_subst_expression
