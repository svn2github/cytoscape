		     ...
		     std::string * const error_message, unsigned number_of_retries)
{
	...
	MsgUtil::Error("in SocketUtil::StringToAddress: undefined state.");
	return true;
}
	       ...
	       const ReuseAddrOptionType reuse_addr_option)
{
	...
	if (unlikely(not socket_fd.isValid())) {
		*error_message = "socket(2) failed (" + MsgUtil::ErrnoToString() + ")!";
		return -1;
	}
	...
	if (reuse_addr_option == REUSE_ADDR) {
		...
		if (::setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, &reuse_addr_flag, sizeof(reuse_addr_flag)) != 0) {
			*error_message = "setsockopt(2) failed for SO_REUSEADDR (" + MsgUtil::ErrnoToString()
					 + ")!";
			return -1;
		}
	}
	...
	if (::connect(socket_fd, (struct sockaddr *)&server_address, sizeof(server_address)) != 0) {
		...
		*error_message = "connect(2) failed ("+ MsgUtil::ErrnoToString() + ")!";
		socket_fd = -1;
	}
...
select_again:
	...
	MSG_UTIL_ASSERT(FD_ISSET(socket_fd, &write_set));
...
select_again:
	...
	MSG_UTIL_ASSERT(FD_ISSET(socket_fd, &read_set));
