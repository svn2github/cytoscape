...
ThreadUtil::Mutex *Downloader::dns_mutex_(NULL);
ThreadUtil::Mutex *Downloader::cookie_mutex_(NULL);
ThreadUtil::Mutex *Downloader::robots_dot_txt_mutex_(NULL);
GNU_HASH_MAP<std::string, RobotsDotTxt> Downloader::url_to_robots_dot_txt_map_;
	  ...
	  banned_reg_exps_(banned_reg_exps), debugging_(debugging), follow_redirects_(follow_redirects)
{
	...
	if (unlikely(max_redirect_count_ < 0 or max_redirect_count_ > MAX_MAX_REDIRECT_COUNT))
		throw Exception("in Downloader::Params::Params: max_redirect_count (= " + StringUtil::ToString(max_redirect_count)
...
bool Downloader::newUrl(const Url &url, const TimeLimit &time_limit)
{
	...
	for (;;) {
		...
		if (::curl_easy_setopt(easy_handle_, CURLOPT_MAXREDIRS, getRemainingNoOfRedirects()) != CURLE_OK)
			throw Exception("in Downloader::newUrlWithHttpEquivRedirect: curl_easy_setopt() failed!");
...
void Downloader::init()
{
	...
	Downloader::InitCurlEasyHandle(params_.dns_cache_timeout_, error_buffer_, params_.debugging_, WriteFunction, LockFunction, UnlockFunction,
				       HeaderFunction, DebugFunction, &easy_handle_, &params_.user_agent_, params_.follow_redirects_);
...
void Downloader::InitCurlEasyHandle(const long dns_cache_timeout, const char * const error_buffer, const bool debugging,
				    ...
				    CURL ** const easy_handle, std::string * const user_agent, const bool follow_redirects)
{
	*easy_handle = ::curl_easy_init();
	if (unlikely(*easy_handle == NULL))
		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_init() failed!");

	if (share_handle_ == NULL) {
		share_handle_ = ::curl_share_init( );
		if (unlikely(share_handle_ == NULL))
			throw Exception("in Downloader::InitCurlEasyHandle: curl_share_init() failed!");
		dns_mutex_ = new ThreadUtil::Mutex;
		cookie_mutex_ = new ThreadUtil::Mutex;
		if (unlikely(::curl_share_setopt(share_handle_, CURLSHOPT_LOCKFUNC, lock_func) != 0))
			throw Exception("in Downloader::InitCurlEasyHandle: curl_share_setopt() failed (1)!");
		if (unlikely(::curl_share_setopt(share_handle_, CURLSHOPT_UNLOCKFUNC, unlock_func) != 0))
			throw Exception("in Downloader::InitCurlEasyHandle: curl_share_setopt() failed (2)!");
		if (unlikely(::curl_share_setopt(share_handle_, CURLSHOPT_SHARE, CURL_LOCK_DATA_DNS) != 0))
			throw Exception("in Downloader::InitCurlEasyHandle: curl_share_setopt() failed (3)!");
		if (unlikely(::curl_share_setopt(share_handle_, CURLSHOPT_SHARE, CURL_LOCK_DATA_COOKIE) != 0))
			throw Exception("in Downloader::InitCurlEasyHandle: curl_share_setopt() failed (4)!");
		robots_dot_txt_mutex_ = new ThreadUtil::Mutex;
	}

	if (unlikely(::curl_easy_setopt(*easy_handle, CURLOPT_SHARE, share_handle_) != CURLE_OK))
		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (0)!");

	if (debugging and unlikely(::curl_easy_setopt(*easy_handle, CURLOPT_VERBOSE, 1L) != CURLE_OK))
		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (1)!");
	...
	if (unlikely(::curl_easy_setopt(*easy_handle, CURLOPT_NOSIGNAL, 1L) != CURLE_OK))
		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (4)!");

	if (unlikely(::curl_easy_setopt(*easy_handle, CURLOPT_WRITEFUNCTION, write_func) != CURLE_OK))
		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (5)!");
...
//		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (6)!");
	...
	if (unlikely(::curl_easy_setopt(*easy_handle, CURLOPT_FOLLOWLOCATION, should_follow) != CURLE_OK))
		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (7)!");

	if (unlikely(::curl_easy_setopt(*easy_handle, CURLOPT_DNS_CACHE_TIMEOUT, dns_cache_timeout) != CURLE_OK))
		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (8)!");

	if (unlikely(::curl_easy_setopt(*easy_handle, CURLOPT_HEADERFUNCTION, header_func) != CURLE_OK))
		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (9)!");

	// User agent information:
	const IniFile ini_file(GetEtcDir() + "/Downloader.conf");
	*user_agent = MsgUtil::GetProgName() + "/Downloader (" + ini_file.getString("User Agent", "default_url") + ")";
	if (unlikely(::curl_easy_setopt(*easy_handle, CURLOPT_USERAGENT, user_agent->c_str()) != CURLE_OK))
		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (10)!");

	// Disable `passive' FTP operation:
	if (unlikely(::curl_easy_setopt(*easy_handle, CURLOPT_FTPPORT, "-") != CURLE_OK))
		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (11)!");

	if (unlikely(::curl_easy_setopt(*easy_handle, CURLOPT_ERRORBUFFER, error_buffer) != CURLE_OK))
		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (12)!");
	...
	if (unlikely(::curl_easy_setopt(*easy_handle, CURLOPT_AUTOREFERER, 1L) != CURLE_OK))
		throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (13)!");

	if (debugging) {
		if (unlikely(::curl_easy_setopt(*easy_handle, CURLOPT_DEBUGFUNCTION, debug_func) != CURLE_OK))
			throw Exception("in Downloader::InitCurlEasyHandle: curl_easy_setopt() failed (14)!");
	}
}
...
bool Downloader::allowedByRobotsDotTxt(const Url &url, const TimeLimit &time_limit)
{
...

	{
		...
		ThreadUtil::MutexLocker mutex_locker(robots_dot_txt_mutex_);
		const GNU_HASH_MAP<std::string, RobotsDotTxt>::const_iterator url_and_robots_dot_txt(url_to_robots_dot_txt_map_.find(robots_txt_url));
	...
	if (not internalNewUrl(Url(robots_txt_url), time_limit)) {
		ThreadUtil::MutexLocker mutex_locker(robots_dot_txt_mutex_);
		url_to_robots_dot_txt_map_.insert(std::make_pair<std::string, RobotsDotTxt>(robots_txt_url, RobotsDotTxt()));
		return true;
	}

	RobotsDotTxt new_robots_txt(body_);
	ThreadUtil::MutexLocker mutex_locker(robots_dot_txt_mutex_);
	url_to_robots_dot_txt_map_.insert(std::make_pair<std::string, RobotsDotTxt>(robots_txt_url, new_robots_txt));
...
void Downloader::GlobalCleanup(const bool forever)
{
	if (unlikely(GetInstanceCount() != 0))
		throw Exception("in Downloader::GlobalCleanup: can't cleanup with existing instances of class Downloader!");
...
const PerlCompatRegExps &Downloader::GetBannedUrlRegExps()
{
	...
	if (not initialised) {
		initialised = true;
		const IniFile ini_file(GetEtcDir() + "/BannedUrlRegExps.conf");
...
const std::string &Downloader::GetDefaultUserAgentString()
{
	if (default_user_agent_string_.empty()) {
		IniFile ini_file(GetEtcDir() + "/Downloader.conf");
