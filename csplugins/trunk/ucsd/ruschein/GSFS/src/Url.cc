	  ...
	  state_(UNINITIALISED), throw_exceptions_(creation_flags & THROW_EXCEPTIONS)
{
	...
	if (creation_flags & AUTO_MAKE_ABSOLUTE)
		throw Exception("in Url::Url: constructor flag AUTO_MAKE_ABSOLUTE used with no base URL!");
	  ...
	  state_(HAS_BEEN_PARSED), throw_exceptions_(creation_flags & THROW_EXCEPTIONS)
{
	...
	if (creation_flags & AUTO_MAKE_ABSOLUTE)
		throw Exception("in Url::Url: constructor flag AUTO_MAKE_ABSOLUTE used with component "
					 "constructor!");
...
bool IsValidUsernamePassword(const std::string &username_password)
{
	...
	for (std::string::const_iterator ch(username_password.begin());
	     ch != username_password.end(); ++ch)
	{
		if (*ch == ':') {
			if (colon_seen)
				return false;
...
//
inline bool IsValidFsegmentChar(std::string::const_iterator &ch,
...
//
inline bool IsValidHsegmentChar(std::string::const_iterator &ch,
				const std::string::const_iterator &end)
{
	if (IsValidUchar(ch, end))
		return true;
...
bool CanonizePath(std::string non_canonical_path, std::string reference_path, std::string * const canonical_path)
{
	...
	if (starts_with_slash and not reference_path.empty())
		throw Exception("in CanonizePath (Url.cc): \"non_canonical_path\" is an absolute path and \"reference_path\" is non-empty!");
...
bool Url::makeCanonical(const unsigned override_timeout)
{
	...
	state_ |= CANONIZATION_ATTEMPTED;
		...
		if (unlikely(user_agent_string.empty()))
			throw Exception("in Url::makeCanonical: need a non-empty user agent string!");
...
unsigned short Url::getPort() const
{
	...
	if (scheme_.empty())
		throw Exception("in Url::getPort: port undefined for relative URL (" + url_ + ")!");

	std::string port = getPortAsString(true);
	if (port.empty())
		throw Exception("in Url::getPort: unimplemented: default port for scheme \"" + scheme_ + "\"!");
...
void LoadUrlConfigFile()
{
	...
	while (input) {
		...
		if (equal_pos == std::string::npos)
			throw Exception("in LoadUrlConfigFile (Url.cc): malformed input in \"" + config_filename
...
std::string Url::getSite() const
{
	...
	for (std::list< std::pair<std::string, std::string> >::const_iterator pair(pattern_to_site_map.begin());
	     pair != pattern_to_site_map.end(); ++pair)
	{
...
std::string UrlMaps::map(const std::string &url) const
{
	for (const_iterator url_map(begin()); url_map != end(); ++url_map) {
		std::string new_url = PerlCompatRegExp::Subst(url_map->getFromPattern(), url_map->getToPattern(), url, false/*= global*/, PCRE_ANCHORED);
		if (new_url != url)
			return new_url;
	}
...
void LoadUrlMaps(UrlMaps * const url_maps)
{
	...
	while (url_maps_conf) {
		...
		if (patterns.size() != 2)
			throw Exception("in LoadUrlMaps(Url.cc): invalid entry in \""
...
std::string GetSuggestedWwwHost(const std::string &hostname)
{
	...
	if (dot_pos == 3 // hostname starts with "www." already
	    ...
	    or !StringUtil::IsUnsignedNumber(hostname.substr(3, dot_pos-3))) // hostname is not "wwwN."
			return hostname;
						   ...
						   const bool canonize_url)
{
	...
	if (not the_url.isValidWebUrl())
		throw Exception("in Url::SuggestLikelyAlternateFormsForWebUrl: not a valid Web URL: " + url);
