...
bool InitializeLocale()
{
	...
	if (std::setlocale(LC_CTYPE, StringUtil::IVIA_STANDARD_LOCALE.c_str()) == NULL) {
		const std::string error_message("setlocale(3) failed: " + StringUtil::IVIA_STANDARD_LOCALE + " " + ErrorInfo() + "\n");
...
char ToHexChar(const unsigned u)
{
	...
	case 0xF:
		return 'F';
	default:
		throw Exception("in ToHexChar: value out of range (" + StringUtil::ToString(u) + ")!");
	}
}
...
char ToHex(const unsigned u)
{
	...
	case 0xF:
		return 'F';
	default:
		throw Exception("in ToHex: number " + ToString(u) + " not in [0,15] range!");
	}
}
...
std::string ToString(long long n, const unsigned radix, const int width, const char grouping_char, const unsigned grouping_size)
{
	MSG_UTIL_ASSERT(radix >= 2 and radix <= 36);
...
std::string ToString(unsigned long long n, const unsigned radix, const int width, const char grouping_char, const unsigned grouping_size)
{
	MSG_UTIL_ASSERT(radix >= 2 and radix <= 36);
...
std::string FromHexString(const std::string &hex_string)
{
	...
	for (std::string::const_iterator ch(hex_string.begin()); ch != hex_string.end(); ++ch) {
		...
		if (unlikely(ch == hex_string.end()))
			MSG_UTIL_THROW("invalid hex string (must contain an even number of characters)!");
...
long ToNumber(const std::string &s, const unsigned base)
{
	...
	if (unlikely(not ToNumber(s, &n, base)))
		throw Exception("in StringUtil::ToNumber: can't convert \"" + s + "\" to a long!");
...
unsigned short ToUnsignedShort(const std::string &s, const unsigned base)
{
	...
	if (unlikely(not ToUnsignedShort(s, &n, base)))
		throw Exception("in StringUtil::ToUnsignedShort: can't convert \"" + s + "\" to an unsigned!");
...
unsigned ToUnsigned(const std::string &s, const unsigned base)
{
	...
	if (unlikely(not ToUnsigned(s, &n, base)))
		throw Exception("in StringUtil::ToUnsigned: can't convert \"" + s + "\" to an unsigned!");
...
unsigned long ToUnsignedLong(const std::string &s, const unsigned base)
{
	...
	if (unlikely(not ToUnsignedLong(s,  &n, base)))
		throw Exception("in StringUtil::ToUnsignedLong: can't convert " + s);
...
unsigned long long ToUnsignedLongLong(const std::string &s, const unsigned base)
{
	...
	if (unlikely(not ToUnsignedLongLong(s, &n, base)))
		throw Exception("in StringUtil::ToUnsignedLongLong: can't convert " + s);
...
double ToDouble(const std::string &s)
{
	...
	if (unlikely(not ToDouble(s, &n)))
		throw Exception("in StringUtil::ToDouble: can't convert \"" + s + "\"!");
...
float ToFloat(const std::string &s)
{
	...
	if (unlikely(not ToFloat(s, &n)))
		throw Exception("in StringUtil::ToFloat: can't convert \"" + s + "\"!");
...
bool ToBool(const std::string &value)
{
	...
	throw Exception("in StringUtil::ToBool: can't convert \"" + value + "\" to a bool!");
}
...
//
std::string &Map(std::string * const s,
...
std::string &Map(std::string * const s, const std::string &old_set, const std::string &new_set)
{
	if (unlikely(old_set.length() != new_set.length()))
		throw Exception("in StringUtil::Subst: old_set.length() != new_set.length()");
...
// CollapseAndTrimWhitespace -- Collapses multiple occurrences of whitespace into a single space and removes leading and trailing whitespace.
//
	...
	while (*s != '\0' or *pattern != '\0') {
		...
		case '*':
			if (asterisk_index == MAX_ASTERISKS)
				throw Exception("StringUtil::Match: too many levels of '*'!");
...
goback:
		...
		case '[': { // match a character class
			...
			if (*pattern != ']')
				throw Exception("in StringUtil::Match: missing ']'!");
			if (invert)
				matched = !matched;
			if (not matched) {
				if (asterisk_index)
...
// ExtractDiacriticalMark -- attempts to extract a diacritical mark starting at "ch."  If this function fails, it returns "end", otherwise it returns
//                           "ch + 1".
//
std::string::const_iterator ExtractDiacriticalMark(std::string::const_iterator ch, const std::string::const_iterator &end, char * const diacritical_mark)
{
	if (unlikely(*ch != '\xC2'))
		return end;
...
std::string UTF8ToISO8859_15(const std::string &text, const char unknown_char, const bool use_overlap_tokens, const unsigned char overlap_token)
{
	std::string result;
	...
	for (std::string::const_iterator ch(text.begin()); ch != text.end(); ++ch) {
		if (use_overlap_tokens and unlikely(static_cast<unsigned char>(*ch) == overlap_token)) {
			...
			if (unlikely(ch == text.begin()))
				throw Exception("in StringUtil::UTF8ToISO8859_15: found an overlap token at the start of a UTF-8 string!");
...
const char CHAR_MAP[] = {
	...
	0xA4, 0x00, 0x27, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0xA6, 0x3C, 0x53, 0x54, 0xB4, 0x5A,
	0x00, 0x60, 0x27, 0x22, 0x22, 0x00, 0x2D, 0x2D, 0x00, 0x00, 0xA8, 0x3E, 0x73, 0x74, 0xB8, 0x7A,
...
size_t strlcat(char * const dest, const char * const src, const size_t dest_size)
{
	...
	MSG_UTIL_ASSERT(dest_len < dest_size);
...
char *FastFormat(void *buffer, const size_t max_length, const char *format, ...)
{
	...
	if (written >= static_cast<int>(max_length))
		throw Exception("Error formatting std::string object: " + ErrorLocation());
...
std::string Format(const char *format, ...)
{
	...
	for (;;) {
		...
		if (written < 0)
			throw Exception("in StringUtil::Format: Error formatting std::string object.");
...
std::string ExtractSensibleSubphrase(const std::string &source_text, const std::string &delimiters, const unsigned minimum, const unsigned maximum)
{
	...
	for (std::string::const_iterator ch(escaped_text.begin()); ch != escaped_text.end(); ++ch) {
		if (*ch == '\\') {
			...
			if (unlikely(ch == escaped_text.end()))
				throw Exception("in StringUtil::CStyleUnescape: unexpected end of escaped string (1)!");
			if (*ch == 'x' or *ch == 'X') { // Hexadecimal escape.
				++ch;
				if (unlikely(ch == escaped_text.end()))
					throw Exception("in StringUtil::CStyleUnescape: unexpected end of escaped string (2)!");
				++ch;
				if (unlikely(ch == escaped_text.end()))
					throw Exception("in StringUtil::CStyleUnescape: unexpected end of hex escape (1)!");
				std::string hex_digits(1, *ch);
				++ch;
				if (unlikely(ch == escaped_text.end()))
					throw Exception("in StringUtil::CStyleUnescape: unexpected end of hex escape (2)!");
				hex_digits += *ch;
				unsigned char_value;
				if (unlikely(not StringUtil::ToUnsigned(hex_digits, &char_value, 16)))
					throw Exception("in StringUtil::CStyleUnescape: bad hex escape!");
				...
				++ch;
				if (unlikely(ch == escaped_text.end()))
					throw Exception("in StringUtil::CStyleUnescape: unexpected end of octal escape (2)!");
				octal_digits += *ch;
				++ch;
				if (unlikely(ch == escaped_text.end()))
					throw Exception("in StringUtil::CStyleUnescape: unexpected end of octal escape (3)!");
				octal_digits += *ch;
				unsigned char_value;
				if (unlikely(not StringUtil::ToUnsigned(octal_digits, &char_value, 8)))
					throw Exception("in StringUtil::CStyleUnescape: bad octal escape (\\0" + octal_digits + ")!");
				unescaped_text += static_cast<char>(char_value);
			}
			else {
				switch (*ch) {
				...
				case '"':
					...
					break;
				default:
					throw Exception("in StringUtil::CStyleUnescape: unknown escape '\\" + CStyleEscape(*ch) + "' (2)!");
				}
			}
		}
		else
...
std::string GenerateRandomString(const unsigned length, const std::string &character_pool)
{
	...
	if (character_pool.empty()) {
		for (unsigned i(0); i < length; ++i)
			random_string += static_cast<char>(uniform(256));
	}
	else {
		for (unsigned i(0); i < length; ++i)
			random_string += character_pool[uniform(character_pool.size())];
	}
