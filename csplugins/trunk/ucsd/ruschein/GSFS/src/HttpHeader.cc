...
public:
	...
	bool operator()(const std::string &s) const
		{ return ::strncasecmp(prefix_.c_str(), s.c_str(), prefix_.length()) == 0; }
};
...
std::string HttpHeader::toString() const
{
	if (unlikely(not is_valid_))
		throw Exception("in HttpHeader::toString: can't create a string representation of an invalid header!");
...
bool HttpHeader::hasAcceptableLanguage(const std::string &acceptable_languages) const
{
	...
	for (std::vector<std::string>::const_iterator content_language(content_languages.begin()); content_language != content_languages.end();
	     ++content_language)
	{
...
bool HttpHeader::IsProbablyNotEnglish(const std::string &charset, const std::string &content_languages)
{
	if (not content_languages.empty()) {
		...
		for (std::list<std::string>::const_iterator language_tag(language_tags.begin()); language_tag != language_tags.end(); ++language_tag) {
