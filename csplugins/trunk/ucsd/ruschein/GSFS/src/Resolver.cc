...
{
	GNU_HASH_MAP<std::string, CacheEntry>::iterator entry(resolved_hostnames_cache_.find(hostname));
...
void Resolver::Cache::insert(const std::string &hostname, const std::set<in_addr_t> &ip_addresses, const uint32_t ttl)
{
	...
	if (resolved_hostnames_cache_.size() > 100000)
		...
		GNU_HASH_MAP<std::string, CacheEntry>::iterator cache_entry(resolved_hostnames_cache_.find(hostname));
	...
	resolved_hostnames_cache_.insert(std::make_pair<std::string, CacheEntry>(hostname, new_cache_entry));
}
	  ...
	  reply_packet_(NULL), reply_packet_size_(0)
{
	...
	if (not dns_servers.empty()) {
		     ...
		     dns_server != dns_servers.end(); ++dns_server)
		{
			...
			dns_server_ip_addresses_and_busy_counts_.insert(
				std::make_pair<in_addr_t, unsigned>(ip_address, 0));
		}
		...
		if (not dns_servers_processed and ini_file.sectionIsDefined("DNS Servers")) {
			...
			for (std::list<std::string>::const_iterator name(names.begin()); name != names.end(); ++name) {
				...
				if (unlikely(not NetUtil::StringToNetworkAddress(ip_address_str, &ip_address)))
					throw Exception("Resolver.conf: \"" + ip_address_str + "\""
	...
	if (not dns_servers_processed) {
		...
		for (std::vector<in_addr_t>::const_iterator server_address(server_ip_addresses.begin());
			...
			dns_server_ip_addresses_and_busy_counts_.insert(
				std::make_pair<in_addr_t, unsigned>(*server_address, 0));
	...
	: verbosity_(logger != NULL ? verbosity : 0), logger_(logger), cleanup_logger_(false)
{
	in_addr_t ip_address;
	if (unlikely(not NetUtil::StringToNetworkAddress(dns_server, &ip_address)))
		throw Exception("in Resolver::Resolver: \"" + dns_server + "\" is not a valid IP address (3)!");

	dns_server_ip_addresses_and_busy_counts_.insert(std::make_pair<in_addr_t, unsigned>(ip_address, 0));
...
Resolver::Resolver(const in_addr_t dns_server, Logger * const logger, const unsigned verbosity)
	: verbosity_(logger != NULL ? verbosity : 0), logger_(logger), cleanup_logger_(false)
{
	dns_server_ip_addresses_and_busy_counts_.insert(std::make_pair<in_addr_t, unsigned>(dns_server, 0));
...
void Resolver::initUdpSocket()
{
	...
	if (unlikely(udp_fd_ == -1))
		throw Exception("in Resolver::Resolver: socket(2) failed (" + MsgUtil::ErrnoToString() + ")!");
...
void Resolver::submitRequest(const std::string &mixed_case_hostname)
{
	...
	if (unlikely(not IsValidHostname(hostname)))
		throw Exception("in Resolver::submitRequest: \"" + hostname + "\" is not a valid hostname!");
...
void Resolver::poll(std::list<Result> * const results)
{
	*results = resolved_addresses_;
	...
	while ((read_retcode = SocketUtil::TimedRead(udp_fd_, time_limit, packet, sizeof(packet))) > 0) {
		...
		uint16_t reply_id;
		bool truncated;
		if (DecodeReply(packet, read_retcode, &domainnames, &ip_addresses, &ttl, &reply_id, &truncated, logger_, verbosity_)) {
			// For now, we ignore truncated packets.  We should really send a TCP request instead:
			if (truncated)
				continue;

			for (std::set<std::string>::const_iterator domainname(domainnames.begin()); domainname!= domainnames.end(); ++domainname)
...
void LogIpAddresses(const std::string &domainname, const std::string &event, const std::set<in_addr_t> &ip_addresses, const TimeLimit &time_limit,
		    Logger * const logger)
{
	if (logger == NULL)
		return;
...
void LogIpAddress(const std::string &domainname, const std::string &event,
		  const in_addr_t &ip_address, const TimeLimit &time_limit, Logger * const logger)
{

	logger->log(domainname + " (" + StringUtil::ToString(time_limit.getRemainingTime()) + "ms): "
...
bool Resolver::resolve(const std::string &mixed_case_domainname, const TimeLimit &time_limit, std::set<in_addr_t> * const ip_addresses)
{
	...
	if (verbosity_ >= 3)
		logger_->log("%s (%ums): Requested...", domainname.c_str(), time_limit.getRemainingTime());
	...
	// Check to see whether "domainname" is already an IP address:
		...
		if (verbosity_ >= 4)
			LogIpAddress(domainname, "Already an address", domainname_as_address.s_addr, time_limit, logger_);
		return true;
	}
	...
	if (cache_.lookup(domainname, ip_addresses)) {
		if (verbosity_ >= 4)
			LogIpAddresses(domainname, "In cache", *ip_addresses, time_limit, logger_);
		return true;
	}
	...
	in_addr_t selected_dns_server;
	{
		...
		GNU_HASH_MAP<in_addr_t, unsigned>::const_iterator dns_server_ip_address_and_busy_count(
			dns_server_ip_addresses_and_busy_counts_.begin());
		...
		if (verbosity_ >= 5)
			LogIpAddress(domainname, "DNS Server", selected_dns_server, time_limit, logger_);
	...
	// Now submit the request...
	if (verbosity_ >= 5)
		logger_->log("in resolve: about to send UPD request (ID %u).", request_id);
	sendUdpRequest(selected_dns_server, packet, static_cast<unsigned>(packet_size));

	// Now wait for the server's response:
	for (;;) {
		if (verbosity_ >= 5)
			logger_->log("in resolve: remaining time %u ms", time_limit.getRemainingTime());
...
process_reply_packet:
		if (actual_packet_size <= 0) {
			if (errno == ETIMEDOUT) {
				if (verbosity_ >= 4)
					logger_->log("%s failed: actual_packet_size <= 0, (timed out)", domainname.c_str());
				return false;
			}
			else {
				if (verbosity_ >= 4)
					logger_->log("in resolve: resolving of \"" + domainname + "\" failed: actual_packet_size <= 0, looping ("
		...
		uint16_t reply_id;
		bool truncated;
		if (DecodeReply(reply_packet_, actual_packet_size, &resolved_domainnames, ip_addresses, &ttl, &reply_id, &truncated, logger_, verbosity_))
		{
			...
			if (reply_id != request_id) {
				if (verbosity_ >= 5)
					logger_->log("in resolve: discarding reply with unexpected ID: %u.", reply_id);
				ip_addresses->clear();
				continue;
			}
			...
			if (truncated) { // => We attempt to get the information using TCP instead of UDP.
				if (verbosity_ >= 4)
					logger_->log("in resolve: UDP packet was truncated.  Attempting TCP request.");
				FileDescriptor tcp_fd(sendTcpRequest(selected_dns_server, time_limit, packet, static_cast<unsigned>(packet_size)));
				if (tcp_fd == -1) {
					if (verbosity_ >= 4)
						logger_->log("in resolve: failed to send request " + MsgUtil::ErrnoToString() + "!");
					return false;
				}
				...
				if (unlikely(not FileUtil::SetBlocking(tcp_fd)))
					throw Exception("in Resolver::resolve: FileUtil::SetBlocking() failed (" + MsgUtil::ErrnoToString() + ")!");
				...
				if (byte_got == -1) {
					if (verbosity_ >= 4)
						logger_->log("in resolve: SocketUtil::TimedRead() failed: " + MsgUtil::ErrnoToString() + "!");
					return false;
				}
				if (verbosity_ >= 5)
					logger_->log("in resolve: actual_packet_size = %zd.", actual_packet_size);

				goto process_reply_packet;
			}

			if (verbosity_ >= 4)
				LogIpAddresses(domainname, "Reply", *ip_addresses, time_limit, logger_);
			return true;
		}
		else {
			if (verbosity_ >= 4)
				logger_->log("%s Failed: could not decode reply", domainname.c_str());
			return false;
		}
	}
}
...
void Resolver::sendUdpRequest(const in_addr_t resolver_ip_address, const unsigned char * const packet, const unsigned packet_size) const
{
	if (unlikely(not SocketUtil::SendUdpRequest(udp_fd_, resolver_ip_address, 53 /* DNS service port */, packet, packet_size)))
		throw Exception("in Resolver::sendUdpRequest: sending a UDP request failed (" + MsgUtil::ErrnoToString() + ")!");
}


int Resolver::sendTcpRequest(const in_addr_t resolver_ip_address, const TimeLimit &time_limit, const unsigned char * const packet, const unsigned packet_size) const
{
	...
	std::string error_message;
        ...
        if (socket_fd == -1) {
                if (verbosity_ >= 4)
                        logger_->log("in Resolver::sendTcpRequest: SocketUtil::TcpConnect() failed (" + error_message + ")!");
                return -1;
        }
	...
	if (unlikely(::setsockopt(socket_fd, SOL_TCP, TCP_CORK, &optval, sizeof optval) == -1))
		throw Exception("in Resolver::sendTcpRequest: setsockopt(2) failed (1) (" + MsgUtil::ErrnoToString() + ")!");
#endif

	uint16_t buf_size((uint16_t)(htons(packet_size)));
	if (SocketUtil::TimedWrite(socket_fd, time_limit, &buf_size, sizeof(buf_size)) == -1) {
                if (verbosity_ >= 4)
                        logger_->log("in Resolver::sendTcpRequest: SocketUtil::TimedWrite() failed (" + MsgUtil::ErrnoToString() + ")!");
		::close(socket_fd);
                return -1;
	}
	...
	optval = 0;
	if (unlikely(::setsockopt(socket_fd, SOL_TCP, TCP_CORK, &optval, sizeof optval) == -1))
		throw Exception("in Resolver::sendTcpRequest: setsockopt(2) failed (2) (" + MsgUtil::ErrnoToString() + ")!");
#endif
...
//                      compression".  This is indicated that the two highest bits of a 16 bit quantity are set.  The
			   ...
			   Logger * const logger, const unsigned _verbosity)
{
	...
	*truncated = dns_header->tc;
	if (verbosity >= 5)
		logger->log("in Resolver::DecodeReply: TC bit is %d", (*truncated ? 1 : 0));
	...
	case 1:
		if (verbosity >= 2)
			logger->log("in Resolver::DecodeReply: the server indicated that we sent an invalid request!");
	default:
		...
		*ttl = ntohl(*ttl);
		...
		if (rr_type == 1 and rr_class == 1) { // Type == A and class == IN.
			if (rdlength != sizeof(in_addr_t)) {
				if (verbosity >= 2)
					logger->log("in Resolver::DecodeReply: unexpected \"rdlength\" for A type RR record!");
				return false;
			}
			...
			if (verbosity >= 5)
				logger->log("in Resolver::DecodeReply: %s: %s", rr_domainname.c_str(),
			...
			if (verbosity >= 5)
				logger->log("in Resolver::DecodeReply: %s: CNAME", rr_domainname.c_str());
...
bool Resolver::ServerIsAlive(const std::string &server_ip_address, const std::string &hostname_to_resolve,
			     const unsigned time_limit, Logger * const logger, const unsigned verbosity)
{
...
unsigned Resolver::GetServersFromResolvDotConf(std::vector<in_addr_t> * const server_ip_addresses)
{
	...
	if (resolv_conf.fail())
		throw Exception("in Resolver::GetServersFromResolvDotConf: can't open \"/etc/resolv.conf\" for reading!");
	...
	while (not resolv_conf.eof()) {
		...
		if (line.substr(0, 10) == "nameserver") {
			...
			if (unlikely(not NetUtil::StringToNetworkAddress(resolver_ip_address, &ip_address)))
				throw Exception("in Resolver::GetServersFromResolvDotConf: \"" + resolver_ip_address + "\" is not a valid IP address (2)!");
...
bool ThreadSafeDnsCache::lookup(const std::string &hostname, std::set<in_addr_t> * const ip_addresses)
{
	...
	GNU_HASH_MAP<std::string, ThreadSafeDnsCacheEntry>::iterator entry(resolved_hostnames_cache_.find(hostname));
	if (entry != resolved_hostnames_cache_.end()) {
		const time_t now(std::time(NULL));
		if (entry->second.expire_time_ > now) {
			*ip_addresses = entry->second.ip_addresses_;
...
void ThreadSafeDnsCache::insert(const std::string &hostname, const std::set<in_addr_t> &ip_addresses, const uint32_t ttl)
{
	...
	if (resolved_hostnames_cache_.size() > 100000)
		...
		// Check to see whether we already have information about this "hostname":
		GNU_HASH_MAP<std::string, ThreadSafeDnsCacheEntry>::iterator cache_entry(resolved_hostnames_cache_.find(hostname));
		if (unlikely(cache_entry != resolved_hostnames_cache_.end())) {
	...
	resolved_hostnames_cache_.insert(std::make_pair<std::string, ThreadSafeDnsCacheEntry>(hostname, new_cache_entry));
}
...
SimpleResolver::SimpleResolver(const std::vector<std::string> &dns_servers)
{
	...
	if (not dns_servers.empty()) {
		     ...
		     dns_server != dns_servers.end(); ++dns_server)
		{
			in_addr_t ip_address;
			if (unlikely(not NetUtil::StringToNetworkAddress(*dns_server, &ip_address)))
				throw Exception("in SimpleResolver::init: \"" + *dns_server
						+ "\" is not a valid IP address (1)!");
			dns_server_ip_addresses_and_busy_counts_.push_back(
				std::make_pair<in_addr_t, unsigned>(ip_address, 0));
		}
	}
	else if (FileUtil::Exists(GetEtcDir() + "/Resolver.conf")) { // If a Resolver.conf file exists, read it.
		IniFile ini_file(GetEtcDir() + "/Resolver.conf");
		...
		if (unlikely(dns_server_ip_addresses_and_busy_counts_.empty()))
			throw Exception("in SimpleResolver::init: failed to find DNS server IP address in " + GetEtcDir() +
					"/Resolver.conf!");
	}
		...
		if (resolv_conf.fail())
			throw Exception("in SimpleResolver::init: can't open \"/etc/resolv.conf\" for reading!");
		...
		while (resolv_conf) {
			...
			if (line.substr(0, 10) == "nameserver") {
				...
				in_addr_t ip_address;
				if (unlikely(not NetUtil::StringToNetworkAddress(resolver_ip_address, &ip_address)))
					throw Exception("in SimpleResolver::init: \"" + resolver_ip_address
								 + "\" is not a valid IP address (2)!");
				dns_server_ip_addresses_and_busy_counts_.push_back(
					std::make_pair<in_addr_t, unsigned>(ip_address, 0));
			}
		...
		if (unlikely(dns_server_ip_addresses_and_busy_counts_.empty()))
			throw Exception("in SimpleResolver::init: failed to find DNS server IP address in /etc/resolv.conf!");
	}

	// Ensure we have at least one DNS Server:
	if (dns_server_ip_addresses_and_busy_counts_.empty())
		throw Exception("in SimpleResolver::init: no DNS Servers found");
}
...
bool SimpleResolver::resolve(const std::string &hostname, const TimeLimit &time_limit, std::set<in_addr_t> * const ip_addresses)
{
	...
	if (unlikely(udp_fd == -1))
		throw Exception("in SimpleResolver::resolve: socket(2) failed (" + MsgUtil::ErrnoToString() + ")!");
	...
	if (unlikely(not SocketUtil::SendUdpRequest(udp_fd, resolver_address, 53 /* DNS service port */, packet, static_cast<unsigned>(packet_size))))
		throw Exception("in SimpleResolver::resolve: sending a UDP request failed (" + MsgUtil::ErrnoToString() + ")!");
	...
	for (;;) {
		...
		if (actual_reply_packet_size <= 0) {
			if (likely(errno == ETIMEDOUT))
				return false;
			throw Exception("in SimpleResolver::resolve: SocketUtil::TimedRead() failed (" + MsgUtil::ErrnoToString() + ")!");
		}
...
{
	ThreadUtil::MutexLocker mutex_locker(&dns_server_ip_addresses_and_busy_count_access_mutex_);
...
void SimpleResolver::decDnsServerUsageCount(const in_addr_t server_ip_address)
{
	...
	for (std::vector< std::pair<in_addr_t, unsigned> >::iterator server_and_usage_count(
		     dns_server_ip_addresses_and_busy_counts_.begin());
	     server_and_usage_count != dns_server_ip_addresses_and_busy_counts_.end(); ++server_and_usage_count)
	{
		if (server_and_usage_count->first == server_ip_address) {
			if (unlikely(server_and_usage_count->second == 0))
				throw Exception("in SimpleResolver::decDnsServerUsageCount: this should never happen!");
	...
	throw Exception("in SimpleResolver::decDnsServerUsageCount: we should never get here!");
}
...
bool SimpleResolver::processServerReply(const unsigned char * const reply_packet, const size_t reply_packet_size, const uint16_t expected_reply_id,
					std::set<in_addr_t> * const ip_addresses)
{
	...
	if (not Resolver::DecodeReply(reply_packet, reply_packet_size, &hostnames, ip_addresses, &ttl,
		...
		if (not ip_addresses->empty()) {
			...
			for (std::set<std::string>::const_iterator hostname(hostnames.begin()); hostname != hostnames.end(); ++hostname)
