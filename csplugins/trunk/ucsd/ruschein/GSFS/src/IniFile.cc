...
{
 	return std::string(::getenv("HOME")) + "/etc/" + MsgUtil::GetProgName() + std::string(".conf");
}
...
void IniFile::processSectionHeader(const std::string &line)
{
	if (line[line.length()-1] != ']')
		throw Exception("in IniFile::processSectionHeader: "
					 "garbled section header on line " +
					 StringUtil::ToString(getCurrentLineNo()) + " in file \""
					 + getCurrentFile() + "\"!");
	...
	if (section != sections_.end())
		throw Exception("in IniFile::processSectionHeader: duplicate section \""
...
void IniFile::processInclude(const std::string &line)
{
	...
	if (unlikely(equal_sign != std::string::npos))
		throw Exception("in IniFile::processInclude: unexpected '=' on line "
					 + StringUtil::ToString(getCurrentLineNo()) + " in file \""
					 + getCurrentFile() + "\"!");
	...
	if (include_filename[0] == '"') {
		if (include_filename.length() < 3 or include_filename[include_filename.length() - 1] != '"')
			throw Exception("in IniFile::processSectionEntry: garbled include file name on line " + StringUtil::ToString(getCurrentLineNo())
...
// IsValidVariableName -- only allow names that start with a letter followed by letters, digits, hyphens and underscores.
//
...
void IniFile::processSectionEntry(const std::string &line)
{
	...
	if (equal_sign == std::string::npos) { // Not a normal "variable = value" type line.
		...
		if (unlikely(not IsValidVariableName(trimmed_line)))
			throw Exception("in IniFile::processSectionEntry: invalid variable name \"" + trimmed_line + "\" on line "
					+ StringUtil::ToString(getCurrentLineNo()) + " in file \"" + getCurrentFile() + "\"!");
		...
		if (variable_name.empty())
			throw Exception("in IniFile::processSectionEntry: missing variable name on line " + StringUtil::ToString(getCurrentLineNo())
					+ " in file \"" + getCurrentFile() + "\"!");

		if (not IsValidVariableName(variable_name))
			throw Exception("in IniFile::processSectionEntry: invalid variable name \"" + variable_name + "\" on line "
					+ StringUtil::ToString(getCurrentLineNo()) + " in file \"" + getCurrentFile() + "\"!");

		std::string value = line.substr(equal_sign + 1);
		StringUtil::Trim(" \t", &value);
		if (value.empty())
			throw Exception("in IniFile::processSectionEntry: missing variable value on line " + StringUtil::ToString(getCurrentLineNo())
					+ " in file \"" + getCurrentFile() + "\"!");

		if (value[0] == '"') { // double-quoted string
			if (value.length() == 1 or value[value.length()-1] != '"')
				throw Exception("in IniFile::processSectionEntry: improperly quoted value on line "
						+ StringUtil::ToString(getCurrentLineNo()) + " in file \"" + getCurrentFile() + "!");
...
void IniFile::processFile(const std::string &filename)
{
	...
	if (unlikely(not FileUtil::Exists(filename))) {
		if (not ignore_failed_includes_ or filename == getFilename())
			throw Exception("in IniFile::processFile: file \"" + filename + "\" does not exist!");
	}
	std::ifstream ini_file(filename.c_str());
	if (ini_file.fail())
		throw Exception("in IniFile::processFile: can't open \"" + filename + "\"!");
...
void IniFile::assign(const IniFile &rhs, const bool clear)
{
	...
	if (clear) {
		...
		{
			std::map<std::string, SectionContents>::iterator section_name_and_contents(
				sections_.find(pair->first));
			if (section_name_and_contents == sections_.end())
				...
				sections_.insert(std::make_pair<std::string, SectionContents>(section_name,
		     ...
		     std::string * const s) const
{
	...
	for (SectionContents::const_iterator entry(section_contents->second.begin()); entry != section_contents->second.end(); ++entry) {
...
unsigned IniFile::getUnsigned(const std::string &section_name, const std::string &variable_name) const
{
	...
	if (not lookup(section_name, variable_name, &variable_value))
		throw Exception("can't find \"" + variable_name + "\" in section \"" + section_name + "\" in \"" + ini_file_name_ + "\"!");

	errno = 0;
	char *endp;
	const unsigned number(static_cast<unsigned>(std::strtoul(variable_value.c_str(), &endp, 0)));
	if (errno != 0 or *endp != '\0')
		throw Exception("invalid unsigned number in \"" + variable_name + "\" in section \"" + section_name + "\" in \"" + ini_file_name_ + "\"!");
	return number;
}
...
uint64_t IniFile::getUint64T(const std::string &section_name, const std::string &variable_name) const
{
	...
	if (errno != 0 or *endp != '\0')
		throw Exception("invalid uint64_t number in \"" + variable_name + "\" in section \"" + section_name + "\" in \"" + ini_file_name_ + "\"!");
	return number;
}
...
long IniFile::getInteger(const std::string &section_name, const std::string &variable_name) const
{
	...
	if (errno != 0 or *endp != '\0')
		throw Exception("invalid integer in \"" + variable_name + "\" in section \"" + section_name + "\" in \"" + ini_file_name_ + "\"!");
...
double IniFile::getDouble(const std::string &section_name, const std::string &variable_name) const
{
	...
	if (errno != 0 or *endp != '\0')
		throw Exception("invalid double-precision floating point number in \"" + variable_name + "\" in section \"" + section_name + "\""
				" in \"" + ini_file_name_ + "\"!");
...
char IniFile::getChar(const std::string &section_name, const std::string &variable_name) const
{
	...
	if (variable_value.length() != 1)
		throw Exception("invalid character variable value \"" + variable_name + "\" in section \"" + section_name + "\"" " in \"" + ini_file_name_
...
bool IniFile::getBool(const std::string &section_name, const std::string &variable_name) const
{
	...
	if (not StringUtil::ToBool(variable_value, &retval))
		throw Exception("in IniFile::getBool: invalid value in section \"" + section_name + "\", entry \"" + variable_name + "\" (bad value is \""
				+ variable_value + "\")!");
		     ...
		     const std::list< std::pair<std::string, int> > &string_to_value_map) const
{
	...
	const std::list< std::pair<std::string, int> >::const_iterator pair(std::find_if(string_to_value_map.begin(), string_to_value_map.end(),
											 NameValuePairMatch(variable_value)));
	if (unlikely(pair == string_to_value_map.end()))
		throw Exception("invalid value \"" + variable_value + "\" for \"" + variable_name + "\" in section \"" + section_name + "\""" in \""
				+ ini_file_name_ + "\"!");
...
std::list<std::string> IniFile::getSections() const
{
	...
	for (std::map<std::string, SectionContents>::const_iterator section(sections_.begin()); section != sections_.end(); ++section)
...
std::list<std::string> IniFile::getSectionEntryNamesThatStartWith(const std::string &section_name, const std::string &starting_with) const
{
...
std::list<std::string> IniFile::getSectionEntryValuesHavingNamesStartingWith(const std::string &section_name, const std::string &starting_with) const
{
