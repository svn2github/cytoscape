...
SaveAndRestorePendingTimer::SaveAndRestorePendingTimer()
{
	if (::gettimeofday(&start_time_, NULL) == -1)
	      throw Exception("in SaveAndRestorePendingTimer::SaveAndRestorePendingTimer: gettimeofday(2) failed!");
	if (::getitimer(ITIMER_REAL, &saved_itimerval_) == -1)
	      throw Exception("in SaveAndRestorePendingTimer::SaveAndRestorePendingTimer: getitimer(2) failed!");
	if (::sigaction(SIGALRM, NULL, &old_sigaction_) == -1)
	      throw Exception("in SaveAndRestorePendingTimer::SaveAndRestorePendingTimer: sigaction(2) failed!");
}
...
SaveAndRestorePendingTimer::~SaveAndRestorePendingTimer()
{
	...
	if (::setitimer(ITIMER_REAL, &saved_itimerval_, NULL) == -1)
	      throw Exception("in SaveAndRestorePendingTimer::~SaveAndRestorePendingTimer: setitimer(2) failed!");
}
