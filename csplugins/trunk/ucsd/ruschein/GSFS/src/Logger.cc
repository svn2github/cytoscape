	...
	: destroy_file_(true), already_dead_(false), log_file_(NULL), log_filename_(log_filename), verbosity_(default_verbosity)
{
	...
	if (not dirname.empty() and unlikely(not FileUtil::MakeDirectory(dirname, /* recursive = */ true)))
		throw Exception("in Logger::Logger: can't create directory \"" + dirname + "\"!");

	log_file_ = new File(log_filename_, (open_mode == CLEAR ? "w" : "a"));
	if (log_file_->fail())
		MsgUtil::Error("in Logger::Logger: can't open \"" + log_filename_ + "\" for logging!");
}
...
{
	if (log_file_->fail())
		MsgUtil::Error("in Logger::Logger: logger file not working!");
...
void Logger::reopen(const std::string &log_filename)
{
	...
	if (log_filename.empty() and log_filename_.empty())
		MsgUtil::Error("in Loger::reopen: no log filename available!");
	...
	FileUtil::DirnameAndBasename(log_filename, &dirname, &basename);
	if (not dirname.empty() and unlikely(not FileUtil::MakeDirectory(dirname, /* recursive = */ true)))
		throw Exception("in Logger::reopen: can't create directory \"" + dirname + "\"!");

	log_file_ = new File(log_filename_.c_str(), "a");
	if (log_file_->fail())
		throw Exception("can't open \"" + log_filename_ + "\" for logging!");
}
...
void Logger::log(const std::string &message)
{
	...
	if (log_file_->fail())
		throw Exception("in Logger::log: failed to write to the log file \"" + log_filename_ + "\"!");
}
...
void Logger::sysLog(const std::string &message)
{
	...
	if (log_file_->fail())
		throw Exception("in Logger::syslog: failed to write to the log file \"" + log_filename_
					 + "\"!");
}
...
void Logger::logAndThrow(const std::string &message)
{
	...
	if (log_file_->fail()) {
		already_dead_ = true;
		throw Exception("in Logger::logAndThrow: failed to write to the log file \"" + log_filename_
					 + "\"!");
	}
...
void Logger::logAndDie(const std::string &message)
{
	...
	if (log_file_->fail()) {
		already_dead_ = true;
		throw Exception("in Logger::logAndDie: failed to write to the log file \"" + log_filename_
					 + "\"!");
	}
...
void Logger::sysLogAndDie(const std::string &message)
{
	...
	if (log_file_->fail()) {
		already_dead_ = true;
		throw Exception("in Logger::sysLogAndDie: failed to write to the log file \"" + log_filename_
					 + "\"!");
	}
...
{
	*log_file_ << TimeUtil::GetCurrentDateAndTime() << " [" << MsgUtil::GetProgName() << ", PID:" << ::getpid();
	*log_file_ << "]: ";
	if (log_mask & EXITING)
		*log_file_ << "Exiting: ";
