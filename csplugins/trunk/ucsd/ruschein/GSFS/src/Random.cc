...
void NonUniformRandom::init(const std::vector<double> &distribution)
{
	...
	for (std::vector<double>::const_iterator probability(distribution.begin()); probability != distribution.end(); ++probability) {
		if (unlikely(*probability < 0.0 or *probability > 1.0))
			throw Exception("in Random::NonUniformRandom::NonUniformRandom: invalid probability \"" + StringUtil::ToString(*probability)
					+ "\"!");
		cdf += *probability;
	...
	if (not MathUtil::ApproximatelyEqual(cdf, 1.0))
		throw Exception("in Random::NonUniformRandom::NonUniformRandom: invalid distribution argument (probabilities "
				"must add up to 1.0).(cdf: " + StringUtil::ToString(cdf) + ")!");
...
unsigned NonUniformRandom::operator()(size_t max)
{
	...
	const std::vector<double>::iterator last(((max == 0) or (max > size)) ? cumulative_distribution_.end() : cumulative_distribution_.begin() + max);
