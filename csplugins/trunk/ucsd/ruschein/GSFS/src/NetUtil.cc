...
in_addr_t StringToNetworkAddress(const std::string &s)
{
	...
	throw Exception("in NetUtil::StringToNetworkAddress: \"" + s + "\" is not a valid IPv4 address!");
}
...
std::string NetworkAddressToString(const in_addr_t network_address)
{
	...
	if (unlikely(not NetworkAddressToString(network_address, &network_address_as_string)))
		throw Exception("in NetUtil::NetworkAddressToString: can't convert in_addr_t to an IPv4 address!");
...
void PopulateNetmaskToPrefixTable(GNU_HASH_MAP<in_addr_t, std::string> * const netmask_to_prefix)
{
...
{
	static GNU_HASH_MAP<in_addr_t, std::string> netmask_to_prefix;
	if (unlikely(netmask_to_prefix.empty()))
		PopulateNetmaskToPrefixTable(&netmask_to_prefix);
	GNU_HASH_MAP<in_addr_t, std::string>::const_iterator netmask_and_prefix(netmask_to_prefix.find(netmask));
...
std::string NetworkAddressAndMaskToString(const in_addr_t network_address, const in_addr_t netmask)
{
	...
	if (not NetworkAddressToString(network_address, &address))
		throw Exception("in NetUtil::NetworkAddressAndMaskToString: invalid network address(" + StringUtil::ToString(network_address) + ")");
	...
	if (ip_prefix == "")
		throw Exception("in NetUtil::NetworkAddressAndMaskToString: invalid netmask(" + StringUtil::ToString(netmask) + ")");
...
void GetInterfaces(std::list<ifreq> * const interface_requests)
{
	...
	if (sock_fd == -1)
		throw Exception("in GetInterfaces: can't get IPv4 socket!");
	...
	for (;;) {
		...
		if (::ioctl(sock_fd, SIOCGIFCONF, &ifc) != 0) {
			delete [] ifc.ifc_buf;
			throw Exception("in GetInterfaces: ioctl(2) failed ("
...
void GetLocalIPv4Addrs(std::list<in_addr_t> * const ip_addresses)
{
	...
	for (std::list<ifreq>::const_iterator ifreq(interface_requests.begin()); ifreq != interface_requests.end();
	     ++ifreq)
		ip_addresses->push_back(
			reinterpret_cast<const sockaddr_in *>(&ifreq->ifr_ifru.ifru_addr)->sin_addr.s_addr);
}
