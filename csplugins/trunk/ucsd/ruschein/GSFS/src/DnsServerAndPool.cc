...
{
	if (unlikely(empty()))
		throw Exception("in DnsServer::OutstandingRequests::removeRequest: attempt to remove a request from an empty list!");
	  ...
	  socket_fd_(-1), request_lifetime_(request_lifetime), outstanding_requests_(max_outstanding_request_count + 10)
{
	...
	if (unlikely(protocol_entry == NULL))
		throw Exception("in Resolver::Resolver: can't get protocol entry for \"udp\"!");
	...
	if (::connect(socket_fd_, reinterpret_cast<sockaddr *>(&socket_address), sizeof(struct sockaddr_in)) != 0)
		throw Exception("in DnsServer::DnsServer: can't connect(2) to " + NetUtil::NetworkAddressToString(server_ip_address) + "!");
}
...
bool DnsServer::addLookupRequest(const std::string &valid_hostname, uint16_t * const query_id, uint64_t * const deadline)
{
	...
	*query_id = next_query_id_;
	...
	if (unlikely(retcode == -1))
		throw Exception("in DnsServer::addLookupRequest: write(2) failed (" + MsgUtil::ErrnoToString() + ")!");
	else if (unlikely(retcode != packet_size))
		throw Exception("in DnsServer::addLookupRequest: short write (expected: " + StringUtil::ToString(packet_size) + ", actual: "
				+ StringUtil::ToString(retcode) + ")!");
...
bool DnsServer::processServerReply(std::vector<in_addr_t> * const resolved_ip_addresses, std::vector<std::string> * const resolved_domainnames,
				   uint32_t * const ttl, uint16_t * const query_id)
{
	...
	if (unlikely(outstanding_requests_.empty())) {
		...
		MsgUtil::Warning("in DnsServer::processServerReply: can't process a reply with an empty queue!");
		return false;
	}
	...
	if (Resolver::DecodeReply(packet, read_retcode, &domainnames, &ip_addresses, ttl, &reply_id, &truncated, NULL /* logger */, 0 /* verbosity */)) {
		...
		std::string original_request_hostname;
		outstanding_requests_.removeRequest(reply_id, &original_request_hostname);
		*query_id = reply_id;
...
{
	GNU_HASH_MAP<std::string, DnsCacheEntry>::iterator entry(resolved_hostnames_cache_.find(hostname));
...
void DnsCache::insert(const std::string &hostname, const in_addr_t &ip_address, const uint32_t ttl)
{
	...
	if (resolved_hostnames_cache_.size() >= cache_flush_size_)
		...
		GNU_HASH_MAP<std::string, DnsCacheEntry>::iterator cache_entry(resolved_hostnames_cache_.find(hostname));
	...
	resolved_hostnames_cache_.insert(std::make_pair<std::string, DnsCacheEntry>(hostname, new_cache_entry));
}
	...
	: cache_(max_cache_size), max_queue_length_per_server_(max_queue_length_per_server)
{
	if (unlikely(dns_server_ip_addresses.size() < 2))
		throw Exception("in DnsServerPool::DnsServerPool: can't create a \"pool\" with less than 2 server IP addresses!");

	server_socket_file_descriptors->clear();
	server_socket_file_descriptors->reserve(dns_server_ip_addresses.size());
	for (std::vector<in_addr_t>::const_iterator ip_address(dns_server_ip_addresses.begin()); ip_address != dns_server_ip_addresses.end(); ++ip_address)
	{
...
bool DnsServerPool::isBusy() const
{
	...
	for (std::vector<DnsServer *>::const_iterator server(servers_.begin()); server != servers_.end(); ++server) {
...
bool DnsServerPool::processServerReply(const int socket_fd, in_addr_t * const resolved_ip_address, std::string * const hostname)
{
	...
	throw Exception("in DnsServerPool::processServerReply: received reply for an unknown socket file descriptor " + StringUtil::ToString(socket_fd)
			+ "!");
}
