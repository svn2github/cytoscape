...
void RobotsDotTxt::UserAgentDescriptor::addRule(const RuleType rule_type, const std::string &value)
{
	if (unlikely(value.empty())) {
		...
		case DISALLOW:
			...
			return;
		default:
			throw Exception("in RobotsDotTxt::UserAgentDescriptor::addRule: don't know how to handle rule type ("
...
{
	for (SList<std::string>::const_iterator pattern(user_agent_patterns_.begin()); pattern != user_agent_patterns_.end(); ++pattern) {
...
std::string RobotsDotTxt::UserAgentDescriptor::toString() const
{
	...
	// User-agent:
	for (SList<std::string>::const_iterator user_agent_pattern(user_agent_patterns_.begin()); user_agent_pattern != user_agent_patterns_.end();
	     ++user_agent_pattern)
...
ThreadUtil::Mutex RobotsDotTxtCache::mutex_;
RobotsDotTxtCache *RobotsDotTxtCache::the_singleton_(NULL);
...
void RobotsDotTxtCache::addAlias(const std::string &original_hostname, const std::string &new_hostname)
{
	...
	GNU_HASH_MAP<std::string, RobotsDotTxt *>::const_iterator entry(
	...
	if (unlikely(entry == hostname_to_robots_dot_txt_map_.end()))
		throw Exception("in RobotsDotTxtCache::addAlias: can't add an additional hostname reference for a non-existent entry!");
	hostname_to_robots_dot_txt_map_.insert(std::make_pair<std::string, RobotsDotTxt *>(StringUtil::ToLower(new_hostname), entry->second));
}
...
void RobotsDotTxtCache::setMaxCacheSize(const unsigned new_max_cache_size)
{
	if (unlikely(new_max_cache_size == 0))
		throw Exception("in RobotsDotTxtCache::setMaxCacheSize: new_max_cache_size must be greater than zero!");
...
const RobotsDotTxt *RobotsDotTxtCache::getRobotsDotTxt(const std::string &hostname) const
{
	...
	return entry == hostname_to_robots_dot_txt_map_.end() ? NULL : entry->second;
}
...
{
	GNU_HASH_SET<RobotsDotTxt *> already_deleted;
	for (GNU_HASH_MAP<std::string, RobotsDotTxt *>::iterator entry(hostname_to_robots_dot_txt_map_.begin());
	     entry != hostname_to_robots_dot_txt_map_.end(); ++entry)
	{
