...
std::string GetMediaType(const std::string &document, const bool auto_simplify)
{
	...
	if (unlikely(cookie == NULL))
		throw Exception("in MediaTypeUtil::GetMediaType: could not open libmagic!");

	// Load the default "magic" definitions file:
	if (unlikely(::magic_load(cookie, NULL /* use default magic file */) != 0)) {
		::magic_close(cookie);
		throw Exception("in MediaTypeUtil::GetMediaType: could not load libmagic (" + std::string(::magic_error(cookie)) + ").");
	}
	...
	if (unlikely(magic_mime_type == NULL)) {
		::magic_close(cookie);
		throw Exception("in MediaTypeUtil::GetMediaType: error in libmagic ("+ std::string(::magic_error(cookie)) + ").");
	}
...
//
bool GetMediaType(const Url &url, const HttpHeader &http_header, const std::string &page_content, std::string * const media_type, const bool auto_simplify)
{
	// First, attempt to find the media type in the header:
  	if (http_header.isValid()) {
