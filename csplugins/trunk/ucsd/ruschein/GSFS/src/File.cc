...
File::File(const std::string &path, const std::string &mode, const ThrowOnOpenBehaviour throw_on_error_behaviour,
	...
	: file_(::fopen(path.c_str(), (mode + "m").c_str())), path_(path), mode_(mode), precision_(6), unique_id_(next_unique_id_++),
	  delete_on_close_(delete_on_close_behaviour == DELETE_ON_CLOSE)
{
	if (unlikely(file_ == NULL) and throw_on_error_behaviour == THROW_ON_ERROR)
		throw Exception("in File::File: fopen(3) on \"" + std::string(path) + "\" with mode \"" + mode + "m\" failed (" + MsgUtil::ErrnoToString()
				+ ") (1)!");
}


File::File(const int fd, const std::string &mode)
	: path_("/proc/self/fd/" + StringUtil::ToString(fd)), precision_(6), unique_id_(next_unique_id_++), delete_on_close_(false)
{
	if (not mode.empty())
		mode_ = mode + "m";
	else {
		...
		if (unlikely((flags = ::fcntl(fd, F_GETFL, &flags)) == -1))
			throw Exception("in File::File: fcntl(2) failed (" + MsgUtil::ErrnoToString() + ")!");
		flags &= O_ACCMODE;
		if (flags == O_RDONLY)
			mode_ = "rm";
	...
	if (unlikely(file_ == NULL))
		throw Exception("in File::File: freopen(3) on \"" + StringUtil::ToString(fd) + "\" with mode \"" + mode_ + "\" failed ("
				+ MsgUtil::ErrnoToString() + ") (3)!");
}
...
{
	if (unlikely(file_ == NULL))
		throw Exception("in File::size: can't obtain the size of non-open File \"" + (path_.empty() ? "" : path_) + "\"!");

	struct stat stat_buf;
	if (unlikely(::fstat(fileno(file_), &stat_buf) == -1))
		throw Exception("in File::size: fstat(2) failed on \"" + path_ + "\" ("
...
{
	if (unlikely(file_ == NULL))
		throw Exception("in File::flush: can't flush non-open file \"" + (path_.empty() ? "" : path_) + "\"!");
...
{
	if (unlikely(file_ == NULL))
		throw Exception("in File::write: can't write to non-open file (1) \"" + (path_.empty() ? "" : path_) + "\"!");
...
{
	if (unlikely(file_ == NULL))
		throw Exception("in File::serialize: can't serialize to non-open file (2) \"" + (path_.empty() ? "" : path_) + "\"!");
...
{
	if (unlikely(file_ == NULL))
		throw Exception("in File::serialize: can't serialize to non-open file (3) \"" + (path_.empty() ? "" : path_) + "\"!");
...
{
	if (unlikely(file_ == NULL))
		throw Exception("in File::serialize: can't serialize to non-open file (4) \"" + (path_.empty() ? "" : path_) + "\"!");
...
void File::putback(const char ch)
{
	if (unlikely(std::ungetc(ch, file_) == EOF))
		throw Exception("in File::putback: failed to push back a character ("
				+ StringUtil::ToString(static_cast<unsigned>(ch)) + ") into an input stream!");
}
...
{
	if (unlikely(file_ == NULL))
		throw Exception("in File::read: can't read from non-open file (1) \"" + (path_.empty() ? "" : path_) + "\"!");
...
{
	if (unlikely(file_ == NULL))
		throw Exception("in File::deserialize: can't deserialize from non-open file (2) \"" + (path_.empty() ? "" : path_) + "\"!");
...
{
	if (unlikely(file_ == NULL))
		throw Exception("in File::deserialize: can't deserialize from non-open file (3) \"" + (path_.empty() ? "" : path_) + "\"!");
...
{
	if (unlikely(file_ == NULL))
		throw Exception("in File::seek: can't seek on non-open file \"" + (path_.empty() ? "" : path_) + "\"!");
...
{
	if (unlikely(file_ == NULL))
		throw Exception("in File::tell: can't get a file offset on non-open file \"" + (path_.empty() ? "" : path_) + "\"!");
...
{
	if (unlikely(file_ == NULL))
		throw Exception("in File::setNewSize: can't get non-open file's size \"" + (path_.empty() ? "" : path_) + "\"!");
...
{
	std::map<Callback, void * const>::iterator at_close_function_and_userdata(at_close_function_to_userdata_map_.find(new_callback));
	if (at_close_function_and_userdata == at_close_function_to_userdata_map_.end())
		at_close_function_to_userdata_map_.insert(std::make_pair<Callback, void * const>(new_callback, user_data));
	else
...
const char *File::getReadOnlyMmap() const
{
	...
	if (unlikely(::fstat(fileno(file_), &stat_buf) == -1))
		return NULL;
	const char *data = reinterpret_cast<const char *>(::mmap(0, stat_buf.st_size, PROT_READ, MAP_SHARED,
								 fileno(file_), 0));
...
void File::lock(const LockType lock_type, const short whence, const off_t start, const off_t length)
{
	...
	if (unlikely(::fcntl(fileno(file_), F_SETLKW, &lock_params) == -1))
		throw Exception("in File::lock: fcntl(2) failed (" + MsgUtil::ErrnoToString() + ")!");
}
...
bool File::tryLock(const LockType lock_type, const short whence, const off_t start, const off_t length)
{
	...
	throw Exception("in File::tryLock: fcntl(2) failed (" + MsgUtil::ErrnoToString() + ")!");
}
...
void File::unlock(const short whence, const off_t start, const off_t length)
{
	...
	if (unlikely(::fcntl(fileno(file_), F_SETLK, &lock_params) == -1))
		throw Exception("in File::unlock: fcntl(2) failed (" + MsgUtil::ErrnoToString() + ")!");
}
...
bool File::waitOnAsyncCompletion(aiocb * const control_block_ptr, const unsigned timeout, size_t * const nbytes)
{
	...
	if (success) {
		...
		if (unlikely(errno != 0 or retval < 0))
			throw Exception("in File::waitOnAsyncCompletion: aio_return(3) failed (return code: " + StringUtil::ToString(retval) + ")!");
...
bool File::cancelAsyncRequest(aiocb * const control_block_ptr)
{
	...
	case -1: // Some error occurred.
		throw Exception("in File::cancelAsyncRequest: aio_cancel(3) failed (" + MsgUtil::ErrnoToString() + ")!");
