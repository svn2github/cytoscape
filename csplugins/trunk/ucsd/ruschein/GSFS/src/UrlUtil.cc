...
std::string PrivoxyToPerlRegExpPattern(const std::string &privoxy_pattern)
{
	if (unlikely(privoxy_pattern.empty()))
		throw Exception("in UrlUtil::PrivoxyToPerlRegExpPattern: \"privoxy_pattern\" must not be empty!");
	...
	if (unlikely(not(path.empty() or path[0] == '/')))
		throw Exception("in UrlUtil::PrivoxyToPerlRegExpPattern: \"path\" must be empty or absolute!");
...
const Blacklister::ReferenceCountedPerlCompatRegExp &Blacklister::ReferenceCountedPerlCompatRegExp::operator=(const ReferenceCountedPerlCompatRegExp &rhs)
{
...
{
	for (std::list<std::string>::const_iterator privoxy_pattern(privoxy_patterns.begin()); privoxy_pattern != privoxy_patterns.end();
	     ++privoxy_pattern)
...
void Blacklister::processPrivoxyActionFile(const std::string &action_filename)
{
	...
	if (input.fail())
		throw Exception("in Blacklister::processPrivoxyActionFile: can't open \"" + action_filename
					 + "\" for reading!");
	      ...
	      std::string * const fragment, std::string * const relative_url)
{
	...
	*scheme = url.substr(0, first_colon_pos);
	...
	MSG_UTIL_ASSERT(authority_start != std::string::npos);
