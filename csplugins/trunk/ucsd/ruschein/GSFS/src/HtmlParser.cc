...
void HtmlParser::ungetChar()
{
	if (unlikely(cp_ == cp_start_))
		MsgUtil::Error("in HtmlParser::ungetChar: trying to push back at beginning of input!");
	...
	for (;;) {
		...
		if (unlikely(endOfStream())) {
			Chunk unexpected_eof_chunk(UNEXPECTED_END_OF_STREAM, lineno_,
						   ...
						   + StringUtil::ToString(start_lineno) + ")");
		...
		do {
			...
			if (unlikely(endOfStream())) {
				Chunk unexpected_eof_chunk(UNEXPECTED_END_OF_STREAM, lineno_,
							   ...
							   "on line " + StringUtil::ToString(start_lineno) + ")");
				preNotify(&unexpected_eof_chunk);
				return;
			}
		} while (ch != '*');
		while (ch == '*') {
			...
			if (unlikely(endOfStream())) {
				Chunk unexpected_eof_chunk(UNEXPECTED_END_OF_STREAM, lineno_,
							   ...
							   "line " + StringUtil::ToString(start_lineno) + ")");
				preNotify(&unexpected_eof_chunk);
				return;
			}
			if (ch == '/')
	...
	do {
		...
		if (ch == EOF) {
			Chunk unexpected_eof_chunk(UNEXPECTED_END_OF_STREAM, lineno_,
						   ...
						   + "\" opened on line " + StringUtil::ToString(tag_start_lineno));
			preNotify(&unexpected_eof_chunk);
			return;
		}
...
bool HtmlParser::skipToEndOfScriptOrStyle(const std::string &tag_name, const unsigned tag_start_lineno)
{
	...
	for (;;) {
		...
		if (ch == EOF) {
			Chunk unexpected_eof_chunk(UNEXPECTED_END_OF_STREAM, lineno_,
						   "unexpected end-of-stream while skipping tag \"" + tag_name
						   + "\" opened on line " + StringUtil::ToString(lineno_));
			preNotify(&unexpected_eof_chunk);
			return false;
		}
	...
	*attribute_name += static_cast<char>(ch);
	...
	if (unlikely(endOfStream())) {
		Chunk unexpected_eof_chunk(UNEXPECTED_END_OF_STREAM, lineno_,
					   ...
					   + "\" tag on line " + StringUtil::ToString(lineno_));
		preNotify(&unexpected_eof_chunk);
		return false;
	}
	ungetChar();
	...
	if (ch != '=') {
		if (unlikely(endOfStream())) {
			Chunk unexpected_eof_chunk(UNEXPECTED_END_OF_STREAM, lineno_,
						   ...
						   + "\" on line " + StringUtil::ToString(lineno_));
			preNotify(&unexpected_eof_chunk);
			return false;
		}
		ungetChar();
...
bool HtmlParser::parseTag()
{
	...
	if (unlikely(endOfStream())) {
		Chunk unexpected_eof_chunk(UNEXPECTED_END_OF_STREAM, lineno_,
					   "tag opened on line " + StringUtil::ToString(lineno_) + " was never closed");
		preNotify(&unexpected_eof_chunk);
		return false;
	}
	...
	if (unlikely(endOfStream())) {
		Chunk unexpected_eof_chunk(UNEXPECTED_END_OF_STREAM, lineno_,
					   "tag \"" + tag_name + "\" opened on line " + StringUtil::ToString(lineno_)
					   + " was never closed");
		preNotify(&unexpected_eof_chunk);
		return false;
	}
...
std::string HtmlParser::ChunkTypeToString(const unsigned chunk_type)
{
	...
	case EVERYTHING:
		return "EVERYTHING";
	default:
		throw Exception("in HtmlParser::ChunkTypeToString: unknown chunk type: " + StringUtil::ToString(chunk_type) + "!");
	}
}
