...
/** \brief PtrOrRef is a smart pointer with limited utility, holding a pointer to a class object, that tries to act as if it were a member of
 *         the actual class. The standard template library container classes are limited in what they can hold. We often need to look at a
 *         subset of a container but we don't need a separate copy of the items, just const references to them. Copying heavy items just to
 *         peek at them is performance prohibitive. Unfortunately, the stl containers cannot hold references but they can hold PtrOrRefs that
 *         hold a pointer but act like an instance of the class. IMPORTANT: The PtrOrRef does NOT manage the pointer. It doesn't create, nor
 *         does it destroy its referent. If you destroy the referent the PtrOrRef will be left with a dangling pointer. If you destroy the PtrOrRef,
 *         there is no clean up necessary. It is merely useful because it uses object syntax, while being a pointer.
 *
 *         Most of the algorithms in this file take an PtrOrRef as a parameter. This makes it so that you can pass either a reference, an
 *         instance or a pointer to any of the functions and they will work just the same.
 */
...
public:
	...
	inline operator const Class &() const {
		if (item_ == NULL)
			throw Exception("PtrOrRef has NULL: " + ErrorLocation());
		return *item_;
	}
...
/** Constructing an PtrOrRef directly means you need to specify the class, like PtrOrRef<std::string> aString("test"). This helper function will
    figure out the class for you automagically, construct an PtrOrRef and return it to you. Example, return MakePtrOrRef(aString); Because the PtrOrRef
    contains only a pointer, construction and copying are virtually CPU free (the cost of assigning a pointer). */
 ...
 *  \note  Useful in STL searches and where comparsions of pairs should be based solely on operator< for the "first" components of the pairs
 *         and "second" should not be taken into consideration. Sometimes pair::second doesn't have an operator< and it's a hassle to write
 *         one.
 */
...
/** \brief Comparison function for std::pairs that returns true if lhs.first < rhs.first.  \note Useful in STL searches and where
 *  comparsions of pairs should be based solely on operator< for the "first" components of the pairs and "second" should not be taken into
 *  consideration.
 */
...
/**
 * Helps bridge the gap between sequence containers and associative containers. If passed a scalar value, will just return the value, but if
 * passed an std::pair, will return the second element.  This is to help you write predicates that can work on both elements of associate
 * containers and containers of pairs
 */
...
/**
 *  These simulate "template typedefs". Value<aType> will create a member typedef. Value::Type will be the element type for normal
 *  containers. Value::Type will be the type of the second member of pair for containers of pairs.
 */
...
template <typename First, typename Second> inline bool operator<(const std::pair<First, Second> &lhs, const std::pair<First, Second> &rhs)
{
	return std::operator<(lhs, rhs);
}


/** Creates a pair from a string with some delimiters. Forms a basis for a class that constructs pairs of whatever, from a string. The first
 *  word in the string becomes the key and the remaining text becomes the value. From the source string, a pair of "FirstType", "SecondType"
 *  is created. The pair can consist of any types that AutoConvert can convert to, so that a string like "population=12000" will construct a
 *  pair with a string key, and an int value. This can then be used wherever an std::pair is used.
 *
 *  The PairFromString being created can consist of any two types. To turn the string elements which are derived from "source" into the
 *  proper types, a Convert template parameter can be specified. The default one works pretty well for converting from std::string to other
 *  built in types. If you get more complicated than that, you must provide a functor class whose operator() takes an std::pair<const char
 *  *, const char *> and returns and std::pair of the types that go into your PairFromString. For an example, see the AutoConvertPair()
 *  above which is the default Converter.
 */
...
public:
	...
	explicit PairFromString(const std::string &source, const std::string &delimiting_chars = " \t\r\n/.:=")
	{
		...
		if (unlikely(first_delimiter == NULL))
			throw Exception("No delimiters \"" + delimiting_chars + "\" found in \"" + source + "\": " + ErrorLocation());
	...
	PairFromString(const std::string &source, const PerlCompatRegExp &splitter)
	{
		if (not splitter.match(source))
			throw Exception("Pattern " + splitter.getPattern() + " not found in string " + source + ": " + ErrorLocation());

		unsigned substring_count(splitter.getSubstringMatchCount());
		if (substring_count < 2)
			throw Exception("Not enough substrings found matching pattern " + splitter.getPattern() + " to string " + source + ": " + ErrorLocation());
...
const typename MapContainerType::value_type::second_type &FindOrThrow(const SearchItemType &find_this, const MapContainerType &container)
{
	typename MapContainerType::const_iterator item = container.find(find_this);
	if (item == container.end())
		throw Exception("Unable to find item in container: " + ErrorLocation());
	return item->second;
}
...
template <typename StlContainer> StlContainer GrepContainerNot(const StlContainer &source, const PerlCompatRegExp &regex)
{
	StlContainer matches;
...
GNU_HASH_MAP<First, Second> GrepContainer(const GNU_HASH_MAP<First, Second> &source, const PerlCompatRegExp &regex)
{
	GNU_HASH_MAP<First, Second> matches;
	for (typename GNU_HASH_MAP<First, Second>::const_iterator item(source.begin()); item != source.end(); ++item)
		if (regex.match(ElementToString(*item)))
			matches.insert(*item);
	return matches;
}
 ...
 */
template <typename ForwardIterator> std::string ContainerToString(ForwardIterator first, ForwardIterator last, const std::string &separator = "")
{
	std::string resultant_string;
	while (first != last) {
		resultant_string += AutoConvert(*first);
		++first;
		if (first != last)
...
public:
	...
	template <typename Container> bool operator()(const Container &comparee) const { return comparee.size() < cutoff_size_; }
};


/** \brief  Determines membership in a container. */
template<typename Container> inline bool Contains(const Container &container, const typename Container::value_type entry_candidate)
{
...
namespace Functional {
	...
	/** \brief Inserts a copy of every element in the original container into the new container if FilterFunction(element) returns true for it*/
	template<typename ContainerIterator, typename InsertableContainer, typename FilterFunction>
	inline void Filter(const ContainerIterator &start, const ContainerIterator &end, InsertableContainer &new_container, FilterFunction &filter)
	{
		for (ContainerIterator iterator(start); iterator != end; ++iterator) {
			if (filter(*iterator))
