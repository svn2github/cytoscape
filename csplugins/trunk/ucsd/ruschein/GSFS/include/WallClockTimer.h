 ...
 *
 *  To use a WallClockTimer, create it, call start() to begin timing, perform the action(s) you wish to time, then call stop().  The getTime() member
 *  function will return the time elapsed between the calls to start() and stop().
 */
class WallClockTimer {
	bool is_running_;
...
public:
	enum WallClockTimerType {
		...
		/** Like "NON_CUMULATIVE", constructor automatically calls start() and destructor automatically calls stop() if neccessary. */
...
public:
	 ...
	 *  \param  name        Allows assignment of an optional name to a timer.  This is particularly useful for debugging.  The name, if provided, will
	 *                      also be used in error reporting,
	 */
	...
	/** \brief   Returns either the cumulative wall clock time between all pairs of calls to to start() and stop() or else just the last pair.
...
private:
	WallClockTimer(const WallClockTimer &rhs);                  // Intentionally unimplemented!
	const WallClockTimer &operator=(const WallClockTimer &rhs); // Intentionally unimplemented!
};
