...
class Uniform {
	...
	double max_;
public:
	/** \brief  Constructs an object representing a pseudo-random uniform distribution over the interval [0.0, 1.0), with an initial seed.
	...
	/** \brief  Constructs an object representing a pseudo-random uniform distribution over the interval [min, max), with an initial seed.
...
class Exponential {
	double mean_;
public:
	/** \brief  Constructs an object representing a pseudo-random exponential distribution with a given mean and an initial seed.
...
class Rand {
public:
	/** \brief  Constructs an object representing a pseudo-random uniform distribution with an initial seed, used to select "uniformly" distributed
	 *          unsigned integers between 0 and a given integer.
	 *  \param  seed  A random number generator seed.
	 */
	...
	unsigned operator()(const unsigned n) { return static_cast<unsigned>(static_cast<double>(n) * (::random() / static_cast<double>(RAND_MAX))); }
private:
 ...
 */
template <typename RandomAccessIterator> RandomAccessIterator RandomSample(const size_t size, RandomAccessIterator first, RandomAccessIterator last)
{
	static Rand rand;
 ...
 *  \return  An iterator to the beginning of the random sample, like std::remove().
 */
template <typename RandomAccessContainer> typename RandomAccessContainer::iterator RandomSample(size_t size, RandomAccessContainer * const container)
{
...
public:
	 ...
	 *  \param  distribution  Specifies the probability for each index.  Please note that the sum over all indices must add up to 1.0.
	 *  \param  seed          A random number generator seed.
	 */
	NonUniformRandom(const std::vector<double> &distribution, const unsigned seed): uniform_(seed) { init(distribution); }
 ...
 *  \return  An iterator to the beginning of the random sample, like std::remove().
 */
template <typename RandomAccessContainer> typename RandomAccessContainer::iterator RandomSample(const size_t size, RandomAccessContainer * const container,
												const std::vector<double> &distribution,
												const unsigned seed)
{
 ...
 */
template <typename Entry> void RemoveRandomSample(const size_t size, GNU_HASH_SET<Entry> * const source, GNU_HASH_SET<Entry> * const target)
{
	if (unlikely(size > source->size()))
		throw Exception("in Random::RandomSample: source container is too small for request sample size!");
	...
	for (typename GNU_HASH_SET<Entry>::iterator entry(source->begin()); entry != source->end(); ++selected) {
		if (*selected) {
			target->insert(*entry);
			source->erase(entry++);
		}
