...
class Resolver {
	...
	GNU_HASH_MAP<in_addr_t, unsigned> dns_server_ip_addresses_and_busy_counts_;
	...
	class Cache {
		...
		GNU_HASH_MAP<std::string, CacheEntry> resolved_hostnames_cache_;
	public:
		Cache() { }
...
class ThreadSafeDnsCache {
	...
	GNU_HASH_MAP<std::string, ThreadSafeDnsCacheEntry> resolved_hostnames_cache_;
	ThreadUtil::Mutex cache_access_mutex_;
public:
	ThreadSafeDnsCache() { }
	bool lookup(const std::string &hostname, std::set<in_addr_t> * const ip_addresses);
	void insert(const std::string &hostname, const std::set<in_addr_t> &ip_addresses, const uint32_t ttl);
...
class SimpleResolver {
	...
	ThreadUtil::Mutex dns_server_ip_addresses_and_busy_count_access_mutex_;
	uint16_t next_request_id_;
	ThreadUtil::Mutex request_id_mutex_;
public:
