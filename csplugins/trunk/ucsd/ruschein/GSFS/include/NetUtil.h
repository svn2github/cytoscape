...
template<typename Data> struct NetAddrBlockAndData {
	...
	Data data_;
public:
	NetAddrBlockAndData(const in_addr_t network_address, const in_addr_t mask,
			    const Data &data)
...
template<typename Data> Data NetAddrBlocksAndData<Data>::getSelection(const in_addr_t address,
								      const Data &default_data) const
{
	for (typename SList< NetAddrBlockAndData<Data> >::const_iterator net_addr_block_and_data = this->begin(); net_addr_block_and_data != this->end();
	     ++net_addr_block_and_data)
	{
...
{
	for (typename SList< NetAddrBlockAndData<Data> >::const_iterator iter = net_addr_blocks_and_data.begin(); iter != net_addr_blocks_and_data.end();
	     ++iter)
		output << *iter << '\n';
