...
public:
	...
	static const unsigned END_OF_STREAM            = 1u << 8u;
	static const unsigned UNEXPECTED_END_OF_STREAM = 1u << 9u;
	...
	public:
		 ...
		 *
		 *  The pair (name, value) is stored in the AttributeMap.  If there is an existing value associated with name, it is not inserted.
		 */
...
public:
	explicit HtmlParser(const std::string &input_string, unsigned chunk_mask = EVERYTHING, const bool header_only = false);
...
public:
	MetaTagExtractor(const std::string &document_source, const std::string &meta_tag_name,
		...
		: HtmlParser(document_source, HtmlParser::OPENING_TAG, /* header_only = */true), extracted_data_(*extracted_data)
	...
	MetaTagExtractor(const std::string &document_source, const std::list<std::string> &meta_tag_names,
		         std::list< std::pair<std::string, std::string> > * const extracted_data)
		: HtmlParser(document_source, HtmlParser::OPENING_TAG), meta_tag_names_(meta_tag_names), extracted_data_(*extracted_data) { }
...
private:
	...
	/** The urls extracted from the page. */
	std::list<UrlAndAnchorText> &urls_;

	/** The base URL that relative URLs are relative to. */
...
public:
	/** Construct a URL extractor for an HTML document. */
