	...
	bool accessAllowed(const std::string &user_agent, const Url &url) const
		{
			if (not url.isAbsolute())
				throw Exception("in RobotsDotTxt::accessAllowed: cannot check acces for a relative URL!");
...
class RobotsDotTxtCache {
	...
	GNU_HASH_MAP<std::string, RobotsDotTxt *> hostname_to_robots_dot_txt_map_;
	static ThreadUtil::Mutex mutex_;
