/** \file    Downloader.h
 *  \brief   Declaration of class Downloader.
...
class Downloader {
	...
	static ThreadUtil::Mutex *dns_mutex_;
...
public:
	...
	public:
		explicit Params(const std::string &user_agent = GetDefaultUserAgentString(),
				...
				const bool honour_robots_dot_txt = false, const TextTranslationMode text_translation_mode = MAP_TO_LATIN9,
				const PerlCompatRegExps &banned_reg_exps = PerlCompatRegExps(), const bool debugging = false, const bool follow_redirects = true);
	} params_;
...
public:
	 ...
	 *  \param  auto_simplify  If true, anything following the first semicolon, or if that can't be located, the first space will be removed together
	 *                         with the semicolon or space, if present.
	 *  \return A good guess as to the MIME type of the most recently downloaded document on success or an empty string upon failure.
	 */
...
private:
	static void InitCurlEasyHandle(const long dns_cache_timeout, const char * const error_buffer, const bool debugging,
				       ...
				       CURL ** const easy_handle, std::string * const user_agent, const bool follow_redirect);
	void init();
