 ...
 *  \note   The initial reason that this class exists is that the C++ standard stream classes in libstdc++ cannot handle handle 64 bit
 *          offsets and therefore files larger than 2GB on 32 bit platforms.  After implementing this class we also discovered that it
 *          provides for significantly better I/O performance than the stream classes in libstdc++.
 */
class File {
	FILE *file_;
	std::string path_;
...
public:
	 ...
	 *  \param  throw_on_error_behaviour   If true, any open failure will cause an exception to be thrown.  If not true you must use the
	 *                                     fail() member function.
	 *  \param  delete_on_close_behaviour  Determines if "path" will be deleted on close or not.
	 *  \note   Any  created files will have mode S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH (0666), as modified by the process'
	 *          umask value (see umask(2)).
	 */
	File(const std::string &path, const std::string &mode, const ThrowOnOpenBehaviour throw_on_error_behaviour = DO_NOT_THROW_ON_ERROR,
	...
	template <typename NumericType> bool serialize(const NumericType n)
		{
			if (unlikely(file_ == NULL))
				throw Exception("in File::serialize: can't serialize to a non-open file (4)!");
	...
	template <typename NumericType> bool deserialize(NumericType * const n)
		{
			if (unlikely(file_ == NULL))
				throw Exception("in File::deserialize: can't read from a non-open file (4)!");
	 ...
	 *  \return True upon success, else false.  If false, you can consult the global "errno" variable for the type of error encountered.
	 */
	 ...
	 *  \param  new_length  The length to which the file will be truncated if it was previously longer, or the length to which the file
	 *                      will be extended if it was previously shorter.
	 *  \return True if the operation succeeded, else false.  In case of an error, "errno" will be set to the approriate error code.
	 */
	 ...
	 *          waitOnAsyncReadCompletion() that returns true.  Any number of calls to waitOnAsyncReadCompletion() that return false
	 *          before it returns true are valid.
	 */
	 ...
	 *  \param  timeout  How long to wait until we give up.  UINT_MAX stands for infinity.  0 means poll and return immediately.  Any
	 *                   other value is considered to be a timeout in milliseconds.
	 ...
	 *  \note   Only one request may be outstanding per File.  If the file has been opened in append mode, the data will be written to
	 *          the end of the file regardless of the provided offset!
	 *  \note   After successfully calling this function you need to eventually follow up on it by exactly one call to
	 ...
	 *                            "start."
	 *  \note                     This call will block until we gain access and the lock succeeds.  An illegal operation will result in
	 *                            an exception being thrown.  Multiple calls to this function lead to coalescing of specified regions.
