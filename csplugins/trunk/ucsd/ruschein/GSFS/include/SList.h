...
template<typename EntryType> void SList<EntryType>::pop_front()
{
	if (unlikely(head_ == NULL))
		throw Exception("in SList<EntryType>::pop_front: can't pop an empty list!");
...
template<typename EntryType> typename SList<EntryType>::iterator SList<EntryType>::insert(SList<EntryType>::iterator where, const EntryType &new_entry)
{
	if (where == begin()) {
...
{
	if (unlikely(head_ == NULL))
		throw Exception("in SList<EntryType>::erase: can't erase an element from an empty list!");
	...
	if (where == begin()) {
		...
		throw Exception("in SList<EntryType>::erase: can't erase end()!");
	else {
...
template<typename EntryType> typename SList<EntryType>::iterator SList<EntryType>::erase(SList<EntryType>::iterator first, SList<EntryType>::iterator last)
{
	while (first != last)
		first = erase(first);
...
template<typename EntryType> void SList<EntryType>::RandomShuffle(const SList<EntryType>::iterator &first, const SList<EntryType>::iterator &last)
{
