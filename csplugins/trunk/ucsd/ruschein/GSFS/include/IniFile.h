...
public:
	 ...
	 *   libiViaCore etc directory, and is named X.conf (where "X" is the program name, as returned by
	 *   MsgUtil::GetProgName()).
	 */
	IniFile();
	 ...
	 *           specified via "string_to_value_map" or the default value if no entry has been found.
	 *  \note    If the variable is not defined in the section, the program returns the default value.  The expected
	 *           values for the variable are case sensitive.  The caller will have to use a static_cast to convert
	 *           the int-encoded enum to a variable of the approriate enumumerated type.  Any unknown value results
	 ...
	 *  \note             This is used to workaround the limitation that InFiles cannot have two elements in a section
	 *                    with the same name. So, if you want to simulate this, you must call them something like entry1, entry2, entry_last and
	 *                    then use this function like this: getSectionEntryValuesHavingNamesStartingWith("init", "entry");
	 */
	std::list<std::string> getSectionEntryValuesHavingNamesStartingWith(const std::string &section_name, const std::string &starting_with) const;

	/** \brief  Returns entry names from a section that start with a particular string, case insensitive
	 *  \param            section_name     The section name to check
	 *  \param            starting_with    The string which determines which items will be returned.
	 *  \return           the std::string values of all the items that matched.
	 *  \note             This is used to workaround the limiation that InFiles cannot have two elements in a section
	 *                    with the same name. So, if you want to simulate this, you must call them something like entry1, entry2, entry_last and
	 *                    then use this function like this: getSectionEntryNamesThatStartWith("init", "entry");
	 */
	std::list<std::string> getSectionEntryNamesThatStartWith(const std::string &section_name, const std::string &starting_with) const;
...
private:
	unsigned &getCurrentLineNo()
		{
			MSG_UTIL_ASSERT(!include_file_infos_.empty());
