 ...
 *
 * \note This makes use of "return type overloading". The conversion operators make the AutoConvert interval value (string pointer) and
 *       change the data into the type that is wanted by the context where it is used.  Example: functionThatTakesADouble(AutoConvert("27")) or
 *       functionThatTakesAString(AutoConvert(42)); The point is, AutoConvert converts to whatever is wanted in the context you are using
 *       it.  You don't have to know the type explicitly. Why is this necessary? Certain templates require the ability to change a string
 *       into just about any other format without knowing ahead of time what that format is. For example creating an std::pair from a string
 *       of type "key=value". Depending on what the "pair" is, is defined to be, maybe std::pair<int,double>, "key=value" (a string) must be
 *       converted into int and double respectively. If this is done in a template, there is no way to know ahead of time
 *       what specific "StringTo()" function to call. The AutoConvert figures that out at compile or run time as necessary.
 *
 *       For efficiency sake this uses a simple char buffer and is thus even faster than a function which returns a std::string since this
 *       one has NO heap allocation. Since it's in a completely private context no c problems can be caused by the users of this class.
 *
 *       Usage:
 *
 *       The AutoConvert can be used in most places where you'd use a To* style function. StringToSomething, NumberToString etc. You should
 *       use it only with function style semantecs. That is, don't create instances of AutoConvert for use later, use it immediately where
 *       you want the conversion done. Never return AutoConvert from a function. Since it contains a pointer to a string potentially
 *       outside itself. That referent might be destroyed, leaving the AutoConvert::string_pointer_ pointing at invalid memory.
 *
 *       Good Examples:
 ...
 *       operator()
 *
 *       AutoConvert has operator()(const TypeToConvertFrom &item). This allows an instance of AutoConvert to be used in a template class as
 *       a functor. All it does is set the interal values as would the AutoConvert constructor. STL functor semantics require this function.
 */
class AutoConvert {
	char string_value_[24]; // Only used if constructing from a number. Holds the string version of the number.
	const char *string_pointer_;  // Points at the string data being converted. Sometimes that is "string_value_[]" above.
        /**
	 * Prohibit creating on the heap. Since AutoConvert potentially contains a pointer to an external string, keeping it around longer
	 * than necessary increases potential for dangling pointer.
	 */
...
public:
	...
	explicit AutoConvert(const char single_character)
		: string_pointer_(string_value_)
	{
