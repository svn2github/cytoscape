...
/** \brief  A version strndup that uses stack memory. Stack is instantly allocated so there is no performance issue.
 *  \param  source          The source from which to dupicate.
 *  \param  max_size        The maximum amount of string we will be copying.
 *  \note   This must be a macro. If it were a function, it's return would destroy the very stack frame upon which
 ...
 *  \return A string representation for "n."
 */
std::string ToString(unsigned long long n, const unsigned radix = 10, const int width = 0, const char grouping_char = '\0',
...
template<typename StringContainer> unsigned UnescapeAndSplit(const std::string &mushed_data, const char separator, StringContainer * const container,
							     const bool suppress_empty_elements = true, const char escape_char = '\\')
{
...
inline int AlphaWordCompare(const char *lhs, const char *rhs)
	{ return ::strcasecmp(SkipNonAlphanumericChars(SkipLeadingArticle(lhs)), SkipNonAlphanumericChars(SkipLeadingArticle(rhs))); }
 ...
 */
std::string &ReplaceString(const std::string &old_text, const std::string &new_text, std::string * const s, const bool global = true);
 ...
 */
inline std::string ReplaceString(const std::string &old_text, const std::string &new_text, const std::string &s, const bool global = true)
{
	std::string temp_s(s);
 ...
 *  \param  unknown_char        The character to substitute if no ISO8859-15 equivalent exists.  Use a zero byte ('\\0') if you want no substitute.
 *  \param  use_overlap_tokens  If true, "overlap_token" will be used to indicate that the unicode symbol just before and just after the overlap token
 *                              should be combined.  This is usually used to combine standalone diacritical marks with their corresponding letters.
 *  \param  overlap_token       Only used if "use_overlap_token" is true.  This should be a byte code that can never be (part or) a valid UTF-8 character
 *                              sequence, in other words, it should start with at least 5 1-bits.  Beware of confusion with byte-order markers!
 *  \return The converted string (in ISO8859-15).
 */
std::string UTF8ToISO8859_15(const std::string &text, const char unknown_char = '?', const bool use_overlap_tokens = false,
			     const unsigned char overlap_token = 0xFFu);


/** \brief  Determines whether some text could conceivably be UTF-8.
 ...
 *          the string is UTF-8.  All it guarantees is that the string is a possible UTF-8 string.
 */
 ...
 *  \warning  Adler-32 has a weakness for short messages with few hundred bytes, because the checksums for these messages have a poor coverage of the 32
 *            available bits.
...
/** \brief   Tries to determine the best place to truncate a line of text, given that you want the result to be somewhat logical, sensible, a complete
 *           thought.
