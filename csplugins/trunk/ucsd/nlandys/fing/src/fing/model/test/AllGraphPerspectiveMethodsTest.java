...
public final class AllGraphPerspectiveMethodsTest
{
  ...
  public static final void main(String[] args)
  {
    ...
    for (int j = 0; j < nodeInx.length; j++) {
      for (int i = 0;; i++) { if (nodeIndicesArray[i] == nodeInx[j]) break; } }
    ...
    for (int j = 0; j < edgeInx.length; j++) {
      for (int i = 0;; i++) { if (edgeIndicesArray[i] == edgeInx[j]) break; } }
    ...
    // getOutDegree(Node).
    if (persp.getOutDegree(root2Node) != -1 ||
        persp.getOutDegree(nodeNotInPersp) != -1)
    ...
    // getOutDegree(int).
    if (persp.getOutDegree(minNodeInx - 1) != -1 ||
        persp.getOutDegree(0) != -1 ||
    ...
    if (persp.getOutDegree(minNodeInx - 3, false) != -1 ||
        persp.getOutDegree(0, true) != -1 ||
        persp.getOutDegree(2, false) != -1)
      throw new IllegalStateException("not out degree -1");
    ...
    if (!(persp.isMetaParent(persp.getNode(nodeInx[4]),
                             persp.getNode(nodeInx[4]))  &&
          persp.isMetaParent(persp.getNode(nodeInx[1]),
                             persp.getNode(nodeInx[0]))))
      throw new IllegalStateException("expected meta-relationship");
    if (persp.isMetaParent(persp.getNode(nodeInx[1]),
                           persp.getNode(nodeInx[2])) ||
        persp.isMetaParent(persp.getNode(nodeInx[1]),
                           persp.getNode(nodeInx[4])))
    ...
    if (persp.isNodeMetaParent(0, nodeInx[1]) ||
        ...
        persp.isNodeMetaParent(1, 2) ||
        persp.isNodeMetaParent(minNodeInx - 1, nodeInx[1]) ||
        persp.isNodeMetaParent
        (nodeNotInPersp.getRootGraphIndex(), nodeInx[4]) ||
        persp.isNodeMetaParent
    ...
    for (int i = 0;; i++)
      if (((Node) parentsList.get(i)).getRootGraphIndex() == nodeInx[0]) break;
    for (int i = 0;; i++)
    ...
    for (int i = 0;; i++)
      if (((Node) parentsList.get(i)).getRootGraphIndex() == nodeInx[0]) break;
    if (persp.nodeMetaParentsList(0) != null ||
        persp.nodeMetaParentsList(1) != null ||
    ...
    if (persp.getNodeMetaParentIndicesArray(0) != null ||
        ...
        persp.getNodeMetaParentIndicesArray
    ...
    List edgeChildrenList =
      persp.edgeMetaChildrenList(persp.getNode(nodeInx[4]));
    ...
    if (childEdgeInx.length != 2)
      throw new IllegalStateException("expected 2 edge children");
    for (int i = 0;; i++) if (childEdgeInx[i] == edgeInx[2]) break;
    for (int i = 0;; i++) if (childEdgeInx[i] == edgeInx[6]) break;
    if (persp.getEdgeMetaChildIndicesArray(0) != null ||
        ...
        persp.getEdgeMetaChildIndicesArray
        (nodeNotInPersp.getRootGraphIndex()) != null)
