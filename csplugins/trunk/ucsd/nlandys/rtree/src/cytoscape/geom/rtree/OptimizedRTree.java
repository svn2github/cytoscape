 ...
 */
public final class OptimizedRTree implements MutableSpacialIndex2D
{
   ...
   */
  public OptimizedRTree(final int maxBranches)
  {
  ...
  private final static class NodeArray
  {
    ...
    /*
     * This method will never increase the size of the underlying array.
     * Throws ArrayIndexOutOfBoundsException if index is negative.
     */
    private final Node getNodeAtIndex(final int index) {
      if (index >= m_nodeArr.length) return null;
      return m_nodeArr[index]; }

    /*
     * This method will increase the size of the
     * underlying array only if two conditions hold:
     *    1. node is not null and
     *    2. index is greater than or equal to the length of the array.
     * Throws ArrayIndexOutOfBoundsException if index is negative.
     * Throws IllegalArgumentException if index is Integer.MAX_VALUE and
     * node is not null.
     */
    private final void setNodeAtIndex(final Node node, final int index) {
      if (index >= m_nodeArr.length && node == null) return;
      try { m_nodeArr[index] = node; }
      catch (ArrayIndexOutOfBoundsException e) {
        if (index < 0) throw e;
        if (index == Integer.MAX_VALUE)
          throw new IllegalArgumentException("using objKey Integer.MAX_VALUE");
