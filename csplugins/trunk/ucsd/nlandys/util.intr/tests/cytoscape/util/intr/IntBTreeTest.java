...
public class IntBTreeTest extends TestCase {
    ...
    public void testCount() {
        final int[] countThese = new int[] { Integer.MIN_VALUE, -1, 0, 1, 3, 4, 6, 7, 8, 9, 10, 99, Integer.MAX_VALUE };

        for (int i = 0; i < countThese.length; i++)
            System.out.println("The count of integer " + countThese[i] +
                " is " + tree.count(countThese[i]) + ".");
    ...
    public void testMinMaxRange() {
        ...
        final int[] xMins = new int[] {
                Integer.MIN_VALUE, Integer.MIN_VALUE, -23, 1, 3, 2, 3, 8, 4, 4,
                5, 6, -1, 11
            };
        final int[] xMaxs = new int[] {
                Integer.MAX_VALUE, 3, 99, 2, Integer.MAX_VALUE, 6, 4, 8, 4, 8, 6,
                6, -1, 11
            };
