 ...
 * Note 1: this was copied from giny.util because it is being phased out.
 * Eventually the layout API will be available to use (TODO: remove layouter
 * when layout API is available) Note 2: this has been modified so that the
 * doLayout method is interruptable and reports progress to the MCODELoader
 * */
public class SpringEmbeddedLayouter {
	...
	protected HashMap nodeIndexToMatrixIndexMap; // maps a root graph index to a
													// distance matrix index
	protected TreeMap matrixIndexToNodeIndexMap; // maps a distance matrix index
													// to a root graph instance
	 ...
	 * 
	 * @param weightLayout
	 *            Weighting of this process as calculated by
	 *            MCODEUtil.convertNetworkToImage
	 * @param goalTotal
	 *            Numerical aim as calculated by MCODEUtil.convertNetworkToImage
	 *            based on number of processes required
	 * @param progress
	 *            Amount of work completed in finding the cluster before this
	 *            process started
	 * @param loader
	 *            Loading animation which displays the progress of this process
	 * @return true if the layout was completed without interruption, false
	 *         otherwise
	 */
	public boolean doLayout(int weightLayout, int goalTotal, double progress) {
		...
		Iterator<NodeView> nodes = graphView.getNodeViewsIterator();
		int count = 0;
		while (nodes.hasNext()) {
			NodeView nodeView = nodes.next();
			nodeIndexToMatrixIndexMap.put(new Integer(nodeView
					.getRootGraphIndex()), new Integer(count));
			matrixIndexToNodeIndexMap.put(new Integer(count), new Integer(
					nodeView.getRootGraphIndex()));
			count++;
		}
		...
		for (layoutPass = 0; layoutPass < numLayoutPasses; layoutPass++) {
			...
			// Until num_iterations, or the furthest node is not-so-far, move
			// the
			...
			for (int iterations_i = 0; ((iterations_i < num_iterations) && (furthest_node_partials.euclideanDistance >= euclidean_distance_threshold)); iterations_i++) {
				if (interrupted) {
					...
					// Before we shortcircuit the method, we reset the
					// interruption so that the method can run without
				...
				progress += 100.0
						* (((double) 1 / (double) (num_iterations * numLayoutPasses)))
		...
		// Just in case an interruption occured right before we exit the method,
		// we reset it, such an interruption
	...
	protected void setupNodeDistanceSprings() {
		...
		for (Iterator iterator = matrixIndices.iterator(); iterator.hasNext();) {
			...
			nodeList.add(i, graphView.getGraphPerspective().getNode(
					nodeIndex.intValue()));
			i++;
		}
		NodeDistances ind = new NodeDistances(nodeList, graphView
				.getGraphPerspective(), nodeIndexToMatrixIndexMap);
	...
	protected PartialDerivatives calculatePartials(PartialDerivatives partials,
		...
		int node_view_index = ((Integer) nodeIndexToMatrixIndexMap
				.get(new Integer(node_view.getRootGraphIndex()))).intValue();
		...
		while (iterator.hasNext()) {
			...
			if (node_view.getRootGraphIndex() == other_node_view
					.getRootGraphIndex()) {
				// System.out.println( "Nodes are the same. " );
				continue;
			}

			other_node_view_index = ((Integer) nodeIndexToMatrixIndexMap
					.get(new Integer(other_node_view.getRootGraphIndex())))
					.intValue();
