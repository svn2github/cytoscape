...
import BiNGO.reader.TextFileReader;
import BiNGO.reader.TextHttpReader;
...
public class BiNGOOntologyDefaultReader {
	...
	public BiNGOOntologyDefaultReader(File file, Map synonymHash) throws IllegalArgumentException, IOException,
			Exception {
		this(file.getPath(), synonymHash);
	}

	public BiNGOOntologyDefaultReader(String filename, Map synonymHash) throws IllegalArgumentException,
	...
	private void parse() throws Exception {
		...
		for (int i = 1; i < lines.length; i++) {
			...
			// adjusted : to avoid incorrect names if they contain [ in the
			// name...
			...
			// if(!ontology.containsTerm(((Integer)
			// synonymHash.get(id2)).intValue())){
			term = new OntologyTerm(name, ((Integer) synonymHash.get(id2)).intValue());
			ontology.add(term);
			/*
			 * } else{ term = ontology.getTerm(((Integer)
			 * synonymHash.get(id2)).intValue()); }
			 */
			...
			if (isaStart >= 0) {
				...
				for (int j = 0; j < allIsas.length; j++) {
					...
					// if((id3 == 6944) && (id2 ==
					// 6906)){System.out.println("isa OK " + ((Integer)
					// synonymHash.get(id2)).intValue() + "\t" + ((Integer)
					// synonymHash.get(id3)).intValue());}
			...
			if (partofStart >= 0) {
				...
				for (int j = 0; j < allPartofs.length; j++) {
					...
					// if((id3 == 6944) && (id2 ==
					// 6906)){System.out.println("partof OK " + ((Integer)
					// synonymHash.get(id2)).intValue() + "\t" + ((Integer)
					// synonymHash.get(id3)).intValue());}
