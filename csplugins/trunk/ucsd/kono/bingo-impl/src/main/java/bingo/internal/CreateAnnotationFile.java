 ...
 * <p/>
 * Class which creates a custom annotation file from the selected annotation and
...
public class CreateAnnotationFile {
	 ...
	 *            the Ontology.
	 * @param fileName
	 *            String with the name for the anno-file.
	 * @param dirName
	 *            Directory
	 ...
	 *            aliases of genes
	 */
	public CreateAnnotationFile(Annotation annotation, Map alias, Ontology ontology, String dirName,
			String fileName, Set selectedCanonicalNameVector) {
	...
	public void makeFile() {
		...
		try {
			...
			output.write("(species=" + parsedAnnotation.getSpecies() + ")(type=" + parsedAnnotation.getType()
					+ ")(curator=" + parsedAnnotation.getCurator() + ")\n");
	...
	private Annotation parse(final Set selectedCanonicalNameVector) {
				...
				while (it2.hasNext()) {
					...
					for (int k = 0; k < nodeClassifications.length; k++) {
						...
						for (int l = 0; l < nodeClassifications.length; l++) {
							if (ontology.getTerm(nodeClassifications[k]).isParentOrContainerOf(
									ontology.getTerm(nodeClassifications[l]))) {
								b = false;
							}
						}
						if (b == true) {
