...
public class GoBin extends javax.swing.JFrame {
	...
	/*
	 * private Color[]
	 * automaticColorArray={Color.BLUE,Color.RED,Color.GREEN,Color
	 * .YELLOW,Color.WHITE,Color.GRAY,
	...
	public GoBin(BiNGO.internal.ui.SettingsPanel settingsPanel, CyNetworkView networkView, final CyPluginAdapter adapter) {
	...
	private void initComponents() {
		 ...
		 * launchBiNGOButton.addActionListener(new ActionListener() { public
		 * void actionPerformed(ActionEvent ev){ startBiNGO(); } });
		 */
		 ...
		 * getGenesLinkedView().addActionListener(new ActionListener(){ public
		 * void actionPerformed(ActionEvent ev){ Component comp =
		 * getJTabbedPane().getSelectedComponent(); if (comp instanceof
		 * ResultPanel)
		 * ((ResultPanel)comp).getDisplayPieChart().doIt(((ResultPanel
		 * )comp).displayPieChartListener.APPLY_CHANGE, true, networkView); else
		 * if (comp instanceof StartPanel){ Component comp2 =
		 * getStartPanel().jTabbedPane.getSelectedComponent(); if (comp2
		 * instanceof StartPanelPanel)
		 * ((StartPanelPanel)comp2).getDisplayPieChart
		 * ().doIt(((StartPanelPanel)comp2).getDisplayPieChart().
		 * APPLY_CHANGE,true,networkView); else { ResultAndStartPanel all =
		 * getStartPanel().tabAll;
		...
		// JLabel("ouahhhhh"),java.awt.BorderLayout.SOUTH);

		// setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);

		// this.startPanel=new
		// StartPanel(this);//startPanel.validate();this.validate();
		// pack();
		...
		this.addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				windowClosed = true;
				/*
				 * if (getBingoWindow()!=null) getBingoWindow().dispose();
				 */
				/*
				 * if (getResultPanelCount()!=0){ GoBin
				 * goBin=(GoBin)e.getComponent();
				 * goBin.getResultPanelAt(1).displayPieChartListener
				 * .resetAll(goBin);
				 * 
				 * }
				 */
		...
		/*
		 * getStartPanel().jTabbedPane.setAlignmentY(getStartPanel().jTabbedPane.
		 * CENTER_ALIGNMENT);
		 * getStartPanel().jTabbedPane.setBorder(BorderFactory
		 * .createLineBorder(Color.BLACK)); getStartPanel().validate();
		 */
	 ...
	 * 
	 * String tmp = System.getProperty("user.dir") ; String bingoDir = new
	 * File(tmp,"plugins").toString() ; SettingsPanel settingsPanel = new
	 * SettingsPanel(bingoDir,this);
	 * getBingoWindow().getContentPane().removeAll();
	 * getBingoWindow().getContentPane().add(settingsPanel);
	 * 
	 * 
	 * getBingoWindow().addWindowListener(new WindowAdapter(){ public void
	 * windowClosed(WindowEvent e){ setBingoLaunched(false); } });
	 * 
	 * 
	 * 
	 * getBingoWindow().pack(); Dimension screenSize =
	 * Toolkit.getDefaultToolkit().getScreenSize();
	 * 
	 * getBingoWindow().setLocation(screenSize.width/2 -
	 * (getBingoWindow().getWidth()/2), screenSize.height/2 -
	 * (getBingoWindow().getHeight()/2)); getBingoWindow().setVisible(true);
	...
	public void createResultTab(Map testMap, Map correctionMap, Map mapSmallX, Map mapSmallN, Map mapBigX, Map mapBigN,
		...
		resultPanelCount++;
		/*
		 * Iterator it = this.getStartPanelIterator(); while (it.hasNext()){
			...
			for (int j = 0; j < jtable.getRowCount(); j++) {
				...
				while (it.hasNext()) {
					...
					if (goTerm.equals(goTerm2)) {
						...
						((JLabel) jtable.getValueAt(j, ResultPanel.DESCRIPTION_COLUMN)).setBackground((Color) this
								.getGoColor().get(goTerm2));
					}
				}
			}
	...
	/*
	 * void synchroSelections(){ ResultPanel result; JTable jtable; String
	 * goTerm; int temp;
	 * 
	 * CyNetworkView currentNetworkView = Cytoscape.getCurrentNetworkView(); Set
	 * goHashSet=new
	 * HashSet();//=((PieChartInfo)this.network_Options.get(currentNetworkView
	 * )).goToDisplay; PieChartInfo p =
	 * (PieChartInfo)this.getNetwork_Options().get(currentNetworkView); if
	 * (p!=null) goHashSet=p.goToDisplay;
	 * 
	 * 
	 * 
	 * for (int i =1;i<getResultPanelCount()+1;i++){
	 * 
	 * result = (ResultPanel)getResultPanelAt(i); jtable=result.jTable1; for
	 * (int j=0;j<jtable.getRowCount();j++){
	 * goTerm=(String)jtable.getValueAt(j,result.getGoTermColumn()); if
	 * (goHashSet.contains(goTerm)) jtable.setValueAt(new
	 * Boolean(true),j,result.getSelectColumn()); else jtable.setValueAt(new
	 * Boolean(false),j,result.getSelectColumn());
	 * 
	 * }
	 * 
	 ...
	 * getStartPanel().tabAll.unselectAll(); while (spit.hasNext()){
	 ...
	 * while(it.hasNext()){
	 * 
	 * goTerm=(String)it.next(); spit = this.getStartPanelIterator(); while
	 * (spit.hasNext()){ StartPanelPanel tab=(StartPanelPanel)spit.next(); temp
	 * = tab.getTermIndex(goTerm);
	 * 
	 * if (temp==-1){ Annotation ann =
	 * (Annotation)(getGoTerm_Annotation().get(goTerm));
	 * 
	 * if (ann!=null) tab.addLine(goTerm,ann,ann.getOntology());
	 * 
	 * } else { tab.getJTable().setValueAt(new
	 * Boolean(true),temp,tab.getSelectColumn());
	...
	Color getNextAutomaticColor(String term) {
		for (int i = 0; i < this.getAutomaticColorUsed().length; i++) {
			if (!getAutomaticColorUsed()[i]) {
				...
				return new Color(this.getAutomaticColorArray()[i][0], getAutomaticColorArray()[i][1],
						getAutomaticColorArray()[i][2]);
...
/*
 * class GoBin implements PropertyChangeListener { GoBin goBin; public
 * GoBin(GoBin goB){ this.goBin=goB; }
 * 
 * 
 * 
 * 
 * public void propertyChange(PropertyChangeEvent event) { //String net_id =
 * (String)event.getNewValue(); //CyNetwork network =
 * Cytoscape.getNetwork(net_id); if
 * (!event.getPropertyName().equals(Cytoscape.getDesktop
 * ().NETWORK_VIEW_DESTROYED)){ if
 * (goBin.getNetwork_Options().containsKey(Cytoscape.getCurrentNetworkView()))
 * goBin.synchroSelections(); else goBin.resetPieSelection();
 * //System.out.println("truc"); } else { CyNetworkView cv;
 * 
 * Iterator it=goBin.getNetwork_Options().keySet().iterator();
 * 
 * while (it.hasNext()){ cv= (CyNetworkView)it.next(); if
 * (!Cytoscape.getNetworkViewMap().containsValue(cv)){
 * goBin.getNetwork_Options().remove(cv); break; } } goBin.resetPieSelection();
 * 
 * 
 * }
 * 
