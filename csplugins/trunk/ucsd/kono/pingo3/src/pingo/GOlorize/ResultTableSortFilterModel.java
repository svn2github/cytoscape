...
public class ResultTableSortFilterModel extends AbstractTableModel{
    ...
    private class Row implements Comparable {
        ...
        public int compareTo(Object other){
            ...
            if (model.getColumnName(sortColumn).equals("Gene name") || model.getColumnName(sortColumn).equals("Gene alias")){
                //return ((Comparable)a).compareTo(b);
                return ((String)a).compareTo((String)b);
            }
            if (model.getColumnName(sortColumn).equals("GO Description") || model.getColumnName(sortColumn).equals("Gene Description")){
                return ((String)a).compareTo((String)b);
                //return ((Comparable)((JLabel)a).getText()).compareTo(((JLabel)b).getText()) ; 
            }
            
            if (model.getColumnName(sortColumn).equals("P value")){
                Double sa= new Double(((String)a).replaceAll(" ",""));
                Double sb= new Double(((String)b).replaceAll(" ",""));  
                return sa.compareTo(sb);
            }
            if (model.getColumnName(sortColumn).equals("Neighbor freq")||model.getColumnName(sortColumn).equals("Data freq")){
                int offs = ((String)a).indexOf(" ");
                Double sa = new Double(((String)a).substring(0,offs));
                offs = ((String)b).indexOf(" ");
                Double sb = new Double(((String)b).substring(0,offs));
                return -sa.compareTo(sb);
            }
            else return 0;
