...
public class MCODEAlgorithm {
	 ...
	 * 
	 * @param networkID
	 *            Allows the algorithm to get the parameters of the focused
	 *            network
	 */
	 ...
	 * 
	 * @param cancelled
	 *            Set to true if the algorithm should be cancelled
	 */
	 ...
	 *            Integer which is used to identify the nodes in the
	 *            score-sorted tree map
	 * @param resultTitle
	 *            Title of the results for which we are retrieving a node score
	 ...
	 * 
	 * @param inputNetwork
	 *            The network that will be scored
	 * @param resultTitle
	...
	public void scoreGraph(CyNetwork inputNetwork, String resultTitle) {
		...
		while (nodes.hasNext() && (!cancelled)) {
			...
			if (taskMonitor != null) {
				i++;
				taskMonitor.setPercentCompleted((i * 100)
	 ...
	 * 
	 * @param inputNetwork
	 *            The scored network to find clusters in.
	 * @param resultTitle
	 *            Title of the result
	...
	public MCODECluster[] findClusters(CyNetwork inputNetwork,
		...
		// want to use the
		// current scores that were generated the last time the network was
		// scored and store them
			...
			for (int j = 0; j < alNodesWithSameScore.size(); j++) {
				...
				if (!nodeSeenHashMap.containsKey(currentNode)) {
					...
					if (alCluster.size() > 0) {
						...
						// create an input graph for the filter and haircut
						// methods
						GraphPerspective gpCluster = createGraphPerspective(
								alCluster, inputNetwork);
						if (!filterCluster(gpCluster)) {
							...
							if (params.isFluff()) {
								fluffClusterBoundary(alCluster,
										nodeSeenHashMap, nodeInfoHashMap);
							}
							currentCluster.setALCluster(alCluster);
							gpCluster = createGraphPerspective(alCluster,
									inputNetwork);
	 ...
	 * 
	 * @param cluster
	 *            cluster being explored
	 * @param nodeScoreCutoff
	 *            slider source value
	 * @param inputNetwork
	 *            network
	 * @param resultTitle
	 *            title of the result set being explored
	 * @return explored cluster
	 */
	public MCODECluster exploreCluster(MCODECluster cluster,
			double nodeScoreCutoff, CyNetwork inputNetwork, String resultTitle) {
		// This method is similar to the finding method with the exception of
	 ...
	 * 
	 * @param nodeInfo
	 *            The internal data structure to fill with node information
	 ...
	 * 
	 * @param cluster
	 *            - The GINY GraphPerspective version of the cluster
	 ...
	 * 
	 * @param inputNetwork
	 *            The input network for reference
	 * @param nodeIndex
	 *            The index of the node in the input network to score
	...
	private NodeInfo calcNodeInfo(CyNetwork inputNetwork, int nodeIndex) {
		...
		if (neighbors.length < 2) {
			// if there are no neighbors or just one neighbor, nodeInfo
		...
		// density
		if (gpNodeNeighborhood != null) {
			nodeInfo.density = calcDensity(gpNodeNeighborhood, params
					.isIncludeLoops());
		}
		...
		// calculate the core density - amplifies the density of heavily
		// interconnected regions and attenuates
	 ...
	 * 
	 * @param startNode
	 *            The node that is the seed of the cluster
	 * @param nodeSeenHashMap
	 *            The list of nodes seen already
	 * @param nodeScoreCutoff
	 *            Slider input used for cluster exploration
	 * @param maxDepthFromStart
	 *            Limits the number of recursions
	 * @param nodeInfoHashMap
	 ...
	 * @param nodeSeenHashMap
	 *            The list of nodes seen already
	 * @param startNodeScore
	 *            The score of the seed node
	 * @param currentDepth
	 *            The depth away from the seed node that we are currently at
	 * @param cluster
	 *            The cluster to add to if we find a cluster node in this method
	 * @param nodeScoreCutoff
	 *            Helps determine if the nodes being added are within the given
	 *            threshold
	 * @param maxDepthFromStart
	...
	private boolean getClusterCoreInternal(Integer startNode,
		...
		for (i = 0; i < (((NodeInfo) nodeInfoHashMap.get(startNode)).numNodeNeighbors); i++) {
			...
			if ((!nodeSeenHashMap.containsKey(currentNeighbor))
				...
				getClusterCoreInternal(currentNeighbor, nodeSeenHashMap,
						startNodeScore, currentDepth + 1, cluster,
						nodeScoreCutoff, maxDepthFromStart, nodeInfoHashMap);
			}
		}
	 ...
	 * this method
	 * 
	 * @param gpClusterGraph
	 *            The cluster to check if it passes the filter
	 ...
	 *            The cluster graph
	 * @param cluster
	 *            The cluster node ID list (in the original graph)
	 * @param gpInputGraph
	 ...
	 * 
	 * @param gpInputGraph
	 *            The input graph to calculate the density of
	 * @param includeLoops
	 *            Include the possibility of loops when determining the number
	 *            of possible edges.
	 ...
	 *         object is the highest k value i.e. objectArray[0] The second
	 *         object is the highest k-core as a GraphPerspective i.e.
	 *         objectArray[1]
	 */
