...
public class MCODEUtil {
	 ...
	 */
	public static Image convertNetworkToImage(MCODELoader loader,
			MCODECluster cluster, int height, int width,
			SpringEmbeddedLayouter layouter, boolean layoutNecessary) {
		DGraphView view;
		Image image;
		...
		for (Iterator in = view.getNodeViewsIterator(); in.hasNext();) {
			...
			// First we check if the MCODECluster already has a node view of
			// this node (posing the more generic condition
			// first prevents the program from throwing a null pointer exception
			// in the second condition)
			if (cluster.getDGView() != null
					&& cluster.getDGView().getNodeView(
							nv.getNode().getRootGraphIndex()) != null) {
				// If it does, then we take the layout position that was already
				// generated for it
				nv.setXPosition(cluster.getDGView().getNodeView(
						nv.getNode().getRootGraphIndex()).getXPosition());
				nv.setYPosition(cluster.getDGView().getNodeView(
						nv.getNode().getRootGraphIndex()).getYPosition());
			} else {
				...
				nv.setXPosition((view.getCanvas().getWidth() + 100)
						* Math.random());
			...
			if (loader != null) {
				progress += 100.0 * (1.0 / (double) view.nodeCount())
		...
		for (Iterator ie = view.getEdgeViewsIterator(); ie.hasNext();) {
			...
			if (loader != null) {
				progress += 100.0 * (1.0 / (double) view.edgeCount())
	...
	public static boolean exportMCODEResults(MCODEAlgorithm alg,
			MCODECluster[] clusters, CyNetwork network, String fileName) {
		...
		try {
			...
			fout.write("Parameters:" + lineSep + alg.getParams().toString()
					+ lineSep);
			fout.write("Cluster	Score (Density*#Nodes)\tNodes\tEdges\tNode IDs"
					+ lineSep);
			...
			// MCODEUtil.convertClusterListToSortedNetworkList(clusters,
			// network, alg);
