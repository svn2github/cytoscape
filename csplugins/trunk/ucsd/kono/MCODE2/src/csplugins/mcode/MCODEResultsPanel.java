...
public class MCODEResultsPanel extends JPanel {
	...
	// Actual cluster data
	CyNetwork network; // Keep a record of the original input record for use in
						// the
	...
	int enumerationSelection = 0; // Keep track of selected attribute for
	 ...
	 * 
	 * @param clusters
	 *            Found clusters from the MCODEScoreAndFindTask
	 * @param alg
	 *            A reference to the alg for this particular network
	 * @param network
	 *            Network were these clusters were found
	 * @param imageList
	 *            A list of images of the found clusters
	 * @param resultTitle
	 *            Title of this result as determined by MCODESCoreAndFindAction
	 */
	public MCODEResultsPanel(MCODECluster[] clusters, MCODEAlgorithm alg,
			CyNetwork network, Image[] imageList, String resultTitle) {
		...
		// the view may not exist, but we only test for that when we need to (in
		// the
	...
	private JPanel createClusterBrowserPanel(Image imageList[]) {
		...
		// main data table
		modelBrowser = new MCODEResultsPanel.MCODEClusterBrowserTableModel(
				imageList);

		table = new JTable(modelBrowser);

		table.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		table.setDefaultRenderer(StringBuffer.class,
				new MCODEResultsPanel.JTextAreaRenderer(defaultRowHeight));
		...
		rowSM
				.addListSelectionListener(new MCODEResultsPanel.TableRowSelectionHandler());
	...
	private JPanel createExploreContent(int selectedRow) {
		...
		labelTable.put(new Integer((int) (currentParamsCopy
				.getNodeScoreCutoff() * 1000)), new JLabel("^"));
		...
		String[] availableAttributes = Cytoscape.getNodeAttributes()
				.getAttributeNames();
		...
		sizeSlider.addChangeListener(new MCODEResultsPanel.SizeAction(
				selectedRow, nodeAttributesComboBox));
		...
		modelEnumerator = new MCODEResultsPanel.MCODEResultsEnumeratorTableModel(
				new HashMap());
		...
		enumerationsTable.setFont(new Font(enumerationsTable.getFont()
				.getFontName(), Font.PLAIN, 11));
		enumerationsTable.setDefaultRenderer(StringBuffer.class,
				new MCODEResultsPanel.JTextAreaRenderer(0));
		...
		nodeAttributesComboBox
				.addActionListener(new MCODEResultsPanel.enumerateAction(
						enumerationsTable, modelEnumerator, selectedRow));
	...
	/**
	 * Creates a panel containing buttons for the cluster explore collapsable
	 * panel
	 * 
	 * @param selectedRow
	 *            Currently selected row in the cluster browser table
	 * @return panel
	 */
	private JPanel createBottomExplorePanel(int selectedRow) {
		...
		createChildButton
				.addActionListener(new MCODEResultsPanel.CreateChildAction(
						this, selectedRow));
	...
	/**
	 * Sets the network node attributes to the current result set's scores and
	 * clusters. This method is accessed from MCODEVisualStyleAction only when a
	 * results panel is selected in the east cytopanel.
	 * 
	...
	public double setNodeAttributesAndGetMaxScore() {
		...
		for (Iterator nodes = network.nodesIterator(); nodes.hasNext();) {
			...
			Cytoscape.getNodeAttributes().setAttribute(n.getIdentifier(),
					"MCODE_Node_Status", "Unclustered");
			for (int c = 0; c < clusters.length; c++) {
				...
				if (cluster.getALCluster().contains(new Integer(rgi))) {
					...
					if (Cytoscape.getNodeAttributes().getAttributeList(
							n.getIdentifier(), "MCODE_Cluster") != null) {
						clusterArrayList = (ArrayList) Cytoscape
								.getNodeAttributes().getAttributeList(
					...
					if (cluster.getSeedNode().intValue() == rgi) {
						Cytoscape.getNodeAttributes().setAttribute(
								n.getIdentifier(), "MCODE_Node_Status", "Seed");
					} else {
						Cytoscape.getNodeAttributes().setAttribute(
								n.getIdentifier(), "MCODE_Node_Status",
								"Clustered");
					}
				}
			}
	...
	private class CreateChildAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			final String title = trigger.getResultTitle() + ": "
					+ cluster.getClusterName() + " (Score: "
					+ nf.format(cluster.getClusterScore()) + ")";
			// create the child network and view
			final SwingWorker worker = new SwingWorker() {
				public Object construct() {
					CyNetwork newNetwork = Cytoscape.createNetwork(gpCluster
							.getNodeIndicesArray(), gpCluster
							.getEdgeIndicesArray(), title, network);
					DGraphView view = (DGraphView) Cytoscape
							.createNetworkView(newNetwork);
					...
					for (Iterator in = view.getNodeViewsIterator(); in
							.hasNext();) {
						nv = (NodeView) in.next();
						if (cluster.getDGView() != null
								&& cluster.getDGView().getNodeView(
										nv.getNode().getRootGraphIndex()) != null) {
							// If it does, then we take the layout position that
							// was already generated for it
							nv.setXPosition(cluster.getDGView().getNodeView(
									nv.getNode().getRootGraphIndex())
									.getXPosition());
							nv.setYPosition(cluster.getDGView().getNodeView(
									nv.getNode().getRootGraphIndex())
									.getYPosition());
						} else {
							// this will likely never occur
							// Otherwise, randomize node positions before layout
							// so that they don't all layout in a line
							// (so they don't fall into a local minimum for the
							// SpringEmbedder)
							// If the SpringEmbedder implementation changes,
							// this code may need to be removed
							nv.setXPosition(view.getCanvas().getWidth()
									* Math.random());
							// height is small for many default drawn graphs,
							// thus +100
							nv
									.setYPosition((view.getCanvas().getHeight() + 100)
											* Math.random());
	...
	private class MCODEClusterBrowserTableModel extends AbstractTableModel {
		...
		public MCODEClusterBrowserTableModel(Image imageList[]) {
			...
			for (int i = 0; i < clusters.length; i++) {
				...
				// create an image for each cluster - make it a nice layout of
				// the cluster
				Image image;
				if (imageList != null) {
					image = imageList[i];
				} else {
					image = MCODEUtil.convertNetworkToImage(null, clusters[i],
	...
	private class MCODEResultsEnumeratorTableModel extends AbstractTableModel {
		...
		public void listIt(HashMap enumerations) {
			// first we sort the hash map of attributes values and their
			// occurances
			...
			// finally we redraw the table, however, in order to prevent
			// constant flickering
			// we only fire the data change if the number or rows is altered.
			// That way,
			// when the number of rows stays the same, which is most of the
			// time, there is no
			// flicker.
	...
	private class enumerateAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			HashMap attributeEnumerations = new HashMap(); // the key is the
															// attribute value
															// and the value is
															// the number of
															// times that value
															// appears in the
															// cluster
			// First we want to see which attribute was selected in the combo
			// box
			String attributeName = (String) ((JComboBox) e.getSource())
					.getSelectedItem();
			int selectionIndex = (int) ((JComboBox) e.getSource())
					.getSelectedIndex();
			// If its the generic 'please select' option then we don't do any
			// enumeration
			if (!attributeName.equals("Please Select")) {
				// otherwise, we want to get the selected attribute's value for
				// each node in the selected cluster
				for (Iterator i = clusters[selectedRow].getGPCluster()
						.nodesIterator(); i.hasNext();) {
					Node node = (Node) i.next();
					// The attribute value will be stored as a string no matter
					// what it is but we need an array list
					// because some attributes are maps or lists of any size
					ArrayList attributeValue = new ArrayList();
					// Every type of attribute has its own get method so we have
					...
					if (Cytoscape.getNodeAttributes().getType(attributeName) == CyAttributes.TYPE_STRING) {
						attributeValue.add(Cytoscape.getNodeAttributes()
								.getStringAttribute(node.getIdentifier(),
										attributeName));
					} else if (Cytoscape.getNodeAttributes().getType(
							attributeName) == CyAttributes.TYPE_FLOATING) {
						attributeValue.add(Cytoscape.getNodeAttributes()
								.getDoubleAttribute(node.getIdentifier(),
										attributeName));
					} else if (Cytoscape.getNodeAttributes().getType(
							attributeName) == CyAttributes.TYPE_INTEGER) {
						attributeValue.add(Cytoscape.getNodeAttributes()
								.getIntegerAttribute(node.getIdentifier(),
										attributeName));
					} else if (Cytoscape.getNodeAttributes().getType(
							attributeName) == CyAttributes.TYPE_BOOLEAN) {
						attributeValue.add(Cytoscape.getNodeAttributes()
								.getBooleanAttribute(node.getIdentifier(),
										attributeName));
					} else if (Cytoscape.getNodeAttributes().getType(
							attributeName) == CyAttributes.TYPE_SIMPLE_LIST) {
						List valueList = Cytoscape.getNodeAttributes()
								.getAttributeList(node.getIdentifier(),
										attributeName);
						for (Iterator vli = valueList.iterator(); vli.hasNext();) {
							attributeValue.add(vli.next());
						}
					} else if (Cytoscape.getNodeAttributes().getType(
							attributeName) == CyAttributes.TYPE_SIMPLE_MAP) {
						Map valueMap = Cytoscape.getNodeAttributes()
								.getAttributeMap(node.getIdentifier(),
										attributeName);
						for (Iterator vmki = valueMap.keySet().iterator(); vmki
								.hasNext();) {
					...
					// Next we must make a non-repeating list with the attribute
					// values and enumerate the repetitions
					for (Iterator avi = attributeValue.iterator(); avi
							.hasNext();) {
						...
						if (aviElement != null) {
							...
							if (!attributeEnumerations.containsKey(value)) {
								...
								// time, we give it an enumeration of 1 and add
								// it to the enumerations
								attributeEnumerations
										.put(value, new Integer(1));
							} else {
								// If it already appeared before, we want to add
								// to the enumeration of the value
								Integer enumeration = (Integer) attributeEnumerations
										.get(value);
								enumeration = new Integer(enumeration
										.intValue() + 1);
			...
			// cluster explorations are looking at the already selected
			// attribute
	...
	private class TableRowSelectionHandler implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			if (!lsm.isSelectionEmpty()) {
				...
				// Upon selection of a cluster, we must show the corresponding
				// explore panel content
				// First we test if this cluster has been selected yet and if
				// its content exists
				// If it does not, we create it
				if (exploreContent[selectedRow] == null) {
					exploreContent[selectedRow] = createExploreContent(selectedRow);
				}
				// Next, if this is the first time explore panel content is
				// being displayed, then the
				// explore panel is not visible yet, and there is no content in
				// it yet, so we do not
				// have to remove it, otherwise, if the panel is visible, then
				...
				// content
				explorePanel.getContentPane().add(exploreContent[selectedRow],
						BorderLayout.CENTER);
				...
				// we get a reference to the combo box within the explore
				// content
				JComboBox nodeAttributesComboBox = (JComboBox) ((JPanel) exploreContent[selectedRow]
						.getComponent(1)).getComponent(0);
				// and fire the enumeration action
				nodeAttributesComboBox.setSelectedIndex(enumerationSelection);

				// TODO:
				// table.scrollRectToVisible(table.getCellRect(selectedRow, 0,
				// true));//this needs to be called when the explore panel shows
				// up so that any selection in the cluster browser is centered
				// in the visible portion of the table, but it doesn't work from
				// here
			}
		}
	}
	...
	/**
	 * Selects a cluster in the view that is selected by the user in the browser
	 * table
	 * 
	 * @param gpCluster
	...
	public void selectCluster(GraphPerspective gpCluster) {
		...
		if (gpCluster != null) {
			...
			if (networkView != null) {
				...
				// We want the focus to switch to the appropriate network view
				// but only if the cytopanel is docked
				// If it is not docked then it is best if the focus stays on the
				// panel
				if (Cytoscape.getDesktop().getCytoPanel(SwingConstants.EAST)
						.getState() == CytoPanelState.DOCK) {
					Cytoscape.getDesktop()
							.setFocus(networkView.getIdentifier());
				}
			} else {
	 ...
	 */
	private class JTextAreaRenderer extends JTextArea implements
			TableCellRenderer {
		int minHeight;
		...
		public JTextAreaRenderer(int minHeight) {
			...
			this
					.setFont(new Font(this.getFont().getFontName(), Font.PLAIN,
							11));
	...
	private class SizeAction implements ChangeListener {
		...
		public void stateChanged(ChangeEvent e) {
			// This method as been written so that the slider is responsive to
			// the user's input at all times, despite
			// the computationally challenging drawing, layout out, and
			// selection of large clusters. As such, we only
			// perform real work if the slider produces a different cluster, and
			// furthermore we only perform the quick
			// processes here, while the heavy weights are handled by the
			// drawer's thread.
			...
			MCODECluster cluster = alg.exploreCluster(clusters[selectedRow],
					nodeScoreCutoff, network, resultTitle);
			...
			if (!newCluster.equals(oldCluster)) {
				...
				nodeAttributesComboBox.setSelectedIndex(nodeAttributesComboBox
						.getSelectedIndex());

				// Ensure that a loader is set with the selected row and table
				// object
				// Also, we want to set the loader only once during continuous
				// exploration
				// It is only set again when a graph is fully loaded and placed
				// in the table
				...
				// There is a small difference between expanding and retracting
				// the cluster size
				// When expanding, new nodes need random position and thus must
				// go through the layout
				// When retracting, we simply use the layout that was generated
				// and stored
				// This speeds up the drawing process greatly
				boolean layoutNecessary = newCluster.size() > oldCluster.size();
				// Draw Graph and select the cluster in the view in a separate
				// thread so that it can be
				// interrupted by the slider movement
				drawer.drawGraph(cluster, layoutNecessary, this,
						drawPlaceHolder);
			}
		}
	}
	...
	private class GraphDrawer implements Runnable {
		 ...
		 *            True only if the cluster is expanding in size or lacks a
		 *            DGView
		 * @param trigger
		 *            Reference to the slider size action
		 * @param drawPlaceHolder
		 *            Determines if the cluster should be drawn or a place
		 *            holder in the case of big clusters
		 */
		public void drawGraph(MCODECluster cluster, boolean layoutNecessary,
				MCODEResultsPanel.SizeAction trigger, boolean drawPlaceHolder) {
		...
		public void run() {
			try {
				while (true) {
					// This ensures that the drawing of this cluster is only
					// attempted once
					// if it is unsuccessful it is because the setup or layout
					// process was interrupted by the slider movement
					// In that case the drawing must occur for a new cluster
					// using the drawGraph method
					if (drawGraph && !drawPlaceHolder) {
						Image image = MCODEUtil.convertNetworkToImage(loader,
								cluster, graphPicSize, graphPicSize, layouter,
								layoutNecessary);
						// If the drawing process was interrupted, a new cluster
						// must have been found and
						// this will have returned null, the drawing will be
						// recalled (with the new cluster)
						// However, if the graphing was successfull, we update
						// the table, sotp the loader from animating and select
						// the new cluster
						if (image != null && drawGraph) {
							// Select the new cluster (surprisingly a time
							// consuming step)
							loader.setProgress(100, "Selecting Nodes");
							selectCluster(cluster.getGPCluster());
							clusterSelected = true;
							// Update the table
							table.setValueAt(new ImageIcon(image), cluster
									.getRank(), 0);
							// Loader is no longer showing, so we let the
							// SizeAction know that
						...
						// draw place holder, only once though (as per the if
						// statement)
						Image image = MCODEUtil.getPlaceHolderImage(
								graphPicSize, graphPicSize);
						// Update the table
						table.setValueAt(new ImageIcon(image), cluster
								.getRank(), 0);
						// select the cluster
						selectCluster(cluster.getGPCluster());
						// Loader is no longer showing, so we let the SizeAction
						// know that
						trigger.loaderSet = false;
						// stop loader from animating and taking up computer
						// processing power
						loader.loaded();
						drawGraph = false;
						// Make sure this block is not run again unless if we
						// need to reload the image
						placeHolderDrawn = true;

					} else if (!drawGraph && drawPlaceHolder
							&& !clusterSelected) {
						selectCluster(cluster.getGPCluster());
						clusterSelected = true;
					}
					// This sleep time produces the drawing response time of 1
					// 20th of a second
					Thread.sleep(100);
				}
			} catch (Exception e) {
