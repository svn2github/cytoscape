...
import cytoscape.data.readers.KEGGtoXGMML;
...
public class ImportGraphFileAction extends CytoscapeAction {
	...
	public void takeArgs(String[] args) {
		...
		if (name.length() > 5) {
			...
			} else if (name.endsWith("xgmml")) {
				fileType = Cytoscape.FILE_XGMML;
			} else if (name.endsWith("xml")) {
				fileType = Cytoscape.FILE_KEGG;
			}
		}
...
private CyNetwork createNetwork(String location, int file_type,
		...
		} else if (file_type == Cytoscape.FILE_KEGG) {
			reader = new XGMMLReader(KEGGtoXGMML.transform(location));
		}else {
