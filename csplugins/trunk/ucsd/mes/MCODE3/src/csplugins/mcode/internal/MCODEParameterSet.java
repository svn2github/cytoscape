...
public class MCODEParameterSet {
	 ...
	 * NETWORK, selectedNodes = new Integer[0], loops = false, degree cutoff =
	 * 2, max depth = 100, k-core = 2, node score cutoff = 0.2, fluff = false,
	 * haircut = true, fluff node density cutoff = 0.1, row height for results
	 * table = 80 pixels.
	 */
	...
	/**
	 * Constructor for non-default algorithm parameters. Once an alalysis is
	 * conducted, new parameters must be saved so that they can be retrieved in
	 * the result panel for exploration and export purposes.
	 * 
	 * @param scope
	 *            scope of the search (equal to one of the two fields NETWORK or
	 *            SELECTION)
	 * @param selectedNodes
	 *            Node selection for selection-based scope
	 * @param includeLoops
	 *            include loops
	 * @param degreeCutoff
	 *            degree cutoff
	 ...
	 *            K-core
	 * @param optimize
	 *            determines if parameters are customized by user/default or
	 *            optimized
	 * @param maxDepthFromStart
	 *            max depth from start
	 ...
	 */
	public MCODEParameterSet(String scope, Integer[] selectedNodes,
		...
		setAllAlgorithmParams(scope, selectedNodes, includeLoops, degreeCutoff,
				...
				haircut, fluffNodeDensityCutoff);
	...
	public void setDefaultParams() {
		setAllAlgorithmParams(NETWORK, new Integer[0], false, 2, 2, false, 100,
				0.2, false, true, 0.1);
	}
	 ...
	 *            scope
	 * @param selectedNodes
	 *            Node selection for selection-based scopes
	 * @param includeLoops
	 *            include loops
	 * @param degreeCutoff
	 *            degree cutoff
	 ...
	 *            fluff node density cutoff
	 */
	public void setAllAlgorithmParams(String scope, Integer[] selectedNodes,
			boolean includeLoops, int degreeCutoff, int kCore,
			boolean optimize, int maxDepthFromStart, double nodeScoreCutoff,
			boolean fluff, boolean haircut, double fluffNodeDensityCutoff) {
	...
	/**
	 * Generates a summary of the parameters. Only parameters that are necessary
	 * are included. For example, if fluff is not turned on, the fluff density
	 * cutoff will not be included.
	 * 
	...
	public String toString() {
		...
		sb.append("   Network Scoring:" + lineSep + "      Include Loops: "
				+ includeLoops + "  Degree Cutoff: " + degreeCutoff + lineSep);
		sb
				.append("   Cluster Finding:"
						+ lineSep
						+ "      Node Score Cutoff: "
						+ nodeScoreCutoff
						+ "  Haircut: "
						...
						+ ((fluff) ? ("  Fluff Density Cutoff " + fluffNodeDensityCutoff)
								: "") + "  K-Core: " + kCore
						+ "  Max. Depth from Seed: " + maxDepthFromStart
						+ lineSep);
		return sb.toString();
	}
}
