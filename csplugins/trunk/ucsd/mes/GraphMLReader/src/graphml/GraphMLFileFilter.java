...
/**
 * FileFilter for Reading in GraphML Files.
 *
 * @author Cytoscape Development Team.
 */
public class GraphMLFileFilter extends CyFileFilter {
	/**
	...
	public GraphReader getReader(String fileName) {
		reader = new GraphMLReader(fileName);

		return reader;
	}

	/**
	 * Indicates which files the GraphMLFileFilter accepts.
	 *
	 * This method will return true only if:
	 * <UL>
	 * <LI>File ends in .xml;  and
	 * <LI>File headers includes the graphml.graphdrawing.org namespace declaration.
	 * </UL>
	 *
	 * @param file File
	...
	public boolean accept(File file) {
		...
		if (firstPass) {
			...
			try {
				...
				if (header.indexOf("graphml.graphdrawing.org") > 0) {
					return true;
				}
			} catch (IOException e) {
			}
