...
public class VennAnalytic {
	...
	private VennDiagram collectResults() {
		...
		return new VennDiagram(centers, diameters, polyAreas, residuals, circleLabels, residualLabels, 
		                       colors, intersectionCounts, warnings, luneCenters, stress, stress01, stress05);
	}
	...
	private void minimizeGlobal() {
		...
		for (int iter = 0; iter < 50; iter++) {
			...
			if (stress > lastStress)
				...
				copyCircles(centers, previousCenters);
			logger.info("global iteration, loss " + iter + " " + stress);
	...
	private void minimizeLocal() {
		...
		for (int iter = 0; iter < 50; iter++) {
			...
			recenter();
			stress = computeStress();
			logger.debug("local iteration, loss " + iter + " " + stress);
			if (stress > lastStress)
				copyCircles(previousCenters, centers);
			else
				copyCircles(centers, previousCenters);
