 ...
 * * Authors: Steven Maere, Karel Heymans
 * * Date: Mar.25.2005
 * * Description: Class which creates the new GraphPerspective and GraphView of the 
 ...
 * <p/>
 * Class which creates the new GraphPerspective and GraphView of the
...
public class DisplayBiNGOWindow {
    ...
    */
    ...
    /**
     * Method that builds up the new GraphPerspective and shows it to the user.
     */
    public void makeWindow() {
        GraphPerspective network = buildNetwork();
        buildNodeAttributes(network);
        buildEdgeAttributes(network);
        GraphView bingoGraphView = Cytoscape.createNetworkView(network, clusterName);
        ...
        NodeView nv;
        for (Iterator in = bingoGraphView.getNodeViewsIterator(); in.hasNext();) {
            nv = (NodeView) in.next();
        ...
        SpringEmbeddedLayouter spring = new SpringEmbeddedLayouter(bingoGraphView);
        //  Configure JTask
        ...
        try{
            ...
            //bingoGraphView.applyVizmapper(visualStyle);
            //bingoGraphView.setVisualStyle(visualStyle.getName());
            ...
            //VisualStyle oldStyle = vmm.setVisualStyle(visualStyle.getName());
            vmm.setVisualStyle(vmm.getCalculatorCatalog().getVisualStyle(visualStyle.getName()));
            //bingoGraphView.setVisualStyle(visualStyle.getName());
        }
     ...
     *
     * @return GraphPerspective the network that was built.
     */

    public GraphPerspective buildNetwork() {
        HashSet set;
        if (testMap != null) {
        ...
        GraphPerspective network = Cytoscape.createNetwork(clusterName);
     ...
     */
    public void buildNodeAttributes(GraphPerspective network) {
        ...
        while (i.hasNext()) {
            ...
            CyAttributes nodeAttrs = CyAttributesFactory.getCyAttributes("edge");
     ...
     */
    public void buildEdgeAttributes(GraphPerspective network) {
        Iterator i = network.edgesIterator();
        CyAttributes edgeAttrs = CyAttributesFactory.getCyAttributes("edge");
        while (i.hasNext()) {
