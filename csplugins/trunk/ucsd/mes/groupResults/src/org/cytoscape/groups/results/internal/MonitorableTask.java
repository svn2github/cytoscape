...
/**
 * Classes that perform long tasks (like graph algorithms) can implement this
 * interface so that they can be monitored by a GUI like a
 * <code>javax.swing.plaf.ProgressBarUI</code> or a
 * <code>javax.swing.ProgressMonitor</code> Note: this was copied from giny.util
 * because it is being phased out. Eventually the layout API will be available
 * to use (TODO: remove when layout API is available)
 */
...
public interface MonitorableTask {
	...
	/**
	 * Starts doing the task in a separate thread so that the GUI stays
	 * responsive
	 * 
