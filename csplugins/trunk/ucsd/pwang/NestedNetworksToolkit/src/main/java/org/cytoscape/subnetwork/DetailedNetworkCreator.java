...
//		networkAttr.setUserEditable(VisualStyleObserver.NETWORK_TYPE_ATTRIBUTE_NAME, false);
   	 	...
   	 	// Get parent network
   	 	String parentNetworkId = networkAttr.getAttribute(netID, "parentNetworkId").toString();
		...
		List<CyEdge> edges = (List<CyEdge>) parentNetwrok.getConnectingEdges(getIntersectingNodes(parentNetwrok, nodes));
...
//		}
		...
		{
			CyNode node = (CyNode)Cytoscape.getRootGraph().getNode(selected[i]);
			String parent = node.getIdentifier();
			
			Set<Node> sset = new HashSet<Node>();
			Iterator nodeIt2 = node.getNestedNetwork().nodesList().iterator();
