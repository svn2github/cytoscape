...
class MultiFasta:
    ...
    def get_singlefastafile(self, id):
        tmpfile = tempfile.mktemp()
	fasta_input = "%s -d %s -s %s " % (self.fastacmd_EXEC,
                                           self.mf_file,
                                           id)
...
class MultiFasta_MEM:
    def __init__(self, multifasta_file):
        self.seqs = {}
        self.mf_file = multifasta_file        
        seqs = Useful_Seq1.read_fasta(self.mf_file)
    
        for id in seqs:
            seqid = self.extract_id(id)
            sequence = SingleSeq2.SingleSeq(seqs[seqid])
...
if __name__ == "__main__":
    ...
    sfasta = mf.get_singlefasta(seqid)
    sfasta.set_ID_auto()
...
""")
    mfmem = MultiFasta_MEM(tmpseq.filename())
    for id in mfmem.get_sequences():
        print id
