...
t_query_ID = "Query ID"
t_subject_ID = "Subject ID"
t_e_value = "E-value"
t_identity = "Identity"
t_positive = "Positive"
t_overlap = "Overlap"
t_query_len = "Query length"
t_subject_len = "Subject length"
t_query_start = "Query start"
t_query_end = "Query end"
t_subject_start = "Subject start"
t_subject_end = "Subject end"

class HomologyDescr3:
    def __init__(self, homol_file):

        self.homol_file = homol_file
        self.query2subject = Data_Struct.Hash_A.Hash_headf_A()
        self.querysubject2hitinfo = Data_Struct.Hash_A.Hash_headf_A()
        ...
        self.query2subject.read_file(filename = self.homol_file,
                                     Key_cols_hd = self.term_query,
                                     Val_cols_hd = self.term_subject)

        self.querysubject2hitinfo.read_file(
    ...
    def enable_reverse(self):
        ...
        if not self.reverse_enabled:
            self.subject2query = Data_Struct.Hash_A.Hash_headf_A()
            self.subject2query.read_file(filename = self.homol_file,
    ...
    def query_len(self, query_ID, subject_ID = None): # subject_ID is actually unnecessary.
        ...
        return homol_info[self.info_num[t_query_len]]

    def subject_len(self, query_ID, subject_ID): # query_ID is actually unnecessary.

        homol_info = self.querysubject2hitinfo. \
                     val_force(query_ID, subject_ID)[0].split("\t")
        return homol_info[self.info_num[t_subject_len]]
    ...
    def query_start(self, query_ID, subject_ID):
        ...
        homol_info = self.querysubject2hitinfo. \
                     val_force(query_ID, subject_ID)[0].split("\t")
        return homol_info[self.info_num[t_query_start]]

    def query_end(self, query_ID, subject_ID):

        homol_info = self.querysubject2hitinfo. \
                     val_force(query_ID, subject_ID)[0].split("\t")
        return homol_info[self.info_num[t_query_end]]

    def subject_start(self, query_ID, subject_ID):

        homol_info = self.querysubject2hitinfo. \
                     val_force(query_ID, subject_ID)[0].split("\t")
        return homol_info[self.info_num[t_subject_start]]

    def subject_end(self, query_ID, subject_ID):

        homol_info = self.querysubject2hitinfo. \
                     val_force(query_ID, subject_ID)[0].split("\t")
        return homol_info[self.info_num[t_subject_end]]
...
if __name__ == "__main__":
    ...
    homol = HomologyDescr3(homol_file)
