...
for sat in sats_strand.get_sats():
    ...
    transcr_sense = Transcript_Factory()[s_id]
    transcr_antis = Transcript_Factory()[a_id]
    probes_sense = transcr_sense.get_probes()
    probes_antis = transcr_antis.get_probes()

    if len(probes_sense) != 1:
        raise "Number of probes is not 1 for " + s_id
    if len(probes_antis) != 1:
        raise "Number of probes is not 1 for " + a_id

    exppat_sense_dt = \
        expr_pat_set_dt.expression_pat(Transcript_Factory()[s_id].get_probes()[0])
    exppat_antis_dt = \
        expr_pat_set_dt.expression_pat(Transcript_Factory()[a_id].get_probes()[0])

    exppat_sense_rd = \
        expr_pat_set_rd.expression_pat(Transcript_Factory()[s_id].get_probes()[0])
    exppat_antis_rd = \
        expr_pat_set_rd.expression_pat(Transcript_Factory()[a_id].get_probes()[0])
    ...
    (wab_dt_sense_normal,
     ...
     wab_dt_antis_normal,
     wab_dt_antis_cancer) = \
     wab_dt_sheet.get_four_exp_pat(sat, 
                                   opt.get_normal_keys(), 
                                   opt.get_cancer_keys())
    ...
    (wab_rd_sense_normal,
     ...
     wab_rd_antis_normal,
     wab_rd_antis_cancer) = \
     wab_rd_sheet.get_four_exp_pat(sat, 
                                   opt.get_normal_keys(),
                                   opt.get_cancer_keys())
    ...
    # output.append("Antisense Categories", categ_a)
    # output.append("Antisense Oncogenes / Tumor suppressors", oncts_a)    

    output.append("Sense Coding Potential",     t1.get_info("coding"))
    output.append("Antisense Coding Potential", t2.get_info("coding"))
    output.append("SAT type", sat.get_info("type"))
    output.append("SAT category", sat.get_info("category"))
    output.append("Overlap length", sat.get_info("overlap length"))
    
    output.append("Colon Pos. Ctrl (Sense)", bertu.get(s_id, "")) # ID OK?
    output.append("Colon Pos. Ctrl (Antis)", bertu.get(a_id, "")) # ID OK?
    output.append("Hepat Pos. Ctrl (Sense)", patil.get(s_id, "")) # ID OK?
    output.append("Hepat Pos. Ctrl (Antis)", patil.get(a_id, "")) # ID OK?
...
output_macro_str = \
    output_macro(identifier  = output.get_header_idx("SAT ID") + 1,
