...
class Synonyms:
    def __init__(self, filename, main_p, 
                 ignore_ps = [], case_mode = True):

        self.case_mode = case_mode
        self._syn_table = {}
        self.read_file(filename, main_p, ignore_ps, case_mode)

    def read_file(self, filename, main_p, 
                 ignore_ps = [], case_mode = True):
        ...
        for rline in fh:
            ...
            r = sline.split("\t")
            main_term = r[main_p]
            if case_mode is False:
                main_term = main_term.lower()
            for i in range(len(r)):
                if i not in ignore_ps:
                    if case_mode:
                        syno_term = r[i]
                    else:
                        syno_term = r[i].lower()
    ...
    def to_main(self, term):
        ...
        if self.case_mode is False:
            term = term.lower()
        return self._syn_table[ term ]
...
""")
    ...
    syn2 = Synonyms(syno_test.filename(), 0, [1], False)
