...
hDB.read_file_hd(filename = rsc.BioBasic3_2008_DB,
...
rd_messages = open(rsc.BioBasic3_2008_rnd_m, "r").readlines()

MAIL_Command = "mail"
MAIL_Subject = "MCB3-2008: Result of your exam (Ver. 20081120)"
...
def auto_email(input_ID, send_email = True):
    ...
    alog = {}
    if path.isfile(rsc.BioBasic3_2008_Log):
        alog = pickle.load(open(rsc.BioBasic3_2008_Log, "r"))
        
    if alog.get(input_ID, 0) >= ACCESS_LIMIT:

        print cgi_html_out(open(rsc.BioBasic3_2008_html_ACLM, "r").read())
        ...
        message = open(rsc.BioBasic3_2008_mail_IDOK).read()
        ...
        html_out = open(rsc.BioBasic3_2008_html_IDOK, "r").read()
        ...
        html_out = open(rsc.BioBasic3_2008_html_IDNG, "r").read()
        html_out = replace_by_dict(html_out, m_replace)
        print cgi_html_out(html_out)

    alog[input_ID] = alog.get(input_ID, 0) + 1
    pickle.dump(alog, open(rsc.BioBasic3_2008_Log, "w"))
    open(rsc.BioBasic3_2008_LogTime, "a").write("\t".join((access_time, input_ID, name)) + '\n')
