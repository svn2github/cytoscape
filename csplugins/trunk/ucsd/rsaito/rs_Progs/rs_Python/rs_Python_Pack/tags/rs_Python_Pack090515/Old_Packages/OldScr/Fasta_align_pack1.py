...
class FASTA_pack:
    ...
    def get_alignment(self):
        ...
        if self.q_start() < self.q_end():
            ...
            al_q_end   = self.q_end()   - self.q_disp_start()
        else:
            al_q_start = self.q_disp_start() - self.q_start()
            al_q_end   = self.q_disp_start() - self.q_end() 

        al_s_start = self.s_start() - self.s_disp_start()
        al_s_end = self.s_end() - self.s_disp_start()
        
        return(disp_align1[al_q_start:al_q_end+1],
               disp_align2[al_s_start:al_s_end+1])
