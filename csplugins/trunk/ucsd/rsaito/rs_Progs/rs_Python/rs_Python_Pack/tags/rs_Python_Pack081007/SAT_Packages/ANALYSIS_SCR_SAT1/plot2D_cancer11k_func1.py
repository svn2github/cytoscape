...
def Plot_Cancer11kII(expr_sheet_cancer11k,
                        col_keys_normal,
                        col_keys_cancer,
                        cancer_gene_info,
                        plotting_categs):
    ...
    for afs_id in expr_sheet_cancer11k.conv_afas_onc.keys():
        ...
        categs = cancer_gene_info.get_categ_from_onc_id(onc_id)
        hit_categs = common(categs, plotting_categs, 2)
        if hit_categs:
            categ_descr = cancer_gene_info.get_category_descr(hit_categs[0])
        else:
            categ_descr = "Others"
        ...
        x = median(diff_sense)
        y = median(diff_antis)
        pset.add_point(categ_descr, [ x, y ])
...
if __name__ == "__main__":
    ...
    Plot_Cancer11kII(okay_expr_sheet,
                     normal_keys,
                     cancer_keys,
                     cancer_gene_info, ("FG00", ))
