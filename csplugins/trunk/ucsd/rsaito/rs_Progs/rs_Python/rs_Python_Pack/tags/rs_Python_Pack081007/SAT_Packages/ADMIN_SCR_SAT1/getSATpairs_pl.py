...
foreach my $chr ( sort keys %{$ap}){
	...
	print 
	    $chr, "\t", $sap->{SATID}, "\t" ,
	    $sap->{'bp'}, "\t" ,  #overlapping len(0: non-exon-overlapping)
	    $sap->{'status'}, "\t" , #category(some kind of str)
	    $sap->{clusterIDa}, "\t" , 
	    $sap->{ida}, "\t" , 
	    $sap->{cluster_a}{strand}, "\t",
	    $sap->{cluster_a}{start_boundary}, "\t",
	    $sap->{cluster_a}{end_boundary}, "\t",
	    $sap->{clusterIDb}, "\t" , 
	    $sap->{idb}, "\t" ,
	    $sap->{cluster_b}{strand}, "\t",
	    $sap->{cluster_b}{start_boundary}, "\t",
	    $sap->{cluster_b}{end_boundary}, "\n";
	last if( $TEST == 1 && $test1 == 2 ) ;
    }
