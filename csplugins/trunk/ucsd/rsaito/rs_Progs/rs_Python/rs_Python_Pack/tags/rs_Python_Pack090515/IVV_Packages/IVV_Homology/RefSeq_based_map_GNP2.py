...
hm_thres = HM(1.0e-30, 0.90, None, 30, None, 0.90)
...
if __name__ == "__main__":
    ...
    for refseqid in refseq_based.get_all_refseq():
        cds_start, cds_end = refseq_based.get_refseq_CDS(refseqid)
        print "\t".join((refseqid,
                         "RefSeq", refseqid,
                         `1`, `prey_refseq_len[ refseqid ]`,
                        `cds_start` + "-" + `cds_end`))
        for map_info in \
            refseq_based.get_refseq_based_map(refseqid).mapped_positions():
            mobj, start, end = map_info
            otype, id = mobj.get_ID().split("\t")
            cmnt = "-" # Used as bait name for prey
            if otype == "Prey":
                cmnt = (preyid_to_bait.val_accord_hd(id, "Bait Symbol")
                        + "_" +
