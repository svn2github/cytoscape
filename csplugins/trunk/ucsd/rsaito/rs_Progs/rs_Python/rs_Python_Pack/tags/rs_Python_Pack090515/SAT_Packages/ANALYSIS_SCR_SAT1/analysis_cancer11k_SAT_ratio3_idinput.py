...
from Calc_Packages.Math.Stats_OrderI import ordering
...
from Expr_Packages.Clustering.Cluster_Eisen_frmt1 import Cluster_Eisen_frmt_II_simple
...
# The following is needed to have appropriate strand organizations.
if opt.get_misc3() == "1":
    sats = sats_pc_pc
...
output = Cluster_Eisen_frmt_II_simple(normal_keys + cancer_keys + ["(Flag)"],
                                      [ 1.0 ] * len(normal_keys + cancer_keys) + [ 0.0 ])

fh = open(i_cdt_file, "r")
line_gid = fh.readline()
line_ewt = fh.readline()

for line in fh:

    gid, satid = line.rstrip().split("\t")[:2] 
    sat = sats.get_sat(satid)
    ...
    output.set_expression(gid, satid,
