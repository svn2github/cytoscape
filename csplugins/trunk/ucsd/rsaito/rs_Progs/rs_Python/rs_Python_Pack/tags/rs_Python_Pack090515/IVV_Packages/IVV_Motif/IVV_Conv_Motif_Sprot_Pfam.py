...
from Motif_Sprot_Pfam_info import Motif_Sprot_Pfam_info_Bait
from Motif_Sprot_Pfam_info import Motif_Sprot_Pfam_info_Prey


class IVV_Conv_Motif_Sprot_Pfam(IVV_Conv):
    def __init__(self, ivv_info,
                 motif_info_bait, motif_info_prey,
                 thres, mode = "S"):

        IVV_Conv.__init__(self, ivv_info, mode)
        self.motif_info_bait = motif_info_bait
        self.motif_info_prey = motif_info_prey
        self.thres = thres

    def bait2convid(self, bait_ID):
        return self.motif_info_bait.motif_info(bait_ID, self.thres)

    def prey2convid(self, prey_ID):
        return self.motif_info_prey.motif_info(prey_ID, self.thres)

    def convid2baits(self, motif):
        ids = self.motif_info_bait.get_seqid_from_motif(
            motif, self.thres)
    ...
    def convid2preys(self, motif):
        ids = self.motif_info_prey.get_seqid_from_motif(
            motif, self.thres)
        preys = []
        for id in ids:
