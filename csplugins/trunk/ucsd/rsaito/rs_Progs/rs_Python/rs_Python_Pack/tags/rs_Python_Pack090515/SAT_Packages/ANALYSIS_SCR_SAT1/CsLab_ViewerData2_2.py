...
from SAT_Packages.Integration.SAT_Global_Center import strand_o
...
def read_probe_info():
    ...
    sys.stderr.write("Reading probe info ...\n")
    ...
    for probeid in probe_info:
        ...
        afas_transcript_id = AFAS_symb + transcript_id
        afas_transcript = SAT_Transcr1.Transcript_Factory().make(afas_transcript_id)
        afas_transcript.add_probe(probe)
        probe.set_transcript(afas_transcript)
...
def read_transcr_info():
    ...
    for transcr_id in transcr_info:
        ...
        if chrom and strand and gposs and chrom != "N/A":
            if strand == "plus":
                strand = "+"
            elif strand == "minus":
                strand = "-"
    ...
    for oncid in conv_onc_afas:
        ...
        if transcr_id in SAT_Transcr1.Transcript_Factory():
            ...
            if afas_transcr_id in SAT_Transcr1.Transcript_Factory():
                ...
                open(rsc.CsLabViewerDataOutError, "a").write("Information for " + afas_transcr_id + " not found.\n")
        else:
            open(rsc.CsLabViewerDataOutError, "a").write("Information for " + transcr_id + " not found.\n")
            ...
            for transcript in cluster.get_transcripts():
                for probe in transcript.get_probes():
                    if (probe.get_id() not in conv_afas_onc and 
                        ...
                        continue
                    chr, tstrand = transcript.get_chromosome()
                    if probe.get_id().startswith(AFAS_symb):
                        pstrand = strand_o[ tstrand ]
                    else:
                        pstrand = tstrand                        
