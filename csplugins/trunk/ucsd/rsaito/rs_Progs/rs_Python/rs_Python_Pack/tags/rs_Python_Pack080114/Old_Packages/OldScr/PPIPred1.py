...
class PPIPred1(IVV_info.IVV_Conv.IVV_Conv):
    ...
    def __init__(self, ivv_info, mode = "S"):
        ...
        IVV_info.IVV_Conv.IVV_Conv.__init__(self, ivv_info, mode)
    ...
    def bait2convid(self, bait_ID):
        ...
        refseq_hits = self.ivv_to_refseq.val_force(bait_ID)
        ret_geneids = Usefuls.NonRedSet.NonRedSet()
        
        for refseq_hit in refseq_hits:
            refseq, eval = refseq_hit.split("\t")
            geneids = self.refseq_to_geneid.val_force(refseq)
            ret_geneids.append_list(geneids)

        # print bait_ID, ret_geneids.ret_set()
    ...
    def prey2convid(self, prey_ID):
        ...
        refseq_hits = self.ivv_to_refseq.val_force(prey_ID)
        ret_geneids = Usefuls.NonRedSet.NonRedSet()
        ...
        # print prey_ID, ret_geneids.ret_set()
        return ret_geneids.ret_set()
    ...
    def convid2baits(self, convid):
        ...
        for refseq in refseqs:
            bait_and_preyid_hits = self.refseq_to_ivv.val_force(refseq)
            # print "RefSeq --> IVV hits:", refseq, bait_and_preyid_hits
            for ivv_hit in bait_and_preyid_hits:
                ivv_id, eval = ivv_hit.split("\t")
                # print ivv_id, self.ivv_info.ID_Type(ivv_id)
                if self.ivv_info.ID_Type(ivv_id) == "Bait":
                    ret_baitids[ ivv_id ] = ""
        # print "Gene ID --> bait IDs", convid, ret_baitids.keys()
    ...
    def convid2preys(self, convid):
        ...
        for refseq in refseqs:
            ...
            for ivv_hit in bait_and_preyid_hits:
                ivv_id, eval = ivv_hit.split("\t")
                if self.ivv_info.ID_Type(ivv_id) == "Prey":
                    ret_preyids[ ivv_id ] = ""
    ...
    def set_mapping(self,
        ...
        self.ivv_to_refseq = Usefuls.Hash.Hash_headf("A")
        self.ivv_to_refseq.read_file(filename = ivv_to_refseq_homol_file,
                                    Key_cols_hd = ["Query ID"],
                                    Val_cols_hd = ["Subject ID",
                                                   "E-value"])
        
        self.refseq_to_ivv = Usefuls.Hash.Hash_headf("A")
        self.refseq_to_ivv.read_file(filename = ivv_to_refseq_homol_file,
                                    Key_cols_hd = ["Subject ID"],
                                    Val_cols_hd = ["Query ID",
                                                   "E-value"])
        
        self.refseq_to_geneid = Usefuls.Hash.Hash_filt("A")
        self.refseq_to_geneid.set_filt([0, "9606"])
        self.refseq_to_geneid.read_file(filename = refseq_to_geneid_file,
                                       Key_cols = [6],
                                       Val_cols = [1])

        self.geneid_to_refseq = Usefuls.Hash.Hash_filt("A")
        self.geneid_to_refseq.set_filt([0, "9606"])
...
def test():
    ...
    ivv_gene = PPIPred1(ivv_info, mode = "S")


    sys.stderr.write("Reading ID Conversion files...\n")
    ivv_gene.set_mapping(ivv_to_refseq_homol_file,
                         refseq_to_geneid_file)
