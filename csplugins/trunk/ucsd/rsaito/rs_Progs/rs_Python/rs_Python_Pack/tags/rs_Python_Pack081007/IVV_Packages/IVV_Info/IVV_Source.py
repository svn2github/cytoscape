...
class IVV_Source_set:
	...
	in spoke PPIs """
        ...
        for source in sources:
            preys = Usefuls.ListProc1.NonRedList(source.get_preys())
	...
	number of values for the given "key" """
        ...
        return (Usefuls.ListProc1.count_key_list(val, qlist),
                len(qlist))
    ...
    def eval_quals_matrix(self, key, val):
        ...
        for qlist in qlist_list:
            ...
            if self.ivv_info.Bait_info().bait_is_protein(bait):
                qlist1_hit = Usefuls.ListProc1.count_key_list(val, qlist1)
                qlist2_hit = Usefuls.ListProc1.count_key_list(val, qlist2)
...
if __name__ == "__main__":
    ...
    ivv_info = IVV_info1.IVV_info(ivv_info_file, filter)

    sys.stderr.write("Reading homology information...\n")
    homology = HomologyDescr(homology_file)
