...
class Analyze_Sheet:
    ...
    def get_extra_scoreA(self, stuid):
        if self.extra and stuid in self.extra:
            extra_info = self.extra[stuid].split("\t")
            if (len(extra_info) >= 1 and
                extra_info[0] != "" and
                (not extra_info[0].isspace())):
    ...
    def get_extra_scoreB(self, stuid):
        if self.extra and stuid in self.extra:
            extra_info = self.extra[stuid].split("\t")
            if (len(extra_info) >= 2 and
                extra_info[1] != "" and
                (not extra_info[1].isspace())):
                return int(extra_info[1])
    ...
    def get_extra_infoA(self, stuid):
        if self.extra and stuid in self.extra:
            extra_info = self.extra[stuid].split("\t")
            if (len(extra_info) >= 3 and
                extra_info[2] != "" and
                (not extra_info[2].isspace())):
                return extra_info[2]
    ...
    def get_extra_infoB(self, stuid):
        if self.extra and stuid in self.extra:
            extra_info = self.extra[stuid].split("\t")
            if (len(extra_info) >= 4 and
                extra_info[3] != "" and
                (not extra_info[3].isspace())):
                return extra_info[3]

        return ""
    
    def get_validity_from_extra_infoA(self, stuid):
        """ If valid, True. If not, False. """
        if (self.extra and stuid in self.extra
            and invalid_A in self.extra[stuid]):
            return False
        else:
            return True
    
    def get_validity_from_extra_infoB(self, stuid):
        """ If valid, True. If not, False. """
        if (self.extra and stuid in self.extra
            and invalid_B in self.extra[stuid]):
            return False
        else:
            return True
    ...
    def output(self):
        ...
        for stuid in self.get_students():
            department     = self.get_students().val_accord_hd(stuid, "Department")
            grade          = self.get_students().val_accord_hd(stuid, "Grade")
            name_family    = self.get_students().val_accord_hd(stuid, "Family Name")
            name_given     = self.get_students().val_accord_hd(stuid, "Given Name")
            account        = self.get_students().val_accord_hd(stuid, "Account")
            email          = self.get_students().val_accord_hd(stuid, "E-Mail")
            ...
            validityA      = self.get_validity_from_extra_infoA(stuid)
            validityB      = self.get_validity_from_extra_infoB(stuid)
            ...
            if validityA:
                ...
                validityA_str = "Invalid"
            if validityB:
                validityB_str = ""
            else:
                validityB_str = "Invalid"
                
            tb.append("Student ID", stuid)
            tb.append("Department", department)
            tb.append("Grade", grade)
            tb.append("Family Name", name_family)
            tb.append("Given Name", name_given)
            tb.append("Account", account)
            tb.append("E-Mail", email)
            ...
            tb.append("Presentation Score A", extra_scoreA_str) # Extra Score A
            tb.append("Presentation Score B", extra_scoreB_str) # Extra Score B
            tb.append("Presentation Days A",  extra_infoA)      # Extra Info A
            tb.append("Presentation Days B",  extra_infoB)      # Extra Info B
            
            tb.append("Validity A", validityA_str)
            tb.append("Validity B", validityB_str)
...
Student_ID Department Grade Family_Name Given_Name Account E-Mail
70641200   Pol        1     X           xxx        id1     id1@email
70644300   Pol        1     Y           yyy        id2     id2@email
70642300   Pol        1     Z           zzz        id3     id3@mail
70680000   Pol        1     W           www        id4     id4@mail
70649300   Pol        1     A           aaa        id5     id5@email
79151932   Env        3     Saito       Rintaro    rsaito  rsaito@email
""", tospace = "_")
    
    extra_info_list = TmpFile_III("""
70644300 7 15 9/15 10/15
70649300
70641200 0  0  -     -   Invalid-A
""")
    ...
    analyze.output()
    # print analyze.get_validity_from_extra_infoA("70641200")
    # print analyze.get_validity_from_extra_infoB("70641200")
    # print analyze.get_validity_from_extra_infoB("70644300")
