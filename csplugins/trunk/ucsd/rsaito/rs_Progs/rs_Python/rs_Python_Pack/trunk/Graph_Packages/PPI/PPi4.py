...
# Warning!!! Be careful when comparison of multiple PPi4 instances
# is conducted. Protein instances with same name may have different
# object ID.
class PPi4:
    def __init__(self, ref_protein_set = None):
	self.ppi = {}
        self.ppi_rev = {}
	self.protein_set = Protein2.Protein_Set()

        if (not ref_protein_set is None) and \
           (not isinstance(ref_protein_set, Protein2.Protein_Set)):
            raise "Instance type mismatch."

        self.ref_protein_set = ref_protein_set

    def get_ref_protein_set(self):

        return self.ref_protein_set
    ...
    def set_ppi(self, p1, p2):
        ...
        ***************************************************
        ...
        set_ppi and del_ppi, controls ppi, ppi_rev and
        protein set.
        ***************************************************
        """

        if not self.ref_protein_set is None:
            p1 = self.ref_protein_set.add_protein(p1)
            p2 = self.ref_protein_set.add_protein(p2)
    ...
    def add_ppis(self, other_ppi):
        ...
        if not isinstance(other_ppi, PPi4):
            raise "Instance type mismatch: PPi4 expected."
    ...
    def ppi_filter(self):
        ...
        ppi_filtered = PPi4(self.ref_protein_set)
...
""")
    ...
    """
    ref_protein_set = Protein.Protein_Set()

    ppi_a = PPi4(ref_protein_set)
    ppi_a.read_from_file(tmp_obj.filename(), 0, 1, 2)
    print ref_protein_set.get_proteins()
    ppi_a.ppi_info1()

    ppi_b = PPi4(ref_protein_set)
    ppi_b.read_from_file(tmp_obj2.filename(), 0, 1, 2)
    print "..."
    print ref_protein_set.get_proteins()
    print ppi_b.get_ref_protein_set().get_proteins()
    print "..."
    ppi_b.ppi_info1()
    print "OKOK"
    """
    ...
    ppi.ppi_info1()
    print "Interactors of protein", proteinB
    ...
    ppi2 = PPi4()
    proteinA2 = Protein2.Protein_Factory().make("A2")
    proteinB2 = Protein2.Protein_Factory().make("B2")
    proteinC2 = Protein2.Protein_Factory().make("C2")
