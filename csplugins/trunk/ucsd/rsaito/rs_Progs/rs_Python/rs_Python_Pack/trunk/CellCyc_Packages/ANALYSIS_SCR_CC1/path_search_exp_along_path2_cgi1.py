...
fw = open(rsc.Cell_cyc_path_analysis_result1, "w")
fw.write(path_search_expr.get_info_I())
fw.close()

fw = open(rsc.Cell_cyc_path_analysis_result2, "w")
for line in path_search_expr.get_exp_along_path(cellcyc_cmplx_exp,
                                                conditions):
    fw.write("\t".join(line) + "\n")
fw.close()

fw = open(rsc.Cell_cyc_path_analysis_result3, "w")
fw.write(path_search_expr.get_nodes_info_goal_path_II_str(mediate_node))
fw.close()

html = open(rsc.Cell_cyc_path_html_analysis_result).read()
