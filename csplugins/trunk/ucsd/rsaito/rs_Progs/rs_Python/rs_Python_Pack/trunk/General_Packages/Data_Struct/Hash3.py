...
class Hash:
    ...
    def enable_ordered_keys_mode(self):
        """ Enable it before reading. """
        self.ordered_keys_mode = True
...
#########################################################
    ...
    def pair_val(self, key1, key2):
        """ Redundant values will be returned if
        key1 and key2 is identical. """
        
        if self.get_val_type() != "A":
            if self.has_key(key1, key2):
            ...
            ret = []
            if self.has_key(key1, key2):
                ret += self.val(key1, key2)
            if self.has_key(key2, key1):
                ret += self.val(key2, key1)
            return ret

    def filt_key(self, *filters):
        """ Gets keys having certain key patterns
        Extraction using header labels not implemented.
        """
    ...
    def record(self, r, Key_cols, Val_cols):
        ...
        """ The following lines are no longer valid since they can be
            controlled by value columns """ 
    ...
    def read_file_hd(self, filename,
                     Key_cols_hd,
                     Val_cols_hd = None,
                     Fil_cols_hd = None):
...
""")
    ...
    h_test = Hash("S")
    h_test.enable_ordered_keys_mode()
#    h_test.verbose_mode()
...
Given     Family     Age Place   Date
Satomi    Nakazawa    -2 Land    1995.3
Tomoko    Yamagishi   -3 Land    1998.4
Reiko     Tomita      -2 Sea     2006.9
Akiko     Sato        -1 Land    2007.12
""")
    disney_with = Hash("A")
...
Given     Family     Age Date
Tomoko    Yamagishi   -3 1998.8
Akiko     Sato        -1 2006.1
Akiko     Shiozawa     0 2006.3
Ayako     Kazamaki    +2 2008.1
""")
    niagara_with = Hash("A")
...
\tCanada\t""", trim_f_line_flag = True)
    ...
    tmp_obj8 = Usefuls.TmpFile.TmpFile_III("""
...
""")
    ...
    h_simple = Hash("A")
    h_simple.read_file_hd(filename = tmp_obj8.filename(),
                          Key_cols_hd = [ "P1", "P2" ],
                          Val_cols_hd = [ "Val" ])
    print h_simple.pair_val("A", "A")
    print h_simple.pair_val("A", "B")
    print h_simple.pair_val("C", "D")
...
""")
    ...
    h_test20 = Hash("S")
    h_test20.read_file(filename = tmp_obj.filename(),
                       Key_cols = [0,1],
                       Val_cols = "L")
    print h_test20[ "Rintaro\tSaito" ], "---"
    
    
    h_test21 = Hash("S")
    h_test21.read_file(filename = tmp_obj.filename(),
                       Key_cols = [0,1],
                       Val_cols = None)
    print h_test21[ "Rintaro\tSaito" ], "---"
...
""")
    ...
    h_test22 = Hash("A")
    h_test22.read_file_hd(filename = tmp_obj2.filename(),
                          Key_cols_hd = [ "Family" ],
                          Val_cols_hd = [ "Family", "Given"])
    print h_test22[ "Saito" ], "---"
    ...
    h_test23 = Hash("S")
    h_test23.read_file_hd(filename = tmp_obj2.filename(),
                          Key_cols_hd = [ "Family" ],
                          Val_cols_hd = None)
    print h_test23[ "Saito" ], "---"
    print h_test.keys()
