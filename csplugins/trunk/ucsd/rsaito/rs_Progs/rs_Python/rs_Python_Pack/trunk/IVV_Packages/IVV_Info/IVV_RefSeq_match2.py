...
class IVV_RefSeq_match:
    ...
    def match_all(self):
        ...
        for ivvseqid in prey_info.preys():
            refseqid_v = prey_info.get_qual_noerror(ivvseqid, "hit_refseqid")
            if not refseqid_v:
                continue
            refseqid, version = refseqid_v.split(".")
...
if __name__ == "__main__":
    ...
    ivm.set_db(ivv_info, rsc.IVVSeq, rsc.IWAS_RefSeq)

    """ This part should be used to calculate
    relationship between IVV and RefSeq """

    sys.stderr.write("Matching IVV with IWAS RefSeq ...\n")
