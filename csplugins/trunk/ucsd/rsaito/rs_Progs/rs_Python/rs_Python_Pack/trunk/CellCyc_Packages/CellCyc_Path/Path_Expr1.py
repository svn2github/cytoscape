...
class Path_Search_expr(Path_search.Path_Search):
    ...
    def loop_steps_until_goal_incorp_expr(self,
                                          ...
                                          thres,
                                          goal_end_mode = False):
        instance_class_check(bot_expr, Botstein.Botstein_Sheet)

        gene_names = bot_expr.row_labels()
        on_gene_names = bot_expr.get_genes_above_thres_simp(
            col_label,
            thres) # May be synonyms must be considered...
        on_nodes = []
        ...
        for step in steps:
            for node in step2nodes[step]:
                ...
                for col_label in col_labels:
                    exp_level = bot_expr.get_datum_filter(node.get_node_name(),
                                                          col_label,
                                                          filter)
    ...
    for node in node_mark_set:
        ...
        for path_rank in path_search_expr.get_node_mark(node).get_paths():
            for path in path_rank:
                print path
                print path.get_path_node_names()
        print
