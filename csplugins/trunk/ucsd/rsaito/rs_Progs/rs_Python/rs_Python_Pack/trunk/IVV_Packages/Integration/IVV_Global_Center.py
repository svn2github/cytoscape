...
def get_reported_ppi2(taxid = "9606"):

    global reported_ppi

    if not 'reported_ppi' in globals():
        sys.stderr.write("Began reading reported PPIs...\n")
        reported_ppi = Data_Struct.Hash2.Hash("A")
        reported_ppi.set_filt([7, "GeneID"],
                              [0, taxid],
                              [5, taxid])
        reported_ppi.read_file(filename = rsc_ncbi.PPI_hs,
                               Key_cols = [1, 6],
                               Val_cols = [13])
        sys.stderr.write("Finished reading %d PPIs\n" % len(reported_ppi.keys()))
    return reported_ppi
...
def get_homol_ivv_to_refseq():
    ...
    if not 'homol_ivv_to_refseq' in globals():
        ...
        sys.stderr.write("Reading homology information (IVV -> RefSeq)...\n")
        homol_ivv_to_refseq = Homology_descr.HomologyDescr(
            rsc.HomolIVVRefSeq_cDNA_NF) # I forgot the reason, but it was rsc.HomolIVVRefSeq_cDNA_NF2
...
def get_gene_info():
    ...
    if not 'gene_info' in globals():
        ...
        gene_info.read_file(rsc_ncbi.GeneInfo_hs,
                            Key_cols = [1],
                            Val_cols = [2,8])
...
def get_geneid_to_refseq(taxid = "9606"):
    ...
    if not 'geneid_to_refseq' in globals():
        ...
        geneid_to_refseq.read_file(filename = rsc_ncbi.gene2refseq_hs,
                                   Key_cols = [1],
                                   Val_cols = [6])
...
if __name__ == "__main__":
    ...
    reported_ppi_tmp        = get_reported_ppi2()
