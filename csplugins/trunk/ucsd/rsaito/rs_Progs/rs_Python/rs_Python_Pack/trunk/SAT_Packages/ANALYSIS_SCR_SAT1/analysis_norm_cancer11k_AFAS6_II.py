...
from SAT_Packages.SAT44K_Info2.Simple_map_hs1 import get_antis_overlap_geneid
...
from analysis_norm_cancer11k_AFAS_VBA5 import output_macro
...
accession2geneid = SAT_Global_Center.get_accession2geneid()
...
sys.stderr.write("Reading Positive Controls ... (Bertucci et al.)\n")
bertu = accession_to_updown_hash()
sys.stderr.write("Reading Positive Controls ... (Patil et al.)\n")
patil = accession_to_foldchange_hash()

sys.stderr.write("Getting Positive and significant ...\n")
pas_dt_sheet = cancer11k_dt.get_pas_sheet()
pas_rd_sheet = cancer11k_rd.get_pas_sheet()

sys.stderr.write("Getting Well above background ...\n")
...
for afs_id in cancer11k_dt.get_exp_sheet().conv_afas_onc.keys():
    ...
    geneid = accession2geneid.val_force(onc_id[4:])
    antis44k, antis44k_len = get_antis_overlap_geneid(geneid)
    ...
    (pas_dt_sense_normal,
     ...
     pas_dt_antis_normal,
     pas_dt_antis_cancer) = \
     pas_dt_sheet.get_four_exp_pat(afs_id, 
                                   opt.get_normal_keys(), 
                                   opt.get_cancer_keys())
    ...
    (pas_rd_sense_normal,
     ...
     pas_rd_antis_normal,
     pas_rd_antis_cancer) = \
     pas_rd_sheet.get_four_exp_pat(afs_id, 
                                   opt.get_normal_keys(),
                                   opt.get_cancer_keys())
    ...
    output.append("AFAS ID", "AFAS-" + afs_id.replace("AFAS-Onc-Anti-", ""))
    ...
    output.append("AFAS probe condition", afas_probe_conds[ afs_id ])

    output.append("SAT information 44K", antis44k)
    output.append("SAT 44K average overlap length", `antis44k_len`)

    output.append("PositiveAndSignificant num. Normal sense dT", `sum(pas_dt_sense_normal)`)
    output.append("PositiveAndSignificant num. Normal antis dT", `sum(pas_dt_antis_normal)`)
    output.append("PositiveAndSignificant num. Cancer sense dT", `sum(pas_dt_sense_cancer)`)
    output.append("PositiveAndSignificant num. Cancer antis dT", `sum(pas_dt_antis_cancer)`)
    output.append("PositiveAndSignificant num. Normal sense Rd", `sum(pas_rd_sense_normal)`)
    output.append("PositiveAndSignificant num. Normal antis Rd", `sum(pas_rd_antis_normal)`)
    output.append("PositiveAndSignificant num. Cancer sense Rd", `sum(pas_rd_sense_cancer)`)
    output.append("PositiveAndSignificant num. Cancer antis Rd", `sum(pas_rd_antis_cancer)`)
    
    output.append("WellAboveBackground num. Normal sense dT", `sum(wab_dt_sense_normal)`)
    output.append("WellAboveBackground num. Normal antis dT", `sum(wab_dt_antis_normal)`)
    output.append("WellAboveBackground num. Cancer sense dT", `sum(wab_dt_sense_cancer)`)
    output.append("WellAboveBackground num. Cancer antis dT", `sum(wab_dt_antis_cancer)`)
    output.append("WellAboveBackground num. Normal sense Rd", `sum(wab_rd_sense_normal)`)
    output.append("WellAboveBackground num. Normal antis Rd", `sum(wab_rd_antis_normal)`)
    output.append("WellAboveBackground num. Cancer sense Rd", `sum(wab_rd_sense_cancer)`)
    output.append("WellAboveBackground num. Cancer antis Rd", `sum(wab_rd_antis_cancer)`)
...
output_macro_str = \
    output_macro(identifier  = output.get_header_idx("AFAS ID") + 1,
                 annotation  = output.get_header_idx("Annotation") + 1,
                 dt_sn_start = output.get_header_idx("dT-S:" + opt.get_normal_keys()[0]) + 1,
