...
from Usefuls.Revolver1 import Revolver
...
class IVV_Info_for_Frame:
	...
	self.f.clear()
		...
		self.f.graduate(pos1, pos2, y_pos, 100)
            ...
            self.f.draw_line(pos1, pos2, y_pos, line_width, line_color)

            if seqtype != "Motif":
		self.f.draw_text(int(pos1) + IVV_disp.label_offset,
				 y_pos, seqid, 14)
            else:
		self.f.draw_text((int(pos1)+int(pos2))/2, y_pos, seqid, 14)

            if seqtype == "Prey":
		self.f.draw_text(IVV_disp.bait_label_offset, y_pos, bait, 14)
...
if __name__ == '__main__':
    ...
    mapfile = rsc.IVV_disp_map_test1
