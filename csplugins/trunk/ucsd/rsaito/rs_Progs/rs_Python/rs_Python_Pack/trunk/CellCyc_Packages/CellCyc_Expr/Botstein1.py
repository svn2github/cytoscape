...
class Botstein_Sheet(Data_Struct.Data_Sheet3.Data_Sheet):
    ...
    def get_datum_z(self, row_key, col_key):
        if self.synonyms:
            row_key = self.synonyms.to_main_force(row_key)
        return Data_Struct.Data_Sheet3.Data_Sheet.get_datum_zscore_row_based(self, row_key, col_key)

    def get_datum_filter(self, row_key, col_key,
                         filter = ("Scaled Fourier", 0, lambda dat, thr: dat >= thr)):
        if filter is None:
            return self.get_datum(row_key, col_key)
        else:
            f_key, thres, jfunc = filter
            datum = self.get_datum(row_key, f_key)
            if jfunc(datum, thres):
                return self.get_datum(row_key, col_key)
            else:
                return None
...
if __name__ == "__main__":
    ...
    bot_expr = Botstein_Sheet(rsc.Botstein_expr,
                              synonyms)
    print "HCP5" in bot_expr.row_labels()
    print bot_expr.get_data("HCP5")
    bot_expr.numerize()
    print bot_expr.get_data("STK15")
    print "Z Score:", bot_expr.get_datum_z("STK15", "A#2")
    print "STK15 - A#2", bot_expr.get_datum_filter("STK15", "A#2",
                                                   ("Scaled Fourier", 70, lambda dat, thr: dat >= thr)), \
                                                   bot_expr.get_datum("STK15", "Scaled Fourier")
