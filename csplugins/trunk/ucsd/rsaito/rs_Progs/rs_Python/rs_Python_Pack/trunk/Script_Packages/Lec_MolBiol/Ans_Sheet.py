...
student_id_format   = re.compile(r'^\d{8}$') # Not used when reading any more.
invalid_ans    = '#'
...
class Ans_Sheet:
    """ Object of Answer Sheet, equipped with
    basic analyses. """
    ...
    def match_correct_ans_d(self, correct, stuid, sep = ":", right = "o", wrong = "x"):
        """ For given student ID, mark responses by 'o' or 'x'.
        (Ex. ret[ 3 ] -> "3:x") """
        instance_class_check(correct, Data_Struct.Dict_Ordered.Dict_Ordered)
...
def read_student_list(student_list_file):
    ...
    stulist = Hash("S")
    stulist.enable_ordered_keys_mode()
    stulist.read_file_hd(student_list_file,
...
70641200   Pol        1     ABC         abc        id1     id1@email
70644300   Pol        1     DEF         def        id2     id2@email
79151932   Env        3     Saito       Rintaro    rsaito  rsaito@email
70642300   Env        4     XXX         YYY        xxx     xxx@email
""", tospace = "_")

    extra_info_list = TmpFile_III("""
70644300 3 1 9/15 10/15
70642300 0 0  -     -   Invalid-B
""")
    ...
    for stuid in stulist:
        ...
        print "\t".join([stuid, stulist.val_accord_hd(stuid, "Family Name",) ] + mark + 
