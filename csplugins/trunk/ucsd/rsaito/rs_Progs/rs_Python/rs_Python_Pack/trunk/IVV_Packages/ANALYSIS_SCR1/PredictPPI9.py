...
parser.add_option("-l", dest = "predlog",
                  action = "store_true",
                  help = "enables prediction description loggings")
parser.add_option("--predlog", dest = "predlogdir",
                  help = "enables prediction description loggings and specifies directory to save logs")
parser.add_option("--bfilter", dest = "bfilter_flag",
                  action = "store_true",
                  help = "enables IVV basic filter")
parser.add_option("-r", "--reprod", dest = "reprod",
                  default = "1",
                  help = "Predicted PPI reproducibility threshold")
parser.add_option("-b", "--bait_eval", dest = "baiteval",
                  default = "1.0e-30",
                  help = "e-value for homology search of bait sequences")
parser.add_option("-p", "--prey_eval", dest = "preyeval",
                  default = "1.0e-2",
                  help = "e-value for homology search of prey sequences")
parser.add_option("--bait_motif_eval", dest = "bait_motif_eval",
                  default = "1.0e-2",
                  help = "e-value for motif search of bait sequences")
parser.add_option("--prey_motif_eval", dest = "prey_motif_eval",
                  default = "1.0e-2",
                  help = "e-value for motif search of prey sequences")
parser.add_option("--ident", dest = "ident",
                  default = "0.7",
                  help = "identity (<= 1.0) for homology search of prey sequences")
parser.add_option("--posit", dest = "posit",
                  default = "0.0",
                  help = "rate of positive (<= 1.0) for homology search of prey sequences")
parser.add_option("--overl", dest = "overl",
                  default = "10",
                  help = "overlap length (a.a.) for homology search of prey sequences")
...
import IVV_Packages.PPI_Pred.stepback_KnownPPI1 as stepback_KnownPPI
...
bait_hm  = HM(float(options.baiteval))
prey_hm  = HM(float(options.preyeval),
              float(options.ident),
              float(options.posit),
              float(options.overl))

mode = "S"
rep_thres = int(options.reprod)

bait_motif_thres = float(options.bait_motif_eval)
prey_motif_thres = float(options.prey_motif_eval)
...
bp_knownPPI = stepback_KnownPPI.construct_StepBack_KnownPPI()
...
for p1 in spoke:
    for p2 in spoke[p1]:
        ...
        if gene_info.has_key(p1):
            annot1 = gene_info.val(p1).split("\t")[1]
        else:
            annot1 = "(Annotation not found.)"
        tb.append("Annotation 1", annot1)

        if gene_info.has_key(p2):
            annot2 = gene_info.val(p2).split("\t")[1]
        else:
            annot2 = "(Annotation not found.)"
        tb.append("Annotation 2", annot2)

        pubmedids = ""
        if reported_ppi.has_pair(p1, p2):
            pubmed_id_set = []
            for pubmed_ids in reported_ppi.pair_val(p1, p2):
                pubmed_id_set += pubmed_ids.split(",")
            pubmedids = ",".join(list(set(pubmed_id_set)))
        tb.append("Literature", pubmedids)
        ...
        sb_bait_count, sb_prey_count, sb_kppis_count = \
            bp_knownPPI.validate_by_stepback_info1(p1, p2)

        tb.append("SB bait count", `sb_bait_count`)
        tb.append("SB prey count", `sb_prey_count`)
        tb.append("SB known PPI count", `sb_kppis_count`)

        tb.output("\t")

        if options.predlogdir:
            IVV_Packages.PPI_Pred.Display_Pred_Source2.Save_Pred_Source(
                p1, p2, options.predlogdir)
        elif options.predlog:
            IVV_Packages.PPI_Pred.Display_Pred_Source2.Save_Pred_Source(
                p1, p2)
