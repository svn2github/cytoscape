...
import Seq_Packages.Homology.Align_MultiPair as Align_MultiPair
...
# ivv_info             = IVV_Global.get_ivv_info() #Judgement of filtering mode necessary.
...
def Return_Pred_Source(geneid1, geneid2):
    ...
    for refseqid in refseqids:
        if refseqid == "-":
            continue
        try:
            testID_singleseq = ref_mf.get_singlefasta(refseqid)
        except MultiFasta.FASTACMD_ERROR:
            sys.stderr.write("Warning: ID %s not found in %s.\n" % (refseqid, rsc.RefSeq_Prot_Human))
            continue
        
        ret.append("--- Refseq sequence " + refseqid + " for gene ID %s ---" % geneid2)

        motifdescr =  refseqprot_swisspfam.ret_motif_info2(refseqid)
...
def stepback_KnownPPI_tracker(geneid1, geneid2):
    ret = []
    sb_knownPPI = IVV_Global_II.get_stepback_knownPPI()
    sb_info = sb_knownPPI.validate_by_stepback_info2(geneid1, geneid2)

    ret.append("")
    ret.append("******* IVV Step Back KPPI Tracker *******")
    ret.append("")
    
    for kppi_bait in sb_info:
        kppi, bait = kppi_bait
        preys = sb_info[ kppi_bait ]
        kppi_p1, kppi_p2 = kppi
        ret.append("Known PPI %s - %s" % (kppi_p1, kppi_p2))
        ret.append("Detected by Bait %s and prey(s) %s" % (bait, ",".join(preys)))
...
def Save_Pred_Source(geneid1, geneid2, logdir = None):
    ...
    res2 = stepback_KnownPPI_tracker(geneid1, geneid2)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print stepback_KnownPPI_tracker(geneid1, geneid2)
