...
class Feature_Ext_Set:
    ...
    def set_feature_ext(self, name, filename, scaling_featext = None, info_keys = None):
        self.feature_ext_set[ name ] = Feature_Extraction1.Feature_Ext(filename, info_keys)
        if scaling_featext:
    ...
    def set_feature_exts(self, idict, scaling_featext = None, info_keys = None):
        for name in idict:
            self.set_feature_ext(name, idict[name], scaling_featext, info_keys)
    ...
    def get_exp_pat_feanum(self, names, feanum):
        exppat = []
        for name in names:
            featext = self.get_feature_ext(name)
            exppat.append(featext.
                          get_gProcessedSignal(feanum))
        return exppat
