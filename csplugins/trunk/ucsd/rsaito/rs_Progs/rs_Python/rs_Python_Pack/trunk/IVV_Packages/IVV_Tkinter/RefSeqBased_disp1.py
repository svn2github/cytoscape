...
from Data_Struct.DictSet1 import DictSet_Order
...
class IVV_Info_for_Frame:
	...
	self.f.clear()
        ...
        map_descr = DictSet_Order()
        for m in mapping:
            seqtype, seqid, bait, pos1, pos2 = m.split("\t")
            map_descr.append(seqtype, (seqid, bait, pos1, pos2))

        y_pos = 60
        y_pos_plus = 30

        for seqtype in map_descr.ret_order():
            line_color = self.map_color[ seqtype ]
            line_width = self.map_linew[ seqtype ]

            if seqtype == "Motif":
                for m in map_descr.ret_set(seqtype):
                    seqid, bait, pos1, pos2 = m
