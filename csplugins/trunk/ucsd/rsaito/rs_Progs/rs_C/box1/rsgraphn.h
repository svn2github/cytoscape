...
{    XSetWindowAttributes att;
     ...
     w = XCreateSimpleWindow(d,DefaultRootWindow(d),x,y,width,height,
			     5,MyColor(d,"purple"),MyColor(d,"light gray"));
     ...
     f = XLoadFont(d,"-schumacher-clean-bold-r-normal--10-100-75-75-c-80-iso8859-1"); 
...
rs_xfillarc(x,y,width,height,angle1,angle2,color)
...
rs_xdrawarc(x,y,width,height,angle1,angle2,color)
int x,y,width,height,angle1,angle2;
char *color;
{
     ...
     int *b;
{
     ...
     XQueryPointer(d, w, &root_r, &child_r, &rx_r, &ry_r, x, y, b);
     /* value of *b must be fixed */
}
