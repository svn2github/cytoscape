...
#define RANGE_PARAM1 20
#define RANGE_PARAM2 0.01
...
void self_org(double **data_set, int data_num,
  ...
  for(t = 0;t < iterations;t ++){
    ...
    xvisual(ref_node_has, x_len, y_len);
...
int x_len,y_len;
{
  ...
  for(s = 0;s < y_len;s ++)
    for(n = 0;n < x_len;n ++){
      if(str_board[n + s*x_len] < 0)color_c = black;
      else if(0 <= str_board[n + s*x_len] && str_board[n + s*x_len] < 100)
	color_c = blue;
      else if(100 <= str_board[n + s*x_len] && str_board[n + s*x_len] < 200)
	color_c = yellow;
      else if(200 <= str_board[n + s*x_len] && str_board[n + s*x_len] < 300)
	color_c = red;
      else if(300 <= str_board[n + s*x_len] && str_board[n + s*x_len] < 400)
	color_c = green;
      else if(400 <= str_board[n + s*x_len] && str_board[n + s*x_len] < 500)
	color_c = orange;
      else color_c = white;
