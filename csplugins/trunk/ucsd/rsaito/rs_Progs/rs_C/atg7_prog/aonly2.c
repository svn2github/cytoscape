...
/* パラメータ処理 */
int aonly2_par(int argc, char *argv[], int n)
{
  if(strcmp(argv[n], "-aonly2") == 0){
    last = atoi(argv[n + 1]);
    return 2; /* パラメータが２つ */
  }
  else return 0;
}
void aonly2_head(char *line){
}
/* エントリ処理 */
void aonly2_ent(struct gparam *entry_info, char seqn[], int max,
...
/* ヘルプ */
void aonly2_help(){
  printf("-aonly2\t Only accepts if two same oriented codons are ");
  printf("separated within specified bases.\n");
}
