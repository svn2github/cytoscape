...
sub get_iwas_ivv1($){
    ...
    open(FH, $filename) || return(undef, undef);
    my $header = <FH>;
    $header =~ /^>(\S+)/;
    my $seqid = $1;
...
my $w_seq_prey_file = "ivv_${output_base_fn}_prey_cDNA.tfa";
...
#    my($ivv_seq, $seqid_ivv) = get_iwas_ivv1($ivv_seq_file);
    ...
    if(defined($seqid_cDNA)){ 
	$seq_ivv{ "Prey" }->{ $seqid_cDNA } = $cDNA_seq;
    }
    else {
...
#    if(defined($seqid_cDNA) &&
#       defined($seq_info{ "Prey_Seq_header_RefSeq" }->{ $seqid_cDNA }) &&
#       -e $refseq_file){
