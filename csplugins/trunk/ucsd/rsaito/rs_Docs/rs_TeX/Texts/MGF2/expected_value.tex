...
\noindent
{\bf 問題\thechapter-\theexer}:~数列5,4,6,2,8の平均と分散、標準偏差を求めよ。

\refstepcounter{exer}

\noindent
{\bf 問題\thechapter-\theexer}:~式\ref{eqn_var701}から式\ref{eqn_var702}を導け。

\refstepcounter{exer}

\noindent
{\bf 問題\thechapter-\theexer}:~式\ref{eqn_var_bsc101}および式\ref{eqn_var_bsc102}を導け。

\refstepcounter{exer}

\noindent
{\bf 問題\thechapter-\theexer}:~連続確率について、分散を表す式を導け。

% \refstepcounter{exer}

% \noindent
% {\bf 問題\thechapter-\theexer}:~以下の数式が成立することを示せ。
