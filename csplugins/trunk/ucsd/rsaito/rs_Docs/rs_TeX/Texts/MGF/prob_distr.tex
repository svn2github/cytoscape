...
\refstepcounter{exer}
\noindent
{\bf 問題\thesection-\theexer}:~上記確率関数の分布関数を求めよ。
...
またこの確率関数は、節\ref{sec_desc_prob}(p.\pageref{pd_def1})で述べた確率分布の3つの性質を満
...
積、すなわち積分で表されるということである。節\ref{sec_desc_prob}(p.\pageref{pd_def1})で述べた離散確率分布の
...
\noindent
{\bf 例題\thesection-\thepreexer}:~ サイコロルーレットの目を表す確率変数\(X\)の分布関数\(F(x)\)を求めよう。
...
\noindent
{\bf 定理\thesection-\thetheorem}:\label{prob_conv_single1}
...
\refstepcounter{exer}
\noindent
{\bf 問題\thesection-\theexer}:~ \label{prob_conv_sample190} 確率変数\(X\)の従う確率密度関数が式\ref{eq_prob_den_func_sample1}で与えられ、
