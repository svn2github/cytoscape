...
//------------------------------------------------------------------------------
    ...
    public boolean activePathParametersPresent() {return activePathParametersPresent;}
    
    public boolean haveInputsError() {return inputsError;}
    
    public boolean shouldRunActivePaths() {
        return ( run && activePathParametersPresent() && !haveInputsError() );
    }

    public boolean shouldSaveAndExit() { return saveAndExit; }
...
//------------------------------------------------------------------------------
	...
	System.err.println("    --APmcb <true|false>    use mc correction at all");
        System.err.println("    --APmc <string>         monte carlo file name");
        System.err.println("    --APsig <double>        significance threshold");
        System.err.println("    --APreg <true|false>    regional scoring true/false");
	System.err.println("    --APexit                save and exit after run");
        System.err.println("    --APhelp                prints this help");
        System.err.println("    --APnorun               don't run active modules");
        System.err.println("    --APtable <string>      table file name");
	System.err.println("    --APmt <int>            maximum number of threads");
    }
    ...
    private void parseArgs(String[] args) {
        ...
        longOpts[0] = new LongOpt ("APt0",  LongOpt.REQUIRED_ARGUMENT, null, 0); // initial temp
        longOpts[1] = new LongOpt ("APtf",  LongOpt.REQUIRED_ARGUMENT, null, 1); // final temp
        longOpts[2] = new LongOpt ("APha",  LongOpt.REQUIRED_ARGUMENT, null, 2); // hub adj
        ...
        longOpts[7] = new LongOpt ("APrs",  LongOpt.REQUIRED_ARGUMENT, null, 7); // random seed
        longOpts[8] = new LongOpt ("APsd",  LongOpt.REQUIRED_ARGUMENT, null, 8); // search depth
        longOpts[9] = new LongOpt ("APmd",  LongOpt.REQUIRED_ARGUMENT, null, 9); // max depth
        longOpts[10] = new LongOpt ("APqu",  LongOpt.REQUIRED_ARGUMENT, null, 10); // quenching
        longOpts[11] = new LongOpt ("APed",  LongOpt.REQUIRED_ARGUMENT, null, 11); // edge annealing
        longOpts[12] = new LongOpt ("APmcb",  LongOpt.REQUIRED_ARGUMENT, null, 12); // monte carlo (y/n)
        longOpts[13] = new LongOpt ("APmc",  LongOpt.REQUIRED_ARGUMENT, null, 13); // monte carlo file
        longOpts[14] = new LongOpt ("APreg",  LongOpt.REQUIRED_ARGUMENT, null, 14); // monte carlo file
        longOpts[15] = new LongOpt ("APhelp",  LongOpt.NO_ARGUMENT, null, 15); // help
	longOpts[16] = new LongOpt ("APexit", LongOpt.NO_ARGUMENT, null, 16); // save and exit
	longOpts[17] = new LongOpt ("APnorun", LongOpt.NO_ARGUMENT, null, 17); // don't run
	longOpts[18] = new LongOpt ("APtable", LongOpt.REQUIRED_ARGUMENT, null, 18); // file for table
	longOpts[19] = new LongOpt ("APmt", LongOpt.REQUIRED_ARGUMENT, null, 19);
		...
		tmp = g.getOptarg();
		try {
		    activePathParameters.setHubAdjustment(Double.parseDouble(tmp));;
		    activePathParametersPresent = true;
		}
		catch (NumberFormatException e) {
		    inputsError = true;
		...
		tmp = g.getOptarg();
		try {
		    activePathParameters.setMinHubSize(Integer.parseInt(tmp));
		    activePathParametersPresent = true;
                    }
		catch (NumberFormatException e) {
		    inputsError = true;
		...
		tmp = g.getOptarg();
		try {
		    activePathParameters.setSearchDepth(Integer.parseInt(tmp));
		    activePathParametersPresent = true;
		}
		catch (NumberFormatException e) {
		    inputsError = true;
		...
		tmp = g.getOptarg();
		try {
		    activePathParameters.setMaxThreads(Integer.parseInt(tmp));
		    activePathParametersPresent = true;
		}
		catch (NumberFormatException e) {
		    inputsError = true;
        ...
        if (helpRequested || inputsError) {usage();}
    }
}
