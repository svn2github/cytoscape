...
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_accession (uid INT, nucacc VARCHAR(15), nucgi INT, protacc VARCHAR(15), protgi INT, KEY(uid,protacc,protgi),INDEX(uid))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_genename (uid INT, protgi INT, genename VARCHAR(50), KEY(uid,protgi,genename),INDEX(protgi), INDEX(uid))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_prodname (uid INT, protgi INT, prodname VARCHAR(60), KEY(uid,protgi,prodname),INDEX(protgi), INDEX(uid))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_goa (uid INT, protgi INT, goaid VARCHAR(20), KEY(uid,protgi,goaid), INDEX(uid), INDEX(protgi))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_goid (uid INT, protgi INT, goid INT, KEY(uid,protgi,goid), INDEX(uid), INDEX(protgi))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_definition (nucgi INT, definition VARCHAR(200), KEY(nucgi,definition), INDEX(nucgi))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_sprot (uid INT, protgi INT, sprotid VARCHAR(12), KEY(uid,protgi,sprotid), INDEX(uid))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_trembl (uid INT, protgi INT, tremblid VARCHAR(12), KEY(uid,protgi,tremblid), INDEX(uid))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_pir (uid INT, protgi INT, pirid VARCHAR(15), KEY(uid,protgi,pirid), INDEX(uid))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_interpro (uid INT, protgi INT, interproid VARCHAR(9), KEY(uid,protgi,interproid), INDEX(uid))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_pdb (uid INT, protgi INT, pdbid VARCHAR(4), KEY(uid,protgi,pdbid), INDEX(uid))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_ensembl (nucgi INT, ensemblid INT, INDEX(nucgi))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_geneid (nucgi INT, geneid INT, INDEX(nucgi))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_hgnc (nucgi INT, hgncid INT, INDEX(nucgi))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_locuslink (nucgi INT, locusid INT, INDEX(nucgi))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_mim (nucgi INT, mimid INT, INDEX(nucgi))");
$dbh->do("CREATE TABLE IF NOT EXISTS genbank_taxid (uid INT, protgi INT, taxid INT, INDEX(protgi), INDEX(taxid))");

$fullFilePath1 = getcwd."/genbank/genbank_accession.txt";
$sth = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath1}\' REPLACE INTO TABLE genbank_accession (nucacc,nucgi,protacc,protgi)") or die "Error: $dbh->errstr";

$fullFilePath2 = getcwd."/genbank/genbank_genename.txt";
$sth2 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath2}\' REPLACE INTO TABLE genbank_genename (protgi,genename)") or die "Error $dbh->errstr";

$fullFilePath3 = getcwd."/genbank/genbank_prodname.txt";
$sth3 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath3}\' REPLACE INTO TABLE genbank_prodname (protgi,prodname)") or die "Error $dbh->errstr";

$fullFilePath4 = getcwd."/genbank/genbank_goa.txt";
$sth4 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath4}\' REPLACE INTO TABLE genbank_goa (protgi,goaid)") or die "Error $dbh->errstr";

$fullFilePath5 = getcwd."/genbank/genbank_definition.txt";
$sth5 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath5}\' REPLACE INTO TABLE genbank_definition") or die "Error $dbh->errstr";

$fullFilePath6 = getcwd."/genbank/genbank_sprot.txt";
$sth6 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath6}\' REPLACE INTO TABLE genbank_sprot (protgi,sprotid)") or die "Error $dbh->errstr";

$fullFilePath7 = getcwd."/genbank/genbank_trembl.txt";
$sth7 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath7}\' REPLACE INTO TABLE genbank_trembl (protgi,tremblid)") or die "Error $dbh->errstr";

$fullFilePath8 = getcwd."/genbank/genbank_pir.txt";
$sth8 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath8}\' REPLACE INTO TABLE genbank_pir (protgi,pirid)") or die "Error $dbh->errstr";

$fullFilePath9 = getcwd."/genbank/genbank_goid.txt";
$sth9 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath9}\' REPLACE INTO TABLE genbank_goid (protgi,goid)") or die "Error $dbh->errstr";

$fullFilePath10 = getcwd."/genbank/genbank_interpro.txt";
$sth10 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath10}\' REPLACE INTO TABLE genbank_interpro (protgi,interproid)") or die "Error $dbh->errstr";

$fullFilePath11 = getcwd."/genbank/genbank_pdb.txt";
$sth11 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath11}\' REPLACE INTO TABLE genbank_pdb (protgi,pdbid)") or die "Error $dbh->errstr";

$fullFilePath12 = getcwd."/genbank/genbank_ensembl.txt";
$sth12 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath12}\' REPLACE INTO TABLE genbank_ensembl") or die "Error $dbh->errstr";

$fullFilePath13 = getcwd."/genbank/genbank_geneid.txt";
$sth13 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath13}\' REPLACE INTO TABLE genbank_geneid") or die "Error $dbh->errstr";

$fullFilePath14 = getcwd."/genbank/genbank_hgnc.txt";
$sth14 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath14}\' REPLACE INTO TABLE genbank_hgnc") or die "Error $dbh->errstr";

$fullFilePath15 = getcwd."/genbank/genbank_locuslink.txt";
$sth15 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath15}\' REPLACE INTO TABLE genbank_locuslink") or die "Error $dbh->errstr";

$fullFilePath16 = getcwd."/genbank/genbank_mim.txt";
$sth16 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath16}\' REPLACE INTO TABLE genbank_mim") or die "Error $dbh->errstr";

$fullFilePath17 = getcwd."/genbank/genbank_taxid.txt";
$sth17 = $dbh->prepare_cached("LOAD DATA LOCAL INFILE \'${fullFilePath17}\' REPLACE INTO TABLE genbank_taxid (protgi, taxid)") or die "Error $dbh->errstr";
	...
	while($line = <IN>){
		...
		if($line =~ /^\/\/$/){
			...
			if($nucacc ne "" and $protacc eq ""){
				...
				if($ensemblid ne ""){
					print ENSEMBL "$nucgi\t$ensemblid\n";
					$ensemblid = "";
				}
				
				if($hgnid ne ""){
					print HGN "$nucgi\t$hgnid\n";
					$hgnid = "";
				}
				
				if($locusid ne ""){
					print LOCUSID "$nucgi\t$locusid\n";
					$locusid = "";
				}
				
				if($mimid ne ""){
					print MIM "$nucgi\t$mimid\n";
					$mimid = "";
				}
				
				if($geneid ne ""){
					print GENEID "$nucgi\t$geneid\n";
					$geneid = "";
				}
			...
			while($line = <IN>){
				...
				if($line =~ /^\s\s\s\s\s\w/ or $line =~ /^\w/){
					...
					print ACC "$nucacc\t$nucgi\t$protacc\t$protgi\n";
					if($genename ne ""){
						print GN "$protgi\t$genename\n";
						$genename = "";
					}
					if($prodname ne ""){
						print PN "$protgi\t$prodname\n";
						$prodname = "";
					}
					if($sprotid ne ""){
						print SPROT	"$protgi\t$sprotid\n";
						$sprotid = "";
					}
					if($tremblid ne ""){
						print TREMBL	"$protgi\t$tremblid\n";
						$tremblid = "";
					}
					if($pirid ne ""){
						print PIR "$protgi\t$pirid\n";
						$pirid = "";
					}
					if($goaid ne ""){
						print GOA "$protgi\t$goaid\n";
						$goaid = "";
					}
					if($goid ne ""){
						print GOID "$protgi\t$goid\n";
						$goid = "";
					}
					if(scalar(@interproids) > 0){
						foreach $ipid (@interproids){
							print INTERPRO "$protgi\t$ipid\n";
						}
						@interproids = ();
					}
					if($pdbid ne ""){
						print PDB "$protgi\t$pdbid\n";
						$pdbid = "";
					}
					if($taxid ne ""){
					...
					if($protgi ne "" and scalar(@interproids) > 0){
						foreach $ipid (@interproids){
							print INTERPRO "$protgi\t$ipid\n";
						}
						@interproids = ();
...
$dbh->do("CREATE TABLE IF NOT EXISTS ncbi_taxid_species (taxid INT, name VARCHAR(100), INDEX(taxid), KEY(taxid,name))");
$fullFilePath = getcwd()."/taxonomy/taxid_speciesname.txt";
print "Loading data into ncbi_taxid_species...";
$dbh->do("LOAD DATA LOCAL INFILE \'${fullFilePath}\' REPLACE INTO TABLE ncbi_taxid_species");
print "done.\n";
...
#$dbh->do("CREATE TABLE IF NOT EXISTS nucgi_taxid (nucgi INT, taxid INT, INDEX(nucgi), INDEX(taxid), KEY(nucgi,taxid))");
#$fullFilePath = getcwd()."/taxonomy/gi_taxid_nucl.dmp";
#print "Loading data into nucgi_taxid...\n";
#$dbh->do("LOAD DATA LOCAL INFILE \'${fullFilePath}\' REPLACE INTO TABLE  nucgi_taxid");
#print "done.\n";

#$dbh->do("CREATE TABLE IF NOT EXISTS protgi_taxid (uid INT, protgi INT, taxid INT, INDEX(uid), INDEX(protgi), INDEX(taxid), KEY(uid,protgi,taxid))");
#$fullFilePath = getcwd()."/taxonomy/gi_taxid_prot.dmp";
#print "Loading data into protgi_taxid...\n";
#$dbh->do("LOAD DATA LOCAL INFILE \'${fullFilePath}\' REPLACE INTO TABLE  protgi_taxid (protgi,taxid)");
#print "done.\n";
