...
print "--------------------- mif_interactiondb_creator.pl ------------------\n";

if(scalar(@ARGV) < 4){
	print "USAGE: perl mif_interactiondb_creator.pl <db user> <db password> <db name> <mif file>\n";
 	die;
}
...
system("rm ${dbname}/interactions/*");
system("mkdir ${dbname}");
system("mkdir ${dbname}/interactions");

open DATA, $mifFile;
my $line1;
# get to the first proteinInteractor
while( <DATA> ) {
    $line1 = $_;
...
while( <DATA> ) {
	...
	foreach my $record ( @records ) {
		...
		my ( $id, $ref, $spec, $swp, $pir, $gi, $full, $primaryid );
		...
		for my $i ( 0..$#children ) {
		    ...
		    $primaryid = $1 if /^proteinInteractor\.proteinInteractor\.xref\.primaryRef\.id=(.*)/;
		...
		$infos{$id} = '' . ( $ref ne '' ? "ref=$ref|" : "" ) .
				   ...
				   ( $pir ne '' ? "pir=$pir|" : "" ) .
				   ( $full ne '' ? "long=$full|" : "" ) .
				   ( $primaryid ne '' ? "primaryID=$primaryid" : "" );
...
$dbh->do("CREATE TABLE interactions (id VARCHAR(25), i1 VARCHAR(25), interactionType VARCHAR(2), i2 VARCHAR(25), taxid INT, KEY(id), INDEX(taxid) )");
$sth = $synh->prepare_cached("SELECT taxid FROM ncbi_taxid_species WHERE name like ?");

open X, ">>${dbname}/interactions/interactions\.txt";
...
while( <DATA> ) {
	...
	foreach my $record ( @records ) {
	    ...
	    $info2 = $infos{$id2};
	    ( $info2 ) =~ s/=/2=/g;
	    $infoA = "PubMedID=$pmid|interactionID=$iid|$info1|$info2";
	    $info1 = $infos{$id1};
	    ( $info1 ) =~ s/=/2=/g;
	    $info2 = $infos{$id2};
	    ( $info2 ) =~ s/=/1=/g;
	    $infoB = "PubMedID=$pmid|interactionID=$iid|$info1|$info2";
...
$fullFilePath = getcwd."/${dbname}/interactions/interactions.txt";
print "Loading interactions...\n";
$dbh->do("LOAD DATA LOCAL INFILE \'${fullFilePath}\' IGNORE INTO TABLE interactions") or die "Error: $synh->errstr";
print "done.\n";
