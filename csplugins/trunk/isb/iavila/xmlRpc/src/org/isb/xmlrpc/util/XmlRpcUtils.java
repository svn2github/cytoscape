...
public class XmlRpcUtils {
  ...
  /**
   * The name of the file that contains properties for XML-RPC, existing props are:<p>
   * host=<URL>
   * service.<service name>=<handler fully described name>
   * service.<service name>.<service argument>=<service argument>
   * auth=?
   */
  public static final String PROPS_FILE_NAME = "xmlrpc.props";

  
  /**
   * Looks for a file named PROPS_FILE_NAME<p>
   * - First in the user's home directory<p>
   * - User's current directory (where the java command was invoked)
   *
   * @return null if the file was not found
   */
  public static String FindXmlRpcPropsFile (){
    boolean found;
    File file;
    //1. Try the users home
    try {
      file = new File(System.getProperty("user.home"), PROPS_FILE_NAME);
      System.out.println(PROPS_FILE_NAME + " found at: " + file);
      found = true;
    }catch(Exception e){
    ...
    try {
      file = new File(System.getProperty("user.dir"), PROPS_FILE_NAME);
      System.out.println(PROPS_FILE_NAME + " found at: " + file);
      found = true;
    } catch (Exception e) {
      // not found in the current directory
  ...
  /**
   * @return a Hashtable with the given object's class fields as keys (Strings)
   * and their values as one of the XML-RPC accepted values as Java complex
   * types (so if it is an int, in the Hash it will be an Integer) it also adds 
   * ("className",obj.getClass().getName()) entry to the hash
   */
    ...
    try {
      ...
      for ( int i = 0; i < fields.length; i ++ )
        out.put( fields[ i ].getName(), 
                 translateField( fields[ i ].getName(), fields[ i ].get( obj ) ) );
  ...
  /**
   * Recreates the object that the given hash represents, the hash
   * contains the fields of the object's class mapped to their Java complex type
   * values (which as "untranslated" to create the object) and also it should contain an
   * ("className",obj.getClass().getName()) entry so that the object can be recreated
   */
  public static Object GetObjectFromStruct( Hashtable tab ) {
    ...
    try {
      ...
      if ( constr != null ) out = constr.newInstance( null );
      else throw new Exception( "Could not create new object of class " + className );

      Field fields[] = c.getDeclaredFields();
      for ( int i = 0; i < fields.length; i ++ )
        fields[ i ].set( out, untranslateField( fields[ i ].getName(), 
                                                tab.get( fields[ i ].getName() ),
                                                fields[ i ].getType() ) );

    } catch( Exception e ) {
      out = null;

      try { // as a last ditch effort, see if it was encoded into a byte array 
        // (xml-rpc can handle this)
  ...
  /**
   * @param fname field name (used to throw an informative exception if the inobj is not of
   * one of the supported XML-RPC type)
   * @param inobj the object to be transtaled to a Java complex type (String, Boolean, Integer,
   * Double, Hashtable, Vector, or java.util.Date)
  ...
  protected static Object translateField( String fname, Object inobj ) throws Exception {
    if ( inobj instanceof String || inobj instanceof Boolean || 
         ...
         inobj instanceof java.util.Date ) return inobj;
    ...
    throw new Exception( "Object type " + inobj.getClass() + " for field " + fname + 
                         " not supported by XML-RPC." );
  }

  /**
   * @param fname the field name for informative exception message
   * @param inobj the object to untranslate
   * @param toType the type to which to convert inobj
   * @return an object with one of the types that XML-RPC accepts as a simple Java type
   * (String, boolean, int, double, Hashtable, Vector, java.util.Date)
   */
  protected static Object untranslateField( String fname, Object inobj,
    ...
    if ( toType.equals( String.class ) || toType.equals( boolean.class ) || 
         ...
         toType.equals( Hashtable.class ) || toType.equals( Vector.class ) ||
         toType.equals( java.util.Date.class ) ) return inobj;
  ...
  public static byte[] GetObjectAsBytes( Object obj, boolean gzip ) {
    if ( ! ( obj instanceof Serializable ) ) return new byte[ 0 ];
    try {
      ByteArrayOutputStream bos = new ByteArrayOutputStream();
      java.util.zip.GZIPOutputStream gos = 
        gzip ? new java.util.zip.GZIPOutputStream( bos ) : null;
      ObjectOutputStream out = 
        gzip ? new ObjectOutputStream( gos ) : new ObjectOutputStream( bos );
      out.writeObject( obj );
  ...
  /**
   * If the service is not running, it adds it to the server at hostURL by calling
   * addService(service,className,args)
   */
  public static boolean startService( String service, String hostURL, String className, 
                                      String[] args ) {
    if ( isServiceRunning( service, hostURL ) ) return true;
    boolean out = false;
    try {
      XmlRpcClient client = new XmlRpcClient( hostURL );
      ...
      if ( args != null ) {
        Vector v = new Vector();
        argv.add( v );
        for ( int i = 0; i < args.length; i ++ ) v.add( args[ i ] );
      }
  ...
  /**
   * If the local wMyXmlRpcServer is not running locally, it starts it on the given port
   */
  public static boolean startWebServerIfNeeded( int port ) {
    if ( isLocalWebServerRunning( port ) ) return true;
    boolean out = false;
    try {
      MyXmlRpcServer server = new MyXmlRpcServer( port );
      if ( server != null ) out = true;
    } catch( Exception e ) { out = false; }
    return out;
  }
