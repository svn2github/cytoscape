...
public class AuthenticatedInvoker extends Invoker {
	...
	/**
	 * @param target
	 *            the object that will be introspected
	 */
	 ...
	 * @param params
	 *            the arguments to the method, must include username and
	 *            password at index 0 and 1 respectively Checks for user level:
	 *            must be <= to the set user level returned by
	 *            getLevelForUser(user)
	 */
	public Object execute (String methodName, Vector params) throws Exception {
		if (users == null)
			return super.execute(methodName, params);
		if (params.size() < 2)
			throw new Exception("No username or password supplied.");
		String user = (String) params.get(0);
		String pass = (String) params.get(1);

		if (!users.containsKey(user))
			throw new Exception("Incorrect username supplied.");
		...
		// if object responds with a 2-element hashtable, key1="level" with
		// allowable data level
		// for response, and
		// key2="response" with the actual response, then check the level
		// against the user's
		// permission level.

		if (out instanceof Hashtable && levels != null
				&& levels.containsKey(user)
				&& ((Hashtable) out).containsKey("levelXXX")
				&& ((Hashtable) out).containsKey("responseXXX")) {

			int level = ((Integer) ((Hashtable) out).get("level")).intValue();
			int usrLevel = ((Integer) levels.get(user)).intValue();

			if (level > usrLevel)
				throw new Exception("User's permission level is too low ("
						+ usrLevel + " < " + level + ")");
