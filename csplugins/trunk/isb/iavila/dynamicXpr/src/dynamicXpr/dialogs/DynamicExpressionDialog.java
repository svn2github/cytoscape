...
public class DynamicExpressionDialog extends JDialog {
   ...
   * Constructor
   *
   * @param listener the <code>DynamicExpression</code> object that is listening to the
   * button requests of the dialog
  ...
  protected void createUI () {
    ...
    int numConditions = 81; // So that the speed slider and the conditions slider are
                            // of the same size by default
    if(expData != null){
      numConditions = expData.getNumberOfConditions();
    }
    conditionsSlider = new JSlider(JSlider.HORIZONTAL,0, numConditions-1,0);
  ...
  /**
   * @return the <code>String</code> currently showing in the Expression Data file-name field.
   */
