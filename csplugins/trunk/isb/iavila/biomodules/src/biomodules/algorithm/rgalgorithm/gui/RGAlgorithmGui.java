...
public class RGAlgorithmGui extends JFrame {
	...
	protected JPanel createParamsPanel() {
		...
		this.minSizeField = new JTextField(Integer.toString(this.algorithmData
				.getMinNumMembers()), 3);
		minSizeField.addActionListener(new AbstractAction() {
		...
		JLabel explanation = new JLabel(
				"<html>If molecule-type available, biomodule size<br>is number of <b>proteins</b> within the biomodule.</html>");
		...
		this.numBiomodulesPlot
				.addJoinBarListener(this.numBiomodulesPlotListener);
	...
	protected HCPlot createNumBiomodulesPlot() {
		...
		return new HCPlot(numBiomodulesAtJoin, minNumBiomods, maxNumBiomods,
				cutJoin, "Tree_node_index", "Num_Biomods");
	...
	protected HCPlot createDistancesPlot() {
		...
		return new HCPlot(distancesAtJoin, minJoiningValue, maxJoiningValue,
				cutJoin, "Tree_node_index", "Join_Distance");
	}// createDistancesPlot
	...
	protected JPanel createDataPanel () {
		...
		this.viewDataRadioButton = new JRadioButton(
				"<html>Select this button <b>before</b> calculating<br>biomodules to view APSP and Manhattan data.</html>",
		...
		JButton annotsButton = new JButton(
				"Find Overrepresented Annotations...");
		AbstractAction[] annotsEdgesAction = {
				new DrawAnnotationEdgesAction(this.algorithmData.getNetwork()),
				new SaveAnnotationsToAttribute() };
	...
	protected JPanel createButtonsPanel() {
		...
		calculateBiomodsButton.addActionListener(new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
		...
		closeWindowButton.addActionListener(new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
	...
	protected void calculateBiomodules() {
		...
		// memory
		this.algorithmData.setSaveIntermediaryData(this.viewDataRadioButton
				.isSelected());
		...
		if (hClustering == null) {
			biomodules = RGAlgorithm.calculateBiomodules(this.algorithmData
					.getNetwork());
			updatePlots();
		} else {
		...
		if (this.abstractRbutton.isSelected() && netView != null) {
			...
			if(bioNodes == null){
				System.err.println("ERROR ERROR, bioNodes List is NULL!!!!!");
			}
		...
		JOptionPane.showMessageDialog(this, "Done calculating Biomodules.",
	...
	protected void updatePlots() {
		...
		this.numBiomodulesPlot
				.removeJoinBarListener(this.numBiomodulesPlotListener);
	...
	protected int readInput(JTextField text_field) {
		...
		if (text_field == this.minSizeField) {
			...
			int numIntNodes = hClustering
					.getNumInternalNodes(EisenClustering.ROWS);
	...
	protected class PlotsOptionsListener extends AbstractAction {
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (option.equals(BIOMODS_PLOT)
					&& !currentPlotID.equals(BIOMODS_PLOT)) {

				// Show biomodules plot
				RGAlgorithmGui.this.plotPanel.removeAll();
				RGAlgorithmGui.this.plotPanel
						.add(RGAlgorithmGui.this.numBiomodulesPlot
								.getContentPane());
				int joinNumber = RGAlgorithmGui.this.distancesPlot
						.getSelectedJoinNumber();
				if (joinNumber != RGAlgorithmGui.this.numBiomodulesPlot
						.getSelectedJoinNumber()) {
					RGAlgorithmGui.this.numBiomodulesPlot
							.moveVerticalBarTo(joinNumber);
			...
			if (option.equals(DISTANCES_PLOT)
					&& !currentPlotID.equals(DISTANCES_PLOT)) {

				// Show distances plot
				RGAlgorithmGui.this.plotPanel.removeAll();
				RGAlgorithmGui.this.plotPanel
						.add(RGAlgorithmGui.this.distancesPlot.getContentPane());
				int joinNumber = RGAlgorithmGui.this.numBiomodulesPlot
						.getSelectedJoinNumber();
				if (joinNumber != RGAlgorithmGui.this.distancesPlot
						.getSelectedJoinNumber()) {
					RGAlgorithmGui.this.distancesPlot
							.moveVerticalBarTo(joinNumber);
				}// if the selected join numbers are not the same
				RGAlgorithmGui.this.plotPanel.validate();
				updatePlotLabels();
	...
	protected class PlotListener extends AbstractAction {
		...
		public void actionPerformed(ActionEvent event) {
			...
			HierarchicalClustering hClustering = RGAlgorithmGui.this.algorithmData
					.getHierarchicalClustering();
			int numIntNodes;
			if (hClustering != null) {
				numIntNodes = hClustering
						.getNumInternalNodes(EisenClustering.ROWS);
			} else {
				return;
			}
	...
	protected class XFieldListener extends AbstractAction {
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (input < 0) {
				if (input == BOUNDS_ERROR) {
					...
					if (hClustering != null) {
						int numIntNodes = hClustering
								.getNumInternalNodes(EisenClustering.ROWS);
						showErrorMessageDialog("Enter a number between 0 and "
								+ (numIntNodes - 1) + ".");
					}
			...
			RGAlgorithmGui.this.distancesPlot
					.removeJoinBarListener(RGAlgorithmGui.this.distancesPlotListener);
			RGAlgorithmGui.this.numBiomodulesPlot
					.removeJoinBarListener(RGAlgorithmGui.this.numBiomodulesPlotListener);
			RGAlgorithmGui.this.distancesPlot.moveVerticalBarTo(input);
			RGAlgorithmGui.this.numBiomodulesPlot.moveVerticalBarTo(input);
			RGAlgorithmGui.this.distancesPlot
					.addJoinBarListener(RGAlgorithmGui.this.distancesPlotListener);
			RGAlgorithmGui.this.numBiomodulesPlot
					.addJoinBarListener(RGAlgorithmGui.this.numBiomodulesPlotListener);
...
public void actionPerformed (ActionEvent event){
	...
	protected class DisplayBiomodulesAction extends AbstractAction {
		...
		protected final String[] colNames = { "Biomodule", "Num Members",
				"Members (canonical names)", "Members (common names)" };
		...
		public void actionPerformed(ActionEvent event) {
			if (RGAlgorithmGui.this.biomodulesTable == null || this.update) {
				...
				RGAlgorithmGui.this.biomodulesTable = new DataTable(null,
						colNames, data, ("Biomodules Table: " + net.getTitle()));
			}// if the table is null
			this.update = false;
			RGAlgorithmGui.this.biomodulesTable.pack();
			RGAlgorithmGui.this.biomodulesTable
					.setLocationRelativeTo(RGAlgorithmGui.this);
