...
public class SimpleMetaNodeAttributesHandler implements MetaNodeAttributesHandler {
  ...
  public boolean setNodeAttributes (CyNetwork cy_network,
    ...
    String metaName = 
      (String)cy_network.getNodeAttributeValue(metaNode,Semantics.CANONICAL_NAME);
    if(metaName == null){
    ...
    for(int i = 0; i < childrenAtts.length; i++) {
        ...
        Object value = cy_network.getNodeAttributeValue(childrenNodes[j], 
                                                        attrName);
  ...
  /**
   * @return a String with the concatenated canonical names of the children of the
   * given meta-node
   */
  protected String getCommonMetaName (int metanode_root_index, CyNetwork cy_network) {
    ...
    for (Iterator it = children.iterator(); it.hasNext(); ) {
	    ...
	    commonName += cy_network.getNodeAttributeValue(child, Semantics.CANONICAL_NAME);
  ...
  protected class CopyEdgeAttr implements IntIntProcedure {
    ...
    public boolean apply (int metaEdgeIndex, int childEdgeIndex ) {
      ...
      String childEdgeName = 
        (String)Cytoscape.getEdgeAttributeValue(childEdge,Semantics.CANONICAL_NAME);
	    String metaEdgeName = "unknown";

	    // infer the metaNode and use it to name the metaEdge
	    String interaction = 
        (String)Cytoscape.getEdgeAttributeValue(childEdge,"interaction");
        ...
        metaEdgeName = 
          Cytoscape.getNodeAttributeValue(sourceNode,Semantics.CANONICAL_NAME) + 
          " (" 
          + interaction + ") " 
          + 
        ...
        metaEdgeName = 
          Cytoscape.getNodeAttributeValue(metaEdge.getSource(),
                                          Semantics.CANONICAL_NAME) 
          + " (" + interaction + ") " 
          + Cytoscape.getNodeAttributeValue(targetNode,Semantics.CANONICAL_NAME);
      }
      ...
      if (usedEdgeNames.contains(metaEdgeName)) {
        ...
        for (int i = 0; i < allAttrNames.length; i++) {
          ...
          if (attrName.equals(Semantics.CANONICAL_NAME)) continue; // reserved
          Object childValue = Cytoscape.getEdgeAttributeValue(childEdge,attrName); 
          Object metaValue = Cytoscape.getEdgeAttributeValue(metaEdge,attrName);
          ...
          Cytoscape.setEdgeAttributeValue(metaEdge,attrName,uniqueValues.toArray());
        ...
        System.out.println("adding name mapping : " + 
                           metaEdgeName + "-->" + metaEdge);
        ...
        //String [] attributes = Cytoscape.getEdgeAttributesList(edges);
        ...
        for(int i = 0; i < attributes.length; i++){
          ...
          Object value = Cytoscape.getEdgeAttributeValue(childEdge, attributes[i]);
          Cytoscape.setEdgeAttributeValue(metaEdge, attributes[i], value);
        }
   ...
   * NOTE -- CURRENTLY NOT BEING USED, FUNCTIONALITY WAS SUBSUMED BY
   * CopyEdgeAttr class.
   */
  protected boolean mergeEdgesOfSameName ( int [] metaEdgeArray ) {
    ...
    for (int i=0; i<metaEdgeArray.length; i++) {
	    ...
	    Edge metaEdge = rootGraph.getEdge(metaEdgeIndex);	    
	    String metaEdgeName = 
        (String)Cytoscape.getEdgeAttributeValue(metaEdge,Semantics.CANONICAL_NAME);
