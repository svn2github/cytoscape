...
public class BioGridInteractionsSource extends SimpleInteractionsSource implements InteractionsDataSource {
    
    public static final String NAME = "BioGrid";
    public static final String ID_TYPE = SynonymsSource.REFSEQ_ID; // but alternate names are UniProt, PIR, HGNC in that order
    public static final String EVIDENCE = "EVIDENCE"; // Key in interaction Hashtable to find out type of evidence (for example, Two-hybrid, Invivo, etc).
    
    /**
     * Empty constructor
     */
    public BioGridInteractionsSource() {
        super();
    }
    ...
    public boolean makeConnection (String url){
        ...
        ResultSet rs = query("SELECT dbname FROM db_name WHERE db=\"biogrid\"");
        ...
        System.out.println("Current BioGrid database is: [" + currentDb + "]");
        if(currentDb == null || currentDb.length() == 0){
            ok = false;
            throw new IllegalStateException("Oh no! We don't know the name of the current BioGrid database!!!!!");
        }
     ...
     */
    public BioGridInteractionsSource(String mysql_url) {
        super(mysql_url);
    }
     ...
     * @return
     */
    // If using EVIDENCE, add one more key,value to the interaction Hashtable for it
