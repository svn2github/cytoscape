...
public class ClipRadarNode extends PClip
  implements NodeView,
  ...
  public ClipRadarNode ( int node_index, PGraphView view, Vector data, Vector lamda, String[] conditions ) {
  ...
  public ClipRadarNode ( int        node_index,
                         PGraphView view ) {
    this ( node_index,
           view,
           Double.MAX_VALUE,
  ...
  /**
   * Create a new ClipRadarNode with the given physical attributes.
  ...
  protected void updateWedges () {
    ...
    while ( wedges.hasNext() ) {
      ...
      if ( values[0] < 0 ) {
        wedge.setStrokePaint( MDNodePlugin.colorInterpolatorNegative.colorFromValue( values[1] ) );
      } else {
        wedge.setStrokePaint( MDNodePlugin.colorInterpolatorPositive.colorFromValue( values[1] ) );
      }
    }
  }
  ...
  protected void drawNodeView()
  {
   ...
   // System.out.println( "The label is created " + l );
    ...
    for (int n = 0; n < size; n++)
      {
        ...
        if ( expression < 0 ) {
          ...
          wedge.setStrokePaint( MDNodePlugin.colorInterpolatorNegative.colorFromValue( lamdaValue ) );
	  ...
	  //GradientPaint( bpx, bpy, start, X[n], Y[n], end ) );
          ...
          // POSITIVE
          wedge.setStrokePaint( MDNodePlugin.colorInterpolatorPositive.colorFromValue( lamdaValue ) );
        ...
        addChild(wedge);
        wedge.addClientProperty("tooltip", getLabel().getText()+'\n'+conditions[n] + '\n' +"value: " + ( expression/100 )+ '\n' +"lamda: "+lamdaValue);
    ...
    this.addChild( h );
    h.setParent( this );
    h.addClientProperty("tooltip", getLabel().getText());
    this.visible = true;
    this.selected = false;
  ...
  public void setCenter ( double x, double y)
{
  ...
  public giny.view.Label getLabel () {
    ...
    //return ( giny.view.Label )view.getNodeObjectProperty( rootGraphIndex,
                                                 //PGraphView.NODE_LABEL);
  }
/**
   * @return The Value of the label
