...
// TODO all cases need to handle the "null network" case by either doing the action on all open networks
// or popping up a message that no network was selected (goose)
...
public class CyGoose implements Goose {
    ...
    public String[] getSelection() throws RemoteException {
        ...
        for (int i = 0; (i < Selected.length) && (NodeIter.hasNext()); i++) {
            ...
            Selected[i] = CurrentNode.getIdentifier(); // this could change based on
            // the broadcastId but
    ...
    // TODO: setup some default calculators for color/shape/size in cases where an
    // attribute is not matched so a movie
    // will do something regardless
    /**
     * @param species
     * @param dataTitle
     * @param hashMap   Takes the attributes from the hashMap and adds them to the network goose
    ...
    public void handleMap(String species, String dataTitle, HashMap hashMap)
        ...
        while (attrKeyIter.hasNext()) {
            ...
            // determine the data type of attribute in hashMap (should be DOUBLE,
    ...
    /**
     * @param source
     * @param matrix Adds all attributes given in the matix to all matching nodes.
     */
            ...
            if (SelectNode != null) {
                ...
                for (int col = 0; col < ConditionNames.length; col++) {
                    ...
                    if ((NodeAtts.hasAttribute(SelectNode.getIdentifier(), attributeName))
                        ...
                        NodeAtts.setAttribute(SelectNode.getIdentifier(), attributeName,
                                condVal);
                }
            }
        }
    ...
    /**
     * @param source
     * @param namelist If sent to a network goose (not the default 'null' goose) all matching
     *                 nodes will be selected. Species is ignored.
     */
...
//                    CyNet.setSelectedNodeState(NewNode, true);
    ...
    /**
     * @param source
     * @param gNetwork If this is broadcast to the 'null' goose a network is created in Cytoscape.
     *                 If this is broadcast to a network goose interactions are added to the
     *                 network and all added interactions and matching interactions are selected.
     */
    public void handleNetwork(String source, Network gNetwork)
        ...
        if (this.getNetworkId() == null || this.getNetworkId().equals("0")) {
            ...
            handleNetwork(gNetwork, Cytoscape
                    .getNetwork(this.getNetworkId()), true);
     ...
     * @param CyNet
     * @param SelectNodes
     * @throws RemoteException Create a network from the gaggle network either de novo (if null network is
     *                         handling) or add to the network goose and select added nodes.
     */
            ...
            case NODE:
                ...
                for (String att : gNet.getNodeAttributeNames()) {
                    ...
                    for (String nodeName : Attributes.keySet())
                        setAttribute(Cytoscape.getNodeAttributes(), nodeName, att, Attributes
                                .get(nodeName));
                }
                break;
            ...
            case EDGE:
                ...
                for (String att : gNet.getEdgeAttributeNames()) {
                    ...
                    for (String edgeName : Attributes.keySet())
                        setAttribute(Cytoscape.getEdgeAttributes(), edgeName, att, Attributes
                                .get(edgeName));
                }
                break;
        }
    ...
    // I think this is used to choose the identifier to broadcast/handle nodes by,
    // currently not used
