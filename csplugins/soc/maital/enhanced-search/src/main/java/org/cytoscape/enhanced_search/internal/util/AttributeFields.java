...
package org.cytoscape.enhanced_search.internal.util;
...
public class AttributeFields {
	...
	private void initFields(CyNetwork network) {
		...
		CyDataTable nodeCyDataTable = network.getNodeCyDataTables().get(CyNetwork.DEFAULT_ATTRS);
		nodeColumnTypeMap = nodeCyDataTable.getColumnTypeMap();

		CyDataTable edgeCyDataTable = network.getEdgeCyDataTables().get(CyNetwork.DEFAULT_ATTRS);
