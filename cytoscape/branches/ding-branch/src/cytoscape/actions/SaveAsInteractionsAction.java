...
/*
  File: SaveAsInteractionsAction.java 
 ...
 * interactions format: nodeA edgeType nodeB. for example: <code>
 * <p/>
 * YMR056C pp YLL013C
 * YCR107W pp YBR265W
 * <p/>
...
public class SaveAsInteractionsAction extends CytoscapeAction {
	...
	// MLC 09/19/05 BEGIN:
	/**
	 * User-initiated action to save the current network in SIF format to a
	...
	public void actionPerformed(ActionEvent e) {
		...
		// get the file name
		File file = FileUtil.getFile("Export Network as Interactions (SIF)",
		...
		if (file != null) {
			...
			ExportAsSifTask task = new ExportAsSifTask(fileName);
 ...
 */
class ExportAsSifTask implements Task {
	 ...
	 *            Filename to save to
	 */
	ExportAsSifTask(String fileName) {
	...
	private void saveInteractions() throws IOException {
		...
		giny.model.Node[] nodes = (giny.model.Node[]) nodeList
				.toArray(new giny.model.Node[0]);
		...
		for (int i = 0; i < nodes.length; i++) {
			...
			if (edges.size() == 0) {
				...
				while (it.hasNext()) {
					...
					if (node == edge.getSource()) { // do only for outgoing
													// edges
						...
						String interactionName = edgeAtts.getStringAttribute(
