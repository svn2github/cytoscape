...
public class InstallablePlugin implements Installable {
	...
	public boolean install(cytoscape.task.TaskMonitor taskMonitor)
		...
		Download = new File(PluginDir, createFileName(infoObj));
		URLUtil.download(infoObj.getObjectUrl(), Download, taskMonitor);

		ClassName = getPluginClass(Download.getAbsolutePath(), infoObj
				.getFileType());
				...
				new PluginInquireAction() {
					...
					public void inquireAction(List<DownloadableInfo> Results) {
						...
						for (DownloadableInfo NewInfo : Results) {
							if (!InfoToUpdate.getType().equals(NewInfo.getType()))
								continue;
							
							PluginInfo New = (PluginInfo) NewInfo;
							// ID or classname are unique
							boolean newer = InfoToUpdate.isNewerObjectVersion(New);
							if ( (New.getID().equals(InfoToUpdate.getID()) || New.getPluginClassName().equals(InfoToUpdate.getPluginClassName()))
									&& newer) {
	...
	public boolean update(DownloadableInfo newInfoObj, cytoscape.task.TaskMonitor taskMonitor)
			throws java.io.IOException, cytoscape.plugin.ManagerException {
		...
		// ID or classname
		if ( (infoObj.getID().equals(newObj.getID()) || infoObj.getPluginClassName().equals(newObj.getPluginClassName()))
