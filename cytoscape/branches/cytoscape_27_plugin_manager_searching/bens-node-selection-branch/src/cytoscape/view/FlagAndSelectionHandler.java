...
public class FlagAndSelectionHandler implements SelectEventListener, GraphViewChangeListener {
	...
	private void setNodeSelected(Node node, boolean selectOn) {
		...
		// set selected in node details -
		// used later to render selected nodes on top of unselected nodes
		if (view instanceof DGraphView) {
			DNodeDetails nodeDetails = ((DGraphView)view).getDNodeDetails();
			if (nodeDetails != null) {
				nodeDetails.setSelected(~node.getRootGraphIndex(), selectOn);
			}
		}
	}
