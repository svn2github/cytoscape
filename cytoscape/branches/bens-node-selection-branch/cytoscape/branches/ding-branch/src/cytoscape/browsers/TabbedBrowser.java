...
//---------------------------------------------------------------------------------------
/**
 * create a table in a tab for every attribute; each table has 2 columns:
 * canonical name, common name, attribute. note that canonical and common names
 * are, strictly speaking, themselves attributes, but since they appear in every
...
public class TabbedBrowser extends JFrame implements ClipboardOwner {
	...
	/**
	 * Need a global reference here so we can disable the button if the
	 * customize tab is selected
	 */
	...
	/**
	 * Need a global reference to this so we can check if the tabbed pane has
	 * been switched to this tab
	 */
	...
	HashMap customAttributes; // a copy of one of the previous two that we use
	...
	JTabbedPane createGui(CyAttributes attributes) {
		...
		for (int i = 0; i < attributeNames.length; i++) {
			...
			if (attributeName.equalsIgnoreCase("commonName"))
				continue;
			// String attributeCategory = attributes.getCategory
			// (attributeName);
			// //System.out.println("attribute category = " +
			// attributeCategory);
			...
			BrowserTableModel model = new BrowserTableModel(graphObjects,
					attributes, requestedAttibuteNames);
			JTable table = new JTable(model);
			setPreferredColumnWidths(table);
			table.setCellSelectionEnabled(true);
			table.addMouseListener(new MyMouseListener(table));
			table.setPreferredScrollableViewportSize(new Dimension(
					preferredTableWidth, preferredTableHeight));
			JScrollPane scrollPane = new JScrollPane(table);
			tabbedPane.add(attributeNames[i], scrollPane);
		} // for i

		String[] customTabNames = (String[]) customAttributes.keySet().toArray(
				new String[0]);
	...
	protected JPanel createCustomizerTab(String[] attributeNames) {
		...
		panel.setBorder(BorderFactory.createCompoundBorder(BorderFactory
				.createTitledBorder("Select attributes for custom view"),
	...
	/**
	 * The purpose of this class is to listen to the tabbedPane and determine
	 * when the user has selected the Customize tab. If this is the case, the
	 * "Save Table" button should be disabled, because it is non-functional
	 */
	class TabbedPaneListener implements ChangeListener {
		/**
		 * Create a tabbed pane listener, check to see if the customize tab has
		 * been selected, and if so greys out the save button because it is
		 * non-functional
		 * 
		 */
	...
	public class CreateNewPanelAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (customAttributesList == null
					|| customAttributesList.size() == 0)
				return;
			String title = customTabNameTextField.getText();
			if (title == null)
				title = "";
			// System.out.println ("storing custom tab: " + title + ": " +
	...
	private void createNewPanel(Vector customAttributesList, String title) {
		String[] requestedAttributeNames = (String[]) customAttributesList
				.toArray(new String[0]);
		BrowserTableModel model = new BrowserTableModel(graphObjects,
				attributes, requestedAttributeNames);
		JTable table = new JTable(model);
		setPreferredColumnWidths(table);
		table.setCellSelectionEnabled(true);
		table.addMouseListener(new MyMouseListener(table));
	...
	public class OKAction extends AbstractAction {
		...
		OKAction(JFrame frame) {
			super("");
			this.frame = frame;
		}

		public void actionPerformed(ActionEvent e) {
			// System.out.println ("cancel, customAttributes: " +
			// customAttributes);
			frame.dispose();
		}
	...
	class RowListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			if (!lsm.isSelectionEmpty()) {
				...
				// System.out.println ("Row " + selectedRow + " is now
				// selected.");
	...
	class MyMouseListener implements MouseListener {
		...
		public void mouseClicked(MouseEvent e) {
			...
			// System.out.println ("cellValue (" + cellValue.getClass() + "): " +
			// cellValue);

			try {

				if (cellValue != null
						&& cellValue.getClass() == Class.forName("java.net.URL")) {
					...
					// ignore it
					URL url = new URL((String) cellValue);
					displayWebPage(url);
				}
		...
		} // mouseClicked public void mouseEntered(MouseEvent e) {
		...
		public void mouseReleased(MouseEvent e) {
			...
			StringSelection stringSelection = new StringSelection(stringWriter
					.toString());
	...
	protected void displayWebPage(URL url) {
		...
		// }
		// catch (Exception ex) {
		// String msg = "<html>You need to invoke Cytsocape through Java Web
		// Start <br>" +
		// "in order to make use of your web browser from within the
		// program.<br><br>" +
