...
public class DynamicLayoutMenu extends JMenu implements MenuListener {
	...
	private void addNodeAttributeMenus(JMenu parent, boolean selectedOnly) {
		...
		addAttributeMenus(parent, nodeAttributes, 
											layout.supportsNodeAttributes(), selectedOnly);
	}

	private void addEdgeAttributeMenus(JMenu parent, boolean selectedOnly) { 
		// Get the edge attributes
		CyAttributes edgeAttributes = Cytoscape.getEdgeAttributes();
		addAttributeMenus(parent, edgeAttributes, 
											layout.supportsEdgeAttributes(), selectedOnly);
	}

	private void addAttributeMenus(JMenu parent, CyAttributes attributes, 
																 byte[] typeList, boolean selectedOnly) {
			...
			if (layout.supportsSelectedOnly()) {
				...
				if (selectedOnly && selectedNodes.size() > 0) {
					...
					while (nodeViews.hasNext()) {
						...
						if (!selectedNodes.contains(node)) layout.lockNode(nv);
					}
				}
			}
