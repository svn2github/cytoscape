 ...
 * setting the state, or getting the full set of states for
 ...
 * the removal of graph objects. A currently selected object that is
 * removed from the graph will lose its selected state, even if it is later
 * added back to the graph.<P>
 *
 * When the selected state of a node or edge is changed, a event of type SelectEvent
 ...
 *
 * Performance note: the implementation is a HashSet of selected objects,
 * so most methods are O(1). Operations on groups of nodes are O(N) where
 * N is either the number of selected objects or the number of objects in
...
public class SelectFilter implements Filter, GraphPerspectiveChangeListener {
    ...
    /**
     * Returns the set of all selected nodes in the referenced GraphPespective.<P>
     *
     * WARNING: the returned set is the actual data object, not a copy. Don't
     * directly modify this set.
     */
    public Set getSelectedNodes() {return selectedNodes;}
    
    /**
     * Returns the set of all selected edges in the referenced GraphPespective.<P>
     *
     * WARNING: the returned set is the actual data object, not a copy. Don't
     * directly modify this set.
     */
    public Set getSelectedEdges() {return selectedEdges;}
     ...
     * GraphPerspective, false otherwise.
     */
    public boolean isSelected(Edge edge) {return selectedEdges.contains(edge);}
    
    /**
     * Implementation of the Filter interface. Returns true if the argument
     * is a selected Node or Edge in the referenced GraphPerspective, false otherwise.
     */
     ...
     * sets its selected state to the value of the second argument. An event
     ...
     */
    public Set setSelectedNodes(Collection nodesToSet, boolean newState) {
        
      //System.out.println( "SettingSelectedNodes" );
     ...
     */
    public Set setSelectedEdges(Collection edgesToSet, boolean newState) {
        Set returnSet = new HashSet();
    ...
    public Set unselectAllNodes() {
        if (selectedNodes.size() == 0) {return new HashSet();}
    ...
    public Set unselectAllEdges() {
        if (selectedEdges.size() == 0) {return new HashSet();}
     ...
     * whose selected state changed, or a Set of such objects. The second argument
