...
public class PluginManageDialog extends javax.swing.JDialog implements
   ...
   */
	public void switchDownloadSites()
		{
		java.util.Vector<TreeNode> AvailableNodes = new java.util.Vector<TreeNode>(availableNode.getChildren());
		for (TreeNode child: AvailableNodes) 
			{
			treeModel.removeNodeFromParent(child);
			}
		
//		availableNode = new TreeNode(PluginInstallStatus.AVAILABLE.toString(), true);
//		rootTreeNode.addChild(availableNode);

		}

	/**
   * Adds a category and it's list of plugins to the appropriate tree (based on
   * Status) in the dialog.
   * 
   * @param CategoryName
   ...
   */
	...
	private void installButtonActionPerformed(java.awt.event.ActionEvent evt)
		{
		final TreeNode node = (TreeNode) pluginTree.getLastSelectedPathComponent();
	...
	private void initTree()
		...
		installedNode = new TreeNode(PluginInstallStatus.INSTALLED.toString(), true);
		availableNode = new TreeNode(PluginInstallStatus.AVAILABLE.toString(), true);
	...
	private void initComponents()
		...
		layout.setHorizontalGroup(layout.createParallelGroup(
				...
				layout.createSequentialGroup().add(43, 43, 43).add(
						layout.createParallelGroup(
								org.jdesktop.layout.GroupLayout.TRAILING, false).add(
								org.jdesktop.layout.GroupLayout.LEADING, msgLabel).add(
								org.jdesktop.layout.GroupLayout.LEADING, availablePluginsLabel,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
								.add(
										layout.createSequentialGroup().add(changeSiteButton).add(
												18, 18, 18).add(installButton).add(18, 18, 18).add(
												deleteButton).add(22, 22, 22).add(closeButton)).add(
										org.jdesktop.layout.GroupLayout.LEADING, jSplitPane1,
										org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 574,
										Short.MAX_VALUE)).addContainerGap(41, Short.MAX_VALUE)));
		layout.setVerticalGroup(layout.createParallelGroup(
				...
				layout.createSequentialGroup().addContainerGap().add(
						...
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE).addPreferredGap(
						org.jdesktop.layout.LayoutStyle.RELATED).add(msgLabel,
						org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 33, Short.MAX_VALUE)
						.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED).add(
								layout.createParallelGroup(
										org.jdesktop.layout.GroupLayout.BASELINE).add(
										changeSiteButton).add(installButton).add(deleteButton).add(
		...
		public void run()
			...
			try
				...
				taskMonitor.setException(ioe, "Failed to download "
						+ pluginInfo.getName() + " from " + pluginInfo.getUrl());
				}
