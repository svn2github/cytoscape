...
public class PluginTracker {
	...
	private void readDoc() {
		Iterator<Element> pI = TrackerDoc.getRootElement().getChild(this.PluginList)
		                                 .getChildren(this.Plugin).iterator();
		...
		while (pI.hasNext()) {
			...
			Iterator<Element> aI = Plugin.getChild(this.AuthorList).getChildren(this.Author)
			                             .iterator();

			while (aI.hasNext()) {
				Element Author = aI.next();
				Info.addAuthor(Author.getChildTextTrim(this.Name),
				               Author.getChildTextTrim(this.Inst));
			}
	...
	protected void addInstalledPlugin(PluginInfo obj, boolean Overwrite) {
		...
		if (Overwrite && Installed.containsKey(obj.getPluginClassName())) { // replace
