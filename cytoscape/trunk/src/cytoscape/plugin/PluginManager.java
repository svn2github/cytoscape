...
public class PluginManager
   ...
   */
     ...
     */
		...
		switch (obj.getFileType())
			...
			case (PluginInfo.JAR):
				try
					...
					java.io.File Installed = HttpUtils.downloadFile(obj.getUrl(), obj.getName()
							+ ".jar", "plugins/");
			...
			case (PluginInfo.ZIP):
				try
					...
					if (UnzipUtil.zipContains(HttpUtils.getInputStream(obj.getUrl()),
							"plugins/\\w+\\.jar"))
						{
						List<String> InstalledFiles = UnzipUtil.unzip(HttpUtils
								.getInputStream(obj.getUrl()));
		...
		if (installOk) Tracker.addInstalledPlugin(obj, true);
		return installOk;
		}
   ...
   */
	public boolean delete(PluginInfo obj)
		...
		if (deleteOk) this.Tracker.removePlugin(obj);
		return deleteOk;
		}
	...
	public void register(CytoscapePlugin Plugin, String JarFileName)
		...
		if (Plugin.getPluginInfoObject() == null)
			...
			if (JarFileName != null) InfoObj.addFileName(JarFileName);
			}
		else
			{
		...
		try
			...
			if (m != null)
				...
				if (className != null)
					...
					if (pc != null) PluginClassName = className;
					}
				}
			...
			while (entries.hasMoreElements())
				...
				if (entry.endsWith("class"))
					...
					if (pc != null) PluginClassName = entry;
					}
				}
			}
		catch (java.io.IOException E)
