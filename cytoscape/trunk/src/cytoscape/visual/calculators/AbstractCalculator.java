...
public abstract class AbstractCalculator
    ...
    public AbstractCalculator(String name, ObjectMapping m, Class c,
        ...
        if (type == null)
            throw new NullPointerException(
                "Type parameter for Calculator is null");
        ...
        if (!c.isAssignableFrom(m.getRangeClass()))
            throw new ClassCastException("Invalid Calculator: Expected class " +
    ...
    public Object clone()
        ...
        if (dupeCount != 0) {
            int dupeCountIndex = name.lastIndexOf(
                    new Integer(dupeCount).toString());
        ...
        clonedCalc.mappings = new Vector(
                this.mappings.size(),
                2);
    ...
    void updateAttribute(String attrName, CyNetwork network, ObjectMapping m)
        ...
        if (mapIndex == -1)
            throw new IllegalArgumentException(m.getClass().getName() + " " +
                m.toString() + " is not contained in calculator " +
                this.toString());
    ...
    protected class CalculatorUI extends JPanel {
        ...
        public CalculatorUI(CyAttributes attr, JDialog parent, CyNetwork network) {
            ...
            for (i = yPos = 0; i < mappings.size(); i++, yPos++) {
                MiscGB.insert(
                    this.myGBG,
                    new JLabel("Map Attribute:", SwingConstants.RIGHT),
                    0,
                    yPos);
                MiscGB.insert(
                    this.myGBG,
                ...
                if (okClass != null) {
                    ...
                    for (int j = 0; j < attrNames.length; j++) {
                        ...
                        Class attrClass = CyAttributesUtils.getClass(attrNames[j],
                                attr);
                ...
                attrBox.addItemListener(
                    new AttributeSelectorListener(parent, network, i, ++yPos,
                        mapperUI));
        ...
        protected class AttributeSelectorListener
             ...
             */
            protected AttributeSelectorListener(JDialog parent,
                CyNetwork network, int mapIndex, int yPos, JPanel mapperUI) {
                this.parent = parent;
            ...
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.SELECTED) {
                    ...
                    this.mapperUI = ((ObjectMapping) mappings.get(mapIndex)).getUI(this.parent,
                            network);
                    MiscGB.insert(myGBG, this.mapperUI, 0, this.yPos, 3, 1, 2,
