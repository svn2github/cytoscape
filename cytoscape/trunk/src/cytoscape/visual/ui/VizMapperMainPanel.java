...
public class VizMapperMainPanel extends JPanel implements
		PropertyChangeListener, CyNetworkListener {
	...
	private static final ImageIcon rndIcon = new ImageIcon(Cytoscape.class
			.getResource("/cytoscape/images/ximian/stock_filters-16.png"));
	...
	private static final ImageIcon legendIcon = new ImageIcon(
			Cytoscape.class
					.getResource("/cytoscape/images/ximian/stock_graphic-styles-16.png"));
	private static final ImageIcon editIcon = new ImageIcon(Cytoscape.class
			.getResource("/cytoscape/images/ximian/stock_edit-16.png"));
	...
	private void setMenu() {
		...
		deleteVS.addActionListener(new ActionListener() {
		...
		renameVS.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
			}
		});
		...
		duplicateVS.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
			}
		});
	...
	private void initComponents() {
		...
		defaultAppearencePanel.setBorder(javax.swing.BorderFactory
	...
	private ShapeCellRenderer lineCellRenderer = new ShapeCellRenderer(
			VisualPropertyType.EDGE_LINETYPE);
	...
	private static final Map<Object, Icon> nodeShapeIcons = VisualPropertyIconFactory
			.getIconSet(VisualPropertyType.NODE_SHAPE);
	private static final Map<Object, Icon> arrowShapeIcons = VisualPropertyIconFactory
			.getIconSet(VisualPropertyType.EDGE_SRCARROW_SHAPE);
	private static final Map<Object, Icon> lineTypeIcons = VisualPropertyIconFactory
			.getIconSet(VisualPropertyType.EDGE_LINETYPE);
	...
	private void initializePropertySheetPanel() {
		...
		for (Component comp : comps) {
			if (comp.getClass() == JPanel.class) {
				...
				randomize
						.setToolTipText("Randomize Values (Discrete Mapping Only)");
				randomize
						.setIcon(new ImageIcon(
								Cytoscape.class
										.getResource("/cytoscape/images/ximian/stock_filters-16.png")));
				...
				newButton
						.setToolTipText("Switch View (Attribute-Oriented <--> Visual Property Oriented)");
				newButton.setIcon(new ImageIcon(Cytoscape.class
						.getResource("images/ximian/stock_refresh-16.png")));
		...
		visualPropertySheetPanel.getTable().getColumnModel()
				.addColumnModelListener(new TableColumnModelListener() {
					...
					public void columnMarginChanged(ChangeEvent e) {
						final PropertySheetTable table = visualPropertySheetPanel
								.getTable();
						Property shownProp = null;
						int rowCount = table.getRowCount();

						for (int i = 0; i < rowCount; i++) {
							shownProp = ((Item) visualPropertySheetPanel
									.getTable().getValueAt(i, 0)).getProperty();

							if ((shownProp != null)
									&& shownProp.getDisplayName().equals(
											GRAPHICAL_MAP_VIEW)) {
								final Property parent = shownProp
										.getParentProperty();
								...
								if (type instanceof VisualPropertyType) {
									...
									if (mapping instanceof ContinuousMapping) {
										...
										case NODE_BORDER_COLOR:
											...
											final GradientEditorPanel gre = new GradientEditorPanel(
													((VisualPropertyType) type));
											gradientRenderer
													.setIcon(CyGradientTrackRenderer
															.getTrackGraphicIcon(
																	wi,
																	70,
										...
										case NODE_SIZE:
											continuousRenderer
													.setIcon(ContinuousTrackRenderer
															.getTrackGraphicIcon(
																	wi,
																	70,
																	(ContinuousMapping) mapping,
				...
				new MouseListener() {
					public void mouseClicked(MouseEvent e) {
						...
						if (SwingUtilities.isRightMouseButton(e)) {
							...
							if (row >= 0) {
								final Property prop = ((Item) visualPropertySheetPanel
										.getTable().getValueAt(selected, 0))
										.getProperty();
								String controllerName = (String) prop
										.getValue();
								CyAttributes selectedAttr = Cytoscape
										.getNodeAttributes();
							}
						} else {
							/*
							...
							if (0 <= selected) {
								...
								if (prop != null) {
									...
									case NODE_BORDER_COLOR:
									case EDGE_COLOR:
										mep = new GradientEditorPanel(type);
										mep.setPreferredSize(new Dimension(450,
												150));
									...
									case EDGE_LINE_WIDTH:
									case EDGE_FONT_SIZE:
										mep = new C2CMappingEditor(type);
										mep.setPreferredSize(new Dimension(600,
												280));

										break;

									case NODE_SHAPE:
										mep = new C2DMappingEditor(type);
										mep.setPreferredSize(new Dimension(400,
												250));
									...
									if (mep != null) {
										...
										dialog.getContentPane().add(mep);
										dialog.pack();
										dialog.setTitle(type.getName());
										dialog.setLocationRelativeTo(Cytoscape
												.getDesktop());
	...
	private void setPropertyFromCalculator(List<Calculator> calcList,
		...
		for (Calculator calc : calcList) {
			...
			if (firstMap != null) {
				...
				if ((firstMap.getClass() == DiscreteMapping.class)
						&& (attrName != null)) {
					final Map discMapping = ((DiscreteMapping) firstMap)
							.getAll();
					...
					case NODE_SHAPE:
						setDiscreteProps(type, discMapping, attrSet,
								shapeCellEditor, defCellRenderer,
								calculatorTypeProp);
					...
					case EDGE_TGTARROW_SHAPE:
					case EDGE_LABEL:
						discreteRenderer.setIcon(DiscreteTrackRenderer
								.getTrackGraphicIcon(wi, 70,
										(ContinuousMapping) firstMap));
	...
	public void setDefaultPanel(JPanel panel) {
		...
		defaultAppearencePanel.setPreferredSize(new Dimension(mainSplitPane
				.getWidth(), this.mainSplitPane.getDividerLocation()));
		defaultAppearencePanel.setSize(defaultAppearencePanel
				.getPreferredSize());
	...
	public void propertyChange(PropertyChangeEvent e) {
		 ...
		 */
		if (ctrAttrName != null) {
			mapping.setControllingAttributeName(ctrAttrName, vmm.getNetwork(),
					false);
		...
		Object key = ((Item) visualPropertySheetPanel.getTable().getValueAt(
	...
	private void switchMapping(VizMapperProperty prop, String newMapName) {
		...
		if (newCalc == null) {
			...
			vmm.getCalculatorCatalog().addCalculator(
					getNewCalculator(type, newMapName, newCalcName));
		}
		...
		if (oldCalc != null) {
			...
			if (vmm.getCalculatorCatalog().getCalculator(type, oldCalcName) == null)
				vmm.getCalculatorCatalog()
						.addCalculator(
								getNewCalculator(type, oldMappingTypeName,
										oldCalcName));
		}
	...
	private void expandLastSelectedItem(String name) {
		...
		for (int i = 0; i < table.getRowCount(); i++) {
			...
			if ((curProp != null) && (curProp.getDisplayName() == name)) {
				visualPropertySheetPanel.getTable().setRowSelectionInterval(i,
						i);
		...
		visualPropertySheetPanel.getTable().getActionMap().get("toggle")
	...
	private Calculator getNewCalculator(final VisualPropertyType type,
		...
		final Object defaultObj = VizUIUtilities.getDefault(vmm
				.getVisualStyle(), type);
		...
		return CalculatorFactory
				.newDefaultCalculator(type, newCalcName, mapper);
	}
		...
		public void actionPerformed(ActionEvent e) {
			final SwingWorker worker = new SwingWorker() {
				public Object construct() {
					LegendDialog ld = new LegendDialog(parent, vmm
							.getVisualStyle());
					ld.setVisible(true);
	...
	public String getStyleName(VisualStyle s) {
		...
		// name
		while (true) {
			String ret = (String) JOptionPane.showInputDialog(Cytoscape
					.getDesktop(), "Name for new visual style",
					"Visual Style Name Input", JOptionPane.QUESTION_MESSAGE,
	...
	private void createNewMapping(VisualPropertyType type) {
		...
		Calculator calc = vmm.getCalculatorCatalog().getCalculator(type,
				calcName);

		if (calc == null) {
			calc = CalculatorFactory.newDefaultCalculator(type, calcName,
					mapper);
		...
		visualPropertySheetPanel.getTable().setColumnSelectionAllowed(false);
		visualPropertySheetPanel.getTable().setRowSelectionInterval(2, 5);
	...
	private void removeMapping() {
		...
		if (0 <= selected) {
			Item item = (Item) visualPropertySheetPanel.getTable().getValueAt(
					selected, 0);
			...
			if (curProp instanceof VizMapperProperty) {
				...
				int value = JOptionPane.showConfirmDialog(Cytoscape
						.getDesktop(), message, "Remove Mapping",
	...
	class DSourceList extends JList implements Transferable,
			DragGestureListener {
