...
public class VizMapAttrTab extends VizMapTab {
     ...
     * Utility methods in VizUIUtilites are used to convert this type into a
     * particular object or method of the current visual style.
     */
    private byte type;
     ...
     * method to figure out which calculator corresponds to the type field of
     * this object. It is the responsibility of every method in this class to
     * make sure this field is synchronized with the current state of the
    ...
    /**
     * the panel containing the calculator-specific UI provided by the currently
     * selected calculator
     */
     ...
     * @param tabIndex
     *            index of this tab in tabContainer
     * @param n
     *            Underlying network
     * @param type
     *            One of types defined in {@link VisualMappingManager}
     * @param c
     */
    public VizMapAttrTab(VizMapUI mainUI, JTabbedPane tabContainer,
    ...
    /**
     * Internal class to listen for possible changes in the state of the
     * catalog. When triggered, causes this class to rebuild the combo box for
    ...
    /**
     * Refreshes the panel that displays the UI for the currently selected
     * calculator. This method replaces the current panel with the panel
     * provided by the current calculator, or nothing if there is no currently
     * selected calculator.
     */
    public void refreshUI() {
        ...
        if (this.currentCalculator != null) {
            this.calcPanel = this.currentCalculator.getUI(
                    this.mainUIDialog,
                    VMM.getNetwork());
     ...
     *
     * This method should be called whenever the visual style is changed, or
     * when the calculator for this object's visual attribute is switched to a
     * different calculator. If the change is made by a member of this class,
     * they should call this method instead of directly changing the
     * currentCalculator field (to make sure that the listener is properly
     * updated). If the change was instead made directly to the underlying
     * visual style, then a listener should respond by calling this method with
     * the new calculator.
     */
    ...
    protected void drawDefault(Object defaultObj) {
        ...
        this.defaultValueDisplayer = ValueDisplayer.getDisplayFor(
                mainUIDialog,
                getName(),
                defaultObj);

        if (defaultValueDisplayer != null)
            defaultValueDisplayer.addItemListener(
                new DefaultItemChangedListener());
        else
            return;
        ...
        defaultButton.addActionListener(
            defaultValueDisplayer.getInputListener());
        ...
        defPanel.setBorder(
            BorderFactory.createTitledBorder(defBorder, "Default",
    ...
    protected ImageIcon getImageIcon() {
        ...
        if (this.type == VizMapUI.NODE_COLOR) {
            ...
            icon = new ImageIcon(
                    this.getClass().getResource(imageFile),
                    "Node Color");
    ...
    protected void rebuildCalcComboBox() {
        ...
        /*
         * build the list of known calculators - each calculator has a
         * toString() method that returns the name, so calculators can be passed
         * to the JComboBox.
         */
        ...
        if ((currentCalculator != null) &&
                !(calculators.contains(currentCalculator)))
        ...
        if (this.currentCalculator == null)
            /*
             * Index 0 is always the "None" string. However, setSelectedIndex(0)
             * does not call event handlers. Thus, in RmCalcListener,
             * switchCalculator() is called explicitly.
             */
            this.calcComboBox.setSelectedIndex(0);
        else
            this.calcComboBox.setSelectedItem(this.currentCalculator);

        MiscGB.insert(mapPanelGBG, calcComboBox, 0, 0, 4, 1, 1, 0,
    ...
    /**
     * Listens to selection events on the calculator selection combo box. When a
     * selection occurs, calls switchCalculator with the new calculator.
     */
    private class CalcComboSelectionListener
        implements ItemListener {
        public void itemStateChanged(ItemEvent e) {
            if (e.getStateChange() == ItemEvent.SELECTED) {
                if (calcComboBox.getSelectedIndex() == 0) // "None" selected,
                                                          // use null
     ...
     * the current calculator to the selected calculator and place the
     * calculator's UI into the calculator UI panel.
     *
     * @param calc
     *            new Calculator to use for this tab's mapping
     */
     ...
     * current calculator is added later by refreshUI. Called only once;
     * afterwards the UI is refreshed through either rebuildCalcComboBox, which
     * refreshes the calculator selection combo box, or refreshUI which
     * refreshes the current calculator view.
     */
    protected void drawCalc() {
        ...
        // MiscGB.insert(mapPanelGBG, newCalc, 0, 1, 1, 1, 1, 0,
        // GridBagConstraints.HORIZONTAL);
        ...
        // MiscGB.insert(mapPanelGBG, dupeCalc, 1, 1, 1, 1, 1, 0,
        // GridBagConstraints.HORIZONTAL);
        ...
        // MiscGB.insert(mapPanelGBG, renCalc, 2, 1, 1, 1, 1, 0,
        // GridBagConstraints.HORIZONTAL);
        ...
        // MiscGB.insert(mapPanelGBG, rmCalc, 3, 1, 1, 1, 1, 0,
        // GridBagConstraints.HORIZONTAL);
        ...
        MiscGB.insert(mapPanelGBG, btnPanel, 0, 1, 1, 1, 1, 0,
            GridBagConstraints.HORIZONTAL);

        // add to gridbag
        MiscGB.insert(mapPanelGBG, calcContainer, 0, 2, 4, 1, 5, 5,
    ...
    private class NewCalcListener extends AbstractAction {
        public void actionPerformed(ActionEvent e) {
            ...
            String selectedMapper = (String) JOptionPane.showInputDialog(mainUIDialog,
                    "Choose a mapper", "New Calculator",
                    JOptionPane.QUESTION_MESSAGE, null, mapperArray,
                    mapperArray[0]);
            ...
            try {
                ...
                JOptionPane.showMessageDialog(mainUIDialog,
                    "Error creating mapping " + mapperClass.getName(), "Error",
                    JOptionPane.ERROR_MESSAGE);
            }

            Calculator calc = CalculatorFactory.newDefaultCalculator(vizType,
                    calcName, mapper);

            System.out.println("*** Calc type = " +
                calc.getVisualPropertyType().getPropertyLabel());
    ...
    private Calculator duplicateCalculator(Calculator c) {
        ...
        try {
            ...
            System.err.println(
                "Fatal error - Calculator didn't support Cloneable");
            exc.printStackTrace();
    ...
    private String getCalculatorName(Calculator c) {
        ...
        if (c != null)
            suggestedName = this.catalog.checkCalculatorName(
                    c.toString(),
                    this.type);
        ...
        while (true) {
            String ret = (String) JOptionPane.showInputDialog(mainUIDialog,
                    "New name for calculator", "Calculator Name Input",
            ...
            if (ret == null)
                ...
                JOptionPane.showMessageDialog(mainUIDialog,
                    ...
                    "Calculator Name Input Error", JOptionPane.ERROR_MESSAGE);
            ...
            int alt = JOptionPane.showConfirmDialog(mainUIDialog,
                    ...
                    "Duplicate calculator name", JOptionPane.YES_NO_OPTION,
    ...
    private class RenCalcListener extends AbstractAction {
        public void actionPerformed(ActionEvent e) {
            // get the new name, keep prompting for input until a valid input is
            // received
    ...
    private class RmCalcListener extends AbstractAction {
        public void actionPerformed(ActionEvent e) {
            ...
            if ((conflicts.size() != 0) &&
                ...
                errMsg.append(
                    "</ul><br>Do you still want to delete this calculator?");

                JEditorPane errPane = new JEditorPane("text/html",
                        errMsg.toString());
                ...
                int conf = JOptionPane.showConfirmDialog(mainUIDialog, errPane,
                        "Calculator In Use", JOptionPane.YES_NO_OPTION,
                ...
                int conf = JOptionPane.showConfirmDialog(mainUIDialog, s,
                        "Confirm Remove Calculator", JOptionPane.YES_NO_OPTION);
            ...
            switchCalculator(null);
            catalog.removeCalculator(temp); // triggers events that switch the
                                            // calculator
        }
    }
     ...
     *
     * A single instance of this listener is maintained by this class and is
     * removed and reattached when the current calculator changes.
     */
    ...
    /**
     * Check that the calculator is not selected by other objects. If so, pop up
     * a dialog offering to duplicate the calculator.
     *
     * @param c
     ...
     * alerting calcComboSelectionListener which changes the selected
     * calculator.
     */
