...
public class OldStyleCalculatorIO {
	...
	* @deprecated Use CalculatorIO instead. Will be removed 10/2007
	...
	public static void checkForCalculators(Properties props, CalculatorCatalog catalog) {
		...
		for (Enumeration e = props.propertyNames(); e.hasMoreElements();) {
			...
			if (key.startsWith("node.fillColor.") || key.startsWith("node.borderColor.")
			    || key.startsWith("node.selectedColor.") || key.startsWith("node.borderLinetype.")
			    || key.startsWith("node.width.") || key.startsWith("node.height.")
			    || key.startsWith("node.shape.") || key.startsWith("edge.color.")
			    || key.startsWith("background.color.") || key.startsWith("edge.linetype.")
	...
	* @deprecated Use CalculatorIO instead. Will be removed 10/2007
	...
	public static void loadCalculators(Properties props, CalculatorCatalog catalog) {
		...
		loadCalculator(props, catalog, "node.borderColor", nodeColorName2, nodeColorClass,
		               nodeColorIntClass, colorInterpolator);
		...
		loadCalculator(props, catalog, "node.borderLinetype", nodeLineTypeName, nodeLineTypeClass,
		               nodeLineTypeIntClass, flatInterpolator);
		...
		loadCalculator(props, catalog, "node.width", nodeSizeName, nodeSizeClass, nodeSizeIntClass,
		               numberInterpolator);
		...
		loadCalculator(props, catalog, "edge.linetype", edgeLineTypeName, edgeLineTypeClass,
		               edgeLineTypeIntClass, flatInterpolator);
		...
		loadCalculator(props, catalog, "edge.sourceDecoration", edgeSourceName, edgeArrowClass,
		               edgeArrowIntClass, flatInterpolator);

		String edgeTargetName = edgeSourceName + "2";
		loadCalculator(props, catalog, "edge.targetDecoration", edgeTargetName, edgeArrowClass,
		               edgeArrowIntClass, flatInterpolator);
	...
	* @deprecated Use CalculatorIO instead. Will be removed 10/2007
	...
	private static void loadCalculator(Properties props, CalculatorCatalog catalog,
	                                   ...
	                                   String intClassName, String interpolator) {
		Properties newProps = getNewProperties(props, oldBaseKey, newBaseKey, className,
		                                       interpolator);
	...
	private static Properties getNewProperties(Properties props, String oldBaseKey,
	                                           String newBaseKey, String className,
	                                           String interpolator) {
	 ...
	 */
	private static void loadNodeAppearanceCalculator(Properties props, CalculatorCatalog catalog,
	                                                 VisualStyle vs, VisualStyle defVS) {
	 ...
	 * automatically activated the next time the user runs Cytoscape.
	 */
	private static void loadEdgeAppearanceCalculator(Properties props, CalculatorCatalog catalog,
	                                                 VisualStyle vs, VisualStyle defVS) {
	...
	private static void loadGlobalAppearanceCalculator(Properties props, CalculatorCatalog catalog,
	                                                   VisualStyle vs, VisualStyle defVS) {
