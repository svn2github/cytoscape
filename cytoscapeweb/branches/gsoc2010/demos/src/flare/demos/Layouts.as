package flare.demos
{
	...
	import flare.demos.util.DataLoader;
	...
	public class Layouts extends Demo
	{
		...
		public override function init():void
		{
			...
			//By Tomithy to create a function to read from graphical data types: for testing
			var dataLoader:DataLoader = new DataLoader(callBack);
		...
		private function switchTo(name:String):Transition
		{
			...
			//The following function is use to redraw the mesh whenever the graph layout is changed. 
			timer.addEventListener(TimerEvent.TIMER, function onTimerComplete(e:TimerEvent):void
			{
		...
		private function options(w:Number, h:Number):Array
		{
			var a:Array = [
				...
				},
				{
					...
					param: {
						"simulation.dragForce.drag": 0.2, //default 0.2
						defaultParticleMass: 3,
						defaultSpringLength: 20, //default 40
						defaultSpringTension: .1 //default .5
					},
					update: true,
		...
		//to reinitialise graph with loaded data
		private function callBack(d:Data):void
		{
			//initialisation is wrapped inside event listener to handle asynchronous code
			...
			//trace("Layouts : Is onComplete called?");
			
			
			_GBEB = new GBEB(data);
			addChild(_GBEB);
			
			//swapChildren(_GBEB, vis);
