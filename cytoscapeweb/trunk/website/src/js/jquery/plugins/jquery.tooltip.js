...
;(function($){
    ...
    $.tooltip.defaults = {
        ...
        delayAfterHover: 250, // ms delay from mouse over to showing tool tip; do not show tool tip on negative value
        minArrowMargin: 5, // min margin (left or right) from edge of tool tip bubble for the tool tip arrow
        opacity: 1,
    ...
    $.tooltip.show = function(options) {
        ...
        if( $("#" + options.toolTipId).length <= 0 ) {  
            $("body").append("<div id=\"" + options.toolTipId + "\" class=\"" + options.toolTipClass + "\">"
                    + "<div class=\"" + options.toolTipTopArrowId + " " + options.toolTipPointerClass + "\"></div>"
                    + "<div class=\"" + options.toolTipTextId + "\">&nbsp;</div>"
                    + "<div class=\"" + options.toolTipBottomArrowId + " " + options.toolTipPointerClass + "\"></div>"
                    + "</div>");
        }
        ...
        $.tooltip.timeoutList[options.toolTipId] = setTimeout(function(){
            ...
            var offsetBottom = 0;
            var enabled = true;
            var imgWidth = parseInt( $("." + options.toolTipPointerClass).css("width") );
