...
# recurively get all .gl.enrichment files
# -printf "%P" will remove $resultsDir from the filename
my $find_cmd = `find $resultsDir -name "*.gl.enrichment" -printf "%P\n"\n`;
...
foreach my $file (@files)
{
    ...
    print STDERR "file = $file [pub=$pub, model=$model_name]\n";
#    next;
    
    my $model_id = get_model_id($get_model_id_STH, $pub, $model_name);
    
    open(FILE, "< $resultsDir/$file") or die "Cannot open $resultsDir/$file: $!\n";
