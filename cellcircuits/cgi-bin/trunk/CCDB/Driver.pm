...
sub search
{
	...
	#my ($lower_lim, $upper_lim) = 
	#    CCDB::HtmlRoutines::outputResultsHeader($query,
	#					    $publications,
	#					    $species,
	#					    $sort_method,
	#					    $pval_thresh,
	...
	#					    $request_URI
	#					    );
	#    CCDB::HtmlRoutines::outputResultsTable($hash, 
	#					   $page, #page to output
	#					   $lower_lim,
	#					   $upper_lim,
	#					   $pval_thresh,
...
sub inspect_results
{
	...
	foreach my $model_id (keys %{ $hash }){
		...
		print "\tid                         : $id\n";
		print "\tn_genes_in_model_with_term : "
		    . "$n_genes_in_model_with_term\n";
		print "\tn_genes_in_model           : $n_genes_in_model\n";
		print "\tn_genes_with_term          : $n_genes_with_term\n";
		print "\tn_genes_in_GO              : $n_genes_in_GO\n";
		print "\tpval                       : $pval\n";
	...
	foreach my $mid (keys %{ $hash }) {
		...
		printf "mid: $mid\t# enrichment objects = %d\n"
		    , scalar(@{ $hash->{$mid}{'enrichment'} });
	    }
	    else {
		print "hash->{$mid}{'enrichment'} doesnt exist!\n";
	    }
	}
	printf "n_matched_models: %d\n", scalar(keys %{ $hash });
