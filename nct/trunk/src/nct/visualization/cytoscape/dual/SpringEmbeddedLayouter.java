...
//============================================================================
// 
//  file: SpringEmbeddedLayouter.java
// 
...
public class SpringEmbeddedLayouter{
	...
	protected int[][] calculateNodeDistances(List nodesList,GraphPerspective network){
		...
		for(int i=0;i<node_count;i++){
			...
			node2Index.put(n,new Integer(i)); // not used in this method 
			nodeIndexArray[i] = n.getRootGraphIndex();
			graphIndex2arrayIndex.put(-(nodeIndexArray[i]),i);
			Arrays.fill(result[i],20);
		}
		for(int i=0;i<node_count;i++){
			int sourceNodeIndex = nodeIndexArray[i];
			int[] edgeIndexArray = network.getAdjacentEdgeIndicesArray(sourceNodeIndex,true,true,true);
			for ( int edgeNum=0; edgeNum<edgeIndexArray.length;edgeNum++) {
				int edgeIndex = edgeIndexArray[edgeNum];
				int targetNodeIndex = sourceNodeIndex ^ network.getEdgeSourceIndex(edgeIndex) ^ network.getEdgeTargetIndex(edgeIndex); 
				int j = graphIndex2arrayIndex.get(-targetNodeIndex);
				result[i][j] = 1;
				result[j][i] = 1;
			}
			result[i][i] = 0;
		...
		for (int k=0;k<node_count;k++){ /* k -> is the intermediate point */
	...
	protected void setupNodeDistanceSprings () {
		...
		String [] nodeNames = new String[nList.size()];
		{
			int idx = 0;
			for(Iterator nodeIt = graphView.getGraphPerspective().nodesIterator();nodeIt.hasNext();){
				//nodeNames[idx++] = (String)Cytoscape.getNodeAttributeValue((Node)nodeIt.next(),Semantics.CANONICAL_NAME);
				nodeNames[idx++] = nodeAttrs.getStringAttribute(((Node)nodeIt.next()).getIdentifier(),Semantics.CANONICAL_NAME);
			}
		}
		...
		for( int node_i = 0; node_i < nodeCount; node_i++ ) {
			for( int node_j = ( node_i + 1 ); node_j < nodeCount; node_j++ ) {
				...
				if(!homologyPairSet.contains(i,j)){
					...
					//there is a homology interaction between them, make a strong homology interaction
					nodeDistanceSpringRestLengths[ node_i ][ node_j ] = node_distance_rest_length_constant/HOMOLOGY_MULTIPLIER;
					nodeDistanceSpringStrengths[ node_i ][ node_j ] = HOMOLOGY_MULTIPLIER*HOMOLOGY_MULTIPLIER*node_distance_strength_constant;
				}
	...
	protected PartialDerivatives calculatePartials ( PartialDerivatives partials, 
		List partials_list, PotentialEnergy potential_energy, boolean reversed) {
			...
			if( distance_from_touching < 0.0 ) {
				incremental_change =
					( anticollisionSpringScalars[ layoutPass ] *
						( anticollisionSpringStrength[ node_view_index ][ other_node_view_index] *
							( delta_x -
								(
								 ( ( node_view_radius + other_node_view_radius ) *
									 delta_x ) /
				...
				if( other_node_partials != null ) {
					incremental_change =
						( anticollisionSpringScalars[ layoutPass ] *
							( anticollisionSpringStrength[other_node_view_index][ node_view_index] *
								( -delta_x -
									(
									 ( ( node_view_radius + other_node_view_radius ) *
										 -delta_x ) /
