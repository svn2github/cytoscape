...
public class SelectEventTest extends TestCase {
	...
	public void testCtor() throws Exception {
		...
		SelectEvent singleNodeOn = new SelectEventImpl(source, node1, true);
		checkEvent(singleNodeOn, source, node1, SelectEvent.SINGLE_NODE, true);

		SelectEvent singleNodeOff = new SelectEventImpl(source, node2, false);
		checkEvent(singleNodeOff, source, node2, SelectEvent.SINGLE_NODE, false);

		SelectEvent singleEdgeOn = new SelectEventImpl(source, edge1, true);
		checkEvent(singleEdgeOn, source, edge1, SelectEvent.SINGLE_EDGE, true);

		SelectEvent singleEdgeOff = new SelectEventImpl(source, edge2, false);
		checkEvent(singleEdgeOff, source, edge2, SelectEvent.SINGLE_EDGE, false);

		SelectEvent nodeSetOn = new SelectEventImpl(source, nodeSet, true);
		checkEvent(nodeSetOn, source, nodeSet, SelectEvent.NODE_SET, true);

		SelectEvent nodeSetOff = new SelectEventImpl(source, nodeSet, false);
		checkEvent(nodeSetOff, source, nodeSet, SelectEvent.NODE_SET, false);

		SelectEvent edgeSetOn = new SelectEventImpl(source, edgeSet, true);
		checkEvent(edgeSetOn, source, edgeSet, SelectEvent.EDGE_SET, true);

		SelectEvent edgeSetOff = new SelectEventImpl(source, edgeSet, false);
