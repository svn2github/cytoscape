...
public class GraphLayoutRepresentation extends GraphTopologyRepresentation implements GraphLayout {
	 ...
	 *
	 * @param maxWidth       <blockquote>the maximum allowable X position of nodes, as defined by
	 *                       <code>getMaxWidth()</code>.</blockquote>
	 * @param maxHeight      <blockquote>the maximum allowable Y position of nodes, as defined by
	 *                       <code>getMaxHeight()</code>.</blockquote>
	 * @param nodeXPositions <blockquote>an array of length <code>numNodes</code> such that
	 ...
	 * @param nodeYPositions <blockquote>an array of length <code>numNodes</code> such that
	 ...
	 */
	public GraphLayoutRepresentation(int numNodes, int[] directedEdgeSourceNodeIndices,
		...
		super(numNodes, directedEdgeSourceNodeIndices, directedEdgeTargetNodeIndices,
		      undirectedEdgeNode0Indices, undirectedEdgeNode1Indices);
		...
		if (nodeXPositions == null) {
			nodeXPositions = new double[getNumNodes()];
		}

		if (nodeYPositions == null) {
			nodeYPositions = new double[getNumNodes()];
		}
		...
		System.arraycopy(nodeXPositions, 0, m_nodeXPositions, 0, nodeXPositions.length);
		...
		System.arraycopy(nodeYPositions, 0, m_nodeYPositions, 0, nodeYPositions.length);
	...
	public final double getNodePosition(int nodeIndex, boolean xPosition) {
		...
		return (xPosition ? m_nodeXPositions[nodeIndex] : m_nodeYPositions[nodeIndex]);
	}
}
