...
public final class GraphConverter {
	...
	private static final class MyCMutableGraphLayout
		...
		private MyCMutableGraphLayout(int numNodes, int[] directedEdgeSourceNodeIndices,
		                              int[] directedEdgeTargetNodeIndices,
		                              int[] undirectedEdgeNode0Indices,
		                              int[] undirectedEdgeNode1Indices, double width,
		                              double height, double[] nodeXPositions,
		                              double[] nodeYPositions, boolean[] isMovableNode,
		                              double[][] edgeAnchorXPositions,
		                              double[][] edgeAnchorYPositions, NodeView[] nodeTranslation,
		                              EdgeView[] edgeTranslation, double xOff, double yOff) {
			super(numNodes, directedEdgeSourceNodeIndices, directedEdgeTargetNodeIndices,
			      undirectedEdgeNode0Indices, undirectedEdgeNode1Indices, width, height,
			      nodeXPositions, nodeYPositions, isMovableNode, edgeAnchorXPositions,
	...
	private static class MyRMutableGraphLayout implements MutablePolyEdgeGraphLayout {
		...
		private MyRMutableGraphLayout(NodeView[] nodeTranslation, EdgeView[] edgeTranslation,
		                              Hashtable nodeIndexTranslation, double width, double height,
		                              double xOff, double yOff, boolean allMovable) {
			m_nodeTranslation = nodeTranslation;
			m_edgeTranslation = edgeTranslation;
		...
		public int getNumNodes() {
			return m_nodeTranslation.length;
		}

		public int getNumEdges() {
			return m_edgeTranslation.length;
		}
		...
		public int getEdgeNodeIndex(int edgeIndex, boolean sourceNode) {
			...
			if (sourceNode) {
				ginyNInx = edge.getSource().getRootGraphIndex();
			} else {
		...
		public double getNodePosition(int nodeIndex, boolean xPosition) {
			...
			if (xPosition) {
				return node.getXPosition() - m_xOff;
			} else {
				return node.getYPosition() - m_yOff;
			}
		}
		...
		public void setNodePosition(int nodeIndex, double xPos, double yPos) {
			...
			if (!isMovableNode(nodeIndex)) {
				throw new UnsupportedOperationException("node at index " + nodeIndex
				                                        + " is not movable");
			}
		...
		public double getAnchorPosition(int edgeIndex, int anchorIndex, boolean xPosition) {
			Point2D point = (Point2D) m_edgeTranslation[edgeIndex].getBend().getHandles()
			                                                      .get(anchorIndex);

			return (xPosition ? (point.getX() - m_xOff) : (point.getY() - m_yOff));
		}

		private void checkAnchorIndexBounds(int edgeIndex, int anchorIndex, boolean create) {
		...
		public void createAnchor(int edgeIndex, int anchorIndex) {
			...
			checkMutableAnchor(edgeIndex);
			m_edgeTranslation[edgeIndex].getBend().addHandle(anchorIndex, new Point2D.Double());

			Point2D src = ((anchorIndex == 0)
			               ? (new Point2D.Double(getNodePosition(getEdgeNodeIndex(edgeIndex, true),
			                                                     true),
			                                     getNodePosition(getEdgeNodeIndex(edgeIndex, true),
			                                                     false)))
			               : (new Point2D.Double(getAnchorPosition(edgeIndex, anchorIndex - 1, true),
			                                     getAnchorPosition(edgeIndex, anchorIndex - 1, false))));
			Point2D trg = ((anchorIndex == (getNumAnchors(edgeIndex) - 1))
			               ? (new Point2D.Double(getNodePosition(getEdgeNodeIndex(edgeIndex, false),
			                                                     true),
			                                     getNodePosition(getEdgeNodeIndex(edgeIndex, false),
			                                                     false)))
			               : (new Point2D.Double(getAnchorPosition(edgeIndex, anchorIndex + 1, true),
			                                     getAnchorPosition(edgeIndex, anchorIndex + 1, false))));
			setAnchorPosition(edgeIndex, anchorIndex, (src.getX() + trg.getX()) / 2.0d,
			                  (src.getY() + trg.getY()) / 2.0d);
		}

		public void setAnchorPosition(int edgeIndex, int anchorIndex, double xPos, double yPos) {
			checkAnchorIndexBounds(edgeIndex, anchorIndex, false);
			checkMutableAnchor(edgeIndex);
			checkPosition(xPos, yPos);
			m_edgeTranslation[edgeIndex].getBend()
			                            .moveHandle(anchorIndex,
			                                        new Point2D.Double(xPos + m_xOff, yPos + m_yOff));
		}
	}
	...
	public static MutablePolyEdgeGraphLayout getGraphCopy(double percentBorder,
		...
		while (nodeIterator.hasNext()) {
			...
			if (noNodesSelected) {
				mobility[nodeIndex] = true;
			} else {
				mobility[nodeIndex] = currentNodeView.isSelected();
			}
		...
		while (edgeIterator.hasNext()) {
			...
			if (currentEdge.isDirected()) {
				chosenEdgeVector = directedEdgeVector;
			}

			chosenEdgeVector.add(new Object[] {
			                         new int[] { nativeSourceNodeIndex, nativeTargetNodeIndex },
			                         currentEdgeView
			                     });
		}

		final int[] directedEdgeSourceNodeIndices = new int[directedEdgeVector.size()];
		final int[] directedEdgeTargetNodeIndices = new int[directedEdgeVector.size()];

		for (int i = 0; i < directedEdgeVector.size(); i++) {
			int[] edge = (int[]) (((Object[]) (directedEdgeVector.get(i)))[0]);
			EdgeView edgeV = (EdgeView) (((Object[]) (directedEdgeVector.get(i)))[1]);
			directedEdgeSourceNodeIndices[i] = edge[0];
			directedEdgeTargetNodeIndices[i] = edge[1];
			edgeTranslation[i] = edgeV;
		}

		final int[] undirectedEdgeSourceNodeIndices = new int[undirectedEdgeVector.size()];
		final int[] undirectedEdgeTargetNodeIndices = new int[undirectedEdgeVector.size()];

		for (int i = 0; i < undirectedEdgeVector.size(); i++) {
			int[] edge = (int[]) (((Object[]) (undirectedEdgeVector.get(i)))[0]);
			EdgeView edgeV = (EdgeView) (((Object[]) (undirectedEdgeVector.get(i)))[1]);
		...
		{ // Modify minX, maxY, minY and maxY so that the area is a square.
			...
			double extra = Math.max(maxX - minX, maxY - minY) - Math.min(maxX - minX, maxY - minY);
		...
		final double border = Math.max(maxX - minX, maxY - minY) * percentBorder * 0.5d;
		...
		return new MyCMutableGraphLayout(numNodesInTopology, directedEdgeSourceNodeIndices,
		                                 directedEdgeTargetNodeIndices,
		                                 undirectedEdgeSourceNodeIndices,
		                                 undirectedEdgeTargetNodeIndices,
		                                 maxX - minX + border + border,
		                                 maxY - minY + border + border, nodeXPositions,
		                                 nodeYPositions, mobility, edgeAnchorXPositions,
		                                 edgeAnchorYPositions, nodeTranslation, edgeTranslation,
		                                 xOff, yOff);
	}
	 ...
	 *
	 * @throws IllegalArgumentException if <code>layout</code> is not
	...
	public static void updateCytoscapeLayout(MutablePolyEdgeGraphLayout layout) {
		...
		try {
			myLayout = (MyCMutableGraphLayout) layout;
		...
		for (int n = 0; n < nodeTranslation.length; n++) {
			...
			nodeTranslation[n].setXPosition(layout.getNodePosition(n, true) + myLayout.m_xOff);
			nodeTranslation[n].setYPosition(layout.getNodePosition(n, false) + myLayout.m_yOff);
		}
		...
		for (int e = 0; e < edgeTranslation.length; e++) {
			...
			for (int a = 0; a < layout.getNumAnchors(e); a++) {
				anchorList.add(new Point2D.Double(layout.getAnchorPosition(e, a, true)
				                                  + myLayout.m_xOff,
				                                  layout.getAnchorPosition(e, a, false)
				                                  + myLayout.m_yOff));
			}
	...
	public static MutablePolyEdgeGraphLayout getGraphReference(double percentBorder,
		...
		return new MyRMutableGraphLayout(nodeTranslation, edgeTranslation, nodeIndexTranslation,
		                                 maxX - minX + border + border,
		                                 maxY - minY + border + border, xOff, yOff, noNodesSelected);
	}
}
