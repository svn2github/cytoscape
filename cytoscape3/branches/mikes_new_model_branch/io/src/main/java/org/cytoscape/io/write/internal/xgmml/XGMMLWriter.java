 ...
 */
package org.cytoscape.io.write.internal.xgmml;
...
*/
	...
	private void writeAttribute(final CyRow attributes, final String attributeName) 
		...
		// process boolean
			...
			Boolean bAttr = attributes.get(attributeName,Boolean.class);
			writeAttributeXML(attributeName, ObjectType.BOOLEAN, bAttr, true);
		}
		// process simple list
		else if (attType == List.class) {
			// get the attribute list
			final List listAttr = attributes.get(attributeName, List.class);
		...
		// process simple map
			...
			final Map mapAttr = attributes.get(attributeName,Map.class);
