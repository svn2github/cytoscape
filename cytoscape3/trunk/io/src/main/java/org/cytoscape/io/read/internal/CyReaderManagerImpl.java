...
public class CyReaderManagerImpl implements CyReaderManager {
	...
	private Set<CyNetworkReader> readers;

	/**
	 * Constructor.
	 */
	public CyReaderManagerImpl() {
		readers = new HashSet<CyNetworkReader>();	
	}

	public void addReader(CyNetworkReader read, Map props) {
		readers.add(read);
	}

	public void removeReader(CyNetworkReader read, Map props) {
		readers.remove(read);
	}
	 ...
	 */
	public CyNetworkReader getReader(String fileName) throws IOException {
		for ( CyNetworkReader read : readers ) {
			...
			if ( cff.accept( fileName ) ) {
				try {
					read.setInput( new FileInputStream(fileName) );
				} catch (FileNotFoundException fnfe) {
					IOException ioe = new IOException("Couldn't find file: " + fileName);
					ioe.setStackTrace( fnfe.getStackTrace() );
					throw ioe;
				}
				return read;
			}
	 ...
	 */
	public CyNetworkReader getReader(URL url) throws IOException {
	 ...
	 */
	private String[] concatAllExtensions(Set<CyNetworkReader> cffs) {
		Set<String> stringAns = new HashSet<String>();

		for ( CyNetworkReader cff : cffs ) 
	...
	private File downloadFromURL(URL url, TaskMonitor taskMonitor)
		...
		Proxy pProxyServer = null; // TODO ProxyHandler.getProxyServer();
		URLConnection conn = null;
