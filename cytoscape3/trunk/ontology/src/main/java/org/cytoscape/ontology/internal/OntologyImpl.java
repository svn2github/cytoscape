...
public class OntologyImpl extends AbstractChangeable implements org.biojava.ontology.Ontology {
	 ...
	 */
	public OntologyImpl(final String name) {
	 ...
	 * @throws URISyntaxException
	 */
	public OntologyImpl(final String name, final String curator, final String description,
	                final CyNetwork dag) {
	...
	public Set<Term> getTerms() {
		...
		for ( CyNode node : ontologyGraph.getNodeList() ) {
			final String id = node.attrs().get("name",String.class);
			final Term term = new OntologyTerm(id, this.name, node.attrs().get(OBOTags.DEF.toString(), String.class));
			terms.add(term);
		}
	...
	public Triple createTriple(Term subject, Term object, Term predicate, String name,
		...
		Triple newTriple = new org.cytoscape.ontology.internal.TripleImpl(subject, object, predicate, name,
		                                                      description);
