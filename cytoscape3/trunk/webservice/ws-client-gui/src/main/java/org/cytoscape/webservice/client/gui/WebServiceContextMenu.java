 ...
 * 
 */
public class WebServiceContextMenu implements NodeContextMenuListener,
	...
	private void addMenu(Object view, JPopupMenu menu) {
		...
		final List<WebServiceClient<?>> clients = wscm.getAllClients();

		for (WebServiceClient<?> client : clients) {
			if (client instanceof WebServiceClientGUI) {
				if (view instanceof NodeView)
					context = ((WebServiceClientGUI) client)
							.getNodeContextMenuItems((NodeView) view);
				else if (view instanceof EdgeView)
					context = ((WebServiceClientGUI) client)
							.getEdgeContextMenuItems((EdgeView) view);
				...
				if (context != null) {
					...
					clientMap
							.put(client.getDisplayName(), client.getClientID());
				}
			}
		}
