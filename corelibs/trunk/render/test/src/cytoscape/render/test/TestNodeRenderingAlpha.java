...
public final class TestNodeRenderingAlpha
  extends Frame implements MouseListener, MouseMotionListener
{
  ...
  public final static void main(String[] args) throws Exception
  {
    ...
    // Populate the tree with entries.
    {
      ...
      for (int i = 0; i < colors.length; i++) {
        colors[i] = new Color((0x00ffffff & r.nextInt()) | 0x80000000, true); }
    }

    EventQueue.invokeAndWait(new Runnable() {
        public void run() {
          Frame f = new TestNodeRenderingAlpha
            (tree, shape, borderWidth, colors);
          f.show();
  ...
  private final Color m_borderColor = new Color(0, 0, 0, 128);
  ...
  public TestNodeRenderingAlpha(RTree tree, byte shape,
  ...
  private final void updateNodeImage()
  {
    ...
    final SpacialEntry2DEnumerator iter = m_tree.queryOverlap
