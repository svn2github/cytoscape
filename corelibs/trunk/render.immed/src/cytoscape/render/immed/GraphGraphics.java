...
public final class GraphGraphics {
	 ...
	 */
	public final void clear(final Paint bgPaint, final double xCenter, final double yCenter,
	...
	private final void setStroke(final float width, final float dashLength, final int capType,
	                             final boolean ignoreCache) {
		if ((!ignoreCache) && (width == m_currStrokeWidth) && (dashLength == m_currDash[0])
		    && (capType == m_currCapType))
			return;
	 ...
	 */
	public final void drawNodeLow(final float xMin, final float yMin, final float xMax,
	                              final float yMax, final Color fillColor) {
		if (m_debug) {
			if (!EventQueue.isDispatchThread())
				throw new IllegalStateException("calling thread is not AWT event dispatcher");
	 ...
	 */
	public final void drawNodeFull(final byte nodeShape, final float xMin, final float yMin,
	                               final float xMax, final float yMax, final Paint fillPaint,
	                               final float borderWidth, final Paint borderPaint) {
		if (m_debug) {
			if (!EventQueue.isDispatchThread())
				throw new IllegalStateException("calling thread is not AWT event dispatcher");
			...
			if (nodeShape == SHAPE_ROUNDED_RECTANGLE) {
				...
				if (!(Math.max(width, height) < (2.0d * Math.min(width, height))))
					throw new IllegalArgumentException("rounded rectangle does not meet constraint "
		...
		if (borderWidth == 0.0f) {
			...
			m_path2dPrime.reset();
			m_path2dPrime.append(getShape(nodeShape, xMin, yMin, xMax, yMax), false); // Make a copy, essentially.

			final Shape innerShape;

			if (nodeShape == SHAPE_ELLIPSE) {
				// TODO: Compute a more accurate inner area for ellipse + border.
				innerShape = getShape(SHAPE_ELLIPSE, ((double) xMin) + borderWidth,
				                      ((double) yMin) + borderWidth, ((double) xMax) - borderWidth,
				...
				computeRoundedRectangle(((double) xMin) + borderWidth,
				                        ...
				                        ((double) yMax) - borderWidth,
				                        (Math.max(((double) xMax) - xMin, ((double) yMax) - yMin) / 4.0d)
				...
				computeInnerPoint(m_ptsBuff, xPrev, yPrev, xCurr, yCurr, xNext, yNext, borderWidth);
				...
				while (true) {
					if (i == (m_polyNumPoints * 2)) {
						computeInnerPoint(m_ptsBuff, xCurr, yCurr, xNext, yNext, xNot, yNot,
						                  borderWidth);
						m_path2d.lineTo((float) m_ptsBuff[0], (float) m_ptsBuff[1]);
						computeInnerPoint(m_ptsBuff, xNext, yNext, xNot, yNot, xOne, yOne,
						                  borderWidth);
						m_path2d.lineTo((float) m_ptsBuff[0], (float) m_ptsBuff[1]);
						m_path2d.closePath();
						...
						computeInnerPoint(m_ptsBuff, xPrev, yPrev, xCurr, yCurr, xNext, yNext,
						                  borderWidth);
						m_path2d.lineTo((float) m_ptsBuff[0], (float) m_ptsBuff[1]);
					}
				}
	 ...
	 * interface would allow user selection of nodes, for example.  Use the
	 * same parameter values that were used to render corresponding node.
	 ...
	 *   the computed path is closed.
	 */
	public final void getNodeShape(final byte nodeShape, final float xMin, final float yMin,
	                               final float xMax, final float yMax, final GeneralPath path) {
		if (m_debug) {
			if (!EventQueue.isDispatchThread())
				throw new IllegalStateException("calling thread is not AWT event dispatcher");
	 ...
	 *   defined.
	 */
	public final byte defineCustomNodeShape(final float[] coords, final int offset,
		...
		{ // Test all criteria regardless of m_debug.
			...
			for (int i = 0; i < vertexCount; i++) {
				...
				final double distP2fromP0P1 = ((((y0 - y1) * x2) + ((x1 - x0) * y2) + (x0 * y1))
				                              - (x1 * y0)) / distP0P1;
				...
				if (!((float) distCenterFromP0P1 > 0.0f)) {
					throw new IllegalArgumentException("polygon is going counter-clockwise or is not star-shaped with "
					                                   + "respect to center");
				}
	 ...
	 */
	private final Shape getShape(final byte nodeShape, final double xMin, final double yMin,
		...
		switch (nodeShape) {
			...
			case SHAPE_ROUNDED_RECTANGLE:
				...
				computeRoundedRectangle(xMin, yMin, xMax, yMax,
				                        Math.max(xMax - xMin, yMax - yMin) / 4.0d, m_path2d);
			...
			default: // Try a custom node shape or throw an exception.
				...
				m_xformUtil.transform(storedPolyCoords, 0, m_polyCoords, 0, m_polyNumPoints);
	...
	private final static void computeRoundedRectangle(final double xMin, final double yMin,
	                                                  final double xMax, final double yMax,
	                                                  final double radius, final GeneralPath path2d) {
		path2d.reset();
		path2d.moveTo((float) (xMax - radius), (float) yMin);
		path2d.curveTo((float) (((CURVE_ELLIPTICAL - 1.0d) * radius) + xMax), (float) yMin,
		               (float) xMax, (float) (((1.0d - CURVE_ELLIPTICAL) * radius) + yMin),
		               (float) xMax, (float) (radius + yMin));
		path2d.lineTo((float) xMax, (float) (yMax - radius));
		path2d.curveTo((float) xMax, (float) (((CURVE_ELLIPTICAL - 1.0d) * radius) + yMax),
		               (float) (((CURVE_ELLIPTICAL - 1.0d) * radius) + xMax), (float) yMax,
		               (float) (xMax - radius), (float) yMax);
		path2d.lineTo((float) (radius + xMin), (float) yMax);
		path2d.curveTo((float) (((1.0d - CURVE_ELLIPTICAL) * radius) + xMin), (float) yMax,
		               (float) xMin, (float) (((CURVE_ELLIPTICAL - 1.0d) * radius) + yMax),
		               (float) xMin, (float) (yMax - radius));
		path2d.lineTo((float) xMin, (float) (radius + yMin));
		path2d.curveTo((float) xMin, (float) (((1.0d - CURVE_ELLIPTICAL) * radius) + yMin),
		               (float) (((1.0d - CURVE_ELLIPTICAL) * radius) + xMin), (float) yMin,
	 ...
	 */
	private final static void computeInnerPoint(final double[] output, final double xPrev,
	                                            final double yPrev, final double xCurr,
	                                            final double yCurr, final double xNext,
	                                            final double yNext, final double borderWidth) {
		...
		final double distancePrimeToSeg1 = (((segX1 * yNextPrime) - (segY1 * xNextPrime)
	 ...
	 */
	public final void drawEdgeFull(final byte arrow0Type, final float arrow0Size,
	                               final Paint arrow0Paint, final byte arrow1Type,
	                               final float arrow1Size, final Paint arrow1Paint, final float x0,
	                               final float y0, EdgeAnchors anchors, final float x1,
	                               final float y1, final float edgeThickness,
		...
		if (m_debug) {
			edgeFullDebug(arrow0Type, arrow0Size, arrow1Type, arrow1Size, edgeThickness,
			              dashLength, anchors);
		}

		if (!computeCubicPolyEdgePath(arrow0Type, (arrow0Type == ARROW_NONE) ? 0.0f : arrow0Size,
			...
			if (m_edgePtsCount == 2) { // Draw an ordinary edge.
				drawSimpleEdgeFull(arrow0Type, arrow0Size, arrow0Paint, arrow1Type, arrow1Size,
				                   arrow1Paint, (float) m_edgePtsBuff[0], (float) m_edgePtsBuff[1],
		...
		{ // Render the edge polypath.
			...
			setStroke(edgeThickness, dashLength,
			          simpleSegment ? BasicStroke.CAP_ROUND : BasicStroke.CAP_BUTT, false);
			...
			while (inx < count) {
				m_path2d.curveTo((float) m_edgePtsBuff[inx++], (float) m_edgePtsBuff[inx++],
	...
	private final void edgeFullDebug(final byte arrow0Type, final float arrow0Size,
	                                 final byte arrow1Type, float arrow1Size,
		...
		switch (arrow0Type) {
			...
			case ARROW_BIDIRECTIONAL:
				...
				if (arrow1Type != arrow0Type)
					throw new IllegalArgumentException("for ARROW_BIDIRECTIONAL and ARROW_MONO, both arrows must be "
					                                   + "identical");
		...
		if (anchors.numAnchors() > MAX_EDGE_ANCHORS)
			throw new IllegalArgumentException("at most MAX_EDGE_ANCHORS (" + MAX_EDGE_ANCHORS
			                                   + ") edge anchors can be specified");
	}

	private final void drawSimpleEdgeFull(final byte arrow0Type, final float arrow0Size,
	                                      final Paint arrow0Paint, final byte arrow1Type,
	                                      final float arrow1Size, final Paint arrow1Paint,
	                                      final float x0, final float y0, final float x1,
	                                      final float y1, final float edgeThickness,
	                                      final Paint edgePaint, final float dashLength) {
		...
		if (arrow0Type == ARROW_BIDIRECTIONAL) { // Draw and return.
			...
			m_path2d.moveTo((float) (a + (4.0d * f)), (float) (f + (1.5d * edgeThickness)));
			...
			m_path2d.moveTo((float) (len - (a + (4.0d * g))), (float) (-g + (-1.5d * edgeThickness)));
			m_path2d.lineTo((float) (len - a), (float) (-1.5d * edgeThickness));
			...
			setStroke(edgeThickness, dashLength,
			          (dashLength == 0.0f) ? BasicStroke.CAP_ROUND : BasicStroke.CAP_BUTT, false);
			m_g2d.setPaint(edgePaint);
			m_g2d.draw(m_path2d);
		...
		if (arrow0Type == ARROW_MONO) { // Draw and return.
			...
			if (dashLength == 0.0f) { // Render delta wedge cap.
				m_xformUtil.setTransform(-cosTheta, -sinTheta, sinTheta, -cosTheta, midX, midY);
				m_g2d.transform(m_xformUtil);
				m_g2d.scale(edgeThickness, edgeThickness);
				// The paint is already set to edge paint.
		...
		{ // Render the line segment if necessary.
			...
			if ((((((double) x1) - x0) * (x1Adj - x0Adj))
				...
				setStroke(edgeThickness, dashLength,
				          (simpleSegment > 0) ? BasicStroke.CAP_ROUND : BasicStroke.CAP_BUTT, false);
	 ...
	 */
	public final boolean getEdgePath(final byte arrow0Type, final float arrow0Size,
	                                 final byte arrow1Type, final float arrow1Size, final float x0,
	                                 final float y0, EdgeAnchors anchors, final float x1,
	                                 final float y1, final GeneralPath path) {
		final double curveFactor = CURVE_ELLIPTICAL;
		...
		if (!computeCubicPolyEdgePath(arrow0, (arrow0 == ARROW_NONE) ? 0.0f : arrow0Size, arrow1,
		                              (arrow1 == ARROW_NONE) ? 0.0f : arrow1Size, x0, y0, anchors,
		                              x1, y1, curveFactor)) {
		...
		while (inx < count) {
			path.curveTo((float) m_edgePtsBuff[inx++], (float) m_edgePtsBuff[inx++],
			             (float) m_edgePtsBuff[inx++], (float) m_edgePtsBuff[inx++],
			             (float) m_edgePtsBuff[inx++], (float) m_edgePtsBuff[inx++]);
		}
	...
	private final Shape computeUntransformedArrowCap(final byte arrowType, final double ratio) {
		switch (arrowType) {
			...
			case ARROW_DISC:
				...
				m_arc2d.setArc(0.0d, ratio / -2.0d, ratio, ratio, 180.0d - theta, theta * 2,
				               Arc2D.OPEN);
				m_path2d.reset();
	 ...
	 * ARROW_BIDIRECTIONAL or ARROW_MONO allowed).
	 */
	private final static double getT(final byte arrowType) { // I could implement this as an array instead of a switch statement.
	 ...
	 */
	private final boolean computeCubicPolyEdgePath(final byte arrow0Type, final float arrow0Size,
	                                               final byte arrow1Type, final float arrow1Size,
	                                               final float x0, final float y0,
	                                               final EdgeAnchors anchors, final float x1,
	                                               final float y1, final double curveFactor) {
		...
		{ // First set the three points related to point 1.
			...
			double candX1 = m_edgePtsBuff[(m_edgePtsCount * 6) - 4] + (dx * 2.0d * arrow1Size);
			...
			if (Math.abs(candX1 - m_edgePtsBuff[m_edgePtsCount * 2]) > Math.abs(candX2
			...
			double candY1 = m_edgePtsBuff[(m_edgePtsCount * 6) - 3] + (dy * 2.0d * arrow1Size);
			...
			if (Math.abs(candY1 - m_edgePtsBuff[(m_edgePtsCount * 2) + 1]) > Math.abs(candY2
			                                                                          - m_edgePtsBuff[(m_edgePtsCount * 2)
			                                                                          + 1])) {
		...
		while (m_edgePtsCount > 2) {
			...
			m_edgePtsBuff[(m_edgePtsCount * 6) - 6] = midX
			                                          + ((m_edgePtsBuff[(m_edgePtsCount * 2) - 2]
			                                             - midX) * curveFactor);
	 ...
	 */
	public final boolean computeEdgeIntersection(final byte nodeShape, final float xMin,
	                                             final float yMin, final float xMax,
	                                             final float yMax, final float offset,
	                                             final float ptX, final float ptY,
		...
		if (nodeShape == SHAPE_ELLIPSE) {
			...
			final double tangentXformedPtPrimeX = xsectXformedPtPrimeX + xsectXformedPtPrimeY;
			final double tangentXformedPtPrimeY = xsectXformedPtPrimeY - xsectXformedPtPrimeX;
			...
			final double tanLen = Math.sqrt((vTangentX * vTangentX) + (vTangentY * vTangentY));
			final double distPtPrimeToTangent = (((vTangentX * ptPrimeY) - (vTangentY * ptPrimeX)
			                                     + (xsectPtPrimeX * tangentPtPrimeY))
			                                    - (tangentPtPrimeX * xsectPtPrimeY)) / tanLen;
			...
			returnVal[0] = (float) (centerX
			               + (xsectPtPrimeX + (multFactor * (ptPrimeX - xsectPtPrimeX))));
			returnVal[1] = (float) (centerY
			...
			if (nodeShape == SHAPE_ROUNDED_RECTANGLE) {
				final double radius = Math.max(((double) xMax) - xMin, ((double) yMax) - yMin) / 4.0d;
				...
				getShape(SHAPE_RECTANGLE, radius + xMin, radius + yMin, -radius + xMax,
				         -radius + yMax);
			...
			if (trueOffset == 0.0d) {
				...
				for (int i = 0; i < twicePolyNumPoints;) {
					...
					if (segmentIntersection(m_ptsBuff, ptX, ptY, centerX, centerY, x0, y0, x1, y1)) {
			...
			for (int i = 0; i < m_polyNumPoints; i++) {
				...
				(m_ptsBuff, // product to perform this test quicker.
				 ...
				 m_fooPolyCoords[((i * 4) + 7) % (m_polyNumPoints * 4)])) {
					...
					m_foo2PolyCoords[inx++] = m_fooPolyCoords[((i * 4) + 4) % (m_polyNumPoints * 4)];
					m_foo2PolyCoords[inx++] = m_fooPolyCoords[((i * 4) + 5) % (m_polyNumPoints * 4)];
			...
			for (int i = 0; i < m_polyNumPoints; i++) {
				if (m_fooRoundedCorners[i]) {
					if (segmentIntersection(m_ptsBuff, ptX, ptY, centerX, centerY,
						...
						final int numXsections = bad_circleIntersection(m_ptsBuff, ptX, ptY,
						                                                centerX, centerY,
					...
					(m_ptsBuff, // that comes after the arc.
					 ptX, ptY, centerX, centerY, m_foo2PolyCoords[inx++], m_foo2PolyCoords[inx++],
					 m_foo2PolyCoords[inx % foo2Count], m_foo2PolyCoords[(inx + 1) % foo2Count])) {
						returnVal[0] = (float) m_ptsBuff[0];
						returnVal[1] = (float) m_ptsBuff[1];
	 ...
	 */
	private final static boolean segmentIntersection(final double[] returnVal, double x1,
	 ...
	 */
	private final static int bad_circleIntersection(final double[] returnVal, final double x1,
	                                                final double y1, final double x2,
	                                                final double y2, final double cX,
	                                                final double cY, final double radius) {
		final double vX = x2 - x1;
		final double vY = y2 - y1;

		if ((vX == 0.0d) && (vY == 0.0d))
			throw new IllegalStateException("the condition of both line segment endpoint being the same "
	 ...
	 */
	public final void drawTextLow(final Font font, final String text, final float xCenter,
	                              final float yCenter, final Color color) {
		if (m_debug) {
			if (!EventQueue.isDispatchThread())
				throw new IllegalStateException("calling thread is not AWT event dispatcher");
		...
		m_gMinimal.drawString(text, (int) ((-0.5d * fMetrics.stringWidth(text)) + m_ptsBuff[0]),
	 ...
	 *   systems tested, the shape filling method results in a substantial
	 ...
	 */
	public final void drawTextFull(final Font font, final double scaleFactor, final String text,
	                               final float xCenter, final float yCenter, final float theta,
	                               final Paint paint, final boolean drawTextAsShape) {
		if (m_debug) {
			if (!EventQueue.isDispatchThread())
				throw new IllegalStateException("calling thread is not AWT event dispatcher");
		...
		if (drawTextAsShape) {
			...
			glyphV = font.layoutGlyphVector(getFontRenderContextFull(), m_charBuff, 0,
			                                text.length(), Font.LAYOUT_NO_LIMIT_CONTEXT);
	 ...
	 */
	public final void drawCustomGraphicFull(final Shape shape, final float xOffset,
	                                        final float yOffset, final Paint paint) {
		if (m_debug) {
			if (!EventQueue.isDispatchThread())
				throw new IllegalStateException("calling thread is not AWT event dispatcher");
