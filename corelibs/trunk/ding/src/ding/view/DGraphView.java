...
public class DGraphView implements GraphView, Printable {
    ...
    public void setBackgroundPaint(Paint paint) {
        synchronized (m_lock) {
            if (paint instanceof Color) {
                ...
                System.out.println(
                    "DGraphView.setBackgroundPaint(), Color not found!");
            }
        }
    }
    ...
    private NodeView addNodeViewInternal(int nodeInx) {
        final NodeView oldView = (NodeView) m_nodeViewMap.get(new Integer(
                    nodeInx));
    ...
    public EdgeView addEdgeView(int edgeInx) {
        ...
        synchronized (m_lock) {
            final EdgeView oldView = (EdgeView) m_edgeViewMap.get(new Integer(
                        edgeInx));
    ...
    private DEdgeView removeEdgeViewInternal(int edgeInx) {
        final DEdgeView returnThis = (DEdgeView) m_edgeViewMap.remove(new Integer(
                    edgeInx));
        ...
        if (obj instanceof DEdgeView) {
            ...
            synchronized (m_lock) {
                ...
                for (int i = 0; i < edges.length; i++)
                    hideGraphObjectInternal(m_edgeViewMap.get(
                            new Integer(edges[i])), false);
            ...
            if (treatNodeShapesAsRectangle) {
                ...
                for (int i = 0; i < totalHits; i++) {
                    ...
                    if (((m_extentsBuff[0] < xMin) &&
                            ...
                            ((m_extentsBuff[2] > xMax) &&
                            (m_extentsBuff[1] < yMin))) {
                        m_networkCanvas.m_grafx.getNodeShape(m_nodeDetails.shape(
                                node), m_extentsBuff[0], m_extentsBuff[1],
    ...
    public void drawSnapshot(Image img, GraphLOD lod, Paint bgPaint,
        ...
        synchronized (m_lock) {
            GraphRenderer.renderGraph((FixedGraph) m_drawPersp, m_spacial, lod,
                m_nodeDetails, m_edgeDetails, m_hash,
                new GraphGraphics(img, false), bgPaint, xCenter, yCenter,
                scaleFactor);
        }
    }
    ...
    public int print(Graphics g, PageFormat pageFormat, int page) {
        if (page == 0) {
            ...
            // area of the paper
            double image_scale = Math.min(pageFormat.getImageableWidth() / getComponent()
                                                                               .getWidth(),
    ...
    public enum Canvas {BACKGROUND_CANVAS,
        NETWORK_CANVAS,
        FOREGROUND_CANVAS;
    }
    public enum ShapeType {NODE_SHAPE,
        LINE_TYPE,
        ARROW_SHAPE;
    }
}
