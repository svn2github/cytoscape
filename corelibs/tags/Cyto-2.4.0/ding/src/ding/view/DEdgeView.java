...
class DEdgeView
    implements EdgeView, Label, Bend, EdgeAnchors {
     ...
     */
    public Edge getEdge() {
        return m_view.getGraphPerspective()
                     .getEdge(~m_inx);
    }
        ...
        synchronized (m_view.m_lock) {
            ...
            if (segmentDashLength > 0.0f) {
                final float[] dashes = new float[] {
                        segmentDashLength, segmentDashLength
                    };
    ...
    public Point2D getTargetHandlePoint() {
        synchronized (m_view.m_lock) {
            ...
            final Point2D returnThis = new Point2D.Float();
            returnThis.setLocation((Point2D) m_anchors.get(m_anchors.size() -
                    1));
    ...
    public int addHandleFoo(Point2D pt) {
        synchronized (m_view.m_lock) {
            ...
            final Point2D sourcePt = m_view.getNodeView(~((FixedGraph) m_view.m_structPersp).edgeSource(
                        m_inx))
                                           .getOffset();
            final Point2D targetPt = m_view.getNodeView(~((FixedGraph) m_view.m_structPersp).edgeTarget(
                        m_inx))
                                           .getOffset();
            ...
            for (int i = 1; i < m_anchors.size(); i++) {
                final double distCand = (pt.distance((Point2D) m_anchors.get(i -
                            1)) + pt.distance((Point2D) m_anchors.get(i))) -
                    ((Point2D) m_anchors.get(i)).distance((Point2D) m_anchors.get(i -
                            1));
