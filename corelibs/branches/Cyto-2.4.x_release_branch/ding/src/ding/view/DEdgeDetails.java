...
class DEdgeDetails extends IntermediateEdgeDetails {
    ...
    public EdgeAnchors anchors(int edge) {
        ...
        if (graph.edgeSource(edge) == graph.edgeTarget(edge)) { // Self-edge.
            ...
            int i = 0;
            final IntIterator selfEdges = graph.edgesConnecting(node, node,
                    true, true, true);
        ...
        while (true) {
            ...
            int i = (((EdgeAnchors) m_view.getEdgeView(~otherEdge)).numAnchors() == 0)
                ? 1 : 0;
            ...
            final double trgY = (((double) m_extentsBuff[1]) +
                m_extentsBuff[3]) / 2.0d;
            final double nodeSize = Math.max(
                    Math.max(
                        Math.max(srcW, srcH),
                        trgW),
                    trgH);
