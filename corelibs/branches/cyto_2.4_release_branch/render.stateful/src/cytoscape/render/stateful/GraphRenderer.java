...
public final class GraphRenderer {
    ...
    public final static int renderGraph(final FixedGraph graph,
        final SpacialIndex2D nodePositions, final GraphLOD lod,
        final NodeDetails nodeDetails, final EdgeDetails edgeDetails,
        final IntHash nodeBuff, final GraphGraphics grafx, final Paint bgPaint,
        final double xCenter, final double yCenter, final double scaleFactor) {
        ...
        final float yMax;
        xMin = (float) (xCenter -
            ((0.5d * grafx.image.getWidth(null)) / scaleFactor));
        yMin = (float) (yCenter -
            ((0.5d * grafx.image.getHeight(null)) / scaleFactor));
        xMax = (float) (xCenter +
            ((0.5d * grafx.image.getWidth(null)) / scaleFactor));
        yMax = (float) (yCenter +
            ((0.5d * grafx.image.getHeight(null)) / scaleFactor));
        ...
        {
            final SpacialEntry2DEnumerator nodeHits = nodePositions.queryOverlap(xMin,
                    yMin, xMax, yMax, null, 0, false);
            final int visibleNodeCount = nodeHits.numRemaining();
            final int totalNodeCount = graph.nodes()
                                            .numRemaining();
            final int totalEdgeCount = graph.edges()
                                            .numRemaining();
            renderEdges = lod.renderEdges(visibleNodeCount, totalNodeCount,
                    totalEdgeCount);
        ...
        if (renderEdges >= 0) {
            ...
            if (renderEdges > 0)
                ...
                nodeHits = nodePositions.queryOverlap(xMin, yMin, xMax, yMax,
                        null, 0, false);
                ...
                while (nodeHits.numRemaining() > 0) {
                    ...
                    while (touchingEdges.numRemaining() > 0) {
                        ...
                        if (nodeBuff.get(otherNode) < 0) { // Has not yet been rendered.
                            ...
                            // Compute dash length.
                            final float dashLength = (((lodBits &
                                LOD_DASHED_EDGES) == 0) ? 0.0f
                                                        : edgeDetails.segmentDashLength(edge));

                            // Compute the anchors to use when rendering edge.
                            final EdgeAnchors anchors = (((lodBits &
                                LOD_EDGE_ANCHORS) == 0) ? null
                                                        : edgeDetails.anchors(edge));

                            if (!computeEdgeEndpoints(grafx, srcExtents,
                                        srcShape, srcArrow, srcArrowSize,
                                        anchors, trgExtents, trgShape,
                                        trgArrow, trgArrowSize, floatBuff3,
                                        floatBuff4))
                                continue;
                            ...
                            grafx.drawEdgeFull(srcArrow, srcArrowSize,
                                srcArrowPaint, trgArrow, trgArrowSize,
                                trgArrowPaint, srcXAdj, srcYAdj, anchors,
                                trgXAdj, trgYAdj, thickness, segPaint,
                                dashLength);

                            // Take care of edge anchor rendering.
                            if (anchors != null) {
                                for (int k = 0; k < anchors.numAnchors();
                                        k++) {
                                    final float anchorSize;

                                    if ((anchorSize = edgeDetails.anchorSize(
                                                    edge, k)) > 0.0f) {
                            ...
                            if ((lodBits & LOD_EDGE_LABELS) != 0) {
                                ...
                                for (int labelInx = 0; labelInx < labelCount;
                                        labelInx++) {
                                    final String text = edgeDetails.labelText(edge,
                                            labelInx);
                                    final Font font = edgeDetails.labelFont(edge,
                                            labelInx);
                                    final double fontScaleFactor = edgeDetails.labelScaleFactor(edge,
                                            labelInx);
                                    final Paint paint = edgeDetails.labelPaint(edge,
                                            labelInx);
                                    ...
                                    if (text.indexOf('\n') >= 0)
                                        justify = edgeDetails.labelJustify(edge,
                                                labelInx);
                                    else
                                    ...
                                    if (edgeAnchor == EdgeDetails.EDGE_ANCHOR_SOURCE) {
                                        ...
                                        grafx.getEdgePath(srcArrow,
                                            srcArrowSize, trgArrow,
                                            trgArrowSize, srcXAdj, srcYAdj,
                                            anchors, trgXAdj, trgYAdj, path2d);
                                    ...
                                    TextRenderingUtils.computeTextDimensions(grafx,
                                        text, font, fontScaleFactor,
                                    ...
                                    lemma_computeAnchor(textAnchor,
                                        doubleBuff1, doubleBuff2);

                                    final double textXCenter = edgeAnchorPointX -
                                        doubleBuff2[0] + offsetVectorX;
                                    final double textYCenter = edgeAnchorPointY -
                                        doubleBuff2[1] + offsetVectorY;
                                    TextRenderingUtils.renderHorizontalText(grafx,
                                        text, font, fontScaleFactor,
                                        (float) textXCenter,
                                        (float) textYCenter, justify, paint,
                                        (lodBits & LOD_TEXT_AS_SHAPE) != 0);
                                }
                            }
                        }
            ...
            if ((lodBits & LOD_HIGH_DETAIL) == 0) { // Low detail.
                ...
                while (nodeHits.numRemaining() > 0) {
                    ...
                    if ((lodBits & LOD_CUSTOM_GRAPHICS) != 0) {
                        ...
                        for (int graphicInx = 0; graphicInx < graphicCount;
                                graphicInx++) {
                            final Shape gShape = nodeDetails.graphicShape(node,
                                    graphicInx);
                            final Paint paint = nodeDetails.graphicPaint(node,
                                    graphicInx);
                            final byte anchor = nodeDetails.graphicNodeAnchor(node,
                                    graphicInx);
                    ...
                    if ((lodBits & LOD_NODE_LABELS) != 0) { // Potential label rendering.
                        ...
                        for (int labelInx = 0; labelInx < labelCount;
                                labelInx++) {
                            final String text = nodeDetails.labelText(node,
                                    labelInx);
                            final Font font = nodeDetails.labelFont(node,
                                    labelInx);
                            final double fontScaleFactor = nodeDetails.labelScaleFactor(node,
                                    labelInx);
                            final Paint paint = nodeDetails.labelPaint(node,
                                    labelInx);
                            final byte textAnchor = nodeDetails.labelTextAnchor(node,
                                    labelInx);
                            final byte nodeAnchor = nodeDetails.labelNodeAnchor(node,
                                    labelInx);
                            final float offsetVectorX = nodeDetails.labelOffsetVectorX(node,
                                    labelInx);
                            final float offsetVectorY = nodeDetails.labelOffsetVectorY(node,
                            ...
                            if (text.indexOf('\n') >= 0)
                                justify = nodeDetails.labelJustify(node,
                                        labelInx);
                            else
                                justify = NodeDetails.LABEL_WRAP_JUSTIFY_CENTER;
                            ...
                            doubleBuff1[2] = floatBuff1[2];
                            doubleBuff1[3] = floatBuff1[3];
                            lemma_computeAnchor(nodeAnchor, doubleBuff1,
                                doubleBuff2);
                            ...
                            doubleBuff1[2] = 0.5d * floatBuff3[0];
                            doubleBuff1[3] = 0.5d * floatBuff3[1];
                            lemma_computeAnchor(textAnchor, doubleBuff1,
                                doubleBuff2);

                            final double textXCenter = nodeAnchorPointX -
                                doubleBuff2[0] + offsetVectorX;
                            final double textYCenter = nodeAnchorPointY -
                                doubleBuff2[1] + offsetVectorY;
                            TextRenderingUtils.renderHorizontalText(grafx,
                                text, font, fontScaleFactor,
                                (float) textXCenter, (float) textYCenter,
                                justify, paint,
                                (lodBits & LOD_TEXT_AS_SHAPE) != 0);
                        }
                    }
    ...
    private final static void lemma_computeAnchor(final byte anchor,
        final double[] input4x, final double[] rtrn2x) {
        switch (anchor) {
    ...
    public final static boolean computeEdgeEndpoints(
        final GraphGraphics grafx, final float[] srcNodeExtents,
        final byte srcNodeShape, final byte srcArrow, final float srcArrowSize,
        EdgeAnchors anchors, final float[] trgNodeExtents,
        final byte trgNodeShape, final byte trgArrow, final float trgArrowSize,
        final float[] rtnValSrc, final float[] rtnValTrg) {
        final boolean alwaysCompute = true;

        if ((anchors != null) && (anchors.numAnchors() == 0))
            anchors = null;

        final float srcX = (float) ((((double) srcNodeExtents[0]) +
            srcNodeExtents[2]) / 2.0d);
        final float srcY = (float) ((((double) srcNodeExtents[1]) +
            srcNodeExtents[3]) / 2.0d);
        final float trgX = (float) ((((double) trgNodeExtents[0]) +
            trgNodeExtents[2]) / 2.0d);
        final float trgY = (float) ((((double) trgNodeExtents[1]) +
        ...
        synchronized (s_floatBuff) {
            if ((srcNodeExtents[0] == srcNodeExtents[2]) ||
                ...
                if (!_computeEdgeIntersection(srcX, srcY, srcOffset, srcXOut,
                            srcYOut, alwaysCompute, s_floatBuff))
                    return false;
            } else {
                if (!grafx.computeEdgeIntersection(srcNodeShape,
                            srcNodeExtents[0], srcNodeExtents[1],
                    ...
                    { // Compute newSrcXOut and newSrcYOut.
                        ...
                        if (len == 0.0d) {
                            ...
                            newSrcXOut = (float) (((dX / len) * desiredDist) +
                                srcXOut);
                            newSrcYOut = (float) (((dY / len) * desiredDist) +
                                srcYOut);
                        }
                    }

                    grafx.computeEdgeIntersection(srcNodeShape,
                        srcNodeExtents[0], srcNodeExtents[1],
                        srcNodeExtents[2], srcNodeExtents[3], srcOffset,
        ...
        synchronized (s_floatBuff) {
            if ((trgNodeExtents[0] == trgNodeExtents[2]) ||
                ...
                if (!_computeEdgeIntersection(trgX, trgY, trgOffset, trgXOut,
                            trgYOut, alwaysCompute, s_floatBuff))
                    return false;
            } else {
                if (!grafx.computeEdgeIntersection(trgNodeShape,
                            trgNodeExtents[0], trgNodeExtents[1],
                    ...
                    { // Compute newTrgXOut and newTrgYOut.
                        ...
                        if (len == 0.0d) {
                            ...
                            newTrgXOut = (float) (((dX / len) * desiredDist) +
                                trgXOut);
                            newTrgYOut = (float) (((dY / len) * desiredDist) +
                                trgYOut);
                        }
                    }

                    grafx.computeEdgeIntersection(trgNodeShape,
                        trgNodeExtents[0], trgNodeExtents[1],
                        trgNodeExtents[2], trgNodeExtents[3], trgOffset,
        ...
        synchronized (s_floatTemp) {
            ...
            for (int i = 0; i < segs; i++) {
                ...
                case PathIterator.SEG_QUADTO:
                    rtnVal.quadTo(s_floatBuff2[offset++],
                ...
                default: // PathIterator.SEG_CUBICTO.
                    rtnVal.curveTo(s_floatBuff2[offset++],
                        s_floatBuff2[offset++], s_floatBuff2[offset++],
                        s_floatBuff2[offset++], s_floatBuff2[offset++],
                        s_floatBuff2[offset++]);
            ...
            for (int i = segs - 1; i > 0; i--) {
                ...
                case PathIterator.SEG_LINETO:
                    offset -= 2;
                    rtnVal.lineTo(s_floatBuff2[offset - 2],
                ...
                case PathIterator.SEG_QUADTO:
                    offset -= 4;
                    rtnVal.quadTo(s_floatBuff2[offset],
                        s_floatBuff2[offset + 1], s_floatBuff2[offset - 2],
                        s_floatBuff2[offset - 1]);
                ...
                default: // PathIterator.SEG_CUBICTO.
                    offset -= 6;
                    rtnVal.curveTo(s_floatBuff2[offset + 2],
                        s_floatBuff2[offset + 3], s_floatBuff2[offset],
                        s_floatBuff2[offset + 1], s_floatBuff2[offset - 2],
                        s_floatBuff2[offset - 1]);
    ...
    static final boolean _computeEdgeIntersection(final float nodeX,
        final float nodeY, final float offset, final float ptX,
        final float ptY, final boolean alwaysCompute, final float[] returnVal) {
        if (offset == 0.0f) {
            returnVal[0] = nodeX;
            returnVal[1] = nodeY;
