...
public final class GraphRenderer {
	...
	public final static int renderGraph(final FixedGraph graph, final SpacialIndex2D nodePositions,
		...
		// node; it is only on top of the node it belongs to.
		{
			...
			// are we renderering in low or high detail
			boolean renderLowDetail = ((lodBits & LOD_HIGH_DETAIL) == 0);

			// the following three vars added to support
			// rendering selected nodes on top of unselected nodes
			int selectedNodesIndex = -1;
			NodeExtentsInfo nodeExtentsInfo;
			java.util.List<NodeExtentsInfo> selectedNodes = new java.util.ArrayList<NodeExtentsInfo>();

			// interate over node hits
			int totalNodeHits = nodeHits.numRemaining();
			while (totalNodeHits > 0 || selectedNodesIndex >= 0) {

				// ref to node
				final int node;
				// grab next node & extents from node hits
				if (totalNodeHits-- > 0) {
					node = nodeHits.nextExtents(floatBuff1, 0);
					// is this a selected node, if so save info & skip until
					// all unselected nodes are rendered
					if (nodeDetails.getSelected(node)) {
						selectedNodes.add(++selectedNodesIndex, new NodeExtentsInfo(node,
																					floatBuff1[0], floatBuff1[1],
																					floatBuff1[2], floatBuff1[3]));
						continue;
					}
				}
				// we are processing selected nodes, get extents from selected nodes list
				else {
					nodeExtentsInfo = selectedNodes.get(selectedNodesIndex--);
					node = nodeExtentsInfo.node;
					floatBuff1[0] = nodeExtentsInfo.nodeExtents[0];
					floatBuff1[1] = nodeExtentsInfo.nodeExtents[1];
					floatBuff1[2] = nodeExtentsInfo.nodeExtents[2];
					floatBuff1[3] = nodeExtentsInfo.nodeExtents[3];
				}

				// render low detail node
				if (renderLowDetail) {
					if ((floatBuff1[0] != floatBuff1[2]) && (floatBuff1[1] != floatBuff1[3]))
	...
	/**
	 * A private class which encapsulates a node and its extents.
	 * Used to store selected node extents and render them after
	 * (on top of) unselected nodes.
	 * 
	 * Members public for easy access to make class as lightweight
	 * as possible.
	 */
	private final static class NodeExtentsInfo {
		public int node;
		public float[] nodeExtents = new float[4];
		public NodeExtentsInfo(int node,
							   float nodeExtentsA, float nodeExtentsB,
							   float nodeExtentsC, float nodeExtentsD) {
			this.node = node;
			nodeExtents[0] = nodeExtentsA;
			nodeExtents[1] = nodeExtentsB;
			nodeExtents[2] = nodeExtentsC;
			nodeExtents[3] = nodeExtentsD;
		}
	}
}
