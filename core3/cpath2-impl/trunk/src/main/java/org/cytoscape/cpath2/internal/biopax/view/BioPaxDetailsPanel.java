 ...
 **/
package org.cytoscape.cpath2.internal.biopax.view;
...
public class BioPaxDetailsPanel extends JPanel {
	...
	public void showDetails(CyNetwork network, CyNode node) {
        ...
        // name, shortname
        stringRef = row.get(MapBioPaxToCytoscape.BIOPAX_NAME, String.class);

        String shortName = row.get(MapBioPaxToCytoscape.BIOPAX_SHORT_NAME, String.class);
        ...
        // organism
        stringRef = row.get(MapBioPaxToCytoscape.BIOPAX_ORGANISM_NAME, String.class);
        if (stringRef != null) {
        ...
        // synonyms
        addAttributeList(node, MapBioPaxToCytoscape.BIOPAX_SYNONYMS, "Synonyms:", buf);
        
		// cellular location
		addAttributeList(node, MapBioPaxToCytoscape.BIOPAX_CELLULAR_LOCATIONS,
		                 "Cellular Location(s):", buf);

		// chemical modification
		addAttributeList(node, MapBioPaxToCytoscape.BIOPAX_CHEMICAL_MODIFICATIONS_LIST,
    ...
    private void addCPathLink(CyNetwork cyNetwork, CyNode node, StringBuffer buf) {
        ...
        if (serverName != null && serverDetailsUrl != null) {
            ...
            String type = nodeRow.get(MapBioPaxToCytoscape.BIOPAX_ENTITY_TYPE, String.class);
    ...
    private void addType(CyNode node, StringBuffer buf) {
    	CyRow row = node.getCyRow();
        String type = row.get(MapBioPaxToCytoscape.BIOPAX_ENTITY_TYPE, String.class);
        buf.append("<h3>" + type + "</h3>");
    }

    private void addLinks(CyNode node, StringBuffer buf) {
    	CyRow row = node.getCyRow();
		String xrefs1 = row.get(MapBioPaxToCytoscape.BIOPAX_UNIFICATION_REFERENCES, String.class);
        String xrefs2 = row.get(MapBioPaxToCytoscape.BIOPAX_RELATIONSHIP_REFERENCES, String.class);
	...
	private void addIHOPLinks(CyNode node, StringBuffer buf) {
		CyRow row = node.getCyRow();
		String ihopLinks = row.get(MapBioPaxToCytoscape.BIOPAX_IHOP_LINKS, String.class);
