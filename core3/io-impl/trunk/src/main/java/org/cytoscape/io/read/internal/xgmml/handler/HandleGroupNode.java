...
public class HandleGroupNode extends AbstractHandler {
	public ParseState handle(String tag, Attributes atts, ParseState current)
		...
		if (href == null) {
			// Create the node
			manager.currentNode = attributeValueUtil
					.createUniqueNode(label, id);
		} else {
			id = href.substring(1);
		}
		...
		if (manager.idMap.containsKey(id)) {
			...
			List<CyNode> nodeList = manager.groupMap
					.get(manager.currentGroupNode);
			nodeList.add(node);
		} else {
			// Remember it for later -- we'll fix this up in handleGraphDone
			if (!manager.nodeLinks.containsKey(manager.currentGroupNode)) {
				manager.nodeLinks.put(manager.currentGroupNode,
						new ArrayList<String>());
			}

			List<String> links = manager.nodeLinks
					.get(manager.currentGroupNode);
			links.add(id);
		}
		// logger.debug(">");
