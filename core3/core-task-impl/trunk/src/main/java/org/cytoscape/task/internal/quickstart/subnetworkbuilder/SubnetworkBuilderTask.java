...
public class SubnetworkBuilderTask extends AbstractTask {
	...
	public ListSingleSelection<String> selection = new ListSingleSelection<String>(
			LOAD_NEW_NETWORK, USE_EXISTING_NETWORK);

	private final CyNetworkManager networkManager;
	private final SubnetworkBuilderUtil util;

	SubnetworkBuilderTask(final CyNetworkManager networkManager,
		...
		if (selected == LOAD_NEW_NETWORK) {
			...
			if (networks.size() == 0) {
				...
				JOptionPane.showMessageDialog(null,
						"No network is available.  Need to load a new one.",
						"No network found", JOptionPane.ERROR_MESSAGE);
				insertTasksAfterCurrentTask(util.getWebServiceImportTask());
			} else
				insertTasksAfterCurrentTask(new SelectNetworkTask());
		}
	}
