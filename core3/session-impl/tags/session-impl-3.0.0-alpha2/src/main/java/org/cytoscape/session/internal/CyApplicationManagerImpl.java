...
public class CyApplicationManagerImpl implements CyApplicationManager, NetworkAboutToBeDestroyedListener, NetworkViewAboutToBeDestroyedListener
{
		...
		synchronized (this) {
			if (!networkManager.networkExists(networkId))
				throw new IllegalArgumentException(
					"Network is not registered in this ApplicationManager: ID = " + networkId);
	...
	public void setCurrentNetworkView(final long modelID) {
		...
		synchronized (this) {
			if (!networkManager.networkExists(modelID))
				throw new IllegalArgumentException(
						"network is not recognized by this ApplicationManager");

			view = networkViewManager.getNetworkView(modelID);
			if (view == null)
				throw new IllegalArgumentException(
						"network view is not recognized by this ApplicationManager");
	...
	public void setSelectedNetworkViews(final List<Long> modelIDs) {
		...
		cyEventHelper.fireSynchronousEvent(
			new SetSelectedNetworkViewsEvent(this, new ArrayList<CyNetworkView>(selectedNetworkViews)));
	}
	...
	public void setSelectedNetworks(final List<Long> ids) {
		...
		cyEventHelper.fireSynchronousEvent(
			new SetSelectedNetworksEvent(this, new ArrayList<CyNetwork>(selectedNetworks)));
	}
