...
public final class GraphConverter2 {
	...
	public static MutablePolyEdgeGraphLayout getGraphReference(double percentBorder,
		...
		for ( View<CyNode> currentNodeView : graphView.getNodeViews() ) {
			minX = Math.min(minX, currentNodeView.getVisualProperty(TwoDVisualLexicon.NODE_X_LOCATION));
			maxX = Math.max(maxX, currentNodeView.getVisualProperty(TwoDVisualLexicon.NODE_X_LOCATION));
			minY = Math.min(minY, currentNodeView.getVisualProperty(TwoDVisualLexicon.NODE_Y_LOCATION));
	...
	*/
		...
		return new MutablePolyEdgeGraphLayout() {
				...
				public void setNodePosition(CyNode node, double xPos, double yPos) {
					...
					nodeView.setVisualProperty(TwoDVisualLexicon.NODE_X_LOCATION, xPos + xOff);
					nodeView.setVisualProperty(TwoDVisualLexicon.NODE_Y_LOCATION, yPos + yOff);
				}
