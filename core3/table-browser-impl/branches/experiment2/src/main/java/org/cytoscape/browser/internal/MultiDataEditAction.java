...
import org.cytoscape.browser.internal.ui.ActionName;
import org.cytoscape.browser.internal.util.AttrUtil;
...
public class MultiDataEditAction extends AbstractUndoableEdit {
	...
	public void undo() {
		for (int i = 0; i < objects.size(); ++i) {
			CyTableEntry go = (CyTableEntry) objects.get(i);
	...
	public void initEdit() {
		...
		if (action == COPY) {
			...
			if (attType == CyTable.TYPE_UNDEFINED)
				attType = CyTable.TYPE_STRING;
			...
			else if (attType == CyTable.TYPE_FLOATING) {
				Double d = new Double(input);
				doubleAction(d.doubleValue());
			} else if (attType == CyTable.TYPE_INTEGER) {
				try {
			...
			} else if (attType == CyTable.TYPE_STRING) {
				stringAction(input);
			} else if (attType == CyTable.TYPE_BOOLEAN) {
				booleanAction(Boolean.valueOf(input));
			} else if (attType == CyTable.TYPE_SIMPLE_LIST) {
				listAction();
			} else if (attType == CyTable.TYPE_SIMPLE_MAP) {
				// TODO: HANDLE
			}
		}
	...
	private static boolean eqnTypeIsCompatibleWithAttrType(final Class eqnType, final byte attType) {
		if (eqnType == Integer.class || eqnType == Long.class) {
			if (attType != CyTable.TYPE_INTEGER && attType != CyTable.TYPE_FLOATING
			    && attType != CyTable.TYPE_BOOLEAN && attType != CyTable.TYPE_STRING)
				return false;
		}
		else if (eqnType == Double.class) {
			if (attType != CyTable.TYPE_FLOATING && attType != CyTable.TYPE_BOOLEAN
			    && attType != CyTable.TYPE_STRING)
				return false;
		}
		else if (eqnType == Boolean.class) {
			if (attType != CyTable.TYPE_INTEGER && attType != CyTable.TYPE_FLOATING
			    && attType != CyTable.TYPE_BOOLEAN)
				return false;
		}
	...
	private void listAction() {
		...
		for (final Object obj : objects) {
			 final CyTableEntry go = (CyTableEntry)obj;
	...
	private void copyAtt() {
		...
		for (final Object o : objects) {
			final CyTableEntry go = (CyTableEntry)o;
			...
			if ((fromType == CyTable.TYPE_INTEGER) && (toType == CyTable.TYPE_FLOATING))
	...
	private boolean copyAttrsAreCompatible(final Class<?> fromType, final Class<?> toType) {
		if (fromType == toType)
			return true;

		if (fromType == Integer.class && toType == Double.class)
			return true;
		if (fromType == Long.class && toType == Double.class)
			return true;
	...
	private void booleanAction(final Boolean input) {
		...
		for (final Object o : objects) {
			CyTableEntry go = (CyTableEntry)o;
