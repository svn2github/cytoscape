...
public class FilterPlugin extends CytoscapePlugin {
	...
	private void init() {
		...
		ImageIcon icon =  new ImageIcon(getClass().getResource("/cytoscape/images/ximian/stock_filter-data-by-criteria.png"));
		ImageIcon icon2 = new ImageIcon(getClass().getResource("/cytoscape/images/ximian/stock_filter-data-by-criteria-16.png"));
		...
		System.out.println("FilterPlugin.init(): load "+allFilterVect.size() + " filters from filters.prop");
	...
	public void restoreSessionState(List<File> pStateFileList) {
		...
		for (int i =0; i<sessionFilterVect.size(); i++ ){
			...
			for (int j=0; j<currentFilterCount; j++ ) {
				if (sessionFilterVect.elementAt(i).toString().equalsIgnoreCase(allFilterVect.elementAt(j).toString())) {
					isDuplicated= true;
					break;
				}				
			}
		...
		System.out.println("\t" + (currentFilterCount+ sessionFilterVect.size() -allFilterVect.size())+ " duplicated filters are not added");
	}
