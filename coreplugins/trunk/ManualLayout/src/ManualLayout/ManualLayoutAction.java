...
public class ManualLayoutAction extends CytoscapeAction {
	...
	private CytoPanelImp cytoPanel1 = (CytoPanelImp) Cytoscape.getDesktop()
	                                                          .getCytoPanel(SwingConstants.WEST);
	private CytoPanelImp manualLayoutPanel = (CytoPanelImp) Cytoscape.getDesktop()
	                                                                 .getCytoPanel(SwingConstants.SOUTH_WEST);
	private BiModalJSplitPane split;

	/**
	 * Creates a new ManualLayoutAction object.
	 */
	...
	public void actionPerformed(ActionEvent ev) {
		...
		if (_source instanceof JCheckBoxMenuItem) {
			...
			if (menuItem == (JCheckBoxMenuItem) layoutMenu.getMenuComponent(0)) {
				menuItemIndex = 0;
			} else if (menuItem == (JCheckBoxMenuItem) layoutMenu.getMenuComponent(1)) {
				menuItemIndex = 1;
			}
		...
		CytoPanelState curState = Cytoscape.getDesktop().getCytoPanel(SwingConstants.SOUTH_WEST)
		                                   .getState();
		...
		if (curState == CytoPanelState.HIDE) {
			...
			if (nativeGraph == null) {
				nativeGraph = new MutablePolyEdgeGraphLayout[] {
	...
	private boolean isAnyCheckBoxSelected() {
		...
		JCheckBoxMenuItem rotateCheckBoxMenuItem = (JCheckBoxMenuItem) layoutMenu.getMenuComponent(0);
		JCheckBoxMenuItem scaleCheckBoxMenuItem = (JCheckBoxMenuItem) layoutMenu.getMenuComponent(1);
		JCheckBoxMenuItem controlCheckBoxMenuItem = (JCheckBoxMenuItem) layoutMenu.getMenuComponent(2);

		if (rotateCheckBoxMenuItem.getState() || scaleCheckBoxMenuItem.getState()
	...
	private void addEventListeners() {
		...
		Cytoscape.getCurrentNetwork().addSelectEventListener(treeSelectListener);
		Cytoscape.getDesktop().getCytoPanel(SwingConstants.SOUTH_WEST)
	...
	private void removeEventListeners() {
		...
		Cytoscape.getCurrentNetwork().removeSelectEventListener(treeSelectListener);
		Cytoscape.getDesktop().getCytoPanel(SwingConstants.SOUTH_WEST)
	...
	public class RotatePanelSliderListener implements ChangeListener {
		...
		public void stateChanged(ChangeEvent e) {
			...
			double radians = (((double) (rotatePanel.jSlider.getValue() - prevValue)) * 2.0d * Math.PI) / 360.0d;
