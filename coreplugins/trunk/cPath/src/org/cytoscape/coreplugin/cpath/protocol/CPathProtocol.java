...
public class CPathProtocol {
	...
	public String connect() throws CPathException, EmptySetException {
		try {
			...
			String msg = "Network error occurred while tring to connect to "
			             + "the cPath Web Service.  Could not find server:  " + e.getMessage()
	...
	private NameValuePair[] createNameValuePairs() {
		...
		if (taxonomyId == NOT_SPECIFIED) {
			...
			nvps[6] = new NameValuePair(ARG_ORGANISM, Integer.toString(taxonomyId));
		}
		...
		nvps[5] = new NameValuePair(ARG_START_INDEX, Integer.toString(startIndex));

		return nvps;
	}

	private void checkHttpStatusCode(int statusCode) throws CPathException {
		if (statusCode != 200) {
			String msg = new String("Error Connecting to cPath "
			                        + "Web Service (Details:  HTTP Status Code:  " + statusCode
			                        + ")");
	...
	private void checkForErrors(Document document) throws CPathException, EmptySetException {
		...
		if (name.equals("error")) {
			...
			if (errorCode.equals("460")) {
				...
				String msg = new String("Error Connecting to cPath " + "Web Service (Error Code:  "
