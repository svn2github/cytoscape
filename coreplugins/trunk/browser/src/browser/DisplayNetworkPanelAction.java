...
public class DisplayNetworkPanelAction extends CytoscapeAction {
	/**
	 * Creates a new DisplayNetworkPanelAction object.
	 */
	...
	public void actionPerformed(ActionEvent ev) {
		...
		CytoPanelState curState = Cytoscape.getDesktop().getCytoPanel(SwingConstants.WEST).getState();

		int targetIndex = 0;

		if (curState == CytoPanelState.HIDE) {
			Cytoscape.getDesktop().getCytoPanel(SwingConstants.WEST).setState(CytoPanelState.DOCK);
			Cytoscape.getDesktop().getCytoPanel(SwingConstants.WEST).setSelectedIndex(targetIndex);
