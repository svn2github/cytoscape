...
public class ExternalLinkUtil {
    ...
    private static String createDbParameter(ArrayList dbList,
        ...
        if (dbList != null && dbList.size() > 0) {
            for (int i = 0; i < dbList.size(); i++) {
                ...
                String code = (String) ihopMap.get
                        (link.getDbName().toUpperCase());
                if (code != null) {
    ...
    public static String createIHOPLink(String type, ArrayList synList,
        ...
        if (url != null) {
            StringBuffer buf = new StringBuffer();
            buf.append("<A HREF=\"" + url + "\">"
                    + "Search IHOP</A>:  (Information Hyperlinked "
                    + "Over Proteins)");
            return buf.toString();
        } else {
            return null;
    ...
    private static void initMap() {
        ...
        url = "http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?"
                + "db=unigene&cmd=search&term=";
        dbMap.put("UNIGENE", url);

        //  NCBI GenBank
        url = "http://www.ncbi.nlm.nih.gov/entrez/viewer.fcgi?db=protein"
                + "&val=";
