...
public class MapNodeAttributes {
     ...
     */
    public static final String BIOPAX_CELLULAR_LOCATIONS =
            "biopax.cellular_location";
    ...
    private static final String PHOSPHORYLATION_GRAPHICS =
    ...
    public static void doMapping(BioPaxUtil bpUtil, ArrayList nodeList) {
        ...
        while (nodeIterator.hasNext()) {
            ...
            // get node element
            String biopaxID = nodeAttributes.getStringAttribute(nodeID,
                    BIOPAX_RDF_ID);
            Element resource = (Element) rdfMap.get(biopaxID);

            if (resource != null) {
                String stringRef;
                BioPaxEntityParser bpParser = new BioPaxEntityParser
                        (resource, rdfMap);

                // type
                stringRef = addType(bpParser, nodeID, nodeAttributes);
                if (stringRef != null) {
                    nodeAttributes.setAttribute(nodeID, BIOPAX_ENTITY_TYPE,
                            stringRef);
                }
                ...
                if (stringRef != null) {
                    nodeAttributes.setAttribute(nodeID, BIOPAX_SHORT_NAME,
                            stringRef);
                }
                ...
                if (synList != null) {
                    nodeAttributes.setAttributeList(nodeID, BIOPAX_SYNONYMS,
                            synList);
                }
                ...
                if (stringRef != null) {
                    nodeAttributes.setAttribute(nodeID, BIOPAX_ORGANISM_NAME,
                            stringRef);
                }
                ...
                if (stringRef != null) {
                    nodeAttributes.setAttribute(nodeID, BIOPAX_COMMENT,
                            stringRef);
                }
                ...
                if (stringRef != null) {
                    nodeAttributes.setAttribute(nodeID,
                            BIOPAX_UNIFICATION_REFERENCES, stringRef);
                }

                // the following code should replace the old way to set
                // relationship references
                List xrefList = getXRefList(bpParser,
                        BIOPAX_AFFYMETRIX_REFERENCES_LIST);
                if (xrefList != null && xrefList.size() > 0) {
                    nodeAttributes.setAttributeList(nodeID,
                            BIOPAX_AFFYMETRIX_REFERENCES_LIST, xrefList);
                }
                ...
                if (stringRef != null) {
                    nodeAttributes.setAttribute(nodeID,
                            BIOPAX_RELATIONSHIP_REFERENCES, stringRef);
                }
                ...
                if (stringRef != null) {
                    nodeAttributes.setAttribute(nodeID,
                            BIOPAX_PUBLICATION_REFERENCES, stringRef);
                }

                // availability
                stringRef = bpParser.getAvailability();
                if (stringRef != null) {
                    nodeAttributes.setAttribute(nodeID, BIOPAX_AVAILABILITY,
                            stringRef);
                }
                ...
                if (stringRef != null) {
                    nodeAttributes.setAttribute(nodeID, BIOPAX_DATA_SOURCES,
                            stringRef);
                }
                ...
                if (stringRef != null) {
                    nodeAttributes.setAttribute(nodeID, BIOPAX_IHOP_LINKS,
                            stringRef);
                }
    ...
    public static void customNodes(CyNetworkView networkView) {
        ...
        if (nodesIt.hasNext()) {
            ...
            if (mhmdef.getAttributeValueType
                    (BIOPAX_CHEMICAL_MODIFICATIONS_MAP) != -1) {
                MultiHashMap mhmap = nodeAttributes.getMultiHashMap();
                CountedIterator modsIt = mhmap.getAttributeKeyspan
                        (node.getIdentifier(),
            ...
            if (isPhosphorylated) {
                addCustomShapes(networkView, node, PHOSPHORYLATION_GRAPHICS,
                        count);
            }
        }
    }
    ...
    private static void initAttributes(CyAttributes nodeAttributes) {
        ...
        nodeAttributes.setAttributeDescription(BIOPAX_COMMENT,
                "Comments regarding this entity.  ");
        ...
        nodeAttributes.setAttributeDescription
                (BIOPAX_AFFYMETRIX_REFERENCES_LIST,
                        "A list of one or more Affymetrix probeset identifers "
                                + "associated with the entity.");
        nodeAttributes.setAttributeDescription
                (BIOPAX_CHEMICAL_MODIFICATIONS_LIST,
     ...
     */
    private static void addCustomShapes(CyNetworkView networkView, CyNode node,
            String shapeType, int modificationCount) {
     ...
     */
    private static Rectangle2D getCustomShapeRect(BufferedImage image,
        ...
        final double startY[] = {(-1
                * BioPaxVisualStyleUtil.
                BIO_PAX_VISUAL_STYLE_PHYSICAL_ENTITY_NODE_HEIGHT
                * BioPaxVisualStyleUtil.
                BIO_PAX_VISUAL_STYLE_PHYSICAL_ENTITY_NODE_SIZE_SCALE) / 2,
                0,
                (BioPaxVisualStyleUtil.
                        BIO_PAX_VISUAL_STYLE_PHYSICAL_ENTITY_NODE_HEIGHT
                        * BioPaxVisualStyleUtil.
    ...
    private static String addPathwayMembership(String rdfId,
            BioPaxUtil bpUtil) {
        ...
        if (pathwayList != null) {
            ...
            for (int i = 0; i < pathwayList.size(); i++) {
                ...
                if (pathway != null) {
                    BioPaxEntityParser bpParser2 = new BioPaxEntityParser
                            (pathway, rdfMap);
    ...
    private static String addDataSource(RdfQuery rdfQuery, Element resource) {
        ArrayList dataSourceList = rdfQuery.getNodes(resource,
        ...
        if (dataSourceList.size() > 0) {
            ...
            for (int i = 0; i < dataSourceList.size(); i++) {
                ...
                Element dsCommentElement = rdfQuery.getNode
                        (dataSource, "COMMENT");
    ...
    private static String addPublicationXRefs(RdfQuery rdfQuery,
            Element resource) {
        ArrayList pubList = rdfQuery.getNodes(resource,
        ...
        if (pubList.size() > 0) {
            ...
            for (int i = 0; i < pubList.size(); i++) {
                ...
                if (authorElement != null) {
                    temp.append(authorElement.getTextNormalize()
                            + " et. al, ");
                }
    ...
    private static String addXRefs(ArrayList xrefList) {
        if (xrefList.size() > 0) {
            ...
            for (int i = 0; i < xrefList.size(); i++) {
                ...
                temp.append("- ");
                temp.append(ExternalLinkUtil.createLink
                        (link.getDbName(), link.getId()));
    ...
    private static List getXRefList(BioPaxEntityParser bpParser,
            String xrefType) {
    ...
    private static String addIHOPLinks(BioPaxEntityParser bpParser) {
        ...
        if (synList.size() > 0 || dbList.size() > 0) {
            String htmlLink = ExternalLinkUtil.createIHOPLink
                    (bpParser.getType(), synList,
                            dbList, bpParser.getOrganismTaxonomyId());
            if (htmlLink != null) {
                return ("- " + htmlLink);
            }
        }
