...
public class FormulaBuilderPanel extends JPanel implements ActionListener {
	...
	public FormulaBuilderPanel(final DataTableModel tableModel, final DataObjectType graphObjectType) {
		...
		operation.add(functionComboBox, gridBagConstraints);
	...
	private void initFunctionComboBox() {
		...
		for (final AttribFunction function : functions) {
			functionNames[index] = function.getName();
			stringToFunctionMap.put(function.getName(), function);
			++index;
		}
	...
	protected void actionChanged() {
		final ActionName action = ActionName.getValueOf(functionComboBox.getSelectedItem().toString());
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == applyModify) {
			...
			if (ActionName.getValueOf(functionComboBox.getSelectedItem().toString())
			              .equals(ActionName.REPLACE)) {
				input = field1.getText() + "\t" + field2.getText();
			}

			edit = new MultiDataEditAction(input,
			                               ActionName.getValueOf(functionComboBox.getSelectedItem()
			                                                              .toString()),
