    ...
    */
    ...
    private void createEdge(Interactor interactor1, Interactor interactor2,
        ...
        CyEdge edge = Cytoscape.getCyEdge(node1, node2,
                Semantics.INTERACTION, key, true);
        ...
        if (refs != null) {
            ...
            if (dbsList != null && dbsList.size() != 0) {
                Cytoscape.getNodeAttributes().setAttributeList
                        (cyNode.getIdentifier(), CommonVocab.XREF_DB_NAME, dbsList);
            }
            if (idsList != null && idsList.size() != 0) {
                Cytoscape.getNodeAttributes().setAttributeList
                        (cyNode.getIdentifier(), CommonVocab.XREF_DB_ID, idsList);
            }
        }
    }
    ...
    protected void mapEdgeAttributes(Interaction interaction, CyEdge cyEdge) {
        ...
        while (iterator.hasNext()) {
            ...
            if (attrObject instanceof String) {
                ...
                Object object = Cytoscape.getEdgeAttributes().getStringAttribute
                        (key, cyEdge.getIdentifier());
                if (object != null) {
                    ...
                    if (values != null && values.toString().length() != 0) {
                        Cytoscape.getEdgeAttributes().setAttribute
                                (cyEdge.getIdentifier(), key, values.toString());
                    }
                } else {
        ...
        if (refs != null) {
            ...
            if (dbsList != null && dbsList.size() != 0) {
                Cytoscape.getEdgeAttributes().setAttributeList
                        (cyEdge.getIdentifier(), CommonVocab.XREF_DB_NAME, dbsList);
            }
            if (idsList != null && idsList.size() != 0) {
                Cytoscape.getEdgeAttributes().setAttributeList
                        (cyEdge.getIdentifier(), CommonVocab.XREF_DB_ID, idsList);
            }
        }
    }
