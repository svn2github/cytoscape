...
/*var s_account="devagilentcom"*/
/*Added to condition to get the separate traffic for production*/
/* for production comment the below line and uncomment the 13-18 line numbers*/ 
/*  var s_account="devagilentcom" */
/* For production please uncomment the below lines*/

/*var pageURL = document.URL;
if(pageURL.indexOf('prdprewe')>1))
	var s_account="devagilentcom";
  else */ 

	var s_account="agilentcom";
...
var s_linkInternalFilters="javascript:,agilent.com"
var s_linkLeaveQueryString=false
var s_linkTrackVars="s_prop4,s_prop19,s_prop20,s_prop24,s_prop25,s_hier2"
var s_linkTrackEvents="None"

/* campaign pathing list */
/* this is a comma delimited list of campaign IDs to track for pathing, empty=all campaign IDs */
var s_campaignPathList="";
...
function s_doPlugins() {
	...
	//set campaign variable based on query string parameter cid
	s_vp_getCGI("s_campaign","cmpid")
	
	// set s_prop for campaign pathing
	s_vp_cmpPath("s_prop12","cmpPath","cmpid=");
	

	var t_prop; //(temporary prop variable)
	
	// set s_prop26 
	t_prop=s_vp_getValueOrEmpty("s_channel") + "|" + s_vp_getValueOrEmpty("s_prop13");
	s_vpr("s_prop26",t_prop);
	
	// set s_prop27 (end of pageName will be truncated if total over 100 bytes)
	t_prop=s_vp_getValueOrEmpty("s_channel") + "|" + s_vp_getValueOrEmpty("s_pageName");
	s_vpr("s_prop27",t_prop);
	
	// set s_prop28
	t_prop=s_vp_getValueOrEmpty("s_prop22") + "|" + s_vp_getValueOrEmpty("s_prop13");
	s_vpr("s_prop28",t_prop);
	
	// set s_prop29
	t_prop=s_vp_getValueOrEmpty("s_prop23") + "|" + s_vp_getValueOrEmpty("s_prop13");
	s_vpr("s_prop29",t_prop);
	
	// set s_prop30 (end of pageName will be truncated if total over 100 bytes)
	t_prop=s_vp_getValueOrEmpty("s_prop21") + "|" + s_vp_getValueOrEmpty("s_pageName");
	s_vpr("s_prop30",t_prop);
...
/*
 * Plugin: Get Plugin Modified Value or Empty String if null
 */
function s_vp_getValueOrEmpty(vs) {
	var t = s_vp_getValue(vs);
	//if (val(t)=="undefined") {t=""};
	if (!t) {t=""};
	return t;}
/*
 * Plugin: Set s_prop for Campaign Pathing
 */
function s_vp_cmpPath(s,ck,pr) {
   /* 
   ...
   first page sets s_prop = cmpPrefix + campaignID + pagename 
	...
	s = s_prop to use
	ck = cookie name
	pr = report prefix for landing page
	*/
	var tp = s_vp_getValue("s_pageName");
	var tc = s_vp_getValue("s_campaign");
	if(tc) { 
		if (s_campaignPathList=="" || s_vp_inList(s_campaignPathList,tc,",")) {
			s_vpr(s,pr+tc+" : "+tp); 
			s_c_w(ck,tc,0);
		} else {	s_c_w(ck,"",0); }
	} else if (s_c_r(ck)) { s_vpr(s,tp) } 
}
/*
...
+sess+'?[AQB]&ndh=1'+(q?q:'')+(s_q?s_q:'')+'&[AQE]';if(s_ios){
im=s_wd[imn]?s_wd[imn]:s_d.images[imn];if(!im)im=s_wd[imn]=new Image;im.src=rs
