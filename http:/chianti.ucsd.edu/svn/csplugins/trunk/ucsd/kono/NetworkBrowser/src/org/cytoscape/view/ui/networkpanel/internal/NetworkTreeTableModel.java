...
public class NetworkTreeTableModel extends DefaultTreeTableModel implements TreeSelectionListener,
        SelectEventListener {
	
	private Map<String, MutableTreeTableNode> treeNodeMap;
	
	private static final Icon DEF = new ImageIcon(
			NetworkBrowserPlugin.class.getResource(
	...
	protected void addNetwork(String network_id, String parent_id) {
		...
		if (treeNodeMap.get(network_id) == null) {
			...
			NetworkTreeNode dmtn = new NetworkTreeNode(Cytoscape.getNetwork(network_id).getTitle(),
			                                           network_id, this.getColumnCount());
			Cytoscape.getNetwork(network_id).addSelectEventListener(this);

			
			
			if (parent_id != null && treeNodeMap.get(parent_id) != null) {
				((AbstractMutableTreeTableNode) treeNodeMap.get(parent_id)).add(dmtn);
			} else {
				System.out.println("====== To ROOT: " + getRoot().getChildCount());
			...
			//System.out.println("=============> Value Set: " + getValueAt(dmtn, 1) + ", and count = " + dmtn.getValueAt(1));
